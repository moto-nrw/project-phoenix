...
| Demo sidecar only | Privileged `DB_DSN`, pool sizing, `APP_ENV`, `TZ`, operator bootstrap credentials and device PIN. No JWT signing, SMTP or frontend secrets; no filesystem state. The environment and target-host guards reject staging, production and public hosts. | `backend/cmd/demo.go`, `environments/demo-runtime.compose.yml`, [standing demo operations](standing-demo.md) |
