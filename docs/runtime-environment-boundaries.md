...
| Serving backend | Backend JWT key, application-role password, SMTP credentials, device PIN, VAPID private key, metrics token, analytics settings; pool sizing, URLs, auth timing, rate limits, logging, scheduler and checkout configuration. | `backend/cmd/serve.go`, `database/database_config.go`, `services/factory.go`, `api/base.go`, `services/scheduler/` |
