...
Verwandte Dokumente: `docs/adr/0025-hilfe-bleibt-oeffentlich-die-app-verweist.md`,
...
1. Registriert sind heute **162 Einstellungen** (163 Key-Konstanten in `backend/models/config/keys.go`, davon eine ohne Registrierung), nicht die in ADR 0025 genannten „rund 86" — die Zahl ist seit dem ADR gewachsen und sollte dort nachgezogen werden.
...
10. Für die Anleitung heißt das: eine kleine, kuratierte Variantenachse (NFC, Anwesenheits-Modus, Organisationsmodell) statt einer Filterung über alle Einstellungen — genau die Linie aus ADR 0025, aber mit anderen Grenzen als dort angenommen.
...
| **Empfohlene Behandlung** | Eigene Variante auf Anleitungsebene. Die beiden NFC-Einstiege auf `/help` als „nur für Einrichtungen mit NFC-Tablets" kennzeichnen (ADR 0025: kennzeichnen, nicht verstecken). In der Alltags-Anleitung dort, wo NFC-Navigation beschrieben wird, einen bedingten Hinweis setzen. |
...
| Schul-Portal „moto schule" | Nur Klassenansicht und Nachrichten; operative Sicht immer `own`; verlinkt heute auf die vollständige OGS-Anleitung | `backend/auth/authorize/operational_overview.go:27-43`; ADR 0025 |
...
| „NFC Erste Schritte" | `nfc_enabled` | **bedingter Hinweis** — als „nur mit NFC-Tablets" kennzeichnen, nicht verstecken (ADR 0025) |
