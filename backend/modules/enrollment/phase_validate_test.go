...
func validPhase() *Phase {
	return &Phase{
		...
		ServiceStartDate: Date("2026-09-01"),
		ServiceEndDate:   Date("2027-07-31"),
...
func TestPhase_Validate_RejectsEndBeforeStart(t *testing.T) {
	...
	p := validPhase()
	p.ServiceStartDate = Date("2027-01-01")
	p.ServiceEndDate = Date("2026-12-31")
	err := p.Validate()
	require.Error(t, err)
	assert.Contains(t, err.Error(), "service_end_date")
}
...
func TestPhase_Validate_RejectsUnknownRolloverMode(t *testing.T) {
	...
	p := validPhase()
	mode := "weird"
	var src int64 = 1 // In-memory lineage reference; this validation test does not access a database.
	p.RolloverMode = &mode
