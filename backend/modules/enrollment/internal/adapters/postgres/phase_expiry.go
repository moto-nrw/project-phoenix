...
func (r *Store) PhaseExpirySnapshots(ctx context.Context, input enrollment.PhaseExpiryInput) ([]*enrollment.PhaseExpirySnapshot, error) {
...
effective_source_bookings AS (
    ...
    CROSS JOIN LATERAL (
        ...
        WHERE date_part('isodow', source_phase.service_end_date + candidate.day_offset)::integer IN (
                                ...
                                WHERE target_child_offering.tenant_id = target_child.tenant_id
                                  ...
                                  AND EXISTS (
                                      ...
                                      WHERE CASE LOWER(BTRIM(booked_day.value))
                                          ...
                                          WHEN 'sat' THEN 6
                                          WHEN 'sun' THEN 7
                                      END = date_part('isodow', source_student.first_affected_date)::integer
                                  )
                            )
                        )
                    )
