...
func newIdentityEligibilityService(exists bool) *ChangeRequests {
	return NewChangeRequests(ChangeRequestDependencies{
		Students:   &stubEligibilityStudentRepo{exists: exists},
...
func TestValidateChangedChildIdentityEligibility_RejectsRetargetToEnrolledIdentity(t *testing.T) {
	...
	phase := &enrollment.Phase{Audience: enrollment.PhaseAudienceNewStudents}
	...
	require.ErrorIs(t, err, enrollment.ErrInvalidSubmission, "already-enrolled must keep the 400 category")
...
func TestValidateChangedChildIdentityEligibility_ExistingStudentsRejectsUnknownIdentity(t *testing.T) {
	...
	phase := &enrollment.Phase{Audience: enrollment.PhaseAudienceExistingStudents}
	children := []*RequestChild{identityEligibilityChild(11, "Anna")}
	editReq := SubmitRequest{
		TenantID: 77,
...
func TestValidateChangedChildIdentityEligibility_OtherAudiencesPass(t *testing.T) {
	...
	svc := newIdentityEligibilityService(true)
	phase := &enrollment.Phase{Audience: enrollment.PhaseAudienceOpen}
	children := []*RequestChild{identityEligibilityChild(11, "Anna")}
	editReq := SubmitRequest{
		TenantID: 77,
		Children: []SubmitChild{identityEligibilitySubmit(11, "Ben")},
