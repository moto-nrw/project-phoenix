...
type approvedSelectionFixture struct {
	ApprovedSelections
	rows []*enrollment.ApprovedOfferingSelection
	err  error
}

func (f approvedSelectionFixture) ApprovedSelectionsForOfferings(context.Context, []int64, enrollment.Date) ([]*enrollment.ApprovedOfferingSelection, error) {
	return f.rows, f.err
}
...
func TestApprovedOfferingProjectionExcludesAlumniAndMissingStudents(t *testing.T) {
	...
	selections := approvedSelectionFixture{rows: []*enrollment.ApprovedOfferingSelection{
		{Selection: &enrollment.RequestChildOffering{ID: 101}, StudentID: 21},
		{Selection: &enrollment.RequestChildOffering{ID: 102}, StudentID: 22},
		{Selection: &enrollment.RequestChildOffering{ID: 103}, StudentID: 23},
		{Selection: &enrollment.RequestChildOffering{ID: 104}, StudentID: 21},
	}}
...
func TestApprovedOfferingProjectionPreservesDependencyErrors(t *testing.T) {
	...
	require.Nil(t, rows)
	projection = NewApprovedOfferingProjection(approvedSelectionFixture{rows: []*enrollment.ApprovedOfferingSelection{{StudentID: 21}}}, &offeringStudentFixture{err: failure})
	rows, err = projection.ListApprovedChildrenByCareOfferingIDs(t.Context(), []int64{31}, "2026-09-01")
	require.ErrorIs(t, err, failure)
	require.Nil(t, rows)
...
func (f approvedSelectionFixture) ApprovedSelectionsForStudents(context.Context, []int64, enrollment.Date, enrollment.Date) ([]*enrollment.ApprovedOfferingSelection, error) {
	return f.rows, f.err
}
