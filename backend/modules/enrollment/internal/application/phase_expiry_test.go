...
type phaseExpiryRepositoryStub struct {
	snapshots      []*enrollment.PhaseExpirySnapshot
	asOf           calendar.Date
	warningThrough calendar.Date
}
...
func TestPhaseExpiryServiceRejectsMalformedDateWithoutPartialWarnings(t *testing.T) {
	...
	for _, completed := range []bool{false, true} {
		t.Run(fmt.Sprintf("completed=%t", completed), func(t *testing.T) {
			t.Parallel()
			invalid := &enrollment.PhaseExpirySnapshot{
			...
			repo := &phaseExpiryRepositoryStub{snapshots: []*enrollment.PhaseExpirySnapshot{
				{SourcePhaseID: 3, FirstAffectedDate: "2027-02-01", AffectedChildren: 1, UnresolvedChildren: 1},
				invalid,
			}}
			warnings, err := NewPhaseExpiryWarnings(repo).ListWarnings(context.Background(), calendar.NewDate(2027, 1, 2))
			require.Error(t, err)
...
func (s *phaseExpiryRepositoryStub) ListSnapshots(
	_ context.Context,
	asOf, warningThrough calendar.Date,
) ([]*enrollment.PhaseExpirySnapshot, error) {
	s.asOf = asOf
...
func TestPhaseExpiryService_ListWarnings_UsesThirtyDayHorizonAndOmitsCompletedSuccessor(t *testing.T) {
	...
	today := calendar.NewDate(2027, 1, 2)
	successorID := int64(12)
	repo := &phaseExpiryRepositoryStub{snapshots: []*enrollment.PhaseExpirySnapshot{
		{
			SourcePhaseID:      3,
			SourcePhaseName:    "1. Halbjahr",
			FirstAffectedDate:  enrollment.Date("2027-02-01"),
		...
		},
		{
			...
			FirstAffectedDate:  enrollment.Date("2027-01-01"),
			AffectedChildren:   20,
	...
	warnings, err := NewPhaseExpiryWarnings(repo).ListWarnings(context.Background(), today)
	require.NoError(t, err)
	assert.Equal(t, today, repo.asOf)
	assert.Equal(t, calendar.NewDate(2027, 2, 1), repo.warningThrough)
	require.Len(t, warnings, 2)

	assert.Equal(t, enrollment.PhaseExpiryStateMissingSuccessor, warnings[0].State)
	assert.False(t, warnings[0].Overdue)
	assert.Equal(t, 204, warnings[0].UnresolvedChildren)

	assert.Equal(t, enrollment.PhaseExpiryStateIncomplete, warnings[1].State)
