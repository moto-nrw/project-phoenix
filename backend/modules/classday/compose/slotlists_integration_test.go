...
func studentPickupScheduleRepository(db *bun.DB) scheduleModels.StudentPickupScheduleRepository {
	return classdaytest.NewStudentScheduleRepositories(db).PickupSchedule
}

func studentPickupExceptionRepository(db *bun.DB) scheduleModels.StudentPickupExceptionRepository {
	return classdaytest.NewStudentScheduleRepositories(db).PickupException
}
...
func newTestService(db *bun.DB) classday.SlotLists {
...
func TestSlotListEntryPointsRejectDisabledTimetable(t *testing.T) {
	...
	assert.ErrorIs(t, err, classday.ErrTimetableDisabled)

	_, err = svc.ListOptions(context.Background(), classday.Date(listDate.String()))
	assert.ErrorIs(t, err, classday.ErrTimetableDisabled)
...
func (r failingRoomRepo) FindRoom(context.Context, int64) (facilities.Room, error) {
	return facilities.Room{}, r.err
}

// roomReader is the Facilities read seam the lists resolve room names from.
type roomReader interface {
	FindRoom(ctx context.Context, id int64) (facilities.Room, error)
}

func newTestServiceWithSettings(db *bun.DB, settings stubSlotListSettings) classday.SlotLists {
	return newTestServiceWithSettingsReader(db, settings)
}

func newTestServiceWithSettingsReader(db *bun.DB, settings compose.SettingsReader) classday.SlotLists {
	return newTestServiceWithCustomRoomRepo(db, newTestRoomRepository(db), settings)
}

func newTestRoomRepository(db *bun.DB) roomReader {
...
func newTestPeople(db *bun.DB) *peopledirectory.Module {
...
func newTestGroups(db *bun.DB) *schoolstructure.Module {
	groups, err := schoolStructureCompose.New(schoolStructureCompose.Dependencies{DB: db, Observe: func(schoolStructureCompose.Observation) {}})
	if err != nil {
		panic(err)
	}
	return groups
...
func newTestServiceWithCustomRoomRepo(db *bun.DB, rooms roomReader, settings compose.SettingsReader) classday.SlotLists {
	return newTestServiceWithCustomAccess(db, rooms, settings, slotListUserContext{currentStaff: &userModels.Staff{}})
}

func newTestServiceWithCustomAccess(db *bun.DB, rooms roomReader, settings compose.SettingsReader, userCtx slotListUserContext) classday.SlotLists {
	return newTestServiceWithParticipation(db, rooms, settings, userCtx, slotListParticipation{})
}
...
func newTestServiceWithParticipation(db *bun.DB, rooms roomReader, settings compose.SettingsReader, userCtx slotListUserContext, participation scheduleSvc.CareParticipationResolver) classday.SlotLists {
	...
	return tenantScopedSlotLists{db: db, inner: service}
}

// tenantScopedSlotLists runs every read inside the caller's tenant
// transaction, exactly as the request middleware does in production: the
// People Directory owner serves its reads only on that transaction, and the
// lists read children and names through it.
type tenantScopedSlotLists struct {
	db    *bun.DB
	inner classday.SlotLists
}

func (s tenantScopedSlotLists) BuildList(ctx context.Context, params classday.Params) (*classday.Result, error) {
	var result *classday.Result
	err := tenant.WithTenantTx(ctx, s.db, tenant.FromContext(ctx), func(txCtx context.Context, _ bun.Tx) error {
		built, err := s.inner.BuildList(txCtx, params)
		result = built
		return err
	})
	return result, err
...
func (s tenantScopedSlotLists) ListOptions(ctx context.Context, date classday.Date) (*classday.OptionsResult, error) {
	var result *classday.OptionsResult
	err := tenant.WithTenantTx(ctx, s.db, tenant.FromContext(ctx), func(txCtx context.Context, _ bun.Tx) error {
		options, err := s.inner.ListOptions(txCtx, date)
		result = options
		return err
	})
	return result, err
...
func (s tenantScopedSlotLists) RenderList(ctx context.Context, params classday.Params, format listexport.Format) (listexport.File, error) {
	var file listexport.File
	err := tenant.WithTenantTx(ctx, s.db, tenant.FromContext(ctx), func(txCtx context.Context, _ bun.Tx) error {
		rendered, err := s.inner.RenderList(txCtx, params, format)
		file = rendered
		return err
	})
	return file, err
...
func buildMensaFixture(t *testing.T) *mensaFixture {
	t.Helper()
	return buildMensaFixtureOn(t, testpkg.SetupTestDB(t))
}

// buildMensaFixtureOn seeds the Mensa fixture on the given database so a
// test that counts statements can own its pool.
func buildMensaFixtureOn(t *testing.T, db *bun.DB) *mensaFixture {
	t.Helper()
...
// addPlannedPresent seeds n more planned children who attended the Mensa
// slot, so a list over the fixture grows in rows without changing shape.
func (f *mensaFixture) addPlannedPresent(t *testing.T, n int) {
	t.Helper()
	ctx := testpkg.Ctx(t)
	isRepo := newBoundInstanceStudentRepository(f.db)
	entry := atOn(listDate, 11, 40)
	exit := atOn(listDate, 12, 10)
	for i := 0; i < n; i++ {
		student := testpkg.CreateTestStudent(t, f.db, "SL-Extra", fmt.Sprintf("E-%d-%d", i, time.Now().UnixNano()), "3a")
		row := &scheduleModels.InstanceStudent{
		...
		row.SetTenantID(testpkg.Tenant(t))
		require.NoError(t, isRepo.Create(ctx, row))
		testpkg.CreateTestVisit(t, f.db, student.ID, f.activeGroupID, entry, &exit)
	}
}

func rowByStudent(rows []classday.Row, studentID int64) *classday.Row {
	for i := range rows {
...
func TestBuildList_CancelledInstanceExcluded(t *testing.T) {
	...
	options, err := f.svc.ListOptions(ctx, classday.Date(listDate.String()))
	require.NoError(t, err)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, classday.Counters{}, result.Counters)
		})
	}
}
...
func TestBuildList_ListKindRestrictsSlots(t *testing.T) {
	...
	result, err := f.svc.BuildList(ctx, classday.Params{
		Date:     classday.Date(listDate.String()),
		Target:   classday.TargetSlots,
		ListKind: classday.ListKindMensa,
		Source:   classday.SourcePlanned,
	})
	require.NoError(t, err)
	require.Len(t, result.Slots, 1)
	...
	byKind := map[classday.ListKind]classday.ListKindOption{}
	for _, option := range options.ListKinds {
		byKind[option.Kind] = option
	}
	assert.Equal(t, 1, byKind[classday.ListKindMensa].SlotCount)
	assert.Equal(t, 2, byKind[classday.ListKindMensa].RowCount)
	assert.Equal(t, 1, byKind[classday.ListKindLearningTime].SlotCount)
	assert.Equal(t, 1, byKind[classday.ListKindLearningTime].RowCount)
}
...
func TestBuildList_SlotFilter(t *testing.T) {
	...
	full, err := f.svc.BuildList(ctx, classday.Params{
		Date:   classday.Date(listDate.String()),
		Target: classday.TargetSlots,
		Source: classday.SourceReconciliation,
	})
	...
	kept, err := f.svc.BuildList(ctx, classday.Params{
		Date:        classday.Date(listDate.String()),
		Target:      classday.TargetSlots,
		Source:      classday.SourceReconciliation,
	...
	dropped, err := f.svc.BuildList(ctx, classday.Params{
		Date:        classday.Date(listDate.String()),
		Target:      classday.TargetSlots,
		Source:      classday.SourceReconciliation,
	...
	cleared, err := f.svc.BuildList(ctx, classday.Params{
		Date:           classday.Date(listDate.String()),
		Target:         classday.TargetSlots,
		Source:         classday.SourceReconciliation,
		InstanceIDsSet: true,
...
func TestBuildList_ClassFilterAndOptions(t *testing.T) {
	...
	filtered, err := f.svc.BuildList(ctx, classday.Params{
		Date:    classday.Date(listDate.String()),
		Target:  classday.TargetSlots,
		Source:  classday.SourceReconciliation,
		Classes: []string{"3b"},
...
func TestBuildList_MensaPlannedAndActualModes(t *testing.T) {
	...
	planned, err := f.svc.BuildList(ctx, classday.Params{
		Date:   classday.Date(listDate.String()),
		Target: classday.TargetSlots,
		Source: classday.SourcePlanned,
	})
	...
	actual, err := f.svc.BuildList(ctx, classday.Params{
		Date:   classday.Date(listDate.String()),
		Target: classday.TargetSlots,
		Source: classday.SourceActual,
	})
	require.NoError(t, err)
...
func TestBuildList_TimetablePresentStatusCountsAsActual(t *testing.T) {
	...
	recon, err := f.svc.BuildList(ctx, classday.Params{
		Date:   classday.Date(listDate.String()),
		Target: classday.TargetSlots,
		Source: classday.SourceReconciliation,
	})
...
func TestBuildList_RoomLookupErrorsFail(t *testing.T) {
	...
	_, err := svc.BuildList(ctx, classday.Params{
		Date:   classday.Date(listDate.String()),
		Target: classday.TargetSlots,
		Source: classday.SourceReconciliation,
	})
...
func TestBuildList_FullDayPickupCohorts(t *testing.T) {
	...
	cohort1430, err := svc.BuildList(ctx, classday.Params{
		Date:         classday.Date(pickupDate.String()),
		Target:       classday.TargetPickupCohort,
		PickupCohort: classday.PickupCohortShortDay,
		Source:       classday.SourcePlanned,
	})
	require.NoError(t, err)
	...
	cohort1600, err := svc.BuildList(ctx, classday.Params{
		Date:         classday.Date(pickupDate.String()),
		Target:       classday.TargetPickupCohort,
		PickupCohort: classday.PickupCohortLongDay,
		Source:       classday.SourcePlanned,
	})
	require.NoError(t, err)
...
func TestBuildList_FullDayPickupCohortsUseTenantCutoffs(t *testing.T) {
	...
	shortCohort, err := svc.BuildList(ctx, classday.Params{
		Date:         classday.Date(pickupDate.String()),
		Target:       classday.TargetPickupCohort,
		PickupCohort: classday.PickupCohortShortDay,
		Source:       classday.SourcePlanned,
	...
	longCohort, err := svc.BuildList(ctx, classday.Params{
		Date:         classday.Date(pickupDate.String()),
		Target:       classday.TargetPickupCohort,
		PickupCohort: classday.PickupCohortLongDay,
		Source:       classday.SourcePlanned,
...
func TestBuildList_FullDayPickupSettingsErrorsFail(t *testing.T) {
	...
	_, err := svc.BuildList(testpkg.Ctx(t), classday.Params{
		Date:         classday.Date(pickupDate.String()),
		Target:       classday.TargetPickupCohort,
		PickupCohort: classday.PickupCohortShortDay,
		Source:       classday.SourcePlanned,
...
func TestBuildList_GroupByClass(t *testing.T) {
	...
	grouped, err := f.svc.BuildList(ctx, classday.Params{
		Date:    classday.Date(listDate.String()),
		Target:  classday.TargetSlots,
		Source:  classday.SourceReconciliation,
		GroupBy: classday.GroupByClass,
	})
	require.NoError(t, err)
	assert.Equal(t, classday.GroupByClass, grouped.GroupBy)
	...
	_, err = f.svc.BuildList(ctx, classday.Params{
		Date:         classday.Date(pickupDate.String()),
		Target:       classday.TargetPickupCohort,
		PickupCohort: classday.PickupCohortShortDay,
		Source:       classday.SourcePlanned,
		GroupBy:      classday.GroupBySlot,
	})
...
func TestBuildList_FullDayActualScopedToCohort(t *testing.T) {
	...
	actual, err := svc.BuildList(ctx, classday.Params{
		Date:         classday.Date(pickupDate.String()),
		Target:       classday.TargetPickupCohort,
		PickupCohort: classday.PickupCohortShortDay,
		Source:       classday.SourceActual,
	...
	recon, err := svc.BuildList(ctx, classday.Params{
		Date:         classday.Date(pickupDate.String()),
		Target:       classday.TargetPickupCohort,
		PickupCohort: classday.PickupCohortShortDay,
		Source:       classday.SourceReconciliation,
...
func TestRenderList_PDFSmoke(t *testing.T) {
	...
	file, err := f.svc.RenderList(ctx, classday.Params{
		Date:   classday.Date(listDate.String()),
		Target: classday.TargetSlots,
		Source: classday.SourceReconciliation,
...
func TestBuildList_PickupReconciliationDefersNotYetArrivedChild(t *testing.T) {
	...
	plan, err := svc.BuildList(ctx, classday.Params{
		Date:         classday.Date(pickupDate.String()),
		Target:       classday.TargetPickupCohort,
		PickupCohort: classday.PickupCohortLongDay,
		Source:       classday.SourcePlanned,
...
func TestBuildList_PickupCohortExcludesExpiredStudents(t *testing.T) {
	...
	options, err := svc.ListOptions(ctx, classday.Date(pickupDate.String()))
	require.NoError(t, err)
	byCohort := map[classday.PickupCohort]classday.PickupCohortOption{}
	for _, opt := range options.PickupCohorts {
		byCohort[opt.Cohort] = opt
	}
	assert.Equal(t, 1, byCohort[classday.PickupCohortLongDay].RowCount, "expired child must not inflate availability")
}
...
func TestBuildList_PickupCohortRejectsPastDate(t *testing.T) {
	...
	_, err := svc.BuildList(ctx, classday.Params{
		Date:         classday.Date(pastDate.String()),
		Target:       classday.TargetPickupCohort,
		PickupCohort: classday.PickupCohortShortDay,
		Source:       classday.SourcePlanned,
	})
	require.ErrorIs(t, err, classday.ErrPickupCohortPastDate)
	...
	options, err := svc.ListOptions(ctx, classday.Date(pastDate.String()))
	require.NoError(t, err)
	for _, opt := range options.PickupCohorts {
...
func TestBuildList_PickupCohortUsesEnrollmentInterval(t *testing.T) {
	...
	assert.Equal(t, 1, byCohort[classday.PickupCohortShortDay].RowCount, "options availability matches the preview cohort")
}
...
func TestBuildList_PickupCohortKeepsExpiredActualAttendance(t *testing.T) {
	...
	result, err := svc.BuildList(ctx, classday.Params{Date: classday.Date(pickupDate.String()), Target: classday.TargetPickupCohort,
		PickupCohort: classday.PickupCohortShortDay, Source: classday.SourceReconciliation})
	require.NoError(t, err)
...
func TestListOptions_CancelledCareDayCountedInCohort(t *testing.T) {
	...
	preview, err := svc.BuildList(ctx, classday.Params{
		Date:         classday.Date(pickupDate.String()),
		Target:       classday.TargetPickupCohort,
		PickupCohort: classday.PickupCohortLongDay,
		Source:       classday.SourcePlanned,
	...
	assert.Equal(t, 1, byCohort[classday.PickupCohortLongDay].RowCount, "cancelled child must be counted in options")
	assert.True(t, byCohort[classday.PickupCohortLongDay].Available)
}
...
func TestBuildList_SlotListDropsPlannedRowForUnbookedCareDay(t *testing.T) {
	...
	options, err := svc.ListOptions(ctx, classday.Date(listDate.String()))
	require.NoError(t, err)
	var mensaOption *classday.ListKindOption
	for i := range options.ListKinds {
		if options.ListKinds[i].Kind == classday.ListKindMensa {
...
func TestBuildList_SlotListUnbookedButPresentIsUnplanned(t *testing.T) {
	...
	bookedRow.SetTenantID(testpkg.Tenant(t))
...
func TestListOptions_CancelledCareDayAttendedExcludedFromPlannedCount(t *testing.T) {
	...
	attendedRow.SetTenantID(testpkg.Tenant(t))
...
// TestListOptions_CancelledCareDayAttendedViaVisitOnlyExcludedFromPlannedCount
// guards the #1565 review pass 2 P2 fix: the check-in attendance sync is
// deliberately best-effort, so a child can hold a visit in active.visits while
// their roster row is still Expected (the sync that flips the row to Present
// failed). On a CANCELLED care day the list builder still classifies that child
// as unplanned presence — it reads the visit through presentByGroup — so
// /options must NOT count them as a planned "Abgemeldet" row either. The earlier
// fix keyed only on the row status (Status==Present); this proves the count also
// honours the visit-only presence the builder sees, so the option RowCount
...
func TestListOptions_CancelledCareDayAttendedViaVisitOnlyExcludedFromPlannedCount(t *testing.T) {
	...
	room := testpkg.CreateTestRoom(t, db, fmt.Sprintf("SL-CxvRoom-%d", suffix))
	activity := testpkg.CreateTestActivityGroup(t, db, fmt.Sprintf("SL-CxvAct-%d", suffix))
	staff := testpkg.CreateTestStaff(t, db, "SL-CxvStaff", fmt.Sprintf("CXV-%d", suffix))
	activeGroup := testpkg.CreateTestActiveGroup(t, db, activity.ID, room.ID)
	listKind := activitiesModels.ListKindMensa
	instance := &scheduleModels.ActivityInstance{
		Date:            scheduleModels.Date(listDate),
		...
		RoomID:          room.ID,
		Status:          scheduleModels.InstanceStatusActive, // live: no_show not yet finalized
		ActiveGroupID:   &activeGroup.ID,                     // started, so visits are reachable
		ListKind:        &listKind,
	}
	instance.SetTenantID(testpkg.Tenant(t))
	_, err := db.NewInsert().Model(instance).ModelTableExpr(`schedule.activity_instances`).Exec(ctx)
	...
	// The generated list: attended is unplanned presence (the builder reads the
	// visit), noShow is the Abgemeldet planned row.
	result, err := svc.BuildList(ctx, classday.Params{
		Date:   classday.Date(listDate.String()),
		Target: classday.TargetSlots,
		Source: classday.SourceReconciliation,
	...
	attendedResult := rowByStudent(result.Rows, attended.ID)
	require.NotNil(t, attendedResult, "an attended cancellation stays on the reconciliation list")
	assert.True(t, attendedResult.Present, "the visit records physical presence")
	assert.True(t, attendedResult.Unplanned, "attendance on a cancelled care day is unplanned presence")
	assert.False(t, attendedResult.Planned, "an attended cancellation is not a planned row")
	noShowResult := rowByStudent(result.Rows, noShow.ID)
	require.NotNil(t, noShowResult, "a cancelled no-show stays on the reconciliation list")
	...
	// /options must count exactly the one planned ("Abgemeldet") row the list
	// renders — never the visit-only attended cancellation the list shows as
	// unplanned. Without the visit leg in the count this would report 2.
	options, err := svc.ListOptions(ctx, classday.Date(listDate.String()))
	require.NoError(t, err)
	var mensaOption *classday.ListKindOption
	for i := range options.ListKinds {
	...
	require.NotNil(t, mensaOption, "the Mensa list kind is available")
	assert.Equal(t, 1, mensaOption.RowCount,
		"options planned count must exclude the visit-only attended cancellation and count only the Abgemeldet no-show")
}
...
func TestBuildList_SlotReconciliationExcludesNotYetStartedSlot(t *testing.T) {
	...
	reconStarted, err := svc.BuildList(ctx, classday.Params{
		Date:        classday.Date(pickupDate.String()),
		Target:      classday.TargetSlots,
