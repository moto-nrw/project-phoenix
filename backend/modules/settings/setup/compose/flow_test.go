...
// completed school refuses further wizard writes (ADR 0035).
func TestWizardRunsFromFirstAnswerToCompletion(t *testing.T) {
	...
	assert.False(t, step(t, status, configModel.SetupStepBasics).Done)

	response, status = h.do(t, admin.ID, http.MethodPut, "/basics", map[string]any{
		"presence_mode":   configModel.PresenceModeBinary,
	...
	assert.True(t, step(t, status, configModel.SetupStepBasics).Done)
	assert.False(t, step(t, status, configModel.SetupStepRooms).Applies)
	assert.False(t, step(t, status, configModel.SetupStepGuardians).Applies)

	testpkg.CreateTestStudent(t, h.db, "Wizard", "Kind", "1a")
	response, status = h.do(t, admin.ID, http.MethodGet, "/", nil)
	require.Equal(t, http.StatusOK, response.Code)
	assert.True(t, step(t, status, configModel.SetupStepStudents).Done)
	...
	response, _ = h.do(t, admin.ID, http.MethodPut, "/basics", map[string]any{
		"presence_mode":   configModel.PresenceModeDetailed,
...
func TestWizardRejectsBadRequests(t *testing.T) {
	...
	response, _ = h.do(t, admin.ID, http.MethodPut, "/basics", map[string]any{"presence_mode": configModel.PresenceModeBinary})
