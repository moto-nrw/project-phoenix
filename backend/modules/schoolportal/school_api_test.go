...
// the suite shares.
func setupSchoolRoute(t *testing.T, clocks ...func() time.Time) (*bun.DB, *schoolportal.Resource, int64, string) {
	t.Helper()
...
func newSchoolChiRouter(resource *schoolportal.Resource) chi.Router {
	return resource.Router()
}
...
func registerLehrkraft(t *testing.T, db *bun.DB, resource *schoolportal.Resource, tenantID int64, prefix string) (email string, accountID int64) {
	t.Helper()
