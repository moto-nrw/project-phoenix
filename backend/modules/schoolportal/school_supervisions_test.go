...
type supervisionFixture struct {
	db       *bun.DB
	resource *schoolportal.Resource
	tenantID int64
	router   chi.Router
...
func setupSupervisionFixture(t *testing.T) *supervisionFixture {
	...
	router := schoolportal.NewResource(resource.AuthService, resource.MFAService, classDayResource, newSchoolTimetableResource(db, resource, clock), nil, nil).Router()
