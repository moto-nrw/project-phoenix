...
type OutboxEnqueuer interface {
	Enqueue(ctx context.Context, req ports.EnqueueRequest) (*ports.EmailOutbox, error)
...
func (s *service) notifyGuardians(ctx context.Context, appointment *appointmentcap.Appointment, kind string) error {
	if s.cfg.Outbox == nil {
		return nil
	}
	...
	motoLogoURL := s.cfg.MotoLogoURL(s.cfg.ParentsURL)
	...
	for _, id := range guardianIDs {
		...
		if profile.AccountID != nil && *profile.AccountID > 0 && s.cfg.Preferences != nil {
			notOptedOut, err := s.cfg.Preferences.FilterNotOptedOut(ctx, "parent_appointment", []int64{*profile.AccountID})
			if err != nil {
		...
		if _, err := s.cfg.Outbox.Enqueue(ctx, ports.EnqueueRequest{
			Kind:              kind,
			Payload:           payload,
			RelatedEntityType: ports.EmailRelatedTypeAppointment,
...
func guardianAccountIDs(guardianIDs []int64, profiles map[int64]*ports.GuardianProfile) []int64 {
...
func (s *service) notifyGuardianDevices(ctx context.Context, appointment *appointmentcap.Appointment, kind string) {
	if s.cfg.Notifier == nil {
		return
	}
	appointmentCopy := *appointment
	postCommitCtx := s.cfg.Runtime.WithoutTransactionAndHooks(ctx)
	s.cfg.Runtime.AfterCommit(ctx, func() {
...
func (s *service) dispatchGuardianDevicesAfterCommit(ctx context.Context, appointment *appointmentcap.Appointment, kind string) {
	var accountIDs []int64
	err := s.cfg.Runtime.WithinTenant(ctx, appointment.TenantID, func(txCtx context.Context) error {
		...
		if kind != ports.EmailKindAppointmentCancelled && (current.CancelledAt != nil || !current.NotifyGuardians) {
			return nil
		}
...
func (s *service) dispatchGuardianAccountDevicesLocalized(ctx context.Context, appointment *appointmentcap.Appointment, kind string, accountIDs, studentIDs []int64, locale string) (bool, error) {
	if s.cfg.Notifier == nil || len(accountIDs) == 0 {
		return false, nil
	}
	notificationType := "parent_appointment"
	if kind == ports.EmailKindAppointmentReminder {
		notificationType = "parent_appointment_reminder"
	}
	...
	title, body := s.cfg.NotificationCopy(kind, locale)
	priority := "normal"
	if kind == ports.EmailKindAppointmentCancelled {
		priority = "high"
	}

	err = s.cfg.Notifier.Notify(ctx, ports.Event{
...
func (s *service) dispatchGuardianAccountReminderDevicesLocalized(ctx context.Context, tenantID int64, accountIDs, studentIDs []int64, locale string) (bool, error) {
	...
	title, body := s.cfg.NotificationCopy(ports.EmailKindAppointmentReminder, locale)
	if err := s.cfg.ReminderNotifier.NotifySynchronously(ctx, ports.Event{
		Type: "parent_appointment_reminder", Title: title, Body: body,
		DeepLink: parentCalendarDeepLink, Priority: "normal",
		Audience: ports.Audience{
			TenantID: tenantID, Scope: "guardian",
...
func (s *service) cancelPendingNotifications(ctx context.Context, appointmentID int64, reason string) error {
	if s.cfg.Outbox != nil {
		if _, err := s.cfg.Outbox.CancelPendingByRelatedEntity(ctx, ports.EmailRelatedTypeAppointment, appointmentID, reason); err != nil {
			return err
		}
	}
...
func (s *service) resolveSchoolLogo(ctx context.Context, tenantID int64) string {
	...
	return s.cfg.SchoolLogoURL(s.cfg.ParentsURL, raw)
}

// appointmentWhenText renders a human-readable German date/time line for the
// e-mail (the appointment's first occurrence).
func appointmentWhenText(appointment *appointmentcap.Appointment) string {
