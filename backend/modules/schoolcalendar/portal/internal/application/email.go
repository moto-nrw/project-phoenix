...
type EmailConfig struct {
	// Runtime and Guardians re-check the authorization scope a row carries before it
...
func NewAppointmentRenderer(cfg EmailConfig) func(context.Context, *ports.EmailOutbox) (*ports.Message, error) {
	return func(ctx context.Context, row *ports.EmailOutbox) (*ports.Message, error) {
...
func (cfg EmailConfig) ensureGuardianStillPermitted(ctx context.Context, row *ports.EmailOutbox) error {
	...
	if cfg.Runtime == nil || cfg.Guardians == nil || tenantID <= 0 {
		...
		return fmt.Errorf("%w: cannot re-check guardian child access for %s",
			cfg.RenderCancelled, row.Kind)
	}

	var permitted []int64
	if err := cfg.Runtime.WithinTenant(ctx, tenantID, func(txCtx context.Context) error {
		allowed, err := cfg.Guardians.FilterAccountsWithStudentAccess(
			txCtx, []int64{accountID}, studentIDs, tenantID, "parent_portal.access")
		if err != nil {
			return err
		}
		permitted = allowed
...
func appointmentCopy(kind, schoolName string) (kicker, subjectVerb, intro string) {
	...
	switch kind {
	case ports.EmailKindAppointmentUpdated:
		if known {
	...
	case ports.EmailKindAppointmentCancelled:
		if known {
	...
	case ports.EmailKindAppointmentReminder:
		if known {
