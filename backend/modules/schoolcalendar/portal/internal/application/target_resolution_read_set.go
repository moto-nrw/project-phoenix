...
type targetResolutionReadSet struct {
	...
	linksByGuardian   map[int64][]*ports.StudentGuardian
	linksByStudent    map[int64][]*ports.StudentGuardian
	allSchoolStudents []*ports.Student
	studentsByGroup   map[int64][]*ports.Student
	studentsByClass   map[string][]*ports.Student
}
...
func newTargetResolutionReadSet() *targetResolutionReadSet {
	return &targetResolutionReadSet{
		...
		linksByGuardian: make(map[int64][]*ports.StudentGuardian),
		linksByStudent:  make(map[int64][]*ports.StudentGuardian),
		studentsByGroup: make(map[int64][]*ports.Student),
		studentsByClass: make(map[string][]*ports.Student),
	}
}
...
func (c *targetSelectionCollector) add(target AppointmentTarget) error {
	...
	case appointmentcap.TargetTypeStaff, appointmentcap.TargetTypeGuardianProfile,
		appointmentcap.TargetTypeParentsByStudent, appointmentcap.TargetTypeParentsByGroup:
...
func (c *targetSelectionCollector) addID(targetType string, id *int64) error {
	labels := map[string]string{
		appointmentcap.TargetTypeStaff:            "staff",
		appointmentcap.TargetTypeGuardianProfile:  "guardian",
		appointmentcap.TargetTypeParentsByStudent: "student",
		appointmentcap.TargetTypeParentsByGroup:   "group",
	}
	if id == nil || *id <= 0 {
...
func targetStudentIDs(
	selection targetSelection,
	allSchoolStudents, groupStudents []*ports.Student,
	studentsByClass map[string][]*ports.Student,
) []int64 {
	ids := append([]int64(nil), selection.studentIDs...)
	seen := int64SliceSet(ids)
	addActive := func(students []*ports.Student) {
