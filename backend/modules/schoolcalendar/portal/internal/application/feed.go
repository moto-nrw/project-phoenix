...
// Feed window: how far back / forward the subscription includes appointmentcap.
...
type FeedAccountRepo = ports.Accounts
...
func (s *service) ParentCalendarFeedURL(ctx context.Context, accountID int64) (string, string, error) {
	...
	return s.coalesceFeedCreation(fmt.Sprintf("parent:%d", accountID), func() (string, string, error) {
		...
		token, err := s.cfg.NewFeedToken()
		if err != nil {
			return "", "", err
		}
		persisted, err := s.cfg.AccountRepo.EnsureCalendarFeedToken(ctx, accountID, s.feedTokenHash(token))
		if err != nil {
			return "", "", err
		}
		if persisted != s.feedTokenHash(token) {
...
func (s *service) RotateParentCalendarFeed(ctx context.Context, accountID int64) (string, string, error) {
	...
	if err := s.cfg.AccountRepo.SetCalendarFeedToken(ctx, accountID, s.feedTokenHash(token)); err != nil {
		return "", "", err
	}
	httpsURL, webcalURL := feedURLs(s.cfg.ParentsURL, token)
	return httpsURL, webcalURL, nil
...
func (s *service) StaffCalendarFeedURL(ctx context.Context) (string, string, error) {
	...
	return s.coalesceFeedCreation(fmt.Sprintf("staff:%d:%d", owner.accountID, owner.tenantID), func() (string, string, error) {
		token, err := s.cfg.NewFeedToken()
		if err != nil {
			return "", "", err
		}
		persisted, err := s.cfg.StaffFeedRepo.EnsureToken(ctx, owner.accountID, owner.tenantID, s.feedTokenHash(token))
		if err != nil {
			return "", "", err
		}
		if persisted == "" {
...
func (s *service) RotateStaffCalendarFeed(ctx context.Context) (string, string, error) {
	...
	updated, err := s.cfg.StaffFeedRepo.RotateToken(ctx, owner.accountID, owner.tenantID, s.feedTokenHash(token))
	if err != nil {
		return "", "", err
	}
	if !updated {
...
func (s *service) currentStaffFeedOwner(ctx context.Context) (staffFeedOwner, error) {
	...
	if err != nil {
		if s.cfg.UserContext.MissingUser(err) {
			return staffFeedOwner{}, fmt.Errorf("%w: current account required", ErrForbidden)
		}
	...
	if err != nil {
		if s.cfg.UserContext.MissingStaff(err) {
			return staffFeedOwner{}, fmt.Errorf("%w: current staff required", ErrForbidden)
		}
		return staffFeedOwner{}, err
	}
	if staff == nil {
	...
	tenantID := s.cfg.Runtime.TenantID(ctx)
	if tenantID <= 0 {
		return staffFeedOwner{}, fmt.Errorf("%w: tenant is required", ErrForbidden)
	}
	return staffFeedOwner{accountID: account.ID, tenantID: tenantID, email: account.Email}, nil
}

func (s *service) StaffCalendarFeedByToken(ctx context.Context, token string) (string, string, error) {
	owner, err := s.staffCalendarOwnerByToken(ctx, token)
	if err != nil {
		return "", "", err
	}
...
// projectStaffCalendarEvents is the single privacy and visibility projection
// shared by iCalendar and CalDAV. Both transports therefore expose the same
// UIDs, recurrence rules, tombstones, shifts, and timetable assignments.
func (s *service) projectStaffCalendarEvents(ctx context.Context, owner staffFeedOwner) ([]CalendarEvent, error) {
	if s.cfg.StaffFeedTombstoneRepo == nil || s.cfg.PersonRepo == nil || s.cfg.StaffRepo == nil || s.cfg.CalendarRenderer == nil {
		return nil, fmt.Errorf("%w: staff calendar feed not configured", ErrInvalidRequest)
	}
	var events []CalendarEvent
	err := s.cfg.Runtime.WithinTenant(ctx, owner.tenantID, func(txCtx context.Context) error {
		...
		recurrenceByID := make(map[int64]*appointmentcap.RecurrenceRule, len(recurrences))
		...
		overridesByID := make(map[int64][]*appointmentcap.AppointmentOccurrenceOverride, len(cancelledOverrides))
		...
		tombstoneRecurrenceByID := make(map[int64]*appointmentcap.RecurrenceRule, len(tombstoneRecurrences))
...
func staffFeedTombstoneEvent(tombstone portal.FeedCancellation) Event {
	return Event{
		ID:         fmt.Sprintf("%s:%d", tombstone.Source, tombstone.SourceID),
		Source:     tombstone.Source,
		Title:      tombstone.Title,
		StartDate:  tombstone.EventDate,
		EndDate:    tombstone.EventDate,
...
func staffCalendarICSEvent(tenantID int64, event Event) (CalendarEvent, error) {
	startDate, err := appointmentcap.ParseDate(event.StartDate)
	if err != nil {
		return CalendarEvent{}, fmt.Errorf("parse staff calendar start date: %w", err)
	}
	endDate, err := appointmentcap.ParseDate(event.EndDate)
	if err != nil {
...
func staffCalendarClock(value string) (time.Time, error) {
	...
	return normalizeWallClock(time.Date(2000, time.January, 1, hour, minute, 0, 0, time.UTC)), nil
}
...
func (s *service) ParentCalendarFeedByToken(ctx context.Context, token string) (string, string, error) {
	...
	account, err := s.cfg.AccountRepo.FindByCalendarFeedToken(ctx, s.feedTokenHash(token))
	if err != nil {
		return "", "", err
	}
	if account == nil {
