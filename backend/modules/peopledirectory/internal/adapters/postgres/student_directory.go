...
func (s *StudentStore) ListDirectory(
	...
	query := applyStudentDirectoryFilter(
		withStudentTenant(db.NewSelect().TableExpr(`users.students AS "student"`).
	...
	if err != nil {
		return nil, stats, fmt.Errorf("people directory postgres: list student directory: %w", err)
	}
...
func (s *StudentStore) CountDirectory(
	...
	query := applyStudentDirectoryFilter(
		withStudentTenant(db.NewSelect().TableExpr(`users.students AS "student"`), tenantID),
		filter,
	)
	stats := domain.OperationStats{Queries: 1}
	started := time.Now()
	...
	stats.StatementDuration = time.Since(started)
	if err != nil {
		return 0, stats, fmt.Errorf("people directory postgres: count student directory: %w", err)
	}
...
func (s *StudentStore) ListDirectoryIDs(ctx context.Context) ([]int64, domain.OperationStats, error) {
	...
	ids := []int64{}
	query := withStudentTenant(db.NewSelect().TableExpr(`users.students AS "student"`), tenantID).
	...
	stats.StatementDuration = time.Since(started)
	if err != nil {
		return nil, stats, fmt.Errorf("people directory postgres: list student directory ids: %w", err)
	}
...
func applyStudentDirectoryFilter(query *bun.SelectQuery, filter domain.StudentDirectoryFilter) *bun.SelectQuery {
