...
func TestBuildDayLogResponse_OmitsStudentBeforeScheduledArrival(t *testing.T) {
	...
	response := buildDayLogResponse(date, []*SchoolGroup{group}, &dayLogData{
		studentsByGroup:     map[int64][]*Student{group.ID: {student}},
...
func TestBuildDayLogResponse_SkipsDerivedVerdictBeforeEnrollmentStart(t *testing.T) {
	...
	notYetStarted := &Student{Status: studentStatusActive, EnrolledFrom: &startsLater}
	notYetStarted.ID = 10
	notYetStarted.GroupID = &group.ID
	checkedIn := &Student{Status: studentStatusActive, EnrolledFrom: &startsLater}
	checkedIn.ID = 11
	checkedIn.GroupID = &group.ID

	response := buildDayLogResponse(date, []*SchoolGroup{group}, &dayLogData{
		studentsByGroup: map[int64][]*Student{group.ID: {notYetStarted, checkedIn}},
		persons:         map[int64]*peopleModule.Person{},
		attendanceByStudent: map[int64][]*studentpresence.Attendance{
...
func TestLoadDayLogData_PassesFrozenDayToRosterEligibility(t *testing.T) {
	...
	rs.Persons = personRecordsFake{
		listStudentsForDay: func(_ context.Context, _ []int64, date, today timezone.Date) ([]peopleModule.StudentRecord, error) {
