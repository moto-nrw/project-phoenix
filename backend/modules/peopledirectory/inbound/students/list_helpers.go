...
type studentListParams struct {
	...
	// reads the same first digit run schoolClassGrade reads in Go — a
...
func (p *studentListParams) pageOfGroupStudents(students []*Student) []*Student {
...
func (p *studentListParams) buildDirectoryFilter() peopleModule.StudentDirectoryFilter {
	return peopleModule.StudentDirectoryFilter{
		...
		CareStatusOn: renderCalendarDate(&p.careStatusOn),
	}
}
...
func (p *studentListParams) directoryCareStatus() string {
	if p.careStatus == CareStatusRunning {
		return peopleModule.StudentCareStatusAll
	}
	return p.careStatus
}
...
// the whole selection.
func (p *studentListParams) buildPagedDirectoryFilter() peopleModule.StudentDirectoryFilter {
...
func collectIDsFromStudents(students []*Student) (studentIDs, personIDs, groupIDs []int64) {
...
func matchesSearchFilter(person *peopleModule.Person, studentID int64, search string) bool {
	if search == "" {
		return true
	}
...
func matchesNameFilters(person *peopleModule.Person, firstName, lastName string) bool {
...
// Uses schoolClassGrade rather than a naive string-prefix/LIKE check
...
func matchesGradeLevel(schoolClass string, gradeLevels []int) bool {
	...
	prefix := schoolClassGrade(schoolClass)
...
func careStatusFromRequest(r *http.Request) (string, render.Renderer) {
	...
	canReadEnded := securityruntime.HasPermission(permissions.UsersDelete, jwt.PermissionsFromCtx(r.Context()))
...
// schoolClassKey is the matching identity of a class name, trimmed and
// lowercased, as School Structure compares classes
// (internal/schoolclass.Normalize).
func schoolClassKey(class string) string {
