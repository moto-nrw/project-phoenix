...
func TestStudentResponsePropagatesPresenceModeFailure(t *testing.T) {
	...
	_, err := newStudentResponseWithOpts(context.Background(), StudentResponseOpts{Student: &Student{}}, StudentResponseServices{ActiveService: failedPresenceMode{err: injected}})
...
func TestStudentResponsePropagatesLocationGroupReadFailure(t *testing.T) {
	...
	_, err := newStudentResponseWithOpts(testpkg.Ctx(t), StudentResponseOpts{Student: &Student{ID: student.ID, PersonID: student.PersonID}, HasFullAccess: true}, StudentResponseServices{ActiveService: svc})
	require.ErrorIs(t, err, injected)
}
...
func TestStudentResponseDistinguishesLocationReadFailureFromMissingVisit(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			response, err := newStudentResponseWithOpts(context.Background(), StudentResponseOpts{Student: &Student{}, HasFullAccess: true}, StudentResponseServices{ActiveService: tc.svc})
			if tc.wantErr != nil {
...
func TestPopulatePublicStudentFields_PreservesNonCanonicalPickupStatus(t *testing.T) {
	...
	assert.Equal(t, departure.DepartureBus, resp.DepartureDays.ModeFor(departure.PickupDayMonday))
	assert.True(t, resp.BusDays[departure.PickupDayMonday])
...
func TestPopulateSnapshotPublicFields_PreservesNonCanonicalPickupStatus(t *testing.T) {
	...
	assert.Equal(t, status, resp.PickupStatus)
	assert.Equal(t, departure.DeparturePickup, resp.DepartureDays.ModeFor(departure.PickupDayWednesday))
	assert.True(t, resp.PickupDays[departure.PickupDayWednesday])
}
...
func TestResponsePickupStatus_UsesDerivedStatusForCanonicalStoredText(t *testing.T) {
	...
	assert.Equal(t, departure.PickupStatusGoesAlone, resp.PickupStatus)
...
func TestPopulatePublicStudentFields_BusAndAccompaniedSameDayKeepsAccompanied(t *testing.T) {
	...
	student := &Student{
		...
		AllowedDepartureModes: departure.AllowedDepartureModes{
			departure.PickupDayMonday: {departure.DepartureBus, departure.DepartureAccompanied},
		},
	}
	resp := StudentResponse{}
	...
	assert.Equal(t, departure.PickupStatusAccompanied, resp.PickupStatus,
		"bus+accompanied on the same day must not be bucketed as a self-goer")
	assert.True(t, resp.BusDays[departure.PickupDayMonday], "the bus signal must still surface")
}
...
func TestPopulateSnapshotPublicFields_BusAndAccompaniedSameDayKeepsAccompanied(t *testing.T) {
	...
	student := &Student{
		PickupStatus: &stored,
		AllowedDepartureModes: departure.AllowedDepartureModes{
			departure.PickupDayTuesday: {departure.DepartureBus, departure.DepartureAccompanied},
		},
	}
	resp := StudentResponse{}
	...
	assert.Equal(t, departure.PickupStatusAccompanied, resp.PickupStatus,
		"bus+accompanied on the same day must not be bucketed as a self-goer")
	assert.True(t, resp.BusDays[departure.PickupDayTuesday], "the bus signal must still surface")
	assert.True(t, resp.DepartureRuleConfigured)
}
...
func makeStudent(id int64, photoPath *string, consentAt *time.Time, consentBy *int64) *Student {
	s := &Student{
		PhotoPath:           photoPath,
