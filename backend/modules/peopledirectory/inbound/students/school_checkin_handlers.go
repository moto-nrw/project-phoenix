...
func (rs *Resource) schoolCheckinBatchHandler(w http.ResponseWriter, r *http.Request) {
	...
	req, studentIDs, refusal := decodeSchoolCheckinBatch(w, r)
	if refusal != nil {
		common.RenderError(w, r, refusal)
		return
	}
...
// decodeSchoolCheckinBatch reads and validates the batch body: a bounded JSON
// value with nothing after it, a known action and a non-empty, capped,
// deduplicated id list. It returns the refusal to render on bad input.
func decodeSchoolCheckinBatch(w http.ResponseWriter, r *http.Request) (schoolCheckinBatchRequest, []int64, render.Renderer) {
	...
	if err := dec.Decode(&req); err != nil {
		return schoolCheckinBatchRequest{}, nil, common.ErrorInvalidRequest(err)
	}
	...
	if err := dec.Decode(&struct{}{}); !errors.Is(err, io.EOF) {
		return schoolCheckinBatchRequest{}, nil, common.ErrorInvalidRequest(errors.New("unexpected trailing data after JSON body"))
	}
	if req.Action != schoolCheckinActionIn && req.Action != schoolCheckinActionOut {
		return schoolCheckinBatchRequest{}, nil, common.ErrorInvalidRequest(errors.New(`action must be "in" or "out"`))
	}
	...
	if len(req.StudentIDs) > maxSchoolCheckinBatchSize {
		return schoolCheckinBatchRequest{}, nil, common.ErrorInvalidRequest(errors.New("too many students in one batch"))
	}
	parsedIDs, parseErr := parseStudentIDStrings(req.StudentIDs)
	if parseErr != nil {
		return schoolCheckinBatchRequest{}, nil, common.ErrorInvalidRequest(parseErr)
	}
	studentIDs := dedupeStudentIDs(parsedIDs)
	if len(studentIDs) == 0 {
		return schoolCheckinBatchRequest{}, nil, common.ErrorInvalidRequest(errors.New("student_ids must not be empty"))
	}
	return req, studentIDs, nil
}
