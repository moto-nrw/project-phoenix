...
func (rs *Resource) checkAttendanceExportAccess(w http.ResponseWriter, r *http.Request, student *Student) bool {
	logger := rs.attendanceHistoryLogger()
...
func attendanceExportRows(slots []*studentpresence.HistorySlot, attendanceRows []*studentpresence.Attendance) []lists.Row {
	...
	for _, row := range slots {
		...
		coverage := slotCoverage{
			present:    row.Attendance.Status == studentpresence.SessionAttendancePresent,
