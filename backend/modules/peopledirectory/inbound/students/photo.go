...
func (rs *Resource) uploadStudentPhoto(w http.ResponseWriter, r *http.Request) {
	...
	if err := rs.commitStudentPhoto(r.Context(), id, newStoredURL, consentAck); err != nil {
...
func (rs *Resource) deleteStudentPhoto(w http.ResponseWriter, r *http.Request) {
	...
	clearedURL, err := rs.clearStudentPhoto(r.Context(), id)
	if err != nil {
...
// commitStudentPhoto stores an already-saved upload as the child's photo.
func (rs *Resource) commitStudentPhoto(ctx context.Context, studentID int64, storedURL string, consentAck bool) error {
	if err := requirePhotoTenant(ctx); err != nil {
		return err
	}
	return rs.StudentPhotos.CommitStudentPhoto(ctx, studentID, storedURL, consentAck)
}

// clearStudentPhoto detaches the child's photo and returns the stored URL it
// cleared, empty when none was set.
func (rs *Resource) clearStudentPhoto(ctx context.Context, studentID int64) (string, error) {
	if err := requirePhotoTenant(ctx); err != nil {
		return "", err
	}
