	...
	students := []StudentResponse{
		{
			...
			AllowedDepartureModes:   departure.AllowedDepartureModes{departure.PickupDayWednesday: {departure.DepartureAlone}},
		...
		},
		{
			...
			AllowedDepartureModes:   departure.AllowedDepartureModes{departure.PickupDayWednesday: {departure.DeparturePickup}},
			DepartureRuleConfigured: true,
			HasFullAccess:           true,
		},
		{
...
func TestPopulateExportPhotoConsentFilterDataSupportsFeatureOffResponses(t *testing.T) {
	...
	students := []*Student{
		{ID: 101, PhotoConsentGivenAt: &now},
		{ID: 102},
	}
...
func TestWeeklyCellUsesExplicitLabels(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			plan: weeklySchedule{
				ArrivalByWeekday: map[int]string{weekdayMonday: "08:00"},
				PickupByWeekday:  map[int]string{weekdayMonday: "16:00"},
			},
		...
		},
		{
			...
			plan: weeklySchedule{
				ArrivalByWeekday:  map[int]string{},
				CareDaysByWeekday: map[int]bool{weekdayMonday: true},
				PickupByWeekday:   map[int]string{},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := weeklyCell(tt.plan, weekdayMonday); got != tt.want {
...
func TestCareDaysIncludesArrivalWithoutTime(t *testing.T) {
	...
	plan := weeklySchedule{
		ArrivalByWeekday:  map[int]string{},
		CareDaysByWeekday: map[int]bool{weekdayWednesday: true},
		PickupByWeekday:   map[int]string{},
	}
...
func TestDepartureExportCell(t *testing.T) {
	...
	t.Run("falls back to the legacy departure days for the accompanied check", func(t *testing.T) {
		got := departureExportCell(StudentResponse{
			DepartureDays: departure.DepartureDays{
				departure.PickupDayTuesday: departure.DepartureAccompanied,
			},
	...
	t.Run("names the structured companion links", func(t *testing.T) {
		got := departureExportCell(StudentResponse{
			...
			DepartureCompanions: []departure.CompanionLink{
				{CompanionStudentID: 7, FirstName: "Mia", LastName: "Schulz", Weekdays: []string{departure.PickupDayMonday}},
			},
		})
	...
	t.Run("renders links and note together", func(t *testing.T) {
		got := departureExportCell(StudentResponse{
			AllowedDepartureModes: departure.AllowedDepartureModes{
				departure.PickupDayMonday:  []departure.DepartureMode{departure.DepartureAccompanied},
				departure.PickupDayTuesday: []departure.DepartureMode{departure.DepartureAccompanied},
			},
			DepartureCompanions: []departure.CompanionLink{
				{CompanionStudentID: 7, FirstName: "Mia", LastName: "Schulz", Weekdays: []string{departure.PickupDayMonday}},
			},
	...
	tests := []struct {
	...
	}{
		{
			...
			days: departure.DepartureDays{
				...
				departure.PickupDayThursday:  departure.DepartureAlone,
			},
		...
		},
		{
			...
			allowed: departure.AllowedDepartureModes{
				departure.PickupDayMonday:  []departure.DepartureMode{departure.DepartureBus, departure.DeparturePickup},
				departure.PickupDayTuesday: []departure.DepartureMode{departure.DepartureAlone},
			},
			want: "Mo: Bus, Abholung, Di: zu Fuß",
		},
		{name: "empty plan", days: departure.DepartureDays{}, want: "Geht alleine"},
