...
func (rs *Resource) getStudentAttendanceHistory(w http.ResponseWriter, r *http.Request) {
	...
	// 3.+4. Resolve the caps and the requested range clamped against them
	window, err := rs.resolveAttendanceHistoryWindow(r, logger)
	if err != nil {
	...
	// 5.-7. Load the rows and assemble the per-day response
	resp, refusal := rs.buildAttendanceHistory(ctx, logger, student.ID, window)
	if refusal != nil {
		renderError(w, r, refusal)
		return
	}
...
// buildAttendanceHistory loads the child's attendance rows for the window
// and assembles the per-day response.
func (rs *Resource) buildAttendanceHistory(ctx context.Context, logger *slog.Logger, studentID int64, window attendanceHistoryWindow) (attendanceHistoryResponse, render.Renderer) {
	start, end, today, endOfToday := window.start, window.end, window.today, window.endOfToday
	attendanceCap, roomCap, clamped := window.attendanceCap, window.roomCap, window.clamped
	...
	sources, err := rs.loadAttendanceHistorySources(ctx, studentID, startDay, endDay)
	if err != nil {
		logger.Error("attendance history source query failed",
			slog.Int64("student_id", studentID),
			slog.String("error", err.Error()),
		)
		return attendanceHistoryResponse{}, common.ErrorInternalServerWrap("failed to load attendance history", err)
	}

	// 6. Conditionally load visits for days within the room-detail cap
	roomCutoff := today.AddDate(0, 0, -(roomCap - 1))
	visitsByDate, err := rs.loadRoomVisitsByDate(ctx, studentID, start, end, endOfToday, roomCutoff, roomCap)
	if err != nil {
		return attendanceHistoryResponse{}, common.ErrorInternalServerWrap("failed to load attendance history", err)
	}

	// 7. Assemble per-day response
	days := buildAttendanceHistoryDays(sources.Attendance, sources.Statuses, visitsByDate, roomCutoff)
	days = attachSlotAttendance(days, sources.Slots, visitsByDate, roomCutoff)
...
// attendanceHistoryWindow is the range an attendance history read covers and
// the caps it was clamped against.
type attendanceHistoryWindow struct {
	attendanceCap int
	roomCap       int
	today         time.Time
...
// resolveAttendanceHistoryWindow resolves the tenant's visibility caps and
// clamps the requested range against them.
func (rs *Resource) resolveAttendanceHistoryWindow(r *http.Request, logger *slog.Logger) (attendanceHistoryWindow, error) {
	ctx := r.Context()
	...
	if err != nil {
		return attendanceHistoryWindow{}, err
	}
	start, end, clamped, err := clampAttendanceHistoryRange(start, end, endOfToday, attendanceCap)
	if err != nil {
		return attendanceHistoryWindow{}, err
	}
	return attendanceHistoryWindow{
		attendanceCap: attendanceCap, roomCap: roomCap,
		today: today, endOfToday: endOfToday,
		start: start, end: end, clamped: clamped,
	}, nil
}
...
func hasPlannedSlotRow(slots []*studentpresence.HistorySlot) bool {
	for _, row := range slots {
		if row != nil && row.Instance != nil && row.Attendance != nil &&
			!row.Attendance.IsUnplanned && row.Instance.Status != studentpresence.HistoryInstanceCancelled {
			return true
		}
	}
	return false
...
func (rs *Resource) attendanceHistoryScopeAllows(r *http.Request, student *Student) bool {
	return securityruntime.CanReadStudent(r.Context(), jwt.PermissionsFromCtx(r.Context()), student, rs.UserContextService)
}
...
func buildAttendanceHistoryDays(rows []*studentpresence.Attendance, statusRows []*absencerecords.StudentStatusDay, visitsByDate map[string][]*studentpresence.VisitHistoryEntry, roomCutoff time.Time) []attendanceHistoryDay {
	...
	return assembleAttendanceHistoryDays(dayMap, dayOrder, visitsByDate, roomCutoff)
}
...
func assembleAttendanceHistoryDays(dayMap map[string]*attendanceHistoryDay, dayOrder []string, visitsByDate map[string][]*studentpresence.VisitHistoryEntry, roomCutoff time.Time) []attendanceHistoryDay {
	...
	for _, dateKey := range dayOrder {
		...
		// (all required visit reads have already succeeded).
		date, _ := time.Parse("2006-01-02", dateKey)
		if !timezone.DateOf(date).Before(timezone.DateOf(roomCutoff)) {
...
func attachSlotAttendance(
	...
	for _, row := range rows {
		...
		if !ok {
			...
			if !row.Instance.Date.Before(cutoffDay) {
				day.RoomDetailAvailable = true
...
func slotEntryCoverages(slots []attendanceSlotEntry) []slotCoverage {
	...
	for _, slot := range slots {
		coverage := slotCoverage{
			present:    slot.Status == studentpresence.SessionAttendancePresent,
