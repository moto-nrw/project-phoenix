...
// seedExistingGuardian inserts a guardian profile in tenant 1 and returns its
// ID, registering its cleanup. Used by the select-existing (#1513) tests to
// stand in for a sibling's already-existing guardian.
func seedExistingGuardian(t *testing.T, tc *testContext, firstName, lastName, email string) int64 {
	t.Helper()
	ctx := context.Background()

	g := testpkg.CreateTestGuardianProfileWithExactEmail(t, tc.db, testpkg.Tenant(t), testpkg.GuardianProfileWithEmail{
		FirstName:              firstName,
		LastName:               lastName,
		Email:                  email,
...
func TestCreateStudent_SelectExistingGuardian(t *testing.T) {
	...
	existingID := seedExistingGuardian(t, tc, "Sibling", "Parent", "sibling.parent.existing@example.com")
	...
	body := map[string]interface{}{
		...
		"guardians": []map[string]interface{}{
			{
				"guardian_profile_id":  existingID,
...
func TestCreateStudent_SelectExistingGuardian_DuplicateSkipped(t *testing.T) {
	...
	existingID := seedExistingGuardian(t, tc, "Dup", "Selection", "dup.selection.existing@example.com")
	...
	body := map[string]interface{}{
		...
		"school_class": "3b",
		"guardians": []map[string]interface{}{
			{"guardian_profile_id": existingID, "relationship_type": "parent", "emergency_priority": 1},
			{"guardian_profile_id": existingID, "relationship_type": "guardian", "emergency_priority": 2},
		},
	}
...
func TestCreateStudent_MixedNewAndExistingGuardian(t *testing.T) {
	...
	existingID := seedExistingGuardian(t, tc, "Mixed", "Existing", "mixed.existing@example.com")
	...
	body := map[string]interface{}{
		...
		"school_class": "3c",
		"guardians": []map[string]interface{}{
			{"guardian_profile_id": existingID, "relationship_type": "parent", "is_primary": true, "emergency_priority": 1},
			{
				"first_name":         "Brand",
				"last_name":          "New",
