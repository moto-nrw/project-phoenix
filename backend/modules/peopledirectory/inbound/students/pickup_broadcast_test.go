...
// payload must not name the child. An id in that raw SSE stream would name the
// child to every connected client of the tenant, including sessions that may
// not read them. The id-less event makes clients re-check their pickup caches
// broadly; every refetch is server-access-filtered, so no unauthorized data
// comes back.
func requirePickupEventsCarryNoStudentID(t *testing.T, b *testpkg.RecordingBroadcaster) {
	...
	for _, c := range b.CallsByMethod("tenant") {
		...
		assert.Nil(t, c.Event.Data.StudentID,
			"a tenant-wide pickup_schedule_changed must not name the child (GDPR)")
	}
}
...
func deletePickupSchedules(t *testing.T, tc *testContext, studentID int64) {
	t.Helper()
	_, _ = tc.db.NewDelete().TableExpr("schedule.student_pickup_schedules").
...
func deletePickupExceptions(t *testing.T, tc *testContext, studentID int64) {
	t.Helper()
	_, _ = tc.db.NewDelete().TableExpr("schedule.student_pickup_exceptions").
		Where("student_id = ?", studentID).
		Exec(context.Background())
}
...
func latestPickupExceptionID(t *testing.T, tc *testContext, studentID int64) int64 {
	...
	err := tc.db.NewSelect().
		TableExpr(`schedule.student_pickup_exceptions AS "student_pickup_exception"`).
		Column("id").
		Where("student_id = ?", studentID).
		Order("id DESC").
		Limit(1).
		Scan(context.Background(), &exceptionID)
