...
func (rs *Resource) redactUnreadableCompanionNames(r *http.Request, links []departure.CompanionLink) ([]departure.CompanionLink, error) {
	if len(links) == 0 {
		return links, nil
	}
...
func (rs *Resource) redactCompanionNames(ctx context.Context, accessCtx *studentAccessContext, linkSets ...[]departure.CompanionLink) error {
	...
	companions, err := rs.studentsByIDs(ctx, ids)
	if err != nil {
		return err
	}
	...
	for _, links := range linkSets {
		for i := range links {
			...
			if hasFullAccessToStudent(accessCtx, companions[links[i].CompanionStudentID]) {
				continue
			}
			links[i].FirstName = ""
			links[i].LastName = ""
...
func (rs *Resource) lockCompanionRows(ctx context.Context, student *Student, req *UpdateStudentRequest) error {
...
func (rs *Resource) lockStudentCompanionGraph(ctx context.Context, studentID int64, submitted []int64) error {
	return rs.CompanionService.LockCompanionGraph(ctx, []int64{studentID}, submitted)
}
...
func (rs *Resource) checkCompanionConflicts(ctx context.Context, student *Student, req *UpdateStudentRequest, userPermissions []string) (map[int64]map[string]bool, error) {
	accompaniedDays := departure.Plan{AllowedDepartureModes: student.AllowedDepartureModes, DepartureDays: student.DepartureDays}.AccompaniedDays()
	...
	// written.
	if !req.hasCompanionUpdate() {
		return nil, rs.CompanionService.CheckCompanionTrimForPlan(ctx, student.ID, accompaniedDays, req.CompanionsFingerprint)
	}
...
func (rs *Resource) applyCompanionUpdate(ctx context.Context, student *Student, req *UpdateStudentRequest, authorizedExtensions map[int64]map[string]bool) (bool, error) {
	...
	changed := departure.CompanionLinksFingerprint(current) != departure.CompanionLinksFingerprint(links) ||
...
func (rs *Resource) reconcileCompanions(ctx context.Context, student *Student, req *UpdateStudentRequest, authorizedExtensions map[int64]map[string]bool) ([]departure.CompanionLink, error) {
	accompaniedDays := departure.Plan{AllowedDepartureModes: student.AllowedDepartureModes, DepartureDays: student.DepartureDays}.AccompaniedDays()
	...
	if !req.hasCompanionUpdate() {
		...
		student.DepartureCompanionDays = departure.CompanionDaysFromLinks(links)
		return links, nil
	}
