...
func TestBuildAttendanceHistoryDays_EmptyRows(t *testing.T) {
	...
	days := buildAttendanceHistoryDays(nil, nil, nil, time.Now())
	assert.Empty(t, days)
}
...
func TestAttachSlotAttendance_KeepsOpposingStatusesOnSameDay(t *testing.T) {
	...
	days := attachSlotAttendance(nil, []*studentpresence.HistorySlot{
		{Instance: morning, Attendance: &studentpresence.HistorySlotAttendance{Status: studentpresence.SessionAttendancePresent}},
		{Instance: afternoon, Attendance: &studentpresence.HistorySlotAttendance{Status: studentpresence.SessionAttendanceAbsent, Substatus: &sick}},
	...
	assert.Equal(t, studentpresence.SessionAttendancePresent, days[0].Slots[0].Status)
	assert.Equal(t, "Nachmittagsbetreuung", days[0].Slots[1].Title)
	assert.Equal(t, studentpresence.SessionAttendanceAbsent, days[0].Slots[1].Status)
	require.NotNil(t, days[0].Slots[1].Substatus)
	assert.Equal(t, timetable.SlotSubstatusSick, *days[0].Slots[1].Substatus)
}
...
func TestAttachSlotAttendance_SlotOnlyDayRespectsRoomRetention(t *testing.T) {
	...
	rows := []*studentpresence.HistorySlot{
		{Instance: instance, Attendance: &studentpresence.HistorySlotAttendance{Status: studentpresence.SessionAttendancePresent}},
	}

	t.Run("outside window stays unavailable", func(t *testing.T) {
		cutoff := date.AddDays(2).BerlinMidnight()
		days := attachSlotAttendance(nil, rows, nil, cutoff)
		require.Len(t, days, 1)
...
func TestAttachUnassignedAttendance_WindowMatchAndNeutralLeftover(t *testing.T) {
	...
	days := attachUnassignedAttendance([]attendanceHistoryDay{{
		...
		Slots: []attendanceSlotEntry{{
			...
			Status: studentpresence.SessionAttendancePresent, CheckedInAt: &reentry,
		}},
	}}, true)
...
func TestAttachUnassignedAttendance_AbsentSlotDoesNotAbsorbSessions(t *testing.T) {
	...
	days := attachUnassignedAttendance([]attendanceHistoryDay{{
		...
		Slots: []attendanceSlotEntry{{
			...
			Status: studentpresence.SessionAttendanceAbsent, Substatus: &sick,
		}},
	}}, true)
	...
	days := attachUnassignedAttendance([]attendanceHistoryDay{
		{
			...
			Slots: []attendanceSlotEntry{{
				...
				StartTime: "07:00", EndTime: "12:00",
				Status: studentpresence.SessionAttendancePresent, CheckedInAt: &planned,
			}},
		},
		{
			Date: "2026-07-14",
...
func TestAttachUnassignedAttendance_WalkInWithoutExpectationKeepsItsSlotOnly(t *testing.T) {
	...
	days := attachUnassignedAttendance([]attendanceHistoryDay{{
		...
		Slots: []attendanceSlotEntry{{
			...
			Status: studentpresence.SessionAttendancePresent, CheckedInAt: &walkIn,
			IsUnplanned: true,
		}},
	}}, false)
...
func TestHasPlannedSlotRow(t *testing.T) {
	...
	assert.False(t, hasPlannedSlotRow([]*studentpresence.HistorySlot{{
		...
		Attendance: &studentpresence.HistorySlotAttendance{Status: studentpresence.SessionAttendancePresent, IsUnplanned: true},
	}}), "walk-in rows must not count as care-plan evidence")
	assert.False(t, hasPlannedSlotRow([]*studentpresence.HistorySlot{{
		Instance:   &studentpresence.HistorySlotInstance{Date: timezone.Date(date), Title: "Ausgefallene AG", Status: timetable.InstanceStatusCancelled},
		Attendance: &studentpresence.HistorySlotAttendance{Status: studentpresence.SessionAttendanceExpected},
	...
	assert.True(t, hasPlannedSlotRow([]*studentpresence.HistorySlot{{
		...
		Attendance: &studentpresence.HistorySlotAttendance{Status: studentpresence.SessionAttendancePresent},
	}}))
}
...
func TestAttachSlotAttendance_CarriesInstanceLifecycleStatus(t *testing.T) {
	...
	date := timezone.NewDate(2026, 7, 15)
	instance := &studentpresence.HistorySlotInstance{
		Date: timezone.Date(date), Status: timetable.InstanceStatusActive,
	}
	instance.ID = 203
	...
	assert.Equal(t, timetable.InstanceStatusActive, days[0].Slots[0].InstanceStatus)
}
...
func TestAttachSlotAttendance_CarriesTheAttendanceNote(t *testing.T) {
	...
	late := timetable.SlotSubstatusLate
