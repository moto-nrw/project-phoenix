...
func isStudentDocumentNotFound(err error) bool {
	return errors.Is(err, careplan.ErrCareDocumentNotFound) || common.IsNotFound(err)
}
...
func (rs *Resource) uploadStudentDocument(w http.ResponseWriter, r *http.Request) {
	upload, ok := rs.openStudentDocumentUpload(w, r)
	if !ok {
		return
	}
	defer common.CloseFile(upload.uploaded.File)
	if !rs.queueStudentDocumentUpload(w, r, upload) {
		return
	}
	rs.storeStudentDocumentUpload(w, r, upload)
}

// studentDocumentUpload is one child-document upload in flight.
type studentDocumentUpload struct {
	studentID   int64
	actor       careplan.StudentDocumentActor
	tenantID    int64
	coordinator *apiDocuments.Coordinator
	uploaded    *common.UploadedFile
	category    string
	storedName  string
}

// openStudentDocumentUpload resolves the caller, the tenant and the storage
// and parses the multipart body. The caller closes the uploaded file.
func (rs *Resource) openStudentDocumentUpload(w http.ResponseWriter, r *http.Request) (*studentDocumentUpload, bool) {
	id, ok := common.ParseInt64IDWithError(w, r, "id", common.MsgInvalidStudentID)
	if !ok {
		return nil, false
	}
	...
	return &studentDocumentUpload{studentID: id, actor: actor, tenantID: tenantID, coordinator: coordinator, uploaded: uploaded}, true
}

// queueStudentDocumentUpload authorizes the upload and records the cleanup
// intent of the object it is about to write.
func (rs *Resource) queueStudentDocumentUpload(w http.ResponseWriter, r *http.Request, upload *studentDocumentUpload) bool {
	...
	if err := rs.StudentDocumentService.AuthorizeStudentDocumentUpload(r.Context(), upload.studentID, category, upload.actor); err != nil {
		renderStudentDocumentError(w, r, err)
		return false
	}

	storedName, err := apiDocuments.NewStoredName(common.DocumentFileExtension(upload.uploaded.ContentType))
	if err != nil {
		common.RenderError(w, r, common.ErrorInternalServer(err))
		return false
	}

	if err := rs.StudentDocumentService.QueueStudentDocumentFileCleanup(r.Context(), upload.studentID, storedName); err != nil {
	...
	upload.storedName = storedName
	return true
}

// storeStudentDocumentUpload writes the object within the upload deadline and
// commits its metadata, disposing of the object on failure.
func (rs *Resource) storeStudentDocumentUpload(w http.ResponseWriter, r *http.Request, upload *studentDocumentUpload) {
	...
	size, ok := rs.saveStudentDocumentObject(w, r, uploadCtx, upload)
	if !ok {
		return
	}
	...
	if rs.abortExpiredUpload(w, r, uploadCtx, upload.studentID, upload.storedName) {
		return
	}
	...
	doc, err := rs.StudentDocumentService.CreateStudentDocument(uploadCtx, careplan.CreateStudentDocumentInput{
		...
		FilenameDisplay: upload.uploaded.Filename,
		FilenameStored:  upload.storedName,
		SizeBytes:       size,
		ContentType:     upload.uploaded.ContentType,
	}, upload.actor)
	if err != nil {
		...
		upload.coordinator.ReleaseFailedUpload(r.Context(), upload.tenantID, upload.studentID, upload.storedName, rejectedBeforeCommit, err)
		renderStudentDocumentError(w, r, err)
		return
	}
...
// saveStudentDocumentObject writes the upload's bytes. A failed write
// activates the queued cleanup intent instead of settling it.
func (rs *Resource) saveStudentDocumentObject(w http.ResponseWriter, r *http.Request, uploadCtx context.Context, upload *studentDocumentUpload) (int64, bool) {
	size, err := upload.coordinator.Save(uploadCtx, upload.tenantID, upload.storedName, upload.uploaded.File)
	if err != nil {
		...
		if activateErr := rs.StudentDocumentService.ActivateQueuedCleanup(r.Context(), upload.storedName); activateErr != nil {
