...
func TestCreatePartialAbsenceAppliesOnlyToBlocksStartingAfterTheChosenTime(t *testing.T) {
	...
	morningRow := testpkg.CreateTestInstanceStudent(t, tc.db, morning.ID, student.ID, studentpresence.SessionAttendanceExpected)
	overlapRow := testpkg.CreateTestInstanceStudent(t, tc.db, overlap.ID, student.ID, studentpresence.SessionAttendanceExpected)
	afternoonRow := testpkg.CreateTestInstanceStudent(t, tc.db, afternoon.ID, student.ID, studentpresence.SessionAttendanceExpected)
	checkedInAt := time.Date(2026, 9, 15, 15, 5, 0, 0, timezone.Berlin)
	actualRow := testpkg.CreateTestInstanceStudent(
		t, tc.db, actual.ID, student.ID, studentpresence.SessionAttendancePresent,
	...
	assert.Equal(t, studentpresence.SessionAttendanceExpected, rows[morningRow.ID].Status)
	assert.Equal(t, studentpresence.SessionAttendanceExpected, rows[overlapRow.ID].Status)
	assert.Equal(t, studentpresence.SessionAttendanceAbsent, rows[afternoonRow.ID].Status)
	require.NotNil(t, rows[afternoonRow.ID].Substatus)
	assert.Equal(t, timetable.SlotSubstatusExcused, *rows[afternoonRow.ID].Substatus)
	assert.Equal(t, studentpresence.SessionAttendancePresent, rows[actualRow.ID].Status)
	...
	rows = loadAttendanceRows(t, tc, morningRow.ID, overlapRow.ID, afternoonRow.ID, actualRow.ID)
	assert.Equal(t, studentpresence.SessionAttendanceExpected, rows[afternoonRow.ID].Status, "update must restore the slot no longer covered")
	assert.Equal(t, studentpresence.SessionAttendancePresent, rows[actualRow.ID].Status, "update must preserve actual attendance")
	...
	rows = loadAttendanceRows(t, tc, morningRow.ID, overlapRow.ID, afternoonRow.ID, actualRow.ID)
	assert.Equal(t, studentpresence.SessionAttendanceExpected, rows[afternoonRow.ID].Status)
	assert.Equal(t, studentpresence.SessionAttendancePresent, rows[actualRow.ID].Status, "delete must preserve actual attendance")
}
...
func TestPartialAbsenceFromRunningBlockStartExcusesTheRestOfTheDay(t *testing.T) {
	...
	instance := func(start, end, title, status string) int64 {
	...
	earlier := instance("11:30", "12:30", "Mittagessen", timetable.InstanceStatusCompleted)
	running := instance("13:00", "14:00", "Lernzeit", timetable.InstanceStatusActive)
	...
	earlierRow := testpkg.CreateTestInstanceStudent(t, tc.db, earlier, student.ID, studentpresence.SessionAttendancePresent,
		testpkg.InstanceStudentOpts{CheckedInAt: &checkedInAt})
	// The PATCH of „Entschuldigt“ on the running block leaves a manual absence.
	runningRow := testpkg.CreateTestInstanceStudent(t, tc.db, running, student.ID, studentpresence.SessionAttendanceAbsent,
		testpkg.InstanceStudentOpts{ManualStatusAt: &manualAt})
	laterRow := testpkg.CreateTestInstanceStudent(t, tc.db, later, student.ID, studentpresence.SessionAttendanceExpected)
	decidedRow := testpkg.CreateTestInstanceStudent(t, tc.db, decided, student.ID, studentpresence.SessionAttendanceAbsent,
		testpkg.InstanceStudentOpts{ManualStatusAt: &manualAt})
	resetRow := testpkg.CreateTestInstanceStudent(t, tc.db, reset, student.ID, studentpresence.SessionAttendanceExpected,
		testpkg.InstanceStudentOpts{ManualStatusAt: &manualAt})
	...
	assert.Equal(t, studentpresence.SessionAttendancePresent, rows[earlierRow.ID].Status, "earlier block must stay present")
	assert.True(t, checkedInAt.Equal(*rows[earlierRow.ID].CheckedInAt), "earlier check-in must be preserved")
	assert.Equal(t, studentpresence.SessionAttendanceAbsent, rows[runningRow.ID].Status)
	assert.Nil(t, rows[runningRow.ID].PickupExceptionID, "the running block keeps its own manual decision")
	assert.Equal(t, studentpresence.SessionAttendanceAbsent, rows[laterRow.ID].Status)
	require.NotNil(t, rows[laterRow.ID].Substatus)
	assert.Equal(t, timetable.SlotSubstatusExcused, *rows[laterRow.ID].Substatus)
	require.NotNil(t, rows[laterRow.ID].PickupExceptionID)
	assert.Equal(t, partialAbsenceID, *rows[laterRow.ID].PickupExceptionID)
	assert.Equal(t, studentpresence.SessionAttendanceAbsent, rows[decidedRow.ID].Status)
	assert.Nil(t, rows[decidedRow.ID].Substatus, "a manually decided later block must not be overwritten")
	assert.Equal(t, studentpresence.SessionAttendanceExpected, rows[resetRow.ID].Status, "a manual reset to expected must not be overwritten")
	...
	rows = loadAttendanceRows(t, tc, ids...)
	assert.Equal(t, studentpresence.SessionAttendanceExpected, rows[laterRow.ID].Status, "removing the partial absence restores later blocks")
	assert.Equal(t, studentpresence.SessionAttendanceAbsent, rows[runningRow.ID].Status, "the running block stays excused by hand")
	assert.Equal(t, studentpresence.SessionAttendancePresent, rows[earlierRow.ID].Status)
}
...
func loadAttendanceRows(t *testing.T, tc *testContext, ids ...int64) map[int64]*attendanceSlotRow {
	t.Helper()
	rows := make([]*attendanceSlotRow, 0, len(ids))
	...
	byID := make(map[int64]*attendanceSlotRow, len(rows))
