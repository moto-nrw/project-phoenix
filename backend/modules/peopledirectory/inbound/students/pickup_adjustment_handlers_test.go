...
func TestPickupAdjustmentProtectedRouterRequiresExplicitExceptionAndAuditsApply(t *testing.T) {
	...
	mismatchedArrivalBody["arrival_schedules"] = []map[string]any{{
		"weekday": timetable.WeekdayTuesday, "expected_arrival": "08:00",
	}}
	...
	manual := careplan.PickupSchedule{
		TenantID: student.TenantID, StudentID: student.ID, Weekday: 1, PickupTime: manualTime, CreatedBy: staff.StaffID,
		Source: careplan.ScheduleSourceStaff,
	}
	repoFactory := newStudentTestRepositories(tc.db)
	_, err = repoFactory.CarePlan.UpsertPickupSchedule(testpkg.Ctx(t), manual)
	require.NoError(t, err)
	...
	history, err := tc.resource.StudentAuditService.ListStudentChangeHistory(testpkg.Ctx(t), student.ID)
	require.NoError(t, err)
...
func TestPickupAdjustmentProtectedRouterChangesMatchingOfferingThroughSharedPath(t *testing.T) {
	...
	for offeringID, pickupTime := range map[int64]string{
		fixture.ganztag.ID: "16:00",
		fixture.mittag.ID:  "14:30",
	} {
		pickupTimes := map[string]string{
			"mon": pickupTime, "tue": pickupTime, "wed": pickupTime, "thu": pickupTime, "fri": pickupTime,
		}
		_, err := tc.db.NewUpdate().
			TableExpr(`enrollment.care_offerings AS "care_offering"`).
			Set("pickup_times = ?", pickupTimes).
			Where("id = ?", offeringID).Exec(t.Context())
		require.NoError(t, err)
	}
	...
	manual := careplan.PickupSchedule{
		TenantID: student.TenantID, StudentID: student.ID, Weekday: 5, PickupTime: manualTime, Notes: &existingNote, CreatedBy: staff.StaffID,
	}
	_, err = newStudentTestRepositories(tc.db).CarePlan.UpsertPickupSchedule(testpkg.Ctx(t), manual)
	require.NoError(t, err)
	...
	for weekday := 1; weekday <= 5; weekday++ {
		...
		if weekday == timetable.WeekdayFriday {
	...
	assert.Equal(t, timetable.WeekdayFriday, previewWithNewNote.RemovedManualNotes[0].Weekday)
	...
	assert.Equal(t, timetable.WeekdayFriday, confirmedPreview.RemovedManualNotes[0].Weekday)
	...
	require.NoError(t, err)
	concurrentArrival := careplan.ArrivalSchedule{
		TenantID: student.TenantID, StudentID: student.ID, Weekday: timetable.WeekdayMonday,
		ExpectedArrival: concurrentArrivalTime, CreatedBy: staff.StaffID,
	}
	_, err = newStudentTestRepositories(tc.db).CarePlan.UpsertArrivalSchedule(
	...
	require.NoError(t, tc.db.NewSelect().
		TableExpr(`schedule.student_pickup_schedules AS "student_pickup_schedule"`).
		Column("id").
		Where("student_id = ?", student.ID).
		Where("source <> ?", careplan.ScheduleSourceCareOffering).
		Scan(t.Context(), &manualRows))
...
func TestPickupAdjustmentProtectedRouterRollsBackKnownErrorAfterOfferingWrite(t *testing.T) {
	...
	for offeringID, pickupTime := range map[int64]string{
		fixture.ganztag.ID: "16:00", fixture.mittag.ID: "14:30",
	} {
		_, err := tc.db.NewUpdate().
			TableExpr(`enrollment.care_offerings AS "care_offering"`).
...
func TestBulkPickupAdjustmentRequiresAndAuditsExplicitExceptions(t *testing.T) {
	...
	for _, studentID := range []int64{first.ID, second.ID} {
		history, err := tc.resource.StudentAuditService.ListStudentChangeHistory(testpkg.Ctx(t), studentID)
		require.NoError(t, err)
		require.NotEmpty(t, history)
		assert.Equal(t, auditFieldPickupSchedule, history[0].FieldName)
...
func fiveDayArrivalBody(arrivalTime, mondayNote string) []map[string]any {
	...
	for weekday := timetable.WeekdayMonday; weekday <= timetable.WeekdayFriday; weekday++ {
		row := map[string]any{"weekday": weekday, "expected_arrival": arrivalTime}
		if weekday == timetable.WeekdayMonday && mondayNote != "" {
