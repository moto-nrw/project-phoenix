...
// deviceExec authenticates with the given RFID device's API key and runs the request through
...
// X-Staff-PIN.
func deviceExec(t *testing.T, tc *testContext, req *http.Request, apiKey *string) *httptest.ResponseRecorder {
	t.Helper()
...
func TestAssignRFIDTag_WithDeviceAuth(t *testing.T) {
	...
	t.Run("success_assigns_rfid_tag", func(t *testing.T) {
		...
		rr := deviceExec(t, tc, req, device.APIKey)
...
func TestUnassignRFIDTag_WithAssignedTag(t *testing.T) {
	...
	assignRR := deviceExec(t, tc, assignReq, device.APIKey)
...
func TestRFIDTagRoutes_GraduatedStudent(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, deviceExec(t, tc, assignReq, device.APIKey).Code)
