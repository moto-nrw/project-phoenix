...
func TestStudentRequestBind_AccompaniedRequiresCompanionNote(t *testing.T) {
	...
	t.Run("allowed_departure_modes accompanied without note is rejected", func(t *testing.T) {
		modes := departure.AllowedDepartureModes{
		...
		require.ErrorIs(t, req.Bind(nil), departure.ErrDepartureCompanionNoteRequired)
	})

	t.Run("departure_days accompanied with blank note is rejected", func(t *testing.T) {
		days := departure.DepartureDays{departure.PickupDayMonday: departure.DepartureAccompanied}
		blank := "   "
		req := &StudentRequest{
			FirstName:              "Max",
			LastName:               "Muster",
	...
	t.Run("authoritative allowed modes override stale legacy departure_days", func(t *testing.T) {
		...
		staleDays := departure.DepartureDays{departure.PickupDayMonday: departure.DepartureAccompanied}
		req := &StudentRequest{
			FirstName:             "Max",
			LastName:              "Muster",
			SchoolClass:           "1a",
	...
	t.Run("authoritative allowed modes accompanied without note is still rejected", func(t *testing.T) {
		...
		days := departure.DepartureDays{departure.PickupDayMonday: departure.DepartureBus}
		req := &StudentRequest{
			FirstName:             "Max",
			LastName:              "Muster",
			SchoolClass:           "1a",
