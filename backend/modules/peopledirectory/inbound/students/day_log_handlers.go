...
func (rs *Resource) permittedDayLogGroups(ctx context.Context, date timezone.Date, logger *slog.Logger) ([]*SchoolGroup, error) {
	if securityruntime.HasAdminWildcard(jwt.PermissionsFromCtx(ctx)) {
...
type dayLogData struct {
	studentsByGroup     map[int64][]*Student
...
func (rs *Resource) loadDayLogData(ctx context.Context, groups []*SchoolGroup, date timezone.Date, clock dayLogClock) (*dayLogData, error) {
	...
	records, err := rs.Persons.ListStudentsForDay(ctx, groupIDs, date, clock.today)
	if err != nil {
		return nil, err
	}
	...
	data := &dayLogData{
		studentsByGroup:     make(map[int64][]*Student, len(groups)),
		attendanceByStudent: map[int64][]*studentpresence.Attendance{},
		statusByStudent:     map[int64][]*absencerecords.StudentStatusDay{},
		careDays:            map[int64]careplan.CareDayStatus{},
		arrivalTimes:        map[int64]*careplan.EffectiveArrivalTime{},
	...
	studentIDs, personIDs := indexDayLogStudents(data, studentsFromRecords(records))

	data.persons, err = rs.personsByIDs(ctx, personIDs)
	if err != nil {
		return nil, err
	}
...
func indexDayLogStudents(data *dayLogData, students []*Student) (studentIDs, personIDs []int64) {
...
func dayLogEnrollmentNotStarted(student *Student, row dayLogStudent, date timezone.Date) bool {
...
func buildDayLogStudent(student *Student, data *dayLogData) dayLogStudent {
	row := dayLogStudent{
...
func mergeDayLogAttendance(rows []*studentpresence.Attendance) (time.Time, *time.Time) {
