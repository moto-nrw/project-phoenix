// backend/modules/studentpresence/legacy/repositories/active/group.go
package active
...
// GroupRepositoryOption configures optional collaborators at construction.
type GroupRepositoryOption func(*GroupRepository)

// WithRoomDirectory installs the Facilities directory the group reads
// resolve rooms through (#2665). Without it every room-bearing read fails
// loudly instead of falling back to a join this package no longer owns.
func WithRoomDirectory(rooms RoomDirectory) GroupRepositoryOption {
	return func(r *GroupRepository) { r.rooms = rooms }
}
...
func NewGroupRepository(devices DeviceDirectory, records GroupRecords, activityDirectory GroupActivityDirectory, options ...GroupRepositoryOption) active.GroupRepository {
...
func legacyGroupSupervisions(supervisors []GroupSupervisionRecord) ([]*active.GroupSupervisor, error) {
	...
	for _, row := range supervisors {
		start, err := active.ParseDate(row.StartDate)
		if err != nil {
			return nil, err
		}
		var end *active.Date
		if row.EndDate != nil {
			value, err := active.ParseDate(*row.EndDate)
			if err != nil {
				return nil, err
			}
			end = &value
