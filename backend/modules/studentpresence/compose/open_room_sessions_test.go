...
func TestOpenRoomSessionsAreBatchedAndTenantScoped(t *testing.T) {
	...
	require.Nil(t, rows[0].DeviceID)
	require.Equal(t, []int64{staff.ID}, rows[0].SupervisorStaffIDs)
	device := testpkg.CreateTestDevice(t, db, "OpenRoomKiosk")
	_, err = db.NewUpdate().Table("active.groups").Set("device_id = ?", device.ID).Where("tenant_id = ? AND id = ?", testpkg.Tenant(t), group.ID).Exec(ctx)
	require.NoError(t, err)
	rows, err = module.ListOpenRoomSessions(ctx, []int64{room.ID})
	require.NoError(t, err)
	require.Len(t, rows, 1)
	require.NotNil(t, rows[0].DeviceID)
