...
func TestGroupRecoveryMigrationRuntime(t *testing.T) {
	...
	staff := testpkg.CreateTestStaff(t, db, "Runtime", "Recovery")
	activity := testpkg.CreateTestActivityGroup(t, db, "Recovery runtime")
	room := testpkg.CreateTestRoom(t, db, "Recovery runtime")
	...
	endGroup := func() {
		_, err := db.NewUpdate().Table("active.groups").Set("end_time = now()").Where("id = ?", group.ID).Exec(ctx)
		require.NoError(t, err)
		_, err = db.NewUpdate().Table("active.group_supervisors").Set("end_date = CURRENT_DATE").Where("id = ?", supervisor.ID).Exec(ctx)
		require.NoError(t, err)
	}
	ended := func() (groupEnded, supervisorEnded bool) {
		require.NoError(t, db.NewSelect().TableExpr("active.groups").ColumnExpr("end_time IS NOT NULL").Where("id = ?", group.ID).Scan(ctx, &groupEnded))
		require.NoError(t, db.NewSelect().TableExpr("active.group_supervisors").ColumnExpr("end_date IS NOT NULL").Where("id = ?", supervisor.ID).Scan(ctx, &supervisorEnded))
	...
	restore := func(txCtx context.Context, now time.Time, fail bool) error {
		if err := module.LockOpenSupervisors(txCtx, group.ID); err != nil {
			return err
		}
		if err := module.LockSupervisors(txCtx, []int64{supervisor.ID}); err != nil {
			return err
		}
		if err := module.RestoreGroup(txCtx, group.ID, now); err != nil {
			return err
		}
		if err := module.RestoreSupervisors(txCtx, []int64{supervisor.ID}); err != nil {
			return err
		}
		if fail {
			return abort
	...
	for _, flow := range []string{"reopen-recovery", "rollback-after-supervisor-restore"} {
		t.Run(flow, func(t *testing.T) {
			...
			for iteration := range 35 {
				...
				err := tenant.WithinCurrentTenant(measuredCtx, func(txCtx context.Context) error { return module.RestoreGroup(txCtx, group.ID, now) })
