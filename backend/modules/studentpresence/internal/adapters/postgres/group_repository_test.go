...
func TestActiveGroupRepository_CheckRoomConflict(t *testing.T) {
	...
	t.Run("independent room stay is not occupancy", func(t *testing.T) {
		...
		footballSession := &ports.ActiveGroup{
			StartTime:    now,
			LastActivity: now,
...
func TestActiveGroupRepository_FindWithSupervisors(t *testing.T) {
	...
	t.Run("finds group with supervisors", func(t *testing.T) {
		...
		groupSup := &testpkg.GroupSupervisorRow{
			GroupID:   group.ID,
			StaffID:   staff.ID,
			Role:      "supervisor",
