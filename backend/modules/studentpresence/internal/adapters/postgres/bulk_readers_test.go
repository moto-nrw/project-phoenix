...
func TestGroupSupervisorRepository_ListActiveSupervisedRooms(t *testing.T) {
	...
	require.NoError(t, repo.Update(ctx, supervisorOf(openSupervision)))
	require.NoError(t, repo.Update(ctx, supervisorOf(closedSupervision)))
	...
	t.Run("does not leak another tenant's supervisions", func(t *testing.T) {
		...
		foreignSupervision := &testpkg.GroupSupervisorRow{
