...
type mockSlotListsService struct {
	...
	buildErr      error
	renderFile    classday.File
	renderErr     error
	buildCalled   bool
	lastDate      classday.Date
	lastParams    classday.Params
	lastFormat    classday.Format
}
...
func (m *mockSlotListsService) RenderList(_ context.Context, params classday.Params, format classday.Format) (classday.File, error) {
	m.lastParams = params
	m.lastFormat = format
	if m.renderErr != nil {
		return classday.File{}, m.renderErr
	}
...
func TestExportSlotListDefaultsToPDFAndStreamsFile(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, w.Code)
	assert.Equal(t, classday.FormatPDF, mock.lastFormat)
...
func TestExportSlotListReportsRenderErrors(t *testing.T) {
	...
	assert.Equal(t, classday.FormatXLSX, mock.lastFormat)
}
