// Package timetablehttp — instance lifecycle handlers (WP-B9).
//
...
func (rs *Resource) resolveStartedByStaffID(ctx context.Context) int64 {
	...
	personID, err := rs.People.AccountPersonID(ctx, accountID)
	...
	staffID, err := rs.People.PersonStaffID(ctx, personID)
