...
type fakePlanExport struct {
	...
	file   planexport.File
	err    error
}

func (f *fakePlanExport) ExportDienstplan(context.Context, planexport.Params) (planexport.File, error) {
...
func exportRequest(t *testing.T, body map[string]any, perms []string) (*fakePlanExport, int, http.Header, string) {
	t.Helper()
	fake := &fakePlanExport{file: planexport.File{Data: []byte("%PDF-1.7"), ContentType: "application/pdf", Filename: "Betreuungsplan 2026-07-27.pdf"}}
...
func TestBetreuungsplanExportDefaultsTheTemplateAndStreamsTheFile(t *testing.T) {
	...
	assert.Equal(t, planexport.FormatPDF, fake.params.Format)
...
func TestBetreuungsplanExportRefusesBadParameters(t *testing.T) {
	...
	assert.Equal(t, planexport.FormatDOCX, fake.params.Format)
}
