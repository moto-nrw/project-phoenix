// Package timetablehttp — POST /api/timetable/templates/{id}/split handler (WP-B3).
//
...
func renderTemplateRosterRebaseConflict(w http.ResponseWriter, r *http.Request, err error) bool {
	if !errors.Is(err, timetableModule.ErrTemplateRosterRebaseConflict) {
		return false
	}
...
func (rs *Resource) splitTemplate(w http.ResponseWriter, r *http.Request) {
	...
	if !rs.validateSplitTemplateReferences(w, r, req) {
		return
	}

	result, err := rs.Templates.SplitTemplate(r.Context(), in)
	if err != nil {
...
// validateSplitTemplateReferences runs the tenant-scoped period check the
// create/update handlers run and validates the education groups the split
// targets. The returned roster start date of the period check is unused —
// the split anchors rosters on the effective date instead. It returns false
// after rendering the refusal.
func (rs *Resource) validateSplitTemplateReferences(w http.ResponseWriter, r *http.Request, req *splitTemplateRequest) bool {
...
func buildTemplateSplitInput(id int64, req *splitTemplateRequest) (timetableModule.SplitTemplateCommand, error) {
	startTime, err := parseClockTime(req.StartTime)
	if err != nil {
		return timetableModule.SplitTemplateCommand{}, errors.New("invalid start_time format, expected HH:MM")
	}
	...
	if err != nil {
		return timetableModule.SplitTemplateCommand{}, errors.New("invalid materialize_from format, expected YYYY-MM-DD")
	}
	materializeTo, err := parseOptionalSplitDate(req.MaterializeTo)
	if err != nil {
		return timetableModule.SplitTemplateCommand{}, errors.New("invalid materialize_to format, expected YYYY-MM-DD")
	}

	command := splitTemplateCommand(id, req)
	command.EffectiveDate = effectiveDate
	command.StartTime = startTime
	command.EndTime = endTime
	command.MaterializeFrom = materializeFrom
	command.MaterializeTo = materializeTo
	return command, nil
}

// splitTemplateCommand carries the request's successor fields into the
// service input; the parsed dates and clock times are set by the caller.
func splitTemplateCommand(id int64, req *splitTemplateRequest) timetableModule.SplitTemplateCommand {
	return timetableModule.SplitTemplateCommand{
		TemplateID:              id,
		Name:                    req.Name,
		Type:                    req.Type,
