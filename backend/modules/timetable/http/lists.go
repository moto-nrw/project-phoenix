// Package timetablehttp — issue #1565: lists from timetable slots.
//
...
func (rs *Resource) exportSlotList(w http.ResponseWriter, r *http.Request) {
	...
	format := classday.Format(req.Format)
	if format == "" {
		format = classday.FormatPDF
	}
	if format != classday.FormatPDF && format != classday.FormatXLSX {
