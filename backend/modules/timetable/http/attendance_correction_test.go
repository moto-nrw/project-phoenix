...
func correctionRouter(parentCtx context.Context, res *Resource, db *bun.DB) chi.Router {
...
// loadTrail reads the correction trail through the owner's read of it, the
// one the corrections route serves.
func loadTrail(t *testing.T, s *patchSetup) []timetable.AttendanceCorrectionEntry {
	t.Helper()
	rows, err := s.res.AttendanceCorrections.GetAttendanceCorrections(s.ctx, s.instanceID, s.studentID)
	require.NoError(t, err)
	return rows
}
...
func TestCorrectAttendance_CompletedInstance_WritesRowAndTrail(t *testing.T) {
	...
	byField := map[string]timetable.AttendanceCorrectionEntry{}
	...
	statusEntry, ok := byField[timetable.AttendanceCorrectionFieldStatus]
	require.True(t, ok)
	...
	noteEntry, ok := byField[timetable.AttendanceCorrectionFieldNote]
	require.True(t, ok)
...
func TestCorrectAttendance_NoteOnlyPreservesAttendanceProvenance(t *testing.T) {
	...
	assert.Equal(t, timetable.AttendanceCorrectionFieldNote, trail[0].FieldName)
}
...
func TestCorrectAttendance_RejectsOverlongReason(t *testing.T) {
	...
	long := make([]rune, timetable.AttendanceCorrectionReasonMaxLength+1)
	for i := range long {
		long[i] = 'ä'
	}
	w := doJSON(t, router, http.MethodPost, correctionPath(s), map[string]any{
