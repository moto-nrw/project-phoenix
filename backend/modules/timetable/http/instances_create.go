// Package timetablehttp — POST /api/timetable/instances handler.
//
...
func (rs *Resource) createInstance(w http.ResponseWriter, r *http.Request) {
	...
	inst, err := rs.InstanceService.CreateInstance(r.Context(), timetable.CreateInstanceInput{
		...
		CreatedByStaffID: rs.createdByStaffID(r.Context()),
	...
	if err != nil {
		...
		respondCreatedWithoutEnrichment(w, r, inst, err)
		return
	}
...
// createdByStaffID is the caller's staff id for the new block, nil when the
// caller resolves to no staff member.
func (rs *Resource) createdByStaffID(ctx context.Context) *int64 {
...
// respondCreatedWithoutEnrichment answers a committed create whose enriched
// re-read failed with the block's own fields and the enrichment warning.
func respondCreatedWithoutEnrichment(w http.ResponseWriter, r *http.Request, inst *timetable.LifecycleInstance, err error) {
