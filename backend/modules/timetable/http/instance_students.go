// Package timetablehttp — three-field attendance PATCH handler (WP-B10).
//
...
func (rs *Resource) patchInstanceStudent(w http.ResponseWriter, r *http.Request) {
	...
	patch, ok := readAttendancePatch(w, r)
	if !ok {
		return
	}
...
// readAttendancePatch decodes and validates the PATCH body. It returns
// ok=false after rendering the 400 for a malformed body or a patch that
// changes nothing.
func readAttendancePatch(w http.ResponseWriter, r *http.Request) (timetable.AttendancePatch, bool) {
	req, ok := decodePatchBody(w, r)
	if !ok {
		return timetable.AttendancePatch{}, false
	}
