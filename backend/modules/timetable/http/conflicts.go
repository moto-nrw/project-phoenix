// Package timetablehttp — planning-time conflict probe.
//
...
func parsePlannedConflictParams(w http.ResponseWriter, r *http.Request) (plannedConflictParams, bool) {
	out, ok := parsePlannedConflictSlot(w, r)
	if !ok {
		return out, false
	}
	return parsePlannedConflictSubjects(w, r, out)
}

// parsePlannedConflictSlot reads the probed slot: its date and its clock
// window.
func parsePlannedConflictSlot(w http.ResponseWriter, r *http.Request) (plannedConflictParams, bool) {
	q := r.URL.Query()
...
// parsePlannedConflictSubjects reads who and what the slot would bind: at
// least one of the room, the staff and the students.
func parsePlannedConflictSubjects(w http.ResponseWriter, r *http.Request, out plannedConflictParams) (plannedConflictParams, bool) {
	q := r.URL.Query()
