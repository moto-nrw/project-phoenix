// Package timetablehttp — per-student day and week read endpoints (WP-B11).
//
...
func (rs *Resource) getStudentDay(w http.ResponseWriter, r *http.Request) {
	...
	if !rs.resolveStudentForRead(w, r, studentID) {
		return
	}

	// /day is /week with from == to: one round of batch queries covers it.
	days, err := rs.buildStudentDays(ctx, studentID, date, date)
	if err != nil {
...
func (rs *Resource) getStudentWeek(w http.ResponseWriter, r *http.Request) {
	...
	days, err := rs.buildStudentDays(ctx, studentID, from, to)
	if err != nil {
...
// resolveStudentForRead checks that the child attends and enforces the
// read-access rules; it returns false after rendering the refusal.
// Cross-tenant ("student doesn't exist in caller's tenant") is 404;
...
func (rs *Resource) resolveStudentForRead(w http.ResponseWriter, r *http.Request, studentID int64) bool {
	ctx := r.Context()
	...
	// A missing child, a graduated (alumnus) child and a child whose care has
	// ended are all 404. A graduate is soft-deleted and invisible to every
	// staff read; CanReadStudent only decides the permission scope, so
	// without this gate an administrator with a bookmarked student ID could
	// keep reading a departed child's day/week timetable. Same 404 as the
	// students resource applies (#405 review). A child whose care has ended
	// leaves the operational timetable the same way (#2487); their past days
	// stay in the history exports.
	...
	if !securityruntime.CanReadStudent(
		ctx,
