// Package timetablehttp — edited-occurrence probe (#1875).
//
...
func (rs *Resource) editedInWindow(w http.ResponseWriter, r *http.Request) {
	...
	activityGroupID, from, to, ok := parseEditedInWindowQuery(w, r)
	if !ok {
		return
	}
	...
	includeDeletions := r.URL.Query().Get("include_deletions") == "true"
...
// parseEditedInWindowQuery reads the template and the window of the probe.
// It returns ok=false after rendering the 400 for a missing or malformed
// value or a reversed window.
func parseEditedInWindowQuery(w http.ResponseWriter, r *http.Request) (activityGroupID int64, from, to calendar.Date, ok bool) {
	q := r.URL.Query()
	...
	return activityGroupID, from, to, true
}
