...
func (rs *Resource) getTemplate(w http.ResponseWriter, r *http.Request) {
	...
	rs.attachRosterMaintenance(r.Context(), templates[:1], periodID)
...
func (rs *Resource) updateTemplate(w http.ResponseWriter, r *http.Request) {
	...
	existing, ok := rs.loadTemplateForUpdate(w, r, id, parsed)
	if !ok {
		return
	}
	if !rs.prepareTemplateUpdate(w, r, parsed, existing) {
		return
	}
...
// loadTemplateForUpdate loads the existing recurrence independently of the
// requested target period. Its schedules still belong to the source period
// until the update succeeds, so a target-scoped read would incorrectly report
// 404 while moving a template between periods. It returns ok=false after
// rendering the refusal.
func (rs *Resource) loadTemplateForUpdate(w http.ResponseWriter, r *http.Request, id int64, parsed *parsedUpdateTemplate) (templateResponse, bool) {
	templates, err := rs.loadTemplates(r.Context(), &id, nil)
	if err != nil {
		common.RenderError(w, r, common.ErrorInternalServerWrap("load template failed", err))
