...
//
//  1. jwt.SchoolMiddleware (via ProtectedSchoolRoutes) admits only school-scope
...
func (rs *Resource) schoolStudentSheet(w http.ResponseWriter, r *http.Request) {
	if rs.OperationsService == nil || rs.SupervisionSheets == nil {
	...
	sheet, err := rs.SupervisionSheets.SupervisionStudentSheet(
		r.Context(), studentID, rs.todayDate(), boundary, accountID, strings.Join(claims.Roles, ","),
	)
	if err != nil {
		if isSupervisionSheetRefusal(err) {
...
// SupervisionSheets serves the per-child pickup and emergency sheet of the
// school portal (#2527). The sheet carries guardian names and phone numbers,
// so the caller proves the assignment to a block holding the child first;
// every read is audited. The composition root binds it over the enrollment
// report.
type SupervisionSheets interface {
	// SupervisionStudentSheet returns the sheet of one child on date as the
	// JSON body of the route. companionBoundary is the roster of the block:
	// an accompanied departure names only children from it. actorRole
	// carries the caller's roles, comma-joined, for the audit row. A request
	// the report refuses carries SupervisionSheetRefused().
...
// supervisionSheetRefusal is the shape of the report's refusal of a sheet
// request; the route answers 404 with its text.
type supervisionSheetRefusal interface {
	SupervisionSheetRefused()
}

func isSupervisionSheetRefusal(err error) bool {
	var refusal supervisionSheetRefusal
