// Package timetablehttp — Änderungsprotokoll read endpoint (#1886).
//
...
func (rs *Resource) resolveStaffDisplayNames(ctx context.Context, staffIDs map[int64]bool) map[int64]string {
	...
	for staffID := range staffIDs {
		personID, err := rs.People.StaffPersonID(ctx, staffID)
		if err != nil || personID == 0 {
			continue
		}
		personIDByStaff[staffID] = personID
		personIDs = append(personIDs, personID)
	}
	persons, err := rs.People.PersonNames(ctx, personIDs)
	if err != nil {
		return names
	}
	for staffID, personID := range personIDByStaff {
		if name, ok := persons[personID]; ok {
			names[staffID] = name
		}
	}
	return names
...
func (rs *Resource) resolveActorDisplayNames(ctx context.Context, actorIDs map[int64]bool) map[int64]string {
	...
	for accountID := range actorIDs {
		name, err := rs.People.AccountPersonName(ctx, accountID)
		if err != nil {
			continue
		}
