...
func TestUnfilteredListingRejectsOptionsTheOwnerCannotServe(t *testing.T) {
	...
	calls := 0
	listing := compose.UnfilteredListing[*schedule.CalendarPeriod]{
...
func TestDateframeOptionsListingTranslatesTheSchedulesAPIShape(t *testing.T) {
	...
	var seen compose.DateframeListing
	listing := compose.DateframeOptionsListing{
		Source: func(_ context.Context, request compose.DateframeListing) ([]*schedule.Dateframe, error) {
			seen = request
	...
	_, err := listing.List(ctx, options)
	require.NoError(t, err)
	assert.Equal(t, compose.DateframeListing{
		NamePattern: "%Ferien%", Limit: 25, Offset: 25,
		Sort: []compose.DateframeSortField{{Field: "start_date", Descending: true}},
	}, seen)
	...
	require.NoError(t, err)
	assert.Equal(t, compose.DateframeListing{Name: "Projektwoche"}, seen)

	_, err = listing.List(ctx, nil)
	require.NoError(t, err)
	assert.Equal(t, compose.DateframeListing{}, seen)
