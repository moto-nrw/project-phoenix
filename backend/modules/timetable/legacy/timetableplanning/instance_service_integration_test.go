...
type lifecycleSetup struct {
	presence timetableplanning.InstancePresence
	svc      timetableplanning.InstanceService
...
func instanceServiceWithBroadcaster(s *lifecycleSetup, broadcaster realtime.Broadcaster, recovery ...scheduleModels.ActivityRecoveryRepository) timetableplanning.InstanceService {
	return timetableplanning.NewInstanceService(lifecycleDependencies(s, broadcaster, recovery...))
}
...
func lifecycleDependencies(s *lifecycleSetup, broadcaster realtime.Broadcaster, recovery ...scheduleModels.ActivityRecoveryRepository) timetableplanning.InstanceServiceDependencies {
	...
	return timetableplanning.InstanceServiceDependencies{
		...
		CareDayService:      timetableplanning.NewInstanceCareDays(s.factory.CareDay, s.repos.CarePlan()),
...
func TestInstance_SetUnderstaffedAck_CompletedRejected(t *testing.T) {
	...
	assert.ErrorIs(t, err, timetableplanning.ErrInvalidInstanceTransition)
}
...
func TestInstance_SetUnderstaffedAck_RejectedWhileStaffed(t *testing.T) {
	...
	_, err := svc.SetUnderstaffedAck(s.ctx, ai.ID, true, nil, nil)
	assert.ErrorIs(t, err, timetableplanning.ErrUnderstaffedAckStillStaffed)
...
func TestInstance_SetUnderstaffedAck_NotFound(t *testing.T) {
	...
	assert.ErrorIs(t, err, timetableplanning.ErrInstanceNotFound)
}
...
func TestInstance_CreateRejectsDateOutsideActiveCalendarPeriod(t *testing.T) {
	...
	s := buildLifecycle(t)
	_, err := s.svc.Create(s.ctx, timetableplanning.CreateInstanceInput{
	...
	require.ErrorIs(t, err, timetableplanning.ErrInstanceOutsideActiveCalendarPeriod)
}
...
func TestInstance_CreateSpontaneousSkipsActiveCalendarPeriodValidation(t *testing.T) {
	...
	s := buildLifecycle(t)
	isSpontaneous := true
	created, err := s.svc.Create(s.ctx, timetableplanning.CreateInstanceInput{
		Date:          timezone.NewDate(2101, 4, 21),
		StartTime:     time.Date(2000, 1, 1, 14, 0, 0, 0, time.UTC),
		EndTime:       time.Date(2000, 1, 1, 15, 0, 0, 0, time.UTC),
...
func TestInstance_UpdatePlannedRejectsDateOutsideActiveCalendarPeriod(t *testing.T) {
	...
	_, err := s.svc.UpdatePlanned(s.ctx, instance.ID, timetableplanning.UpdateInstanceInput{
		Date:      timezone.NewDate(2101, 4, 21),
...
func TestInstance_UpdatePlannedKeepsDateAfterPeriodDeactivation(t *testing.T) {
	...
	updated, err := s.svc.UpdatePlanned(s.ctx, instance.ID, timetableplanning.UpdateInstanceInput{
...
func TestInstance_CreateIdempotencyRetryReturnsStoredResultAfterPeriodDeactivation(t *testing.T) {
	...
	req := timetableplanning.CreateInstanceInput{
...
func TestInstance_PlannedCRUD_BroadcastsStaffingDeviationChanged(t *testing.T) {
	...
	inst, err := svc.Create(s.ctx, timetableplanning.CreateInstanceInput{
		Date:      timezone.NewDate(2026, 4, 21),
		StartTime: time.Date(2000, 1, 1, 14, 0, 0, 0, time.UTC),
		EndTime:   time.Date(2000, 1, 1, 15, 0, 0, 0, time.UTC),
	...
	_, err = svc.UpdatePlanned(s.ctx, inst.ID, timetableplanning.UpdateInstanceInput{
...
func TestInstance_Complete_ConfirmationMustMatchOpenVisits(t *testing.T) {
	...
	_, err = s.svc.Complete(timetableplanning.WithCompletionConfirmation(s.ctx, []int64{s.student2}), ai.ID)
	require.ErrorIs(t, err, timetableplanning.ErrCompletionConfirmationStale)

	completed, err := s.svc.Complete(timetableplanning.WithCompletionConfirmation(s.ctx, []int64{s.student1}), ai.ID)
	require.NoError(t, err)
	assert.Equal(t, scheduleModels.InstanceStatusCompleted, completed.Status)
}
...
func testReopenWriteRollback(t *testing.T, stage string) {
	...
	broadcaster := testpkg.NewRecordingBroadcaster()
	s.svc = instanceServiceWithBroadcaster(s, broadcaster, fault)
	...
	assert.Equal(t, scheduleModels.InstanceStatusActive, reopened.Instance.Status)
	_, err = s.svc.Reopen(s.ctx, ai.ID, 0, true)
	require.ErrorIs(t, err, timetableplanning.ErrInvalidInstanceTransition)
	visits, err = presence.ListVisits(s.ctx, studentpresence.VisitFilter{ActiveGroupIDs: []int64{started.ActiveGroupID}})
	require.NoError(t, err)
	require.Len(t, visits, 1)
	assert.Equal(t, visit.ID, visits[0].ID)
...
func TestInstance_Create_AssignsUniqueStaffAndStudents(t *testing.T) {
	...
	inst, err := s.svc.Create(s.ctx, timetableplanning.CreateInstanceInput{
...
func TestInstance_CreateAndUpdatePlanned_ReapplyActiveStatusDays(t *testing.T) {
	...
	updated, err := s.svc.UpdatePlanned(s.ctx, inst.ID, timetableplanning.UpdateInstanceInput{
...
func TestInstance_Create_RejectsCrossTenantReferences(t *testing.T) {
	...
	valid := timetableplanning.CreateInstanceInput{
	...
	cases := []struct {
		name   string
		mutate func(*timetableplanning.CreateInstanceInput)
	}{
		{name: "room", mutate: func(in *timetableplanning.CreateInstanceInput) { in.RoomID = foreignRoom.ID }},
		{name: "activity group", mutate: func(in *timetableplanning.CreateInstanceInput) { in.ActivityGroupID = &foreignTemplate.ID }},
		{name: "staff", mutate: func(in *timetableplanning.CreateInstanceInput) { in.StaffIDs = []int64{foreignStaff.ID} }},
		{name: "student", mutate: func(in *timetableplanning.CreateInstanceInput) { in.StudentIDs = []int64{foreignStudent.ID} }},
		{name: "created by", mutate: func(in *timetableplanning.CreateInstanceInput) { in.CreatedByStaffID = &foreignStaff.ID }},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Error(t, err)
			assert.Nil(t, inst)
			assert.ErrorIs(t, err, timetableplanning.ErrInvalidInstanceReference)
...
func TestInstance_Create_SpontaneousAndMissingTenant(t *testing.T) {
	...
	isSpontaneous := true
	linked, err := s.svc.Create(s.ctx, timetableplanning.CreateInstanceInput{
	...
	_, err = s.svc.Create(context.Background(), timetableplanning.CreateInstanceInput{
...
func TestInstance_UpdatePlanned_ReplacesAssignmentsAndFields(t *testing.T) {
	...
	updated, err := s.svc.UpdatePlanned(s.ctx, ai.ID, timetableplanning.UpdateInstanceInput{
		Date:       timezone.NewDate(2026, 5, 7),
...
func TestInstance_Start_TimePolicyAppliesToNoOfferingPlannedBlock(t *testing.T) {
	...
	planned, err := guarded.Create(s.ctx, timetableplanning.CreateInstanceInput{
		Date:      date,
		StartTime: time.Date(1, 1, 1, 14, 0, 0, 0, time.UTC),
		EndTime:   time.Date(1, 1, 1, 15, 0, 0, 0, time.UTC),
	...
	require.ErrorIs(t, err, timetableplanning.ErrInstanceStartTooEarly,
		"planning-module block without offering must hit the lead-time guard")

	isSpontaneous := true
	adhoc, err := guarded.Create(s.ctx, timetableplanning.CreateInstanceInput{
		Date:          date,
		StartTime:     time.Date(1, 1, 1, 14, 0, 0, 0, time.UTC),
		EndTime:       time.Date(1, 1, 1, 15, 0, 0, 0, time.UTC),
		Title:         "Ad-hoc block",
...
func TestInstance_UpdatePlanned_RejectsCrossTenantReferencesBeforeMutation(t *testing.T) {
	...
	ai := seedInstance(t, s, true, true)
	valid := timetableplanning.UpdateInstanceInput{
	...
	cases := []struct {
		name   string
		mutate func(*timetableplanning.UpdateInstanceInput)
	}{
		{name: "room", mutate: func(in *timetableplanning.UpdateInstanceInput) { in.RoomID = foreignRoom.ID }},
		{name: "activity group", mutate: func(in *timetableplanning.UpdateInstanceInput) { in.ActivityGroupID = &foreignTemplate.ID }},
		{name: "staff", mutate: func(in *timetableplanning.UpdateInstanceInput) { in.StaffIDs = []int64{foreignStaff.ID} }},
		{name: "student", mutate: func(in *timetableplanning.UpdateInstanceInput) { in.StudentIDs = []int64{foreignStudent.ID} }},
	}
...
func TestInstance_UpdatePlanned_RejectsNonPlanned(t *testing.T) {
	...
	for _, status := range []string{
		...
		t.Run(status, func(t *testing.T) {
			...
			_, err := s.svc.UpdatePlanned(s.ctx, ai.ID, timetableplanning.UpdateInstanceInput{
...
func TestInstance_DeleteCancelled_PlannedOrCancelled(t *testing.T) {
	...
	t.Run("rejects single delete when template has multiple same-day slots", func(t *testing.T) {
		...
		require.Error(t, err)
		assert.ErrorIs(t, err, timetableplanning.ErrAmbiguousTemplateInstanceDelete)
...
func TestInstance_StartByUnplannedStaffAddsNoSupervisor(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, result.Instance.StartedBy)
	assert.Equal(t, starter.ID, *result.Instance.StartedBy)
