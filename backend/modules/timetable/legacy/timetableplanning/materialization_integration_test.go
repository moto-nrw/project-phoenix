...
type scenarioSetup struct {
	svc           timetableplanning.MaterializationService
...
func TestMaterializeForTenant_EndToEnd(t *testing.T) {
	...
	// --- Happy path ---
	r, err := s.svc.MaterializeForTenant(s.ctx, from, to, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
	...
	// --- Idempotent re-run ---
	r2, err := s.svc.MaterializeForTenant(s.ctx, from, to, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
	...
	_, err = s.db.NewUpdate().
		...
		Exec(s.ctx)
	require.NoError(t, err)
	r3, err := s.svc.MaterializeForTenant(s.ctx, from, to, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestMaterializeForTenant_ExcludesGraduatedStudents(t *testing.T) {
	...
	r, err := s.svc.MaterializeForTenant(s.ctx, materializeDate, materializeDate.AddDays(6), timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestMaterializeForTenant_MultipleDynamicTargetsFollowClassChanges(t *testing.T) {
	...
	first, err := s.svc.MaterializeForTenant(s.ctx, firstMonday, firstMonday, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
	...
	second, err := s.svc.MaterializeForTenant(s.ctx, secondMonday, secondMonday, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestMaterializeForTenant_NoActivePeriod_ReturnsGracefully(t *testing.T) {
	...
	assert.Equal(t, timetableplanning.MaterializationWarningCodeNoActivePeriod, r.Warnings[0].Code)
...
func TestMaterializeForTenant_NoTemplates_ReturnsWarning(t *testing.T) {
	...
	require.Len(t, r.Warnings, 1)
	assert.Equal(t, timetableplanning.MaterializationWarningCodeNoTemplates, r.Warnings[0].Code)
	assert.NotEmpty(t, r.Warnings[0].Message)
}
...
func TestMaterializeForTenant_PreFetchObservesFirstRunThenSkips(t *testing.T) {
	...
	r1, err := s.svc.MaterializeForTenant(s.ctx, materializeDate, materializeDate.AddDays(6), timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
	...
	r2, err := s.svc.MaterializeForTenant(s.ctx, materializeDate, materializeDate.AddDays(6), timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
	assert.Zero(t, r2.InstancesCreated)
...
func TestMaterializeForTenant_ABWeekSmoke(t *testing.T) {
	...
	r2, err := s.svc.MaterializeForTenant(s.ctx, nextMon, nextMon.AddDays(6), timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestMaterializeForTenant_ScheduleValidUntil_SkipsEndedDates(t *testing.T) {
	...
	r0, err := s.svc.MaterializeForTenant(s.ctx, secondMonday, secondMonday, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
	...
	r, err := s.svc.MaterializeForTenant(s.ctx, firstMonday, secondMonday.AddDays(6), timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
