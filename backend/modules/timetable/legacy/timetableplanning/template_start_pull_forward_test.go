...
func TestUpdateTemplate_StartDatePullForward_MovesEnvelopeRosterAndMaterializesGapOnly(t *testing.T) {
	...
	result, err := s.factory.TimetableData.CreateTemplate(s.ctx, timetableplanning.CreateTemplateInput{
		Name:              name,
		Type:              activitiesModels.GroupTypeCare,
		Weekdays:          weekdays,
	...
	materialized, err := s.materialize.MaterializeForTenant(s.ctx, oldStart, oldStart.AddDays(1), timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
	...
	materialized, err = s.materialize.MaterializeForTenant(s.ctx, newStart, oldStart.AddDays(1), timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestUpdateTemplate_StartDatePullForward_MovesWeekdayScopedRoster(t *testing.T) {
	...
	weekdays := []int{activitiesModels.WeekdayMonday, activitiesModels.WeekdayTuesday}
	assignments := []timetableplanning.WeekdayRosterAssignment{
		{
	...
	_, err = s.materialize.MaterializeForTenant(s.ctx, newStart, newStart.AddDays(1), timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestUpdateTemplate_StartDatePullForward_MidweekDateStartsAtNextMatchingWeekday(t *testing.T) {
	...
	_, err = s.materialize.MaterializeForTenant(s.ctx, newStart, oldStart, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestUpdateTemplate_StartDatePullForward_Validation(t *testing.T) {
	...
	require.ErrorIs(t, err, timetableplanning.ErrTemplateStartNotEarlier,
	...
	require.ErrorIs(t, err, timetableplanning.ErrTemplateStartInPast,
	...
	openResult, err := s.factory.TimetableData.CreateTemplate(s.ctx, timetableplanning.CreateTemplateInput{
		Name:            openName,
		Type:            activitiesModels.GroupTypeCare,
		Weekdays:        weekdays,
...
func TestUpdateTemplate_StartDatePullForward_RejectsPredecessorOverlap(t *testing.T) {
	...
	predecessor, err := s.factory.TimetableData.CreateTemplate(s.ctx, timetableplanning.CreateTemplateInput{
	...
	successor, err := s.factory.TimetableData.CreateTemplate(s.ctx, timetableplanning.CreateTemplateInput{
		Name:              successorName,
		Type:              activitiesModels.GroupTypeCare,
		Weekdays:          weekdays,
	...
	require.ErrorIs(t, err, timetableplanning.ErrTemplateStartPredecessorOverlap,
