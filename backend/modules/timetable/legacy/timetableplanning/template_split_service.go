// Package timetableplanning — template split service (WP-B3, "Dieser und alle folgenden").
//
...
type TemplateSplitService struct {
	...
	// (#2137, #2147 review). Supplied through WithOfferingRosterResync; the
	// composition root late-binds it to the enrollment decision service.
...
func NewTemplateSplitService(deps TemplateSplitDependencies, options ...TemplateSplitOption) *TemplateSplitService {
...
// TemplateSplitOption configures an optional collaborator of the split
// service at construction.
type TemplateSplitOption func(*TemplateSplitService)

// WithOfferingRosterResync wires the enrollment-side source resync used when a
// split drops the successor's offering source.
func WithOfferingRosterResync(fn func(context.Context, OfferingRosterResyncInput) error) TemplateSplitOption {
	return func(s *TemplateSplitService) { s.resyncOfferingRoster = fn }
}
