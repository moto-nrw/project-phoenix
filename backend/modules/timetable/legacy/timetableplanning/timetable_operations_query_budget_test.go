...
func (plannedNowBudgetCareDayService) ResolveForDate(context.Context, []int64, timezone.Date) (map[int64]careplan.CareDayStatus, error) {
	return map[int64]careplan.CareDayStatus{}, nil
}

func (plannedNowBudgetCareDayService) ResolveForRange(context.Context, []int64, timezone.Date, timezone.Date) (map[int64]map[timezone.Date]careplan.CareDayStatus, error) {
...
type unusedPlannedNowInstanceService struct {
	timetableplanning.InstanceService
}
type unusedPlannedNowActiveService struct {
	timetableplanning.OperationActiveService
}
type unusedPlannedNowArrivalService struct {
	timetableplanning.OperationArrivalService
}
type unusedPlannedNowPickupService struct {
	timetableplanning.OperationPickupService
}
...
func TestTimetableOperationsPlannedNowQueryBudget(t *testing.T) {
	...
	service := timetableplanning.NewTimetableOperationsService(timetableplanning.TimetableOperationsDependencies{
	...
	run := func() int {
		counter.Reset()
		result, err := service.PlannedNow(ctx, 1, false, date, now, timetableplanning.PlannedNowOptions{})
