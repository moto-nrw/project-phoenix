...
func TestValidateAttendancePatch_CrossFieldRule(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			errs := timetableplanning.ValidateAttendancePatch(tc.patch, tc.current)
			if tc.wantOK {
				assert.Empty(t, errs)
				return
			}
...
func TestValidateAttendancePatch_PerFieldErrors(t *testing.T) {
	...
	t.Run("invalid status", func(t *testing.T) {
		errs := timetableplanning.ValidateAttendancePatch(scheduleModel.AttendanceFieldPatch{Status: testpkg.StrPtr("ghost")}, cur)
		require.Len(t, errs, 1)
		assert.Equal(t, "status", errs[0].Field)
	})

	t.Run("invalid substatus", func(t *testing.T) {
		errs := timetableplanning.ValidateAttendancePatch(scheduleModel.AttendanceFieldPatch{Substatus: testpkg.StrPtr("banana")}, cur)
		require.Len(t, errs, 1)
	...
	t.Run("note too long", func(t *testing.T) {
		...
		errs := timetableplanning.ValidateAttendancePatch(scheduleModel.AttendanceFieldPatch{Note: &tooLong}, cur)
		require.Len(t, errs, 1)
	...
	t.Run("two per-field errors returned together", func(t *testing.T) {
		...
		errs := timetableplanning.ValidateAttendancePatch(scheduleModel.AttendanceFieldPatch{
