...
func (f *sourcedWeekdayStaffFixture) staffOnlyAssignments() []timetableplanning.WeekdayRosterAssignment {
	return []timetableplanning.WeekdayRosterAssignment{
...
func (f *sourcedWeekdayStaffFixture) assertWeekOfTemplate(t *testing.T, templateID int64, monday timezone.Date, wantKids []int64) {
	...
	_, err := f.s.factory.Materialization.MaterializeForTenant(
		f.s.ctx, monday, monday.AddDays(2), timetableplanning.MaterializationSourceManual,
	)
...
func TestTemplateOfferingSource_CreateWithWeekdayStaffMaterializesEachDaysSupervisor(t *testing.T) {
	...
	result, err := f.s.factory.TimetableData.CreateTemplate(f.s.ctx, timetableplanning.CreateTemplateInput{
...
func TestTemplateOfferingSource_UpdateToSourceKeepsWeekdayStaff(t *testing.T) {
	...
	require.NoError(t, f.s.factory.TimetableData.UpdateTemplate(f.s.ctx, timetableplanning.TemplateUpdateInput{
...
func TestTemplateOfferingSource_SplitKeepsWeekdayStaff(t *testing.T) {
	...
	split, err := f.s.factory.TemplateSplit.Split(f.s.ctx, timetableplanning.TemplateSplitInput{
		TemplateID:       result.TemplateID,
...
type offeringRosterResyncer interface {
	ResyncTemplateOfferingRoster(context.Context, timetableplanning.OfferingRosterResyncInput) error
}
...
func TestTemplateOfferingSource_NewlyBookedChildJoinsWithoutTouchingWeekdayStaff(t *testing.T) {
	...
	require.NoError(t, testpkg.WithTenantTx(t, f.s.ctx, f.s.db, f.s.tenantID, func(txCtx context.Context, _ bun.Tx) error {
		return resyncer.ResyncTemplateOfferingRoster(txCtx, timetableplanning.OfferingRosterResyncInput{
			TemplateID:       result.TemplateID,
