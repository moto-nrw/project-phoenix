...
// from today on (#3594), except those of series that include closing days.
...
// re-plan recreates it). Guardians are not notified. dryRun only counts.
...
func (s *instanceService) BulkCancelPlanned(ctx context.Context, from, to timezone.Date, dryRun bool, actorAccountID *int64) (*timetable.BulkCancelResult, error) {
	...
	if !s.hasTx(ctx) {
		...
		err := tenant.WithinCurrentTenant(ctx, func(txCtx context.Context) error {
			var bulkErr error
			result, bulkErr = s.BulkCancelPlanned(txCtx, from, to, dryRun, actorAccountID)
			return bulkErr
		})
		return result, err
	}
	...
	ids, result := timetable.SelectBulkCancel(candidates, from.String(), to.String(), today)
...
// day flag of their series: one instance query and one group query.
func (s *instanceService) bulkCancelCandidates(ctx context.Context, from, to timezone.Date) ([]timetable.BulkCancelCandidate, error) {
	...
	includesClosingDays := make(map[int64]bool, len(groupIDs))
	if len(groupIDs) > 0 {
		...
		for _, group := range groups {
			includesClosingDays[group.ID] = group.IncludeClosingDays
		}
	}
	candidates := make([]timetable.BulkCancelCandidate, 0, len(rows))
	for _, row := range rows {
		candidate := timetable.BulkCancelCandidate{InstanceID: row.ID, Date: row.Date.String(), Status: row.Status}
		if row.ActivityGroupID != nil {
			candidate.SeriesIncludesClosingDays = includesClosingDays[*row.ActivityGroupID]
		}
