...
func poolEntryOf(t *testing.T, pool *timetableplanning.StaffPoolResult, staffID int64) timetableplanning.StaffPoolEntry {
	t.Helper()
	...
	return timetableplanning.StaffPoolEntry{}
}
...
func TestGetStaffPoolForInstance_Categories(t *testing.T) {
	...
	assert.Equal(t, timetableplanning.StaffPoolAssignedElsewhere, moving.Category)
	...
	assert.Equal(t, timetableplanning.StaffPoolAssignedHere, here.Category)
	assert.False(t, here.OnShift, "no shift seeded for the target member")

	freeEntry := poolEntryOf(t, pool, free.ID)
	assert.Equal(t, timetableplanning.StaffPoolOnShiftFree, freeEntry.Category)
	...
	assert.Equal(t, timetableplanning.StaffPoolOnShiftFree, partialEntry.Category)
	...
	assert.Equal(t, timetableplanning.StaffPoolAbsent, absentEntry.Category, "day-wide absence outranks assignments and shifts")
...
func TestGetStaffPoolForInstance_NoShiftsMeansDienstplanNotInUse(t *testing.T) {
	...
	assert.Equal(t, timetableplanning.StaffPoolNotOnShift, entry.Category)
...
func TestGetStaffPoolForInstance_TouchingWindowsDoNotOverlap(t *testing.T) {
	...
	entry := poolEntryOf(t, pool, s.staffID)
	assert.Equal(t, timetableplanning.StaffPoolOnShiftFree, entry.Category)
...
func TestGetStaffPoolForInstance_TerminalBlockAbsenceRemainsDayWide(t *testing.T) {
	...
	for _, status := range []string{
		...
		t.Run(status, func(t *testing.T) {
			...
			assert.Equal(t, timetableplanning.StaffPoolAbsent, absent.Category)
			...
			assert.Equal(t, timetableplanning.StaffPoolOnShiftFree, historicalOnly.Category)
