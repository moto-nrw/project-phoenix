...
func TestMoveStaffBetweenBlocks_RelocatesRowAndLogsEvent(t *testing.T) {
	...
	result, err := s.factory.Instance.MoveStaffBetweenBlocks(s.ctx, s.target.ID, timetableplanning.MoveStaffInput{
		StaffID:          s.staffID,
		SourceInstanceID: &s.source.ID,
	})
	require.NoError(t, err)
	assert.Equal(t, timetableplanning.MoveStaffActionMoved, result.Action)
...
func TestMoveStaffBetweenBlocks_RetryIsIdempotent(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, timetableplanning.MoveStaffActionAlreadyApplied, result.Action)
...
func TestMoveStaffBetweenBlocks_ValidationFailures(t *testing.T) {
	...
	t.Run("not assigned to source", func(t *testing.T) {
		s := setup(t)
		_, err := s.factory.Instance.MoveStaffBetweenBlocks(s.ctx, s.target.ID, timetableplanning.MoveStaffInput{
			StaffID:          s.otherID,
			SourceInstanceID: &s.source.ID,
		})
	...
	t.Run("cross-date move rejected", func(t *testing.T) {
		...
		_, err = s.factory.Instance.MoveStaffBetweenBlocks(s.ctx, otherDay.ID, timetableplanning.MoveStaffInput{
			StaffID:          s.staffID,
			SourceInstanceID: &s.source.ID,
		})
		de := requireDeviationErr(t, err)
	...
	t.Run("cancelled target", func(t *testing.T) {
		...
		_, err := s.factory.Instance.MoveStaffBetweenBlocks(s.ctx, cancelled.ID, timetableplanning.MoveStaffInput{
			StaffID:          s.staffID,
			SourceInstanceID: &s.source.ID,
		})
		de := requireDeviationErr(t, err)
...
func TestMoveStaffBetweenBlocks_ActiveBlocksSyncSupervisionAndAllowRoundTrip(t *testing.T) {
	...
	reverse, err := s.factory.Instance.MoveStaffBetweenBlocks(s.ctx, s.source.ID, timetableplanning.MoveStaffInput{
		StaffID:          s.staffID,
		SourceInstanceID: &s.target.ID,
	})
	require.NoError(t, err)
