...
func instanceServiceWithGuardianNotices(s *lifecycleSetup, publisher announcement.CareCancellationPublisher) timetableplanning.InstanceService {
	deps := lifecycleDependencies(s, nil)
	deps.GuardianNotices = publisher
	return timetableplanning.NewInstanceService(deps)
}
...
func noticeInput(instanceID int64, actor int64, notice *timetableplanning.GuardianNoticeInput) timetableplanning.CancelInstanceInput {
	return timetableplanning.CancelInstanceInput{
...
func TestCancelWithNotice_PublishesForBookedChildrenOnly(t *testing.T) {
	...
	svc := instanceServiceWithGuardianNotices(s, publisher)
	...
	result, err := svc.CancelWithNotice(s.ctx, noticeInput(instance.ID, actor.ID, &timetableplanning.GuardianNoticeInput{
...
func TestCancelWithNotice_RefusesBeforeCancellingWhenInvalid(t *testing.T) {
	...
	t.Run("empty text", func(t *testing.T) {
		instance, _, _ := seedNoticeInstance(t, s, timezone.TodayDate().AddDays(1))
		_, err := svc.CancelWithNotice(s.ctx, noticeInput(instance.ID, actor.ID, &timetableplanning.GuardianNoticeInput{Title: "x"}))
		require.ErrorIs(t, err, timetableplanning.ErrGuardianNoticeInvalid)
	...
	t.Run("whitespace-only text", func(t *testing.T) {
		instance, _, _ := seedNoticeInstance(t, s, timezone.TodayDate().AddDays(1))
		_, err := svc.CancelWithNotice(s.ctx, noticeInput(instance.ID, actor.ID, &timetableplanning.GuardianNoticeInput{Title: "  ", Message: "\t"}))
		require.ErrorIs(t, err, timetableplanning.ErrGuardianNoticeInvalid)
		reloaded, err := s.repos.ActivityInstance.FindByID(s.ctx, instance.ID)
		require.NoError(t, err)
	...
	t.Run("overlong text", func(t *testing.T) {
		instance, _, _ := seedNoticeInstance(t, s, timezone.TodayDate().AddDays(1))
		_, err := svc.CancelWithNotice(s.ctx, noticeInput(instance.ID, actor.ID, &timetableplanning.GuardianNoticeInput{Title: strings.Repeat("x", 201), Message: "Text"}))
		require.ErrorIs(t, err, timetableplanning.ErrGuardianNoticeInvalid)
		reloaded, err := s.repos.ActivityInstance.FindByID(s.ctx, instance.ID)
		require.NoError(t, err)
		assert.Equal(t, scheduleModels.InstanceStatusPlanned, reloaded.Status)
	...
	t.Run("past block", func(t *testing.T) {
		instance, _, _ := seedNoticeInstance(t, s, timezone.TodayDate().AddDays(-1))
		_, err := svc.CancelWithNotice(s.ctx, noticeInput(instance.ID, actor.ID, &timetableplanning.GuardianNoticeInput{Title: "x", Message: "y"}))
		require.ErrorIs(t, err, timetableplanning.ErrGuardianNoticeInvalid)
	})

	t.Run("no actor", func(t *testing.T) {
		instance, _, _ := seedNoticeInstance(t, s, timezone.TodayDate().AddDays(1))
		_, err := svc.CancelWithNotice(s.ctx, timetableplanning.CancelInstanceInput{
			InstanceID:     instance.ID,
			GuardianNotice: &timetableplanning.GuardianNoticeInput{Title: "x", Message: "y"},
		})
		require.ErrorIs(t, err, timetableplanning.ErrGuardianNoticeInvalid)
	})
	...
	t.Run("school switched it off", func(t *testing.T) {
		...
		_, err := svc.CancelWithNotice(s.ctx, noticeInput(instance.ID, actor.ID, &timetableplanning.GuardianNoticeInput{Title: "x", Message: "y"}))
		require.ErrorIs(t, err, timetableplanning.ErrGuardianNoticeDisabled)
		reloaded, err := s.repos.ActivityInstance.FindByID(s.ctx, instance.ID)
		require.NoError(t, err)
		assert.Equal(t, scheduleModels.InstanceStatusPlanned, reloaded.Status)
	})
