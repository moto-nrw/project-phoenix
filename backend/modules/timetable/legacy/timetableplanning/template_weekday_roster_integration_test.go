...
type weekdayRosterScenario struct {
	...
	materialize timetableplanning.MaterializationService
...
func TestCreateTemplate_PerWeekdayRoster_MaterializesTheWeekdaysOwnPeople(t *testing.T) {
	...
	result, err := s.factory.TimetableData.CreateTemplate(s.ctx, timetableplanning.CreateTemplateInput{
		...
		GradeLevelMax:   4,
		WeekdayAssignments: []timetableplanning.WeekdayRosterAssignment{
			{
	...
	materialized, err := s.materialize.MaterializeForTenant(s.ctx, monday, tuesday, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestCreateTemplate_PerWeekdayRoster_UnlistedWeekdayKeepsSharedDefault(t *testing.T) {
	...
	_, err = s.materialize.MaterializeForTenant(s.ctx, monday, tuesday, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestUpdateTemplate_SwitchesBetweenSharedAndPerWeekdayRoster(t *testing.T) {
	...
	updateInput := timetableplanning.TemplateUpdateInput{
...
func TestUpdateTemplate_PinningPeriodRetiresUnscopedEditorRoster(t *testing.T) {
	...
	require.NoError(t, s.factory.TimetableData.UpdateTemplate(s.ctx, timetableplanning.TemplateUpdateInput{
		TemplateID: result.TemplateID,
		Fields: activitiesModels.TemplateFieldsUpdate{
			Name:             name,
			Type:             activitiesModels.GroupTypeCare,
...
func TestUpdateTemplate_UnpinningPeriodRetiresScopedEditorRoster(t *testing.T) {
	...
	_, err = s.materialize.MaterializeForTenant(s.ctx, monday, monday, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestCreateTemplate_WeekdayAssignmentOutsideRecurrence_IsRejected(t *testing.T) {
	...
	_, err := s.factory.TimetableData.CreateTemplate(s.ctx, timetableplanning.CreateTemplateInput{
	...
	require.ErrorIs(t, err, timetableplanning.ErrWeekdayAssignmentUnscheduled)
}
