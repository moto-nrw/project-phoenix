...
func sessionBlocksFixture(t *testing.T) (*timetableOpsTestDeps, timezone.Date, map[int64][]int64) {
	t.Helper()
	...
	day := timezone.NewDate(2026, time.September, 16)
	deps.service.(*timetableOperationsService).deps.Now = func() time.Time {
		return day.BerlinMidnight().Add(12 * time.Hour)
	}

	block := func(id, activeGroupID int64, start, end int, status string) *scheduleModel.ActivityInstance {
		...
		inst.Date = scheduleModel.Date(day.String())
		inst.Title = "GT"
		if activeGroupID > 0 {
	...
	deps.instanceRepo.byDate = []*scheduleModel.ActivityInstance{
		block(41, 51, 13, 14, scheduleModel.InstanceStatusActive),
		block(42, 52, 13, 15, scheduleModel.InstanceStatusActive),
		block(43, 53, 14, 15, scheduleModel.InstanceStatusActive),
		block(44, 54, 14, 16, scheduleModel.InstanceStatusActive),
		block(45, 0, 16, 17, scheduleModel.InstanceStatusPlanned),
	}
	deps.staffRepo.byInstance[41] = []*scheduleModel.InstanceStaff{{StaffID: callerStaffID}, {StaffID: colleagueStaffID}}
	deps.staffRepo.byInstance[42] = []*scheduleModel.InstanceStaff{{StaffID: colleagueStaffID}}
	deps.staffRepo.byInstance[43] = []*scheduleModel.InstanceStaff{{StaffID: colleagueStaffID}, {StaffID: callerStaffID, IsAbsent: true}}
	deps.staffRepo.byInstance[44] = []*scheduleModel.InstanceStaff{{StaffID: callerStaffID}}
...
func blocksBySession(blocks []OperationSessionBlock) map[int64]OperationSessionBlock {
	result := make(map[int64]OperationSessionBlock, len(blocks))
...
func TestTimetableOperationsSessionBlocksMirrorCanOperate(t *testing.T) {
	...
	assert.Equal(t, OperationSessionBlock{
