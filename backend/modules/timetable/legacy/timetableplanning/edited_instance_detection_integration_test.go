...
func materializeSingleInstance(t *testing.T, s *scenarioSetup) *scheduleModels.ActivityInstance {
	t.Helper()
	_, err := s.svc.MaterializeForTenant(s.ctx, editWindowStart, editWindowStart.AddDays(6), timetableplanning.MaterializationSourceManual)
...
func detect(t *testing.T, s *scenarioSetup) []timetableplanning.EditedOccurrence {
	return detectWindow(t, s, false)
}

func detectWindow(t *testing.T, s *scenarioSetup, includeDeletions bool) []timetableplanning.EditedOccurrence {
	t.Helper()
	...
	for _, targetType := range []string{
		...
		t.Run(targetType, func(t *testing.T) {
			...
			result, err := s.svc.MaterializeForTenant(
				s.ctx, editWindowStart, editWindowStart.AddDays(6), timetableplanning.MaterializationSourceManual,
			)
			require.NoError(t, err)
...
func TestDetectEditedInWindow_TitleEdit(t *testing.T) {
	...
	assert.Equal(t, []string{timetableplanning.EditedChangeTitle}, edited[0].Changes)
...
func TestDetectEditedInWindow_NotesEdit(t *testing.T) {
	...
	edited := detect(t, s)
	require.Len(t, edited, 1)
	assert.Equal(t, []string{timetableplanning.EditedChangeNotes}, edited[0].Changes)
}
...
func TestDetectEditedInWindow_DescriptionEdit(t *testing.T) {
	...
	edited := detect(t, s)
	require.Len(t, edited, 1)
	assert.Equal(t, []string{timetableplanning.EditedChangeDescription}, edited[0].Changes)
}
...
func TestDetectEditedInWindow_RoomEdit(t *testing.T) {
	...
	edited := detect(t, s)
	require.Len(t, edited, 1)
	assert.Equal(t, []string{timetableplanning.EditedChangeRoom}, edited[0].Changes)
}
...
func TestDetectEditedInWindow_TimeMove(t *testing.T) {
	...
	edited := detect(t, s)
	require.Len(t, edited, 1)
	assert.Equal(t, []string{timetableplanning.EditedChangeTime}, edited[0].Changes)
}
...
func TestDetectEditedInWindow_StudentRosterEdit(t *testing.T) {
	...
	edited := detect(t, s)
	require.Len(t, edited, 1)
	assert.Equal(t, []string{timetableplanning.EditedChangeStudents}, edited[0].Changes)
}
...
func TestDetectEditedInWindow_AttendanceStateDoesNotChangeRosterMembership(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			edited := detect(t, s)
			require.Len(t, edited, 1)
			assert.Equal(t, []string{timetableplanning.EditedChangeAttendance}, edited[0].Changes,
				"unrestored attendance is a lost edit, but not a roster-membership change")
			assert.NotContains(t, edited[0].Changes, timetableplanning.EditedChangeStudents)
...
func TestDetectEditedInWindow_StaffRosterEdit(t *testing.T) {
	...
	edited := detect(t, s)
	require.Len(t, edited, 1)
	assert.Equal(t, []string{timetableplanning.EditedChangeStaff}, edited[0].Changes)
}
...
func TestDetectEditedInWindow_DeletedOccurrence(t *testing.T) {
	...
	require.Len(t, edited, 1)
	assert.Equal(t, editWindowStart, edited[0].Date)
	assert.Equal(t, []string{timetableplanning.EditedChangeDeleted}, edited[0].Changes)
}
