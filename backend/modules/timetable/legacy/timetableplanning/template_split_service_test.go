...
func baseSplitInput(s *scenarioSetup, effective timezone.Date, name string) timetableplanning.TemplateSplitInput {
...
func linkedTemplateUpdateInput(t *testing.T, s *scenarioSetup, periodID *int64) timetableplanning.TemplateUpdateInput {
	t.Helper()
...
func TestTemplateMutations_RejectCareOfferingSeriesConflictsWithoutPersisting(t *testing.T) {
	...
	t.Run("split", func(t *testing.T) {
		...
		require.ErrorIs(t, err, timetableplanning.ErrTemplateCareOfferingConflict)
		require.ErrorIs(t, err, timetableplanning.ErrSplitInvalidInput)
...
func TestTemplatePeriodChange_RebasesProtectedEnrollmentForMaterialization(t *testing.T) {
	...
	t.Run("split A to B", func(t *testing.T) {
		...
		materialized, err := s.svc.MaterializeForTenant(s.ctx, effective, effective, timetableplanning.MaterializationSourceManual)
		require.NoError(t, err)
...
func TestTemplateSplit_ProtectedWeekdayDoesNotSuppressManualOtherWeekday(t *testing.T) {
	...
	materialized, err := s.svc.MaterializeForTenant(s.ctx, tuesday, tuesday, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
	require.Equal(t, 1, materialized.InstancesCreated)
...
func TestTemplateSplit_SharedRosterDoesNotBroadenProtectedWeekdays(t *testing.T) {
	...
	materialized, err := s.svc.MaterializeForTenant(s.ctx, monday, tuesday, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestTemplateSplit_HappyPath_CarriesRosterAndProtectsHistory(t *testing.T) {
	...
	r0, err := s.svc.MaterializeForTenant(s.ctx, effective, secondMonday, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestTemplateEndFromDate_CapsTemplateAndProtectsHistory(t *testing.T) {
	...
	res, err := s.factory.TemplateSplit.EndFromDate(s.ctx, timetableplanning.TemplateEndInput{
		TemplateID:    s.template.ID,
		EffectiveDate: effective,
	})
	require.NoError(t, err)
	...
	r1, err := s.svc.MaterializeForTenant(s.ctx, effective, secondMonday, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestTemplateSplit_ValidationErrors(t *testing.T) {
	...
	t.Run("past effective_date", func(t *testing.T) {
		in := baseSplitInput(s, timezone.NewDate(2000, 1, 1), fmt.Sprintf("Split-Vergangen-%d", suffix))
		_, err := s.factory.TemplateSplit.Split(s.ctx, in)
		require.ErrorIs(t, err, timetableplanning.ErrSplitInvalidInput)
	})
	...
	t.Run("non-template id", func(t *testing.T) {
		...
		_, err = s.factory.TemplateSplit.Split(s.ctx, in)
		require.ErrorIs(t, err, timetableplanning.ErrSplitTemplateNotFound)
	})
...
func TestTemplateSplit_SuccessorValidFrom_NoPhantomBeforeEffective(t *testing.T) {
	...
	r, err := s.svc.MaterializeForTenant(s.ctx, prevMonday, prevMonday, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestTemplateSplit_UpdateSegmentsPreservesBoundsDuringMaterialization(t *testing.T) {
	...
	err = s.factory.TimetableData.UpdateTemplate(s.ctx, timetableplanning.TemplateUpdateInput{
		TemplateID: s.template.ID,
		Fields: activitiesModels.TemplateFieldsUpdate{
	...
	require.ErrorIs(t, err, timetableplanning.ErrTemplateSegmentNotEditable)
	...
	_, err = s.svc.MaterializeForTenant(s.ctx, previousMonday, effective, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestTemplateSplit_RejectsResplittingBoundedPredecessor(t *testing.T) {
	...
	materialized, err := s.svc.MaterializeForTenant(
		s.ctx, outerBoundary, afterOuter, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestTemplateSplitAndEnd_RespectCurrentSegmentEnvelope(t *testing.T) {
	...
	t.Run("split and end reject predecessor exclusive end", func(t *testing.T) {
		...
		require.ErrorIs(t, splitErr, timetableplanning.ErrSplitInvalidInput)
		_, endErr := s.factory.TemplateSplit.EndFromDate(s.ctx, timetableplanning.TemplateEndInput{
			TemplateID:    s.template.ID,
			EffectiveDate: boundary,
		})
		require.ErrorIs(t, endErr, timetableplanning.ErrSplitInvalidInput)
	})

	t.Run("end before future successor clamps to its start", func(t *testing.T) {
		ended, err := s.factory.TemplateSplit.EndFromDate(s.ctx, timetableplanning.TemplateEndInput{
...
func TestTemplateSplit_SingleEditThenSuccessorUpdateDoesNotDuplicate(t *testing.T) {
	...
	materialized, err := s.svc.MaterializeForTenant(s.ctx, singleDate, singleDate, timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
	require.Equal(t, 1, materialized.InstancesCreated)
...
func TestTemplateEnd_ConcurrentTemplateUpdatePreservesCommittedCap(t *testing.T) {
	...
	update := timetableplanning.TemplateUpdateInput{
		TemplateID: s.template.ID,
		Fields: activitiesModels.TemplateFieldsUpdate{
	...
	go func() {
		endDone <- testpkg.WithTenantTx(t, s.ctx, s.db, s.tenantID, func(txCtx context.Context, _ bun.Tx) error {
			_, err := s.factory.TemplateSplit.EndFromDate(txCtx, timetableplanning.TemplateEndInput{
				TemplateID:    s.template.ID,
				EffectiveDate: effective,
			})
			if err != nil {
...
func TestTemplateEnd_ConcurrentMaterializationCannotInsertPastCommittedCap(t *testing.T) {
	...
	go func() {
		...
		result, err := s.svc.MaterializeForTenant(s.ctx, effective, effective, timetableplanning.MaterializationSourceManual)
...
func TestTemplateArchive_ConcurrentMaterializationCannotInsertStaleOccurrence(t *testing.T) {
	...
	go func() {
		close(materializeStarted)
		result, err := s.svc.MaterializeForTenant(s.ctx, date, date, timetableplanning.MaterializationSourceManual)
		materializeDone <- materializeOutcome{result: result, err: err}
	}()
	<-materializeStarted
	...
	assert.Equal(t, timetableplanning.MaterializationWarningCodeNoTemplates, outcome.result.Warnings[0].Code)
...
func TestTemplateSplit_WindowValidation(t *testing.T) {
	...
	t.Run("window exceeds max days after clamping", func(t *testing.T) {
		...
		from := effective
		to := effective.AddDays(timetableplanning.MaxMaterializationWindowDays) // span = max+1
		in.MaterializeFrom = &from
		in.MaterializeTo = &to
		_, err := s.factory.TemplateSplit.Split(s.ctx, in)
		require.ErrorIs(t, err, timetableplanning.ErrSplitInvalidInput)
...
func TestTemplateSplit_ValidationErrors_RejectsInvalidTargetGroup(t *testing.T) {
	...
	_, err := s.factory.TemplateSplit.Split(s.ctx, in)
	require.Error(t, err)
	assert.ErrorIs(t, err, timetableplanning.ErrSplitInvalidInput)
}
