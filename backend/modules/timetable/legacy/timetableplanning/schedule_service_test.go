...
func setupScheduleService(t *testing.T, db *bun.DB) timetableplanning.Service {
	t.Helper()
...
func TestScheduleService_TimeframeCareOfferingGuard(t *testing.T) {
	...
	require.ErrorIs(t, err, timetableModule.ErrTimeframeRequiredByCareOffering)
	...
	missingLockService := timetableplanning.NewServiceWithConfig(timetableplanning.ServiceConfig{
		RecurrenceEvents:                    timetabletest.New(t, db),
		DateframeRepo:                       repos.Dateframe,
		TimeframeRepo:                       repos.Timeframe,
		RecurrenceRuleRepo:                  repos.RecurrenceRule,
