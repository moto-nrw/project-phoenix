// Package timetableplanning — materialization service (WP-B8).
//
...
// MaterializationOption configures an optional collaborator of the
// materialization service at construction.
type MaterializationOption func(*materializationService)

// WithCareBoundReader wires the per-date care filter (#2487). Without it the
// materializer behaves exactly as before, which keeps bare unit-test services
// compiling and correct.
func WithCareBoundReader(reader CareBoundReader) MaterializationOption {
	return func(s *materializationService) { s.careBounds = reader }
}
