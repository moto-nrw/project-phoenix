...
func TestTemplateEnd_CascadesToCappedPredecessors(t *testing.T) {
	...
	result, err := c.factory.TemplateSplit.EndFromDate(c.ctx, timetableplanning.TemplateEndInput{
...
func TestTemplateEnd_FromCappedPredecessorAlsoEndsLivingSuccessor(t *testing.T) {
	...
	_, err := c.svc.MaterializeForTenant(
		c.ctx, c.secondBoundary, c.secondBoundary.AddDays(6), timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestTemplateEnd_CascadeLeavesUnrelatedSeriesAlone(t *testing.T) {
	...
	other, err := c.factory.TimetableData.CreateTemplate(c.ctx, timetableplanning.CreateTemplateInput{
	...
	_, err = c.svc.MaterializeForTenant(
		c.ctx, c.firstBoundary, c.secondBoundary.AddDays(-1), timetableplanning.MaterializationSourceManual)
	require.NoError(t, err)
	registerSplitInstancesForCleanup(t, c.scenarioSetup,
	...
	_, err = c.factory.TemplateSplit.EndFromDate(c.ctx, timetableplanning.TemplateEndInput{
		TemplateID:    c.livingID,
		EffectiveDate: c.anchor,
	})
	require.NoError(t, err)
