...
func TestMaterializeForTenant_DuplicateInsertRaceDoesNotCopyChildren(t *testing.T) {
	...
	result, err := svc.MaterializeForTenant(
		...
		date,
		date,
		timetable.MaterializationSourceManual,
	)
...
func TestMaterializeForTenant_PreconditionWarnings(t *testing.T) {
	...
	t.Run("warns and no-ops without active periods", func(t *testing.T) {
		...
		result, err := svc.MaterializeForTenant(context.Background(), date, date, timetable.MaterializationSourceManual)

		require.NoError(t, err)
		require.Len(t, result.Warnings, 1)
		assert.Equal(t, timetable.MaterializationWarningCodeNoActivePeriod, result.Warnings[0].Code)
		assert.Zero(t, result.InstancesCreated)
	})
	...
	t.Run("warns and no-ops without templates", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Len(t, result.Warnings, 1)
		assert.Equal(t, timetable.MaterializationWarningCodeNoTemplates, result.Warnings[0].Code)
		assert.Zero(t, result.InstancesCreated)
	})
}
...
func TestMaterializeForTenant_ErrorBranches(t *testing.T) {
	...
	makeSvc := func(
		...
		return newMaterializationForTest(
			groupRepo,
			scheduleRepo,
			enrollmentRepo,
			supervisorRepo,
...
func newMaterializationBranchService(instanceRepo materializationFakeInstanceRepo) (timetable.MaterializationCapability, timezone.Date) {
	date := timezone.NewDate(2026, 4, 20)
...
func newMaterializationBranchServiceForSchedule(
	...
	date timezone.Date,
	weekday int,
) (timetable.MaterializationCapability, timezone.Date) {
	start := time.Date(2024, time.January, 1, 14, 0, 0, 0, time.UTC)
	end := time.Date(2024, time.January, 1, 15, 0, 0, 0, time.UTC)
	roomID := int64(700)
...
// newMaterializationForTest composes the materialization over fakes; db and
// staffing may be nil.
func newMaterializationForTest(
	...
	svc, err := NewMaterialization(MaterializationDependencies{
		GroupRepo:      groupRepo,
