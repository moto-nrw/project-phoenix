...
func (s *TemplateService) CreateTemplate(ctx context.Context, in CreateTemplateInput) (*timetable.CreateTemplateResult, error) {
	tenantID, err := s.validateTemplateCreateRequest(ctx, in)
	if err != nil {
		return nil, err
	}

	var result timetable.CreateTemplateResult
...
func (s *TemplateService) validateTemplateCreateRequest(ctx context.Context, in CreateTemplateInput) (int64, error) {
	if err := validateTemplateCreateInput(in, s.deps.SchoolClasses); err != nil {
	...
	if s.deps.ActivityGroupRepo == nil || s.deps.ActivityScheduleRepo == nil ||
		...
		s.deps.TimeframeRepo == nil || s.deps.EducationGroups == nil ||
...
func validateTemplateCreateInput(in CreateTemplateInput, rules SchoolClassRules) error {
	if in.Name == "" {
		return errors.New("name is required")
	}
	if err := validateTemplateCreateRecurrence(in); err != nil {
		return err
	}
	if in.CategoryID <= 0 {
...
func validateTemplateCreateRecurrence(in CreateTemplateInput) error {
...
func validateOfferingSourceInput(
	...
	if len(sourceOfferingIDs) == 0 {
		return validateFilterWithoutSource(gradeLevels, schoolClasses)
	}
	if err := validateSourceOfferingIDs(sourceOfferingIDs); err != nil {
		return err
	}
	if targetGroupType != activitiesModel.TargetGroupTypeAngebot {
		return fmt.Errorf("%w: source_care_offering_ids requires target group type 'angebot'", timetable.ErrOfferingSourceInvalid)
	}
	if err := validateSourceGradeLevels(gradeLevels, rules); err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf("%w: %s", timetable.ErrOfferingSourceInvalid, err.Error())
	}
	if len(gradeLevels) > 0 && len(normalizedClasses) > 0 {
		return fmt.Errorf("%w: source_school_classes and source_grade_levels cannot be combined", timetable.ErrOfferingSourceInvalid)
	}
	return validateSourcedRosterInput(studentIDs, weekdayAssignments)
}

func validateFilterWithoutSource(gradeLevels []int, schoolClasses []string) error {
	if len(gradeLevels) > 0 {
		return fmt.Errorf("%w: source_grade_levels requires source_care_offering_ids", timetable.ErrOfferingSourceInvalid)
	}
	if len(schoolClasses) > 0 {
		return fmt.Errorf("%w: source_school_classes requires source_care_offering_ids", timetable.ErrOfferingSourceInvalid)
	}
	return nil
}

func validateSourceOfferingIDs(sourceOfferingIDs []int64) error {
	seenOfferings := make(map[int64]bool, len(sourceOfferingIDs))
	for _, offeringID := range sourceOfferingIDs {
		if offeringID <= 0 {
			return fmt.Errorf("%w: source_care_offering_ids entries must be positive", timetable.ErrOfferingSourceInvalid)
		}
		if seenOfferings[offeringID] {
			return fmt.Errorf("%w: source_care_offering_ids must not contain duplicates", timetable.ErrOfferingSourceInvalid)
		}
...
func validateSourceGradeLevels(gradeLevels []int, rules SchoolClassRules) error {
	seenGrades := make(map[int]bool, len(gradeLevels))
	for _, level := range gradeLevels {
		if level < rules.MinGradeLevel || level > rules.MaxGradeLevel {
			return fmt.Errorf(
				"%w: source_grade_levels entries must be between %d and %d",
				timetable.ErrOfferingSourceInvalid, rules.MinGradeLevel, rules.MaxGradeLevel,
			)
		}
		if seenGrades[level] {
			return fmt.Errorf("%w: source_grade_levels must not contain duplicates", timetable.ErrOfferingSourceInvalid)
		}
...
// validateSourcedRosterInput rejects a manual child roster next to a source.
...
// Per-weekday STAFF stays with the planner (#3165): the source decides which
// children come, not who supervises them on which weekday.
func validateSourcedRosterInput(studentIDs []int64, weekdayAssignments []timetable.WeekdayRosterAssignment) error {
	if len(studentIDs) > 0 {
		return fmt.Errorf("%w: student_ids must be empty when an offering source is set", timetable.ErrOfferingSourceInvalid)
	}
	for _, assignment := range weekdayAssignments {
		if len(assignment.StudentIDs) > 0 {
			return fmt.Errorf("%w: weekday_assignments must not contain student_ids when an offering source is set", timetable.ErrOfferingSourceInvalid)
		}
	}
	return nil
}
...
func (s *TemplateService) createTemplateLocked(
	...
	tenantID int64,
	result *timetable.CreateTemplateResult,
) error {
	...
	if err := s.lockRecurrence(ctx, createTemplateOp); err != nil {
		return err
	}
	targets, err := s.validateTemplateCreateReferences(ctx, &in)
	if err != nil {
		return err
	}
	...
	group, err := s.createTemplateGroup(ctx, in, targets, tenantID)
	if err != nil {
		return err
	}
	scheduleIDs, err := s.createTemplateSchedules(ctx, group.ID, timeframeID, in, tenantID)
	if err != nil {
		return err
	}
	...
	if err := s.seedTemplateOfferingRoster(ctx, group.ID, in); err != nil {
		return err
	}
...
// validateTemplateCreateReferences runs every check before the first write: a
// rejected request must not strand a timeframe or a half-built template. It
// returns the normalized dynamic targets and mirrors the first one onto in.
func (s *TemplateService) validateTemplateCreateReferences(ctx context.Context, in *CreateTemplateInput) ([]*activitiesModel.GroupTarget, error) {
	if err := s.ValidateTemplateEducationGroup(ctx, in.EducationGroupID); err != nil {
		return nil, err
	}
...
func (s *TemplateService) createTemplateGroup(
	ctx context.Context,
	in CreateTemplateInput,
	targets []*activitiesModel.GroupTarget,
	tenantID int64,
) (*activitiesModel.Group, error) {
	roomID := in.RoomID
	...
	if err := s.deps.ActivityGroupRepo.Create(ctx, group); err != nil {
		return nil, &ScheduleError{Op: "create template: create group", Err: err}
	}
	targetRepo, ok := s.deps.ActivityGroupRepo.(activitiesModel.GroupTargetRepository)
	if !ok && len(targets) > 0 {
		return nil, &ScheduleError{Op: "create template: replace targets", Err: errors.New("target repository is not configured")}
	}
	if ok {
		if err := targetRepo.ReplaceTargets(ctx, group.ID, targets); err != nil {
			return nil, &ScheduleError{Op: "create template: replace targets", Err: err}
		}
	}
	return group, nil
}

func (s *TemplateService) createTemplateSchedules(
	ctx context.Context,
	...
	for _, weekday := range in.Weekdays {
		...
		if err := s.deps.ActivityScheduleRepo.Create(ctx, sched); err != nil {
			return nil, &ScheduleError{Op: "create template: create schedule", Err: err}
		}
...
// seedTemplateOfferingRoster lets Enrollment seed an offering-sourced
// template's roster from the approved enrollments (#2137).
func (s *TemplateService) seedTemplateOfferingRoster(ctx context.Context, templateID int64, in CreateTemplateInput) error {
...
func normalizeDynamicTargets(targetType string, grade *int16, class *string, groupID *int64, targets []*activitiesModel.GroupTarget) ([]*activitiesModel.GroupTarget, error) {
	if len(targets) == 0 {
		targets = mirroredDynamicTarget(targetType, grade, class, groupID)
	}
	seen := make(map[string]struct{}, len(targets))
	normalized := make([]*activitiesModel.GroupTarget, 0, len(targets))
	for _, target := range targets {
		copy, key, err := normalizeDynamicTarget(targetType, target)
		if err != nil {
			return nil, err
		}
...
// mirroredDynamicTarget builds the single target a legacy request expresses
// through the group mirror fields when it lists no targets.
func mirroredDynamicTarget(targetType string, grade *int16, class *string, groupID *int64) []*activitiesModel.GroupTarget {
	switch {
	case targetType == activitiesModel.TargetGroupTypeJahrgang && grade != nil:
		return []*activitiesModel.GroupTarget{{TargetGroupType: targetType, TargetGradeLevel: grade}}
	case targetType == activitiesModel.TargetGroupTypeKlasse && class != nil:
		return []*activitiesModel.GroupTarget{{TargetGroupType: targetType, TargetSchoolClass: class}}
	case targetType == activitiesModel.TargetGroupTypeGruppe && groupID != nil:
		return []*activitiesModel.GroupTarget{{TargetGroupType: targetType, EducationGroupID: groupID}}
	}
	return nil
}

// normalizeDynamicTarget validates one target against the template's
// Zielgruppe and returns a copy plus its de-duplication key.
func normalizeDynamicTarget(targetType string, target *activitiesModel.GroupTarget) (*activitiesModel.GroupTarget, string, error) {
	if target == nil {
...
func validateDynamicTargets(ctx context.Context, s *TemplateService, gradeLevelMax int, existing *activitiesModel.Group, existingTargets, targets []*activitiesModel.GroupTarget) error {
	if err := validateTemplateTargetsGradeLimit(s.deps.SchoolClasses, gradeLevelMax, existing, existingTargets, targets); err != nil {
		return err
	}
	for _, target := range targets {
