...
func TestClearUnderstaffedAckIfStaffed_NoAckIsNoop(t *testing.T) {
	...
	svc := &InstanceLifecycleService{deps: InstanceLifecycleDependencies{
		...
		Protocol:          &recordingDeviationProtocol{},
...
func TestClearUnderstaffedAckIfStaffed_FullyStaffedClearsAck(t *testing.T) {
	...
	inst := ackUnitInstance(true)
	instanceRepo := &deleteUnitInstanceRepo{instance: inst}
	events := &recordingDeviationProtocol{}
...
func TestIsUnderstaffed_SkipsNilRows(t *testing.T) {
	...
	assert.False(t, timetable.IsUnderstaffed(staffingRowsOf(rows)))
	assert.True(t, timetable.IsUnderstaffed(staffingRowsOf([]*scheduleModel.InstanceStaff{nil})))
}
