...
func TestEvaluateLifecycleAvailability_PlannedBoundaries(t *testing.T) {
	...
	before := evaluateLifecycleAvailability(instance, time.Date(2026, 8, 13, 13, 29, 59, 0, timezone.Berlin), 15, true)
	assert.False(t, before.CanStart)
	assert.False(t, before.CanComplete)

	atStartBoundary := evaluateLifecycleAvailability(instance, time.Date(2026, 8, 13, 13, 30, 0, 0, timezone.Berlin), 15, true)
	assert.True(t, atStartBoundary.CanStart)

	atEndBoundary := evaluateLifecycleAvailability(instance, time.Date(2026, 8, 13, 14, 30, 0, 0, timezone.Berlin), 15, true)
...
func TestEvaluateLifecycleAvailability_SettingsAndSpontaneous(t *testing.T) {
	...
	assert.True(t, evaluateLifecycleAvailability(instance, now, 120, false).CanStart)
	assert.True(t, evaluateLifecycleAvailability(instance, now, 0, false).CanComplete)

	instance.IsSpontaneous = true
	got := evaluateLifecycleAvailability(instance, now, 0, true)
