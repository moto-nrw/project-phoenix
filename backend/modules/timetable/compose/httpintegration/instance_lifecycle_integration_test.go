...
func TestInstance_SetUnderstaffedAck_CompletedRejected(t *testing.T) {
	...
	assert.ErrorIs(t, err, timetable.ErrInvalidInstanceTransition)
}
...
func TestInstance_SetUnderstaffedAck_RejectedWhileStaffed(t *testing.T) {
	...
	_, err := svc.SetUnderstaffedAck(s.ctx, ai.ID, true, nil, nil)
	assert.ErrorIs(t, err, timetable.ErrUnderstaffedAckStillStaffed)
...
func TestInstance_SetUnderstaffedAck_NotFound(t *testing.T) {
	...
	assert.ErrorIs(t, err, timetable.ErrInstanceNotFound)
}
...
func TestInstance_Cancel_WithReason_Persists(t *testing.T) {
	...
	require.NoError(t, err)
	// The lifecycle's public view carries no cancel reason; the stored row
	// is the contract.
...
func TestInstance_Start_HappyPath(t *testing.T) {
	...
	sups, err := s.presence.QueryGroupSupervisions(s.ctx, studentpresence.GroupSupervisionFilter{GroupIDs: []int64{result.ActiveGroupID}, ActiveOn: new(calendar.TodayDate().String())})
	require.NoError(t, err)
	assert.Len(t, sups, 1)
...
func TestInstance_Start_BroadcastsActiveSupervisionChanged(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = s.presence.QueryGroupSupervisions(s.ctx, studentpresence.GroupSupervisionFilter{GroupIDs: []int64{result.ActiveGroupID}, ActiveOn: new(calendar.TodayDate().String())})
	})
}
...
func TestInstance_CreateRejectsDateOutsideActiveCalendarPeriod(t *testing.T) {
	...
	s := buildLifecycle(t)
	_, err := s.svc.CreateInstance(s.ctx, timetable.CreateInstanceInput{
		Date:      calendar.NewDate(2101, 4, 21),
	...
	require.ErrorIs(t, err, timetable.ErrInstanceOutsideActiveCalendarPeriod)
}
...
func TestInstance_UpdatePlannedKeepsDateAfterPeriodDeactivation(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, calendar.Date(instance.Date), updated.Date)
}
...
func TestInstance_UpdatePlannedMovesSpontaneousOutsideActiveCalendarPeriod(t *testing.T) {
	...
	targetDate := calendar.NewDate(2101, 4, 21)
...
func TestInstance_CreateIdempotencyRetryReturnsStoredResultAfterPeriodDeactivation(t *testing.T) {
	...
	req := timetable.CreateInstanceInput{
		Date:           calendar.NewDate(2026, 4, 21),
		StartTime:      time.Date(2000, 1, 1, 14, 0, 0, 0, time.UTC),
		EndTime:        time.Date(2000, 1, 1, 15, 0, 0, 0, time.UTC),
	...
	created, err := s.svc.CreateInstance(s.ctx, req)
	require.NoError(t, err)

	testpkg.SetCalendarPeriodActive(t, s.db, s.period, false)

	replayed, err := s.svc.CreateInstance(s.ctx, req)
	require.NoError(t, err)
...
func TestInstance_PlannedCRUD_BroadcastsStaffingDeviationChanged(t *testing.T) {
	...
	inst, err := svc.CreateInstance(s.ctx, timetable.CreateInstanceInput{
		Date:      calendar.NewDate(2026, 4, 21),
		StartTime: time.Date(2000, 1, 1, 14, 0, 0, 0, time.UTC),
		EndTime:   time.Date(2000, 1, 1, 15, 0, 0, 0, time.UTC),
	...
	_, err = svc.UpdatePlanned(s.ctx, inst.ID, timetable.UpdateInstanceInput{
		Date:      calendar.Date(inst.Date),
...
func TestInstance_Complete_ConfirmationMustMatchOpenVisits(t *testing.T) {
	...
	now := time.Now()
	testpkg.CreateTestVisit(t, s.db, s.student1, started.ActiveGroupID, now, nil)

	_, err = s.svc.Complete(timetable.WithCompletionConfirmation(s.ctx, []int64{s.student2}), ai.ID)
	require.ErrorIs(t, err, timetable.ErrCompletionConfirmationStale)

	completed, err := s.svc.Complete(timetable.WithCompletionConfirmation(s.ctx, []int64{s.student1}), ai.ID)
	require.NoError(t, err)
	assert.Equal(t, scheduleModels.InstanceStatusCompleted, completed.Status)
}
...
type failingRecoveryRestore struct {
	scheduleModels.ActivityRecoveryRepository
	afterRestore error
	restores     int
}

func (r *failingRecoveryRestore) Restore(ctx context.Context, id int64, snapshot scheduleModels.ActivityCompletionSnapshot, now time.Time) error {
	if err := r.ActivityRecoveryRepository.Restore(ctx, id, snapshot, now); err != nil {
		return err
	}
	r.restores++
	return r.afterRestore
}

func TestInstance_Reopen_RollsBackRestorationAndRetries(t *testing.T) {
	t.Parallel()
	for _, stage := range []string{"group", "visits", "supervisors", "first-assignment", "second-assignment", "instance", "complete"} {
		t.Run(stage, func(t *testing.T) { testReopenWriteRollback(t, stage) })
	}
}

func testReopenWriteRollback(t *testing.T, stage string) {
	t.Helper()
	testpkg.OwnTenant(t)
	...
	started, err := s.svc.Start(s.ctx, ai.ID, 0)
	require.NoError(t, err)
	visit := testpkg.CreateTestVisit(t, s.db, s.student1, started.ActiveGroupID, time.Now().Add(-time.Minute), nil)
	_, err = s.svc.Complete(s.ctx, ai.ID)
	require.NoError(t, err)
	injected := errors.New("failure after recovery writes")
	completedSupervisors, err := s.repos.GroupSupervisor.FindByActiveGroupID(s.ctx, started.ActiveGroupID, false)
	require.NoError(t, err)
	require.Len(t, completedSupervisors, 1)
	...
	expectedRestores := 0
	if stage == "complete" {
		fault.afterRestore = injected
		expectedRestores = 1
	} else {
		target := targets[stage]
		removeFault = installRecoveryUpdateFailure(t, s, target.table, target.id)
	}
	broadcaster := testpkg.NewRecordingBroadcaster()
	s.svc = instanceServiceWithBroadcaster(t, s, broadcaster, fault)
	_, err = s.svc.Reopen(s.ctx, ai.ID, 0, true)
	require.ErrorContains(t, err, injected.Error())
	require.Equal(t, expectedRestores, fault.restores, "fault must stop at its named write boundary")
	assert.Empty(t, broadcaster.Calls())
	stored, err := s.repos.ActivityInstance.FindByID(s.ctx, ai.ID)
	require.NoError(t, err)
	assert.Equal(t, scheduleModels.InstanceStatusCompleted, stored.Status)
	assert.NotEmpty(t, stored.CompletionSnapshot)
	supervisors, err := s.repos.GroupSupervisor.FindByActiveGroupID(s.ctx, started.ActiveGroupID, false)
	require.NoError(t, err)
	assert.Equal(t, completedSupervisors, supervisors, "supervisor restoration must roll back")
	assert.Equal(t, completedFirst, fetchAttendance(t, s, ai.ID, s.student1), "first assignment restoration must roll back")
	assert.Equal(t, completedSecond, fetchAttendance(t, s, ai.ID, s.student2), "second assignment restoration must roll back")
	group, err := s.factory.Active.GetActiveGroup(s.ctx, started.ActiveGroupID)
	require.NoError(t, err)
	...
	assert.NotNil(t, visits[0].ExitTime)
	removeFault()
	fault.afterRestore = nil
	reopened, err := s.svc.Reopen(s.ctx, ai.ID, 0, true)
	require.NoError(t, err)
	assert.Equal(t, scheduleModels.InstanceStatusActive, reopened.Instance.Status)
	_, err = s.svc.Reopen(s.ctx, ai.ID, 0, true)
	require.ErrorIs(t, err, timetable.ErrInvalidInstanceTransition)
	visits, err = presence.ListVisits(s.ctx, studentpresence.VisitFilter{ActiveGroupIDs: []int64{started.ActiveGroupID}})
	require.NoError(t, err)
	require.Len(t, visits, 1)
	assert.Equal(t, visit.ID, visits[0].ID)
...
// Each case owns a database clone, so these AFTER UPDATE triggers cannot affect
// parallel tests. Raising from the trigger proves the named row was updated
// before aborting the transaction, rather than failing a pre-write mock.
func installRecoveryUpdateFailure(t *testing.T, s *lifecycleSetup, table string, id int64) func() {
	t.Helper()
	_, err := s.db.NewRaw(`CREATE FUNCTION public.fail_recovery_write() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF NEW.id = TG_ARGV[0]::bigint THEN
    RAISE EXCEPTION 'failure after recovery writes';
  END IF;
  RETURN NEW;
END;
$$`).Exec(s.ctx)
	require.NoError(t, err)
	_, err = s.db.NewRaw(`CREATE TRIGGER fail_recovery_write AFTER UPDATE ON ?
FOR EACH ROW EXECUTE FUNCTION public.fail_recovery_write(?)`, bun.Ident(table), fmt.Sprint(id)).Exec(s.ctx)
	require.NoError(t, err)
	return func() {
		_, err := s.db.NewRaw(`DROP TRIGGER fail_recovery_write ON ?`, bun.Ident(table)).Exec(s.ctx)
		require.NoError(t, err)
		_, err = s.db.NewRaw(`DROP FUNCTION public.fail_recovery_write()`).Exec(s.ctx)
		require.NoError(t, err)
	}
}
...
func TestInstance_Reopen_RejectsRoomCapacity(t *testing.T) {
	...
	now := time.Now()
	for _, studentID := range []int64{s.student1, s.student2} {
		testpkg.CreateTestVisit(t, s.db, studentID, started.ActiveGroupID, now, nil)
	}
...
func TestInstance_Start_StaffSameRoomIsNotAConflict(t *testing.T) {
	...
	sup := &studentpresence.GroupSupervision{StaffID: s.staffID, GroupID: preGroup.ID, Role: "supervisor", StartDate: calendar.DateFromTime(now).String()}
...
func TestInstance_Start_ConflictWarning_Student(t *testing.T) {
	...
	testpkg.CreateTestVisit(t, s.db, s.student1, preGroup.ID, now, nil)
...
func TestInstance_ReplanWeek_RemovesFutureLegacyWeekendInstances(t *testing.T) {
	...
	from := calendar.NewDate(2026, 8, 24)
...
func TestInstance_ReplanWeek_ScopedToActivityGroup(t *testing.T) {
	...
	from := calendar.NewDate(2026, 7, 20)
	to := from.AddDays(6)
...
func TestInstance_Create_AssignsUniqueStaffAndStudents(t *testing.T) {
	...
	inst, err := s.svc.CreateInstance(s.ctx, timetable.CreateInstanceInput{
		Date:             calendar.NewDate(2026, 5, 4),
	...
	// The lifecycle's public view carries no creator; the stored row does.
	stored := reloadInstance(t, s, inst.ID)
...
func TestInstance_CreateAndUpdatePlanned_ReapplyActiveStatusDays(t *testing.T) {
	...
	s := buildLifecycle(t)
	createdDate := calendar.NewDate(2026, 5, 12)
	updatedDate := calendar.NewDate(2026, 5, 13)
...
func TestInstance_Create_RejectsCrossTenantReferences(t *testing.T) {
	...
	valid := timetable.CreateInstanceInput{
		Date:            calendar.NewDate(2026, 5, 9),
		StartTime:       time.Date(1, 1, 1, 9, 0, 0, 0, time.UTC),
		EndTime:         time.Date(1, 1, 1, 10, 0, 0, 0, time.UTC),
	...
	cases := []struct {
		name   string
		mutate func(*timetable.CreateInstanceInput)
	}{
		{name: "room", mutate: func(in *timetable.CreateInstanceInput) { in.RoomID = foreignRoom.ID }},
		{name: "activity group", mutate: func(in *timetable.CreateInstanceInput) { in.ActivityGroupID = &foreignTemplate.ID }},
		{name: "staff", mutate: func(in *timetable.CreateInstanceInput) { in.StaffIDs = []int64{foreignStaff.ID} }},
		{name: "student", mutate: func(in *timetable.CreateInstanceInput) { in.StudentIDs = []int64{foreignStudent.ID} }},
		{name: "created by", mutate: func(in *timetable.CreateInstanceInput) { in.CreatedByStaffID = &foreignStaff.ID }},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			inst, err := s.svc.CreateInstance(s.ctx, req)
...
func TestInstance_Create_SpontaneousAndMissingTenant(t *testing.T) {
	...
	inst, err := s.svc.CreateInstance(s.ctx, timetable.CreateInstanceInput{
		Date:      calendar.NewDate(2026, 5, 5),
	...
	isSpontaneous := true
	linked, err := s.svc.CreateInstance(s.ctx, timetable.CreateInstanceInput{
		Date:            calendar.NewDate(2026, 5, 7),
	...
	_, err = s.svc.CreateInstance(context.Background(), timetable.CreateInstanceInput{
...
func TestInstance_UpdatePlanned_ReplacesAssignmentsAndFields(t *testing.T) {
	...
	updated, err := s.svc.UpdatePlanned(s.ctx, ai.ID, timetable.UpdateInstanceInput{
		Date:       calendar.NewDate(2026, 5, 7),
...
// guardedLifecycleSettings satisfies compose.LifecycleSettings for the
...
var _ compose.LifecycleSettings = guardedLifecycleSettings{}

func (s guardedLifecycleSettings) StartLeadMinutes(context.Context) (int, error) {
	return s.leadMinutes, nil
}

func (s guardedLifecycleSettings) EnforcePlannedEnd(context.Context) (bool, error) {
...
func TestInstance_Start_TimePolicyAppliesToNoOfferingPlannedBlock(t *testing.T) {
	...
	s := buildLifecycle(t)
	date := calendar.NewDate(2026, 4, 22)
	// 08:00 Berlin — far before the 14:00 start minus the 15-minute lead.
	now := time.Date(2026, 4, 22, 8, 0, 0, 0, calendar.Berlin)
	deps := lifecycleDependencies(s, nil)
	deps.Settings = guardedLifecycleSettings{leadMinutes: 15, enforcePlannedEnd: true}
	deps.Now = func() time.Time { return now }
	deps.EnforceTimePolicy = true
	guarded := newLifecycle(t, deps)

	planned, err := guarded.CreateInstance(s.ctx, timetable.CreateInstanceInput{
		Date:      date,
		StartTime: time.Date(1, 1, 1, 14, 0, 0, 0, time.UTC),
		EndTime:   time.Date(1, 1, 1, 15, 0, 0, 0, time.UTC),
	...
	require.ErrorIs(t, err, timetable.ErrInstanceStartTooEarly,
		"planning-module block without offering must hit the lead-time guard")

	isSpontaneous := true
	adhoc, err := guarded.CreateInstance(s.ctx, timetable.CreateInstanceInput{
		Date:          date,
		StartTime:     time.Date(1, 1, 1, 14, 0, 0, 0, time.UTC),
		EndTime:       time.Date(1, 1, 1, 15, 0, 0, 0, time.UTC),
		Title:         "Ad-hoc block",
...
func TestInstance_UpdatePlanned_RejectsCrossTenantReferencesBeforeMutation(t *testing.T) {
	...
	ai := seedInstance(t, s, true, true)
	valid := timetable.UpdateInstanceInput{
		Date:            calendar.NewDate(2026, 5, 11),
	...
	cases := []struct {
		name   string
		mutate func(*timetable.UpdateInstanceInput)
	}{
		{name: "room", mutate: func(in *timetable.UpdateInstanceInput) { in.RoomID = foreignRoom.ID }},
		{name: "activity group", mutate: func(in *timetable.UpdateInstanceInput) { in.ActivityGroupID = &foreignTemplate.ID }},
		{name: "staff", mutate: func(in *timetable.UpdateInstanceInput) { in.StaffIDs = []int64{foreignStaff.ID} }},
		{name: "student", mutate: func(in *timetable.UpdateInstanceInput) { in.StudentIDs = []int64{foreignStudent.ID} }},
	}
...
func TestInstance_UpdatePlanned_RejectsNonPlanned(t *testing.T) {
	...
	for _, status := range []string{
		...
		t.Run(status, func(t *testing.T) {
			...
			_, err := s.svc.UpdatePlanned(s.ctx, ai.ID, timetable.UpdateInstanceInput{
				Date:      calendar.NewDate(2026, 5, 8),
				StartTime: time.Date(1, 1, 1, 9, 0, 0, 0, time.UTC),
				EndTime:   time.Date(1, 1, 1, 10, 0, 0, 0, time.UTC),
				Title:     "Should fail",
				RoomID:    s.roomID,
...
func TestInstance_DeleteCancelled_PlannedOrCancelled(t *testing.T) {
	...
	t.Run("deletes spontaneous planned occurrence without exception", func(t *testing.T) {
		s := buildLifecycle(t)
		id := insertInstance(t, s, calendar.NewDate(2026, 4, 20), scheduleModels.InstanceStatusPlanned, true)
	...
	t.Run("rejects single delete when template has multiple same-day slots", func(t *testing.T) {
		...
		require.Error(t, err)
		assert.ErrorIs(t, err, timetable.ErrAmbiguousTemplateInstanceDelete)
