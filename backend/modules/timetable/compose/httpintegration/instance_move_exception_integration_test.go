...
func moveInput(s *scenarioSetup, inst *scheduleModels.ActivityInstance, date calendar.Date, startShiftHours int) timetable.UpdateInstanceInput {
	return timetable.UpdateInstanceInput{
		Date:            date,
...
func TestUpdatePlanned_DateMove_WritesExceptionAndBlocksRematerialization(t *testing.T) {
	...
	origDate := calendar.NewDate(2026, time.April, 20) // Mon
...
func TestUpdatePlanned_StartTimeOnlyMove_WritesException(t *testing.T) {
	...
	origDate := calendar.NewDate(2026, time.April, 20)
	s := makeScenario(t, activitiesModels.WeekdayMonday, origDate)
	defer s.runCleanup(t)
