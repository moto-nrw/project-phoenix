...
func futureMondayForStartPull(t *testing.T) calendar.Date {
	t.Helper()
...
) timetable.UpdateTemplateCommand {
	return timetable.UpdateTemplateCommand{
		...
		RosterValidFrom: calendar.NewDate(2030, 8, 26),
		GradeLevelMax:   4,
...
func TestUpdateTemplate_StartDatePullForward_MovesEnvelopeRosterAndMaterializesGapOnly(t *testing.T) {
	...
	materialized, err := s.materialize.MaterializeForTenant(s.ctx, oldStart, oldStart.AddDays(1), timetable.MaterializationSourceManual)
	require.NoError(t, err)
	...
	require.NoError(t, s.factory.TimetableData.Templates.UpdateTemplate(
		s.ctx,
	...
	materialized, err = s.materialize.MaterializeForTenant(s.ctx, newStart, oldStart.AddDays(1), timetable.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestUpdateTemplate_StartDatePullForward_MovesWeekdayScopedRoster(t *testing.T) {
	...
	weekdays := []int{activitiesModels.WeekdayMonday, activitiesModels.WeekdayTuesday}
	assignments := []timetable.WeekdayRosterAssignment{
		{
	...
	require.NoError(t, s.factory.TimetableData.Templates.UpdateTemplate(s.ctx, in))
	...
	_, err = s.materialize.MaterializeForTenant(s.ctx, newStart, newStart.AddDays(1), timetable.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestUpdateTemplate_StartDatePullForward_MidweekDateStartsAtNextMatchingWeekday(t *testing.T) {
	...
	_, err = s.materialize.MaterializeForTenant(s.ctx, newStart, oldStart, timetable.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestUpdateTemplate_StartDatePullForward_Validation(t *testing.T) {
	...
	openResult, err := s.factory.TimetableData.Templates.CreateTemplate(s.ctx, timetable.CreateTemplateCommand{
		Name:            openName,
		Type:            activitiesModels.GroupTypeCare,
		Weekdays:        weekdays,
...
func TestUpdateTemplate_StartDatePullForward_RejectsPredecessorOverlap(t *testing.T) {
	...
	predecessor, err := s.factory.TimetableData.Templates.CreateTemplate(s.ctx, timetable.CreateTemplateCommand{
	...
	successor, err := s.factory.TimetableData.Templates.CreateTemplate(s.ctx, timetable.CreateTemplateCommand{
		Name:              successorName,
		Type:              activitiesModels.GroupTypeCare,
		Weekdays:          weekdays,
	...
	require.ErrorIs(t, err, timetable.ErrTemplateStartPredecessorOverlap,
