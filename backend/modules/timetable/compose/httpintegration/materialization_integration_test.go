...
func makeScenario(t *testing.T, weekday int, materializeDate calendar.Date) *scenarioSetup {
	t.Helper()
	...
	svc := newTestMaterialization(t, db, repoFactory)
...
func TestMaterializeForTenant_EndToEnd(t *testing.T) {
	...
	// --- Happy path ---
	r, err := s.svc.MaterializeForTenant(s.ctx, from, to, timetable.MaterializationSourceManual)
	require.NoError(t, err)
	...
	// --- Idempotent re-run ---
	r2, err := s.svc.MaterializeForTenant(s.ctx, from, to, timetable.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestMaterializeForTenant_ExcludesGraduatedStudents(t *testing.T) {
	...
	r, err := s.svc.MaterializeForTenant(s.ctx, materializeDate, materializeDate.AddDays(6), timetable.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestMaterializeForTenant_MultipleDynamicTargetsFollowClassChanges(t *testing.T) {
	...
	firstMonday := calendar.NewDate(2026, time.April, 20)
	...
	first, err := s.svc.MaterializeForTenant(s.ctx, firstMonday, firstMonday, timetable.MaterializationSourceManual)
	require.NoError(t, err)
	...
	second, err := s.svc.MaterializeForTenant(s.ctx, secondMonday, secondMonday, timetable.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestMaterializeForTenant_ExceptionCancelled_Skips(t *testing.T) {
	...
	materializeDate := calendar.NewDate(2026, time.April, 20)
	s := makeScenario(t, activitiesModels.WeekdayMonday, materializeDate)
	defer s.runCleanup(t)
...
func TestMaterializeForTenant_NoActivePeriod_ReturnsGracefully(t *testing.T) {
	...
	from := calendar.NewDate(2026, 4, 20)
	to := from.AddDays(6)

	r, err := svc.MaterializeForTenant(ctx, from, to, timetable.MaterializationSourceManual)
	require.NoError(t, err)
	assert.Zero(t, r.InstancesCreated)
	...
	assert.Equal(t, timetable.MaterializationWarningCodeNoActivePeriod, r.Warnings[0].Code)
...
func TestMaterializeForTenant_NoTemplates_ReturnsWarning(t *testing.T) {
	...
	require.Len(t, r.Warnings, 1)
	assert.Equal(t, timetable.MaterializationWarningCodeNoTemplates, r.Warnings[0].Code)
	assert.NotEmpty(t, r.Warnings[0].Message)
}
...
func TestMaterializeForTenant_PreFetchObservesFirstRunThenSkips(t *testing.T) {
	...
	r1, err := s.svc.MaterializeForTenant(s.ctx, materializeDate, materializeDate.AddDays(6), timetable.MaterializationSourceManual)
	require.NoError(t, err)
	...
	r2, err := s.svc.MaterializeForTenant(s.ctx, materializeDate, materializeDate.AddDays(6), timetable.MaterializationSourceManual)
	require.NoError(t, err)
	assert.Zero(t, r2.InstancesCreated)
...
func TestMaterializeForTenant_ABWeekSmoke(t *testing.T) {
	...
	materializeDate := calendar.NewDate(2026, time.April, 20) // Mon, "Week A"
	s := makeScenario(t, activitiesModels.WeekdayMonday, materializeDate)
	defer s.runCleanup(t)
	...
	r2, err := s.svc.MaterializeForTenant(s.ctx, nextMon, nextMon.AddDays(6), timetable.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestMaterializeForTenant_ScheduleValidUntil_SkipsEndedDates(t *testing.T) {
	...
	firstMonday := calendar.NewDate(2026, time.April, 20) // Mon
	secondMonday := firstMonday.AddDays(7)
	s := makeScenario(t, activitiesModels.WeekdayMonday, firstMonday)
	defer s.runCleanup(t)
	...
	r0, err := s.svc.MaterializeForTenant(s.ctx, secondMonday, secondMonday, timetable.MaterializationSourceManual)
	require.NoError(t, err)
	...
	r, err := s.svc.MaterializeForTenant(s.ctx, firstMonday, secondMonday.AddDays(6), timetable.MaterializationSourceManual)
	require.NoError(t, err)
...
// newTestMaterialization composes the Timetable owner's materialization over
// the retained repositories, the way the composition root does, without the
// per-date care filter and the staffing announcement.
func newTestMaterialization(t *testing.T, db *bun.DB, repos *repositories.Factory) timetable.MaterializationCapability {
	t.Helper()
	svc, err := compose.NewMaterialization(compose.MaterializationDependencies{
		...
		PeriodRepo:     repos.CalendarPeriod,
		InstanceRepo:   repos.ActivityInstance,
		StaffRepo:      repos.InstanceStaff,
		StudentRepo:    repos.InstanceStudent,
