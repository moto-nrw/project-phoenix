...
func activityDatePointer(date calendar.Date) *activitiesModels.Date {
...
func futureMonday(offsetWeeks int) calendar.Date {
	d := calendar.NewDate(2030, 8, 26).AddDays(1)
...
// it for cleanup.
func splitInsertInstance(t *testing.T, s *scenarioSetup, groupID *int64, date calendar.Date, status string, spontaneous bool, startHour int) int64 {
	t.Helper()
...
func baseSplitInput(s *scenarioSetup, effective calendar.Date, name string) timetable.SplitTemplateCommand {
...
func createLinkedCareOffering(
	...
	offering := testpkg.NewSourceCareOffering(phase.ID,
		fmt.Sprintf("Linked care offering %d", time.Now().UnixNano()), []string{"mon"}, &s.template.ID)
...
func createShortCalendarPeriod(t *testing.T, s *scenarioSetup, start, end calendar.Date) *scheduleModels.CalendarPeriod {
	t.Helper()
...
func createCompatibleCalendarPeriod(t *testing.T, s *scenarioSetup) *scheduleModels.CalendarPeriod {
	t.Helper()
	period := createShortCalendarPeriod(t, s, calendar.Date(s.period.StartDate), calendar.Date(s.period.EndDate))
...
func linkedTemplateUpdateInput(t *testing.T, s *scenarioSetup, periodID *int64) timetable.UpdateTemplateCommand {
	t.Helper()
	...
	return timetable.UpdateTemplateCommand{
		...
		CalendarPeriodID: periodID,
		RosterValidFrom:  calendar.Date(s.period.StartDate),
...
func TestTemplateMutations_RejectCareOfferingSeriesConflictsWithoutPersisting(t *testing.T) {
	...
	t.Run("split", func(t *testing.T) {
		...
		_, err := s.factory.TimetableData.Templates.SplitTemplate(s.ctx, in)
		require.ErrorIs(t, err, timetable.ErrTemplateCareOfferingConflict)
	...
	t.Run("update", func(t *testing.T) {
		...
		err := s.factory.TimetableData.Templates.UpdateTemplate(s.ctx, linkedTemplateUpdateInput(t, s, &shortPeriod.ID))
		require.ErrorIs(t, err, timetable.ErrTemplateCareOfferingConflict)
	...
	t.Run("end", func(t *testing.T) {
		...
		_, err := s.factory.TimetableData.Templates.EndTemplateFromDate(s.ctx, timetable.EndTemplateCommand{
			TemplateID:    s.template.ID,
			EffectiveDate: effective,
		})
		require.ErrorIs(t, err, timetable.ErrTemplateCareOfferingConflict)
	...
	t.Run("end inactive offering still selected by approved child", func(t *testing.T) {
		...
		_, err := s.db.NewRaw(
			`UPDATE enrollment.care_offerings SET is_active = false WHERE id = ? AND tenant_id = ?`,
			offering.ID, s.tenantID,
		).Exec(s.ctx)
		require.NoError(t, err)
	...
	t.Run("update removes advertised weekday", func(t *testing.T) {
		...
		err := s.factory.TimetableData.Templates.UpdateTemplate(s.ctx, in)
		require.ErrorIs(t, err, timetable.ErrTemplateCareOfferingConflict)
		schedules := loadSplitSchedules(t, s, s.template.ID)
		require.Len(t, schedules, 1)
	...
	t.Run("archive", func(t *testing.T) {
		...
		_, err := s.factory.TimetableData.Templates.ArchiveTemplate(s.ctx, s.template.ID)
		require.ErrorIs(t, err, timetable.ErrTemplateCareOfferingConflict)
...
func TestTemplatePeriodChange_RebasesProtectedEnrollmentForMaterialization(t *testing.T) {
	...
	t.Run("split A to B", func(t *testing.T) {
		...
		materialized, err := s.svc.MaterializeForTenant(s.ctx, effective, effective, timetable.MaterializationSourceManual)
		require.NoError(t, err)
	...
	t.Run("update A to B", func(t *testing.T) {
		...
		err := s.factory.TimetableData.Templates.UpdateTemplate(s.ctx, linkedTemplateUpdateInput(t, s, &periodB.ID))
		require.NoError(t, err)
...
func TestTemplateSplit_ProtectedWeekdayDoesNotSuppressManualOtherWeekday(t *testing.T) {
	...
	in.WeekdayAssignments = []timetable.WeekdayRosterAssignment{{
	...
	materialized, err := s.svc.MaterializeForTenant(s.ctx, tuesday, tuesday, timetable.MaterializationSourceManual)
	require.NoError(t, err)
	require.Equal(t, 1, materialized.InstancesCreated)
...
func TestTemplateSplit_SharedRosterDoesNotBroadenProtectedWeekdays(t *testing.T) {
	...
	materialized, err := s.svc.MaterializeForTenant(s.ctx, monday, tuesday, timetable.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestTemplateSplit_HappyPath_CarriesRosterAndProtectsHistory(t *testing.T) {
	...
	r0, err := s.svc.MaterializeForTenant(s.ctx, effective, secondMonday, timetable.MaterializationSourceManual)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	for range []calendar.Date{effective, secondMonday} {
	}
...
func TestTemplateMutationsRejectUnknownPlanningTrack(t *testing.T) {
	...
	_, err := s.factory.TimetableData.Templates.SplitTemplate(s.ctx, split)
	...
	err = s.factory.TimetableData.Templates.UpdateTemplate(s.ctx, update)
	require.ErrorIs(t, err, timetable.ErrPlanningTrackNotFound)
}
...
func TestTemplateEndFromDate_CapsTemplateAndProtectsHistory(t *testing.T) {
	...
	res, err := s.factory.TimetableData.Templates.EndTemplateFromDate(s.ctx, timetable.EndTemplateCommand{
		TemplateID:    s.template.ID,
		EffectiveDate: effective,
	})
	require.NoError(t, err)
	...
	r1, err := s.svc.MaterializeForTenant(s.ctx, effective, secondMonday, timetable.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestTemplateSplit_ValidationErrors(t *testing.T) {
	...
	t.Run("past effective_date", func(t *testing.T) {
		in := baseSplitInput(s, calendar.NewDate(2000, 1, 1), fmt.Sprintf("Split-Vergangen-%d", suffix))
		_, err := s.factory.TimetableData.Templates.SplitTemplate(s.ctx, in)
		require.ErrorIs(t, err, timetable.ErrSplitInvalidInput)
	})
	...
	t.Run("non-template id", func(t *testing.T) {
		...
		_, err = s.factory.TimetableData.Templates.SplitTemplate(s.ctx, in)
		require.ErrorIs(t, err, timetable.ErrSplitTemplateNotFound)
	})
...
func TestTemplateSplit_SuccessorValidFrom_NoPhantomBeforeEffective(t *testing.T) {
	...
	r, err := s.svc.MaterializeForTenant(s.ctx, prevMonday, prevMonday, timetable.MaterializationSourceManual)
	require.NoError(t, err)
...
func TestTemplateSplit_UpdateSegmentsPreservesBoundsDuringMaterialization(t *testing.T) {
	...
	require.ErrorIs(t, err, timetable.ErrTemplateSegmentNotEditable)
	...
	_, err = s.svc.MaterializeForTenant(s.ctx, previousMonday, effective, timetable.MaterializationSourceManual)
	require.NoError(t, err)
	registerSplitInstancesForCleanup(t, s, []int64{s.template.ID, res.NewTemplateID}, []calendar.Date{previousMonday, effective})
...
func assertPredecessorRosterBounds(
	t *testing.T,
	s *scenarioSetup,
	replacementFrom, effective calendar.Date,
) {
	t.Helper()
...
func assertPreservedSourcedEnrollment(
	...
	wantFrom, wantUntil calendar.Date,
) {
	t.Helper()
...
func assertSuccessorStudentRoster(
	...
	s *scenarioSetup,
	groupID int64,
	effective, futureStart calendar.Date,
	weekdaySpecific, sourced *activitiesModels.StudentEnrollment,
	requestChildID int64,
	sourcedUntil calendar.Date,
) {
	t.Helper()
...
func assertSuccessorStaffRoster(t *testing.T, s *scenarioSetup, groupID int64, effective calendar.Date) {
	t.Helper()
	active := 0
...
func registerSplitInstancesForCleanup(t *testing.T, s *scenarioSetup, groupIDs []int64, dates []calendar.Date) {
	t.Helper()
}
...
func TestTemplateSplit_RejectsResplittingBoundedPredecessor(t *testing.T) {
	...
	first, err := s.factory.TimetableData.Templates.SplitTemplate(s.ctx,
		baseSplitInput(s, outerBoundary, fmt.Sprintf("Split-Later-%d", time.Now().UnixNano())))
	require.NoError(t, err)

	_, err = s.factory.TimetableData.Templates.SplitTemplate(s.ctx,
	...
	materialized, err := s.svc.MaterializeForTenant(
		s.ctx, outerBoundary, afterOuter, timetable.MaterializationSourceManual)
	require.NoError(t, err)
	...
	registerSplitInstancesForCleanup(t, s,
		...
		[]calendar.Date{outerBoundary, afterOuter})

	for _, date := range []calendar.Date{outerBoundary, afterOuter} {
...
func TestTemplateSplitAndEnd_RespectCurrentSegmentEnvelope(t *testing.T) {
	...
	first, err := s.factory.TimetableData.Templates.SplitTemplate(s.ctx, firstInput)
	require.NoError(t, err)
	...
	t.Run("split and end reject predecessor exclusive end", func(t *testing.T) {
		...
		_, splitErr := s.factory.TimetableData.Templates.SplitTemplate(s.ctx, in)
		require.ErrorIs(t, splitErr, timetable.ErrSplitInvalidInput)
		_, endErr := s.factory.TimetableData.Templates.EndTemplateFromDate(s.ctx, timetable.EndTemplateCommand{
			TemplateID:    s.template.ID,
			EffectiveDate: boundary,
		})
		require.ErrorIs(t, endErr, timetable.ErrSplitInvalidInput)
	})

	t.Run("end before future successor clamps to its start", func(t *testing.T) {
		ended, err := s.factory.TimetableData.Templates.EndTemplateFromDate(s.ctx, timetable.EndTemplateCommand{
...
func assertFutureRosterCarried(t *testing.T, s *scenarioSetup, groupID int64, wantStart calendar.Date) {
	t.Helper()
...
func TestTemplateEnd_ConcurrentTemplateUpdatePreservesCommittedCap(t *testing.T) {
	...
	go func() {
		endDone <- testpkg.WithTenantTx(t, s.ctx, s.db, s.tenantID, func(txCtx context.Context, _ bun.Tx) error {
			_, err := s.factory.TimetableData.Templates.EndTemplateFromDate(txCtx, timetable.EndTemplateCommand{
				TemplateID:    s.template.ID,
				EffectiveDate: effective,
			})
			if err != nil {
	...
	go func() {
		close(updateStarted)
		updateDone <- s.factory.TimetableData.Templates.UpdateTemplate(s.ctx, update)
	}()
	<-updateStarted
...
func TestTemplateEnd_ConcurrentMaterializationCannotInsertPastCommittedCap(t *testing.T) {
	...
	go func() {
		...
		result, err := s.svc.MaterializeForTenant(s.ctx, effective, effective, timetable.MaterializationSourceManual)
...
func TestTemplateArchive_ConcurrentMaterializationCannotInsertStaleOccurrence(t *testing.T) {
	...
	go func() {
		archiveDone <- testpkg.WithTenantTx(t, s.ctx, s.db, s.tenantID, func(txCtx context.Context, _ bun.Tx) error {
			archived, err := s.factory.TimetableData.Templates.ArchiveTemplate(txCtx, s.template.ID)
			if err != nil {
				return err
			}
			if archived != 1 {
	...
	go func() {
		close(materializeStarted)
		result, err := s.svc.MaterializeForTenant(s.ctx, date, date, timetable.MaterializationSourceManual)
		materializeDone <- materializeOutcome{result: result, err: err}
	}()
	<-materializeStarted
	...
	assert.Equal(t, timetable.MaterializationWarningCodeNoTemplates, outcome.result.Warnings[0].Code)
...
func TestTemplateSplit_WindowValidation(t *testing.T) {
	...
	t.Run("window exceeds max days after clamping", func(t *testing.T) {
		...
		from := effective
		to := effective.AddDays(timetable.MaxMaterializationWindowDays) // span = max+1
		in.MaterializeFrom = &from
		in.MaterializeTo = &to
		_, err := s.factory.TimetableData.Templates.SplitTemplate(s.ctx, in)
		require.ErrorIs(t, err, timetable.ErrSplitInvalidInput)
...
func TestTemplateSplit_SecondSuccessorKeepsOriginalSeriesRoot(t *testing.T) {
	...
	second, err := s.factory.TimetableData.Templates.SplitTemplate(s.ctx, secondInput)
	require.NoError(t, err)
