...
) *timetable.CreateTemplateResult {
	t.Helper()
...
const classSourceResyncDate calendar.Date = "2030-08-26"
...
// offeringSourceResync is the tenant-wide sourced-template resync the
// decision service offers School Structure's grade transitions.
type offeringSourceResync interface {
	ResyncOfferingSourcedTemplates(ctx context.Context, effectiveFrom calendar.Date) error
}

// offeringScopedResync is the per-offering resync the care-offering edit
// paths call.
type offeringScopedResync interface {
...
func TestTemplateSourceClassFilter_RejectsCombinedFilters(t *testing.T) {
	...
	require.Error(t, err)
	assert.ErrorIs(t, err, timetable.ErrOfferingSourceInvalid)
...
func endLinkAt(t *testing.T, s *scenarioSetup, studentID int64, until calendar.Date) {
	t.Helper()
	_, err := s.db.NewRaw(`
		UPDATE enrollment.care_offering_bookings AS rco
		SET valid_until = ?
...
func TestTemplateSourceClassFilter_HonoursBookedWeekdays(t *testing.T) {
	...
	mat, err := s.factory.Materialization.MaterializeForTenant(
		s.ctx, monday, friday, timetable.MaterializationSourceManual,
	)
	require.NoError(t, err)
	require.NotNil(t, mat)
...
func TestTemplateSourceClassFilter_OfferingDayChangeReshapesTheRoster(t *testing.T) {
	...
	today := calendar.Date(classSourceResyncDate)
