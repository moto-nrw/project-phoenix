...
func createSourceCareOffering(
	t *testing.T,
	s *scenarioSetup,
	serviceStart, serviceEnd calendar.Date,
) *testpkg.CareOffering {
	t.Helper()
...
func createSourceCareOfferingOnDays(
	...
	phase := &capability.Phase{
		...
		Kind:                      capability.PhaseKindCustom,
	...
	offering := testpkg.NewSourceCareOffering(phase.ID, fmt.Sprintf("Frühbetreuung %d", suffix), days, nil)
...
func TestTemplateOfferingSource_CreateAndMaterializeCopiesSourcedKids(t *testing.T) {
	...
	offering := createSourceCareOffering(t, s, calendar.Date(s.period.StartDate), calendar.Date(s.period.EndDate))
	...
	mat, err := s.factory.Materialization.MaterializeForTenant(
		s.ctx, monday, monday, timetable.MaterializationSourceManual,
	)
	require.NoError(t, err)
...
func TestTemplateOfferingSource_SplitSuccessorInheritsTheRule(t *testing.T) {
	...
	split, err := s.factory.TimetableData.Templates.SplitTemplate(s.ctx, timetable.SplitTemplateCommand{
		TemplateID:       result.TemplateID,
...
func TestTemplateOfferingSource_RejectsOfferingOutsideTheTemplatePeriod(t *testing.T) {
	...
	fitting := createSourceCareOffering(t, s, calendar.Date(s.period.StartDate), calendar.Date(s.period.EndDate))
	overhanging := createSourceCareOffering(t, s, calendar.Date(s.period.StartDate), calendar.Date(s.period.EndDate).AddDays(30))
	name := fmt.Sprintf("Angebots-Termin-%d", time.Now().UnixNano())

	createInput := timetable.CreateTemplateCommand{
		Name:                  name,
		Type:                  activitiesModels.GroupTypeCare,
		Weekdays:              []int{activitiesModels.WeekdayMonday},
		StartTime:             time.Date(2000, 1, 1, 15, 0, 0, 0, time.UTC),
	...
	_, err := s.factory.TimetableData.Templates.CreateTemplate(s.ctx, createInput)
	...
	result, err := s.factory.TimetableData.Templates.CreateTemplate(s.ctx, createInput)
	require.NoError(t, err)
	registerSourcedTemplateCleanup(t, s, result.TemplateID, result.TimeframeID)
...
func TestTemplateOfferingSource_SplitRejectsOfferingOutsideNewPeriod(t *testing.T) {
	...
	_, err = s.factory.TimetableData.Templates.SplitTemplate(s.ctx, timetable.SplitTemplateCommand{
		TemplateID:       result.TemplateID,
		EffectiveDate:    monday,
		Name:             name,
		Type:             activitiesModels.GroupTypeCare,
