...
func makeRosterChain(t *testing.T, weekdays []int) *rosterChain {
	...
	_, err := chain.svc.MaterializeForTenant(
		chain.ctx, firstBoundary, secondBoundary.AddDays(-1), timetable.MaterializationSourceManual)
...
func instanceStudentIDsOn(t *testing.T, s *scenarioSetup, groupID int64, date calendar.Date) []int64 {
	t.Helper()
	ids := make([]int64, 0)
	err := s.db.NewSelect().
...
func instanceStaffIDsOn(t *testing.T, s *scenarioSetup, groupID int64, date calendar.Date) []int64 {
	t.Helper()
	ids := make([]int64, 0)
	err := s.db.NewSelect().
...
func deleteInstanceStudentRow(t *testing.T, s *scenarioSetup, groupID, studentID int64, date calendar.Date) {
	t.Helper()
...
func TestSeriesRoster_AddsChildAcrossCappedPredecessor(t *testing.T) {
	...
	require.NoError(t, c.factory.TimetableData.Templates.UpdateTemplate(c.ctx, in))
...
func TestSeriesRoster_PastAnchorClampsToTodayAndSegmentStart(t *testing.T) {
	...
	added := c.students[2]
	staleAnchor := calendar.NewDate(2026, 8, 24).AddDays(-7)
...
func TestSeriesRoster_RemovedStaffLosesPlannedOccurrenceRows(t *testing.T) {
	...
	require.NoError(t, c.factory.TimetableData.Templates.UpdateTemplate(c.ctx, added))
	...
	require.NoError(t, c.factory.TimetableData.Templates.UpdateTemplate(c.ctx, removed))
...
func TestSeriesRoster_PreservesHandRemovedChildOnPredecessorOccurrence(t *testing.T) {
	...
	require.NoError(t, c.factory.TimetableData.Templates.UpdateTemplate(c.ctx, first))
	...
	require.NoError(t, c.factory.TimetableData.Templates.UpdateTemplate(c.ctx, second))
...
func TestSeriesRoster_PrimaryChangeReachesMaterializedOccurrences(t *testing.T) {
	...
	require.NoError(t, c.factory.TimetableData.Templates.UpdateTemplate(c.ctx, joined))
	...
	require.NoError(t, c.factory.TimetableData.Templates.UpdateTemplate(c.ctx, moved))
...
func instanceStaffPrimaryOn(t *testing.T, s *scenarioSetup, groupID int64, date calendar.Date) int64 {
	t.Helper()
	ids := make([]int64, 0)
	err := s.db.NewSelect().
		TableExpr(`schedule.instance_staff AS "row"`).
...
func (c *seriesChain) markInstanceStaffAbsent(t *testing.T, staffID int64, date calendar.Date) {
	t.Helper()
