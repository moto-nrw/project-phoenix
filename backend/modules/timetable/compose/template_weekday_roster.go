...
// deviations (timetable.WeekdayRosterAssignment, #2129) into the concrete rows
// to persist.
//
...
// indexWeekdayAssignments rejects an assignment for a weekday the template
// does not run on (dropping it would lose the planner's input, adding the
// weekday would change the recurrence behind their back), a duplicate
// weekday, and a primary supervisor outside the weekday's staff.
func indexWeekdayAssignments(
	weekdays []int,
	assignments []timetable.WeekdayRosterAssignment,
) (map[int]timetable.WeekdayRosterAssignment, error) {
	scheduled := make(map[int]struct{}, len(weekdays))
	for _, weekday := range weekdays {
		scheduled[weekday] = struct{}{}
	}
	byWeekday := make(map[int]timetable.WeekdayRosterAssignment, len(assignments))
	for _, assignment := range assignments {
		...
		if _, ok := scheduled[assignment.Weekday]; !ok {
			return nil, fmt.Errorf("%w: %d", timetable.ErrWeekdayAssignmentUnscheduled, assignment.Weekday)
		}
		if _, dup := byWeekday[assignment.Weekday]; dup {
			return nil, fmt.Errorf("%w: %d", timetable.ErrWeekdayAssignmentDuplicate, assignment.Weekday)
		}
		if assignment.PrimaryStaffID != nil && !rosterContainsPersonID(assignment.StaffIDs, *assignment.PrimaryStaffID) {
			return nil, fmt.Errorf("%w: %d", timetable.ErrWeekdayAssignmentPrimaryStaffMissing, assignment.Weekday)
		}
