...
// Split-series segment resolution (#2187).
//
...
func loadTemplateSeriesSegments(
	...
	for _, group := range groups {
		...
		segment, ok, err := templateSeriesSegmentOf(group, schedulesByGroup[group.ID], templateID, logger)
		if err != nil {
			return nil, err
		}
		if ok {
...
// templateSeriesSegmentOf builds one segment from its schedule rows. A
// template without schedule rows has no recurrence to resolve against and
// cannot be the living segment either; it is skipped like a sibling with an
// inconsistent envelope.
func templateSeriesSegmentOf(
	group *activitiesModel.Group,
	schedules []*activitiesModel.Schedule,
	templateID int64,
	logger *slog.Logger,
) (templateSeriesSegment, bool, error) {
	if len(schedules) == 0 {
		return templateSeriesSegment{}, false, nil
	}
	validFrom, validUntil, err := commonScheduleValidityEnvelope(schedules)
	if err != nil {
		if group.ID == templateID {
			return templateSeriesSegment{}, false, &ScheduleError{Op: "load template series: inspect schedule envelope", Err: err}
		}
		orDefaultLogger(logger).Warn("skipping template series segment with inconsistent schedule envelope",
...
func (s *TemplateService) ResolveLivingTemplateSegment(
	...
	if living == nil {
		return 0, false, timetable.ErrTemplateSeriesFullyEnded
	}
