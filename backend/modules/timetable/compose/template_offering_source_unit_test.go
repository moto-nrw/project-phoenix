...
func TestResolveSuccessorOfferingSource(t *testing.T) {
	...
	t.Run("omitted fields inherit sources and filter", func(t *testing.T) {
		...
		require.NoError(t, resolveSuccessorOfferingSource(in, sourcedOld(), testSchoolClassRules))
	...
	t.Run("merged source next to explicit student_ids is rejected", func(t *testing.T) {
		...
		err := resolveSuccessorOfferingSource(in, sourcedOld(), testSchoolClassRules)
...
func TestOfferingRosterFeedChanged(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			assert.Equal(t, tc.wantChanged, offeringRosterFeedChanged(tc.old, tc.newG, testSchoolClassRules.Normalize))
		})
	}
}
...
func TestResyncUpdatedTemplateOfferingRoster(t *testing.T) {
	...
	deps := func(resync func(context.Context, timetable.OfferingRosterResyncInput) error) TemplateServiceDependencies {
	...
	t.Run("a template without a source before and after skips the hook", func(t *testing.T) {
		...
		svc := NewTemplateService(deps(
			func(context.Context, timetable.OfferingRosterResyncInput) error {
				called = true
				return nil
			},
		))
	...
	t.Run("removing a source still reconciles, using the previous offering", func(t *testing.T) {
		var got timetable.OfferingRosterResyncInput
		svc := NewTemplateService(deps(
			func(_ context.Context, in timetable.OfferingRosterResyncInput) error {
				got = in
				return nil
			},
		))
...
func TestValidateOfferingSourceReference(t *testing.T) {
	...
	t.Run("no source skips the hook", func(t *testing.T) {
		called := false
		svc := NewTemplateService(TemplateServiceDependencies{CareOfferings: CareOfferingChecks{
	...
	t.Run("an unknown source surfaces as timetable.ErrOfferingSourceInvalid before any write", func(t *testing.T) {
		svc := NewTemplateService(TemplateServiceDependencies{CareOfferings: CareOfferingChecks{
			ValidateOfferingSource: func(context.Context, []int64, []int64, *int64) error {
				return fmt.Errorf("%w: care offering %d not found", timetable.ErrOfferingSourceInvalid, int64(12))
			},
		}})
