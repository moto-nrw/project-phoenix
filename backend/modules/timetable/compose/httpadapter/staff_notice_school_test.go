...
import (
	...
	"github.com/moto-nrw/project-phoenix/modules/timetable/compose/httpadapter"
...
// init seeds JWT viper defaults before any test builds a resource through the
...
// newStaffNoticeService verdrahtet die Tagesinformationen (#2180, #2208) so wie
// die Wurzel: eigenes Repository, Kalenderzeiträume für das Wochenmuster und
// das echte Personenverzeichnis hinter der Bestätigungsliste.
func newStaffNoticeService(t *testing.T, db *bun.DB) timetable.StaffNotices {
	t.Helper()
	timetableRepos, err := repositories.NewTimetableTestRepositories(db)
	require.NoError(t, err)
	persons, err := repositories.NewPeopleDirectory(db)
	require.NoError(t, err)
	return compose.NewStaffNotices(compose.StaffNoticeDependencies{
		DB:      db,
		Periods: timetableRepos.CalendarPeriod,
		Names: func(ctx context.Context, accountIDs []int64) (map[int64]string, error) {
			rows, err := persons.ListPersonsByAccount(ctx, accountIDs)
			if err != nil {
				return nil, err
			}
