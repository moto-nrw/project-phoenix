...
// NewStaffShiftPlanning binds the /api/staff-shifts capability to the shift,
// series, overview and plan export services. PlanExport may be nil, which
// leaves ExportPlan reporting that the export is not configured: the printable
// Dienstplan is a separate owner and a composition without it must still serve
// the planning routes.
func NewStaffShiftPlanning(deps PlanningDependencies) workforce.StaffShiftPlanning {
	if deps.Shifts == nil || deps.Series == nil || deps.Overview == nil {
		panic("staff shift planning: the shift, series and overview services are required")
	}
...
func (p staffShiftPlanning) ExportPlan(ctx context.Context, request workforce.PlanExportRequest) (workforce.PlanExportFile, error) {
	if p.deps.PlanExport == nil {
		return workforce.PlanExportFile{}, errors.New("plan export is not configured")
	}
...
func overviewToCapability(overview *StaffScheduleOverview) workforce.StaffScheduleOverview {
	...
	for _, assignment := range overview.Assignments {
		...
		assignments = append(assignments, workforce.OverviewAssignment{
			...
			ActivityTitle: assignment.ActivityTitle, ActivityGroupID: assignment.ActivityGroupID,
			RoomID: assignment.RoomID, RoomName: assignment.RoomName,
