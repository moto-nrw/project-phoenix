...
// cascade directions take the same shape. The absence service owns the same
// vocabulary as its port; the composition root maps between the two.
...
// schedule.instance_staff). The contract is FAIL-CLOSED: the linkage is the
...
// forbids). All methods run inside the caller's tenant transaction.
