...
func TestStaffScheduleOverview_TenantIsolationAcrossEveryProjectionRead(t *testing.T) {
	...
	service := shiftplanning.NewStaffScheduleOverviewService(shiftplanning.StaffScheduleOverviewDependencies{
	...
	localAssignments := make(map[int64]shiftplanning.StaffScheduleAssignment)
	...
	assert.Equal(t, shiftplanning.CoverageStatusNotApplicable, localAssignments[local.instanceID].CoverageStatus)
	...
	assert.Equal(t, shiftplanning.CoverageStatusCovered, foreignOverview.Assignments[0].CoverageStatus)
}
...
func createOverviewShift(t *testing.T, db *bun.DB, tenantID, staffID int64, date timezone.Date, start, end string, breakMinutes int) int64 {
	...
	shift.SetTenantID(tenantID)
	require.NoError(t, repositories.NewFactory(db, repositories.NewUnobservedTimetableDependencies(db)).StaffShift.Create(testpkg.TenantContext(tenantID), shift))
	return shift.ID
}
...
func TestStaffScheduleOverview_WeeklySummariesResolveSollAndIsolateTenant(t *testing.T) {
	...
	summaries := make(map[int64]shiftplanning.StaffWeeklySummary, len(overview.WeeklySummaries))
	...
	require.NoError(t, err)
	foreignSummaries := make(map[int64]shiftplanning.StaffWeeklySummary, len(foreignOverview.WeeklySummaries))
...
func TestStaffScheduleOverview_WeeklySummariesIncludeShiftsOutsideViewport(t *testing.T) {
	...
	var summary *shiftplanning.StaffWeeklySummary
