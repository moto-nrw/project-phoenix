...
func TestShiftTypeService_CreateValidationAndNameTaken(t *testing.T) {
	...
	svc := shiftplanning.NewShiftTypeService(repos.ShiftType, slog.Default())
	ctx := testpkg.Ctx(t)
	...
	require.ErrorIs(t, err, shiftplanning.ErrShiftTypeNameTaken)

	// Empty name is an input error.
	_, err = svc.CreateShiftType(ctx, &scheduleModels.ShiftType{Name: "   ", Color: "#5080D8"})
	require.ErrorIs(t, err, shiftplanning.ErrShiftTypeInvalid)
}
...
func TestShiftTypeService_UpdateAndDelete(t *testing.T) {
	...
	require.ErrorIs(t, err, shiftplanning.ErrShiftTypeNotFound)
}
