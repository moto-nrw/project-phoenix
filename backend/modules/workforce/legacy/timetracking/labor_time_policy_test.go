...
func runningBreak(now time.Time, minutesAgo int) *WorkSessionBreak {
...
func TestNetMinutesWithBreaks(t *testing.T) {
	...
	t.Run("deducts a running break from an open session", func(t *testing.T) {
		...
		breaks := []*WorkSessionBreak{runningBreak(fixedNow, 30)}
...
func TestIsBreakCompliant(t *testing.T) {
	...
	t.Run("running break counts toward the requirement", func(t *testing.T) {
		...
		breaks := []*WorkSessionBreak{runningBreak(fixedNow, 35)}
	...
	t.Run("a too-short running break stays non-compliant", func(t *testing.T) {
		...
		breaks := []*WorkSessionBreak{runningBreak(fixedNow, 10)}
...
func TestEvaluateWorkSessionsLaborTime(t *testing.T) {
	...
	block := func(id int64, inHour, inMin, outHour, outMin, breakMin int) *WorkSession {
	...
	t.Run("does not combine a short gap with a break", func(t *testing.T) {
		...
		eval := EvaluateWorkSessionsLaborTime([]*WorkSession{first, second}, breaks, fixedNow)
	...
	t.Run("single block matches EvaluateLaborTime", func(t *testing.T) {
		...
		day := EvaluateWorkSessionsLaborTime([]*WorkSession{ws}, nil, fixedNow)
...
func TestNetMinutesByDateLegacyBreakAcrossRangeStart(t *testing.T) {
	...
	checkIn := time.Date(2024, 1, 1, 22, 0, 0, 0, Berlin)
	checkOut := time.Date(2024, 1, 2, 4, 0, 0, 0, Berlin)
	session := &WorkSession{
	...
	firstDay := NewDate(2024, time.January, 1)
	secondDay := NewDate(2024, time.January, 2)
	...
	t.Run("full span consumes the cache on the day it starts", func(t *testing.T) {
		...
		assert.Equal(t, map[Date]int{firstDay: 60, secondDay: 240}, byDate)
	})
	...
	t.Run("range starting on the second day keeps that day whole", func(t *testing.T) {
		...
		assert.Equal(t, map[Date]int{secondDay: 240}, byDate)
	})
}
