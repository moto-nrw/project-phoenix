...
func (f *overviewFixture) newExportService() timetracking.StaffTimeExportService {
...
func (f *overviewFixture) newWorkSessionService() timetracking.WorkSessionService {
...
func TestMonthExportRows_MatchMonthSummary(t *testing.T) {
	...
	byStaff := make(map[int64]timetracking.MonthExportRow, len(rows))
...
func TestMonthExportRows_ClosedMonthCarriesFrozenValue(t *testing.T) {
	...
	closeSvc := timetracking.NewStaffMonthCloseService(services.MonthSnapshotCapability(f.repos.StaffMonthSnapshot), monthSvc, services.MonthCloseStaff(f.repos.Staff), settings, nil)
...
func TestMonthExportRows_RejectsFutureMonth(t *testing.T) {
	...
	require.Error(t, err)
	assert.ErrorIs(t, err, timetracking.ErrTimeExportInvalid)
...
func TestStaffTimeExport_CSVAndAudit(t *testing.T) {
	...
	file, err := exportSvc.Export(f.ctx, timetracking.TimeExportRequest{
		Year:  f.today.Year(),
...
func (failingAccessLogRepo) Create(context.Context, *timetracking.DataAccessEvent) error {
...
func TestStaffTimeExport_NoFileWithoutAudit(t *testing.T) {
	...
	f := newOverviewFixture(t, 1)
	actorID := f.newActorAccount(t)
	exportSvc := timetracking.NewStaffTimeExportService(
...
func TestStaffTimeExport_DayRowsMatchSingleExport(t *testing.T) {
	...
	exportSvc := timetracking.NewStaffTimeExportService(
		f.svc, sessionSvc, services.TimeExportStaff(f.repos.Staff), services.NewTimeTrackingDataAccessAudit(f.repos.DataAccessLog), nil, nil,
		services.RenderTimeTrackingWorkbook,
	)
	...
	file, err := exportSvc.Export(f.ctx, timetracking.TimeExportRequest{
		Year:        f.today.Year(),
		Month:       int(f.today.Month()),
		Granularity: timetracking.ExportGranularityDay,
	}, actorID, "admin")
	require.NoError(t, err)
...
func TestStaffTimeExport_RejectsUnknownParameters(t *testing.T) {
	...
	for _, req := range []timetracking.TimeExportRequest{
