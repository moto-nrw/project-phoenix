...
type wsMockStaffShiftRepository struct {
	findByStaffIDsAndDateFunc func(ctx context.Context, staffIDs []int64, date Date) ([]*TimeTrackingShift, error)
}

func (m *wsMockStaffShiftRepository) FindByStaffIDsAndDate(ctx context.Context, staffIDs []int64, date Date) ([]*TimeTrackingShift, error) {
...
func autoCheckoutFixture(openSessions []*WorkSession, shifts []*TimeTrackingShift) (*workSessionService, *wsMockWorkSessionRepository, *wsMockWorkSessionBreakRepository, *wsMockWorkSessionEditRepository, *wsMockStaffShiftRepository) {
	service, sessionRepo, breakRepo, auditRepo, _ := wsCreateTestService()
	shiftRepo := &wsMockStaffShiftRepository{}
	service.staffShiftRepo = shiftRepo

	sessionRepo.getOpenSessionsFunc = func(_ context.Context, _ Date) ([]*WorkSession, error) {
		return openSessions, nil
	}
	sessionRepo.lockOpenByIDFunc = func(_ context.Context, id int64) (*WorkSession, error) {
...
func shiftFor(staffID int64, date Date, startHour, endHour int) *TimeTrackingShift {
...
func TestAutoCheckout_SkipsReopenedAfterShiftEndFromLockedRow(t *testing.T) {
	...
	service, sessionRepo, _, auditRepo, _ := autoCheckoutFixture(
		[]*WorkSession{listedSession},
		[]*TimeTrackingShift{shift},
	)
	sessionRepo.lockOpenByIDFunc = func(_ context.Context, _ int64) (*WorkSession, error) {
...
func TestAutoCheckout_EndsActiveBreakAfterCloseClaim(t *testing.T) {
	...
	activeBreak := &WorkSessionBreak{StartedAt: shift.EndInstant().Add(-time.Hour)}
	activeBreak.ID = 9
	breakRepo.getBySessionIDFunc = func(_ context.Context, _ int64) ([]*WorkSessionBreak, error) {
		return []*WorkSessionBreak{activeBreak}, nil
	}
	breakEnded := false
	closed := false
...
func TestAutoCheckout_BreakCappedAtShiftEnd(t *testing.T) {
	...
	activeBreak := &WorkSessionBreak{StartedAt: shift.EndInstant().Add(-30 * time.Minute)}
	activeBreak.ID = 9
	breakRepo.getBySessionIDFunc = func(_ context.Context, _ int64) ([]*WorkSessionBreak, error) {
		return []*WorkSessionBreak{activeBreak}, nil
	}
...
func TestAutoCheckout_SkipsBreakStartedAfterShiftEnd(t *testing.T) {
	...
	activeBreak := &WorkSessionBreak{StartedAt: shift.EndInstant().Add(10 * time.Minute)}
	activeBreak.ID = 9
	breakRepo.getBySessionIDFunc = func(_ context.Context, _ int64) ([]*WorkSessionBreak, error) {
		return []*WorkSessionBreak{activeBreak}, nil
	}
...
func TestAutoCheckout_SkipsCompletedBreakStartedAfterShiftEnd(t *testing.T) {
	...
	completedBreak := &WorkSessionBreak{
		StartedAt: shift.EndInstant().Add(10 * time.Minute),
		EndedAt:   &endedAt,
	}
	completedBreak.ID = 9
	breakRepo.getBySessionIDFunc = func(_ context.Context, _ int64) ([]*WorkSessionBreak, error) {
		return []*WorkSessionBreak{completedBreak}, nil
	}
...
func TestAutoCheckout_RechecksBreaksAfterCloseBeforeAudit(t *testing.T) {
	...
	lateBreak := &WorkSessionBreak{StartedAt: shift.EndInstant().Add(10 * time.Minute)}
	lateBreak.ID = 9
	getBreaksCalls := 0
	breakRepo.getBySessionIDFunc = func(_ context.Context, _ int64) ([]*WorkSessionBreak, error) {
		getBreaksCalls++
		...
		return []*WorkSessionBreak{lateBreak}, nil
	}
	breakRepo.endBreakFunc = func(_ context.Context, _ int64, _ time.Time, _ int) error {
...
func TestAutoCheckout_QueriesOpenSessionsIncludingToday(t *testing.T) {
	...
	var beforeDate Date
	sessionRepo.getOpenSessionsFunc = func(_ context.Context, d Date) ([]*WorkSession, error) {
		beforeDate = d
		return nil, nil
	}
	...
	require.NoError(t, err)
	assert.Equal(t, NewDate(2026, 8, 24).AddDays(1), beforeDate,
...
func (m *wsMockStaffShiftRepository) FindByStaffIDsAndDateRange(context.Context, []int64, Date, Date) (map[int64][]*TimeTrackingShift, error) {
	return nil, nil
}
