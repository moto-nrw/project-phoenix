...
func TestTimeTrackingAuditReaderPreservesQueryAndPartialRows(t *testing.T) {
	...
	for _, day := range []*timezone.Date{nil, &zero, &date} {
		...
		got, err := reader.ListEntries(ctx, timetracking.TimeTrackingAuditFilter{From: day, To: day, StaffID: 7, ActorStaffID: 8, Sources: []string{"session_edit"}, Limit: 51, Cursor: &timetracking.TimeTrackingAuditCursor{OccurredAt: now, Source: "session_edit", EntryID: 9}})
		require.ErrorIs(t, err, readErr)
		require.Len(t, got, 2)
		require.Nil(t, got[0])
		require.Equal(t, &timetracking.TimeTrackingAuditEntry{OccurredAt: row.OccurredAt, Source: row.Source, EntryID: row.EntryID, StaffID: row.StaffID, ActorStaffID: row.ActorStaffID, ActorIsSystem: row.ActorIsSystem, Reason: row.Reason, Detail: row.Detail}, got[1])
