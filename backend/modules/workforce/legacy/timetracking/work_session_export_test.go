...
func TestWSExportSessions_CSV_Success(t *testing.T) {
	...
	staffID := int64(100)
	from := NewDate(2024, 1, 1)
	to := NewDate(2024, 1, 7)

	date1 := NewDate(2024, 1, 2)
	...
	absenceRepo.getByStaffAndDateRangeFunc = func(_ context.Context, _ int64, _, _ Date) ([]*StaffAbsence, error) {
		return nil, nil
	}
...
func TestWSExportSessions_GetHistoryError(t *testing.T) {
	...
	file, err := svc.ExportSessions(context.Background(), 100, TodayDate(), TodayDate(), "csv")
	require.Error(t, err)
	assert.Nil(t, file)
...
func TestWSBuildExportRows_AbsencesOnly(t *testing.T) {
	...
	dateStart := NewDate(2024, 1, 2)
	dateEnd := NewDate(2024, 1, 4)
...
func TestClampAbsencesToRange_BoundsOverlappingAbsence(t *testing.T) {
	...
	clamped := clampAbsencesToRange([]*StaffAbsence{absence}, from, to)
...
func TestDayExportRowsByStaffIDs_BatchesAllRepositoryLoads(t *testing.T) {
	...
	date := NewDate(2024, 1, 2)
	checkIn := time.Date(2024, 1, 2, 8, 0, 0, 0, time.UTC)
	checkOut := time.Date(2024, 1, 2, 16, 0, 0, 0, time.UTC)

	var sessionLoads, absenceLoads, breakLoads, manualAuditLoads int
	sessionRepo.getHistoryByStaffIDsFunc = func(_ context.Context, gotStaffIDs []int64, _, _ Date) (map[int64][]*WorkSession, error) {
		sessionLoads++
		assert.Equal(t, staffIDs, gotStaffIDs)
		return map[int64][]*WorkSession{
			staffIDs[0]: {{
				Model:        Model{ID: 1},
				StaffID:      staffIDs[0],
				Date:         date,
	...
	sessionRepo.getHistoryByStaffIDFunc = func(context.Context, int64, Date, Date) ([]*WorkSession, error) {
		return nil, errors.New("single-staff session query must not run")
	}
	absenceRepo.getByStaffIDsAndDateRangeFunc = func(_ context.Context, gotStaffIDs []int64, _, _ Date) (map[int64][]*StaffAbsence, error) {
		absenceLoads++
		assert.Equal(t, staffIDs, gotStaffIDs)
		return map[int64][]*StaffAbsence{}, nil
	}
	...
	breakRepo.listFunc = func(context.Context, *QueryOptions) ([]*WorkSessionBreak, error) {
		breakLoads++
		return nil, nil
	}
	breakRepo.getBySessionIDFunc = func(context.Context, int64) ([]*WorkSessionBreak, error) {
...
func TestDayExportRowsByStaffIDsStampsCustomAbsenceLabels(t *testing.T) {
	...
	sessionRepo.getHistoryByStaffIDsFunc = func(context.Context, []int64, Date, Date) (map[int64][]*WorkSession, error) {
		return map[int64][]*WorkSession{}, nil
	}
	absenceRepo.getByStaffIDsAndDateRangeFunc = func(context.Context, []int64, Date, Date) (map[int64][]*StaffAbsence, error) {
		return map[int64][]*StaffAbsence{staffID: []*StaffAbsence{{
			StaffID:       staffID,
...
func TestWSBuildExportRows_CompTimeUsesGermanLabel(t *testing.T) {
	...
	svc, _, _, _, _, _ := wsCreateTestServiceWithAbsenceRepo()
	date := NewDate(2026, time.July, 24)

	rows := svc.buildExportRows(nil, []*StaffAbsence{{
		StaffID:     100,
...
func TestWSBuildExportRows_SortsByDate(t *testing.T) {
	...
	date1 := NewDate(2024, 1, 5)
	date2 := NewDate(2024, 1, 2)
...
func TestWSSessionToRow_LegacyUnknownSource(t *testing.T) {
	...
	sr := &SessionResponse{
		WorkSession: &WorkSession{
			...
			CheckInTime: checkIn,
			Status:      WorkSessionStatusPresent,
			Source:      WorkSessionSourceUnknown,
		},
		NetMinutes: 0,
	}
...
func TestWSExportSessions_PDF_RenderFailure(t *testing.T) {
	...
	file, err := svc.ExportSessions(context.Background(), 100, NewDate(2024, 1, 1), NewDate(2024, 1, 7), "pdf")
	require.ErrorIs(t, err, renderErr)
	require.Nil(t, file)
}
...
func TestWSUpdateSession_StatusChange(t *testing.T) {
	...
	newStatus := WorkSessionStatusHomeOffice
...
func TestWSCleanupOpenSessions_CloseSessionError(t *testing.T) {
	...
	svc, sessionRepo, _, _, _ := wsCreateTestService()
	yesterday := TodayDate().AddDays(-1)

	sessionRepo.getOpenSessionsFunc = func(_ context.Context, _ Date) ([]*WorkSession, error) {
		return []*WorkSession{
			{Model: Model{ID: 1}, Date: yesterday},
		}, nil
	}
...
func TestWSGetHistory_AuditCountError(t *testing.T) {
	...
	responses, err := svc.GetHistory(context.Background(), 100, NewDate(2026, 8, 24), NewDate(2026, 8, 24))
	require.Error(t, err)
