...
type vacationOpeningFixture struct {
	...
	staff    int64
	admin    int64
	svc      timetracking.StaffAbsenceService
...
func newVacationOpeningFixture(t *testing.T) *vacationOpeningFixture {
	...
	svc := timetracking.NewStaffAbsenceService(
		repos.StaffAbsence, repos.WorkSession, repos.StaffVacationQuota,
		repos.StaffAbsenceAudit, wtmIntSettings{}, nil,
		timetracking.WithAbsenceToday(func() timezone.Date { return timezone.NewDate(2026, 8, 24) }),
		// Wired exactly like services/factory.go does.
		timetracking.WithVacationOpenings(repos.StaffVacationOpening),
		timetracking.WithAbsenceDeletionAudit(services.NewTimeTrackingDeletionAudit(repos.TimeTrackingDeletion)),
	)
...
func TestSetVacationOpening_RejectsSecondOpeningForSameYear(t *testing.T) {
	...
	_, err = f.set(8)
	require.ErrorIs(t, err, timetracking.ErrVacationOpeningExists)
...
func TestSetVacationOpening_RejectsOpenCutoff(t *testing.T) {
	...
	for _, effectiveDate := range []timezone.Date{
		...
		_, err := f.svc.SetVacationOpening(f.ctx, f.staff, f.admin, timetracking.SetVacationOpeningRequest{
			EffectiveDate: effectiveDate,
			RemainingDays: 12,
			Note:          "Übernahme aus Altsystem",
		})
		require.ErrorIs(t, err, timetracking.ErrVacationOpeningInvalid)
...
func TestDeleteVacationOpening_MissingRowIsNotFound(t *testing.T) {
	...
	require.ErrorIs(t, err, timetracking.ErrVacationOpeningNotFound)
}
