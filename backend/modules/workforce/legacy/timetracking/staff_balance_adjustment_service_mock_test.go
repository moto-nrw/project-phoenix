...
type recordingBalanceAdjustmentRepo struct {
	StaffBalanceAdjustmentRepository
	events     *[]string
	adjustment *StaffBalanceAdjustment
	resets     []*StaffBalanceAdjustment
	findErr    error
}
...
func (r *recordingBalanceAdjustmentRepo) Create(_ context.Context, adjustment *StaffBalanceAdjustment) error {
	*r.events = append(*r.events, "create")
	r.adjustment = adjustment
	return nil
}

func (r *recordingBalanceAdjustmentRepo) FindByID(context.Context, any) (*StaffBalanceAdjustment, error) {
...
func (r *recordingBalanceAdjustmentRepo) List(context.Context, *QueryOptions) ([]*StaffBalanceAdjustment, error) {
...
func (s *recordingBalanceMonthService) GetClosingBalanceAsOf(context.Context, int64, Date) (int, error) {
	*s.events = append(*s.events, "as-of")
	return s.balance, nil
}

func (s *recordingBalanceMonthService) GetBalanceReductionCapacity(_ context.Context, _ int64, effectiveDate Date) (int, error) {
...
func newRecordingBalanceAdjustmentService(
	...
	service := NewStaffBalanceAdjustmentService(
		...
		// Deletes require the tombstone writer (#1417); the recording repo keeps
		// its event log limited to the adjustment repo, so assertions are unchanged.
		WithAdjustmentDeletionAudit(noopDeletionAuditRepo{}),
	).(*staffBalanceAdjustmentService)
	return service
}
...
func TestStaffBalanceAdjustmentService_AnnouncesTimeTrackingChange(t *testing.T) {
	t.Parallel()
	...
	_, err := service.CreateAdjustment(ctx, 41, 42, CreateBalanceAdjustmentRequest{
		...
		MinutesDelta:  -60,
		EffectiveDate: NewDate(2026, time.July, 7),
		Note:          "Auszahlung",
	})
...
func TestStaffBalanceAdjustmentService_ChecksFrozenMonthAfterLock(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			service.snapshotRepo = &recordingAdjustmentFreezeReader{
				events:   &events,
				snapshot: frozen,
			}
...
func TestStaffBalanceAdjustmentService_RejectsWritesThatPrecedeReset(t *testing.T) {
	...
	resetDate := TodayDate().AddDays(-1)
	reset := &StaffBalanceAdjustment{
		StaffID:       staffID,
		Type:          BalanceAdjustmentTypeReset,
	...
	t.Run("adjustment on reset date", func(t *testing.T) {
		...
		repo := &recordingBalanceAdjustmentRepo{
			events: &events,
			resets: []*StaffBalanceAdjustment{reset},
		}
		service := newRecordingBalanceAdjustmentService(&events, repo, nil)
	...
	t.Run("adjustment after reset", func(t *testing.T) {
		...
		_, err := service.CreateAdjustment(context.Background(), staffID, decidedBy, CreateBalanceAdjustmentRequest{
			Type:          BalanceAdjustmentTypeCompTime,
			MinutesDelta:  -60,
...
func TestStaffBalanceAdjustmentService_RejectsResetOnOpenDay(t *testing.T) {
	...
	for _, effectiveDate := range []Date{
		TodayDate(),
		TodayDate().AddDays(1),
	} {
		events := []string{}
...
func TestStaffBalanceAdjustmentService_BlocksDeleteWhenLaterResetDependsOnAdjustment(t *testing.T) {
	...
	events := []string{}
	effectiveDate := NewDate(2026, time.July, 1)
	adjustment := &StaffBalanceAdjustment{
		StaffID:       staffID,
		Type:          BalanceAdjustmentTypeReset,
		EffectiveDate: effectiveDate,
	...
	adjustment.ID = 10
	laterReset := &StaffBalanceAdjustment{
		StaffID:       staffID,
		Type:          BalanceAdjustmentTypeReset,
	...
	repo := &recordingBalanceAdjustmentRepo{
		...
		resets:     []*StaffBalanceAdjustment{adjustment, laterReset},
	}
	service := newRecordingBalanceAdjustmentService(&events, repo, nil)
...
func TestStaffBalanceAdjustmentService_RejectsFarFutureEffectiveDate(t *testing.T) {
	...
	_, err := service.CreateAdjustment(context.Background(), int64(41), int64(42), CreateBalanceAdjustmentRequest{
		Type:          BalanceAdjustmentTypePayout,
		MinutesDelta:  -60,
		EffectiveDate: TodayDate().AddDays(420),
		Note:          "Auszahlung",
	})
...
func TestStaffBalanceAdjustmentService_DeletePropagatesLookupFailures(t *testing.T) {
	...
	t.Run("no rows maps to not found", func(t *testing.T) {
		events := []string{}
		repo := &recordingBalanceAdjustmentRepo{events: &events, findErr: ErrNotFound}
		service := newRecordingBalanceAdjustmentService(&events, repo, nil)
