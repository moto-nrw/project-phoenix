...
func (m *wtmMockHolidayReader) HolidayDates(_ context.Context, from, to Date) (map[Date]bool, error) {
	if m.err != nil {
		return nil, m.err
	}
	result := make(map[Date]bool)
	for d := range m.dates {
...
func TestWTMDailyTargets_HolidayZeroesSoll(t *testing.T) {
	...
	// a holiday.
	holiday := NewDate(2026, time.June, 8)
	f.svc.holidayReader = &wtmMockHolidayReader{dates: map[Date]bool{holiday: true}}

	targets, err := f.svc.GetDailyProjection(context.Background(), wtmStaffID,
		NewDate(2026, time.June, 1), NewDate(2026, time.June, 15))
	require.NoError(t, err)

	byDate := make(map[Date]int, len(targets))
	for _, target := range targets {
		byDate[target.Date] = target.TargetMinutes
	}
	assert.Equal(t, 480, byDate[NewDate(2026, time.June, 1)], "regular Monday keeps its Soll")
	assert.Equal(t, 0, byDate[holiday], "holiday Monday must have Soll 0")
	assert.Equal(t, 480, byDate[NewDate(2026, time.June, 15)], "Monday after the holiday keeps its Soll")
}
...
func TestWTMMonthSummary_HolidayReaderErrorPropagates(t *testing.T) {
	...
	f := newWTMFixture()
	f.svc.holidayReader = &wtmMockHolidayReader{err: errors.New("settings unavailable")}
...
func TestHolidayScheduleMinutes(t *testing.T) {
	...
	entries := []*WorkScheduleRow{
		{StaffID: wtmStaffID, DayOfWeek: DayMonday, TargetMinutes: 480, RotationLength: 1, ValidFrom: NewDate(2020, time.January, 1)},
		{StaffID: wtmStaffID, DayOfWeek: DayThursday, TargetMinutes: 240, RotationLength: 1, ValidFrom: NewDate(2020, time.January, 1)},
	}
	weekStart := NewDate(2026, time.June, 8) // Monday

	// Holiday on the Monday only: the week loses 480 of its 720 minutes.
	holidaySet := map[Date]bool{weekStart: true}
	...
	holidaySet = map[Date]bool{weekStart.AddDays(5): true}
...
func TestHolidayModelMinutes(t *testing.T) {
	...
	anchor := NewDate(2020, time.January, 6)
	...
	weekStart := NewDate(2026, time.June, 8)

	assert.Equal(t, 480, holidayTemplateMinutes(model, anchor, weekStart, map[Date]bool{weekStart: true}))
