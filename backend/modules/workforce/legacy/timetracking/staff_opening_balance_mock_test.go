...
func TestStaffBalanceAdjustmentService_OpeningAllowsNegativeTarget(t *testing.T) {
	...
	effectiveDate := NewDate(2026, 8, 24).AddDays(-1)
	...
	assert.Equal(t, BalanceAdjustmentTypeOpening, adjustment.Type)
...
func TestStaffBalanceAdjustmentService_OpeningRejectsSecondOpening(t *testing.T) {
	...
	events := []string{}
	existing := &StaffBalanceAdjustment{
		StaffID:       staffID,
		Type:          BalanceAdjustmentTypeOpening,
		EffectiveDate: TodayDate().AddDays(-10),
	}
	repo := &recordingBalanceAdjustmentRepo{events: &events, resets: []*StaffBalanceAdjustment{existing}}
...
func (r *typeFilteringBalanceAdjustmentRepo) List(ctx context.Context, options *QueryOptions) ([]*StaffBalanceAdjustment, error) {
	r.listCalls++
	if r.listCalls == 1 {
...
func TestStaffBalanceAdjustmentService_OpeningRejectsExistingLaterReset(t *testing.T) {
	...
	laterReset := &StaffBalanceAdjustment{
		...
		EffectiveDate: TodayDate().AddDays(-2),
	}
	repo := &typeFilteringBalanceAdjustmentRepo{
		recordingBalanceAdjustmentRepo: &recordingBalanceAdjustmentRepo{events: &events, resets: []*StaffBalanceAdjustment{laterReset}},
	}
	month := &recordingBalanceMonthService{events: &events}
...
func (r *failingBalanceAdjustmentRepo) List(ctx context.Context, options *QueryOptions) ([]*StaffBalanceAdjustment, error) {
	if r.listErr != nil {
...
func (r *failingBalanceAdjustmentRepo) Create(ctx context.Context, adjustment *StaffBalanceAdjustment) error {
	if r.createErr != nil {
...
func (s *failingBalanceMonthService) GetClosingBalanceAsOf(ctx context.Context, staffID int64, date Date) (int, error) {
	if s.err != nil {
		return 0, s.err
	}
...
func newOpeningTestService(events *[]string, repo StaffBalanceAdjustmentRepository, month WorkTimeMonthService) StaffBalanceAdjustmentService {
...
func TestStaffBalanceAdjustmentService_OpeningRejectsInvalidRequests(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			effectiveDate: Date(""), minutes: 600, note: "Übernahme",
		...
		},
		{
			...
			effectiveDate: TodayDate(), minutes: 600, note: "Übernahme",
...
func TestStaffBalanceAdjustmentService_OpeningRejectsDateBeforeAccountStart(t *testing.T) {
	...
	_, err := service.CreateOpeningBalance(
		...
		NewDate(2026, time.May, 31), 600, "Übernahme",
	)
...
func TestStaffBalanceAdjustmentService_ValidateOpeningTakesNoLock(t *testing.T) {
	...
	err := service.ValidateOpeningBalance(
		context.Background(), 41, 42,
		TodayDate().AddDays(-1), -330, "Übernahme",
	)
...
func TestStaffBalanceAdjustmentService_ValidateOpeningReportsTheSameRejection(t *testing.T) {
	...
	existing := &StaffBalanceAdjustment{
		StaffID:       41,
		Type:          BalanceAdjustmentTypeOpening,
		EffectiveDate: TodayDate().AddDays(-30),
	}
	repo := &recordingBalanceAdjustmentRepo{events: &events, resets: []*StaffBalanceAdjustment{existing}}
	service := newOpeningTestService(&events, repo, &recordingBalanceMonthService{events: &events})
	...
	err := service.ValidateOpeningBalance(
		context.Background(), 41, 42,
		TodayDate().AddDays(-1), 600, "Übernahme",
	)
