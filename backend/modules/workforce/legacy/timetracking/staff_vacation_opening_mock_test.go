...
type voOpeningRepoMock struct {
	getByStaffAndYearFunc func(ctx context.Context, staffID int64, year int) (*StaffVacationOpening, error)
	createFunc            func(ctx context.Context, opening *StaffVacationOpening) error
	deleteFunc            func(ctx context.Context, id any) error
}

func (m *voOpeningRepoMock) Create(ctx context.Context, opening *StaffVacationOpening) error {
	if m.createFunc != nil {
...
func (m *voOpeningRepoMock) FindByID(context.Context, any) (*StaffVacationOpening, error) {
	return nil, errors.New("not found")
}

func (m *voOpeningRepoMock) Update(context.Context, *StaffVacationOpening) error {
	return nil
}
...
func (m *voOpeningRepoMock) List(context.Context, *QueryOptions) ([]*StaffVacationOpening, error) {
	return nil, nil
}

func (m *voOpeningRepoMock) GetByStaffAndYear(ctx context.Context, staffID int64, year int) (*StaffVacationOpening, error) {
...
func (m *voOpeningRepoMock) GetByStaffIDsAndYear(context.Context, []int64, int) (map[int64]*StaffVacationOpening, error) {
	return nil, nil
}
...
func voRequest() SetVacationOpeningRequest {
	return SetVacationOpeningRequest{
		EffectiveDate: TodayDate().AddDays(-1),
		RemainingDays: 17.5,
...
func TestValidateSetVacationOpening(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing actor", decided: -1, wantMsg: "decided_by is required"},
		{
			...
			mutate:  func(r *SetVacationOpeningRequest) { r.EffectiveDate = Date("") },
		...
		{
			name:    "Stichtag today",
			mutate:  func(r *SetVacationOpeningRequest) { r.EffectiveDate = TodayDate() },
			wantMsg: "must be before today",
		},
		{
			name:    "Stichtag in the future",
			mutate:  func(r *SetVacationOpeningRequest) { r.EffectiveDate = TodayDate().AddDays(1) },
			wantMsg: "must be before today",
		},
		{
			...
			mutate: func(r *SetVacationOpeningRequest) {
				r.EffectiveDate = NewDate(TodayDate().Year()-1, time.June, 1)
			},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := validateSetVacationOpening(actualStaff, actualDecided, req, TodayDate())
...
func TestSetVacationOpening_CurrentYearGuardAlsoAppliesPerStaff(t *testing.T) {
	...
	req := voRequest()
	req.EffectiveDate = NewDate(TodayDate().Year()-1, time.June, 1)
...
func TestSetVacationOpening_RejectsSecondTakeoverForTheSameYear(t *testing.T) {
	...
	existing := &StaffVacationOpening{EffectiveDate: TodayDate().AddDays(-30)}
	svc, _, _ := voService(&voOpeningRepoMock{
		getByStaffAndYearFunc: func(context.Context, int64, int) (*StaffVacationOpening, error) {
			return existing, nil
		},
	})
...
func TestSetVacationOpening_PropagatesRepositoryFailures(t *testing.T) {
	...
	t.Run("insert", func(t *testing.T) {
		svc, _, _ := voService(&voOpeningRepoMock{
			createFunc: func(context.Context, *StaffVacationOpening) error { return boom },
		})
...
func TestSetVacationOpening_DerivesTakenBeforeFromTheQuota(t *testing.T) {
	...
	var created *StaffVacationOpening
	svc, _, _ := voService(&voOpeningRepoMock{
		createFunc: func(_ context.Context, opening *StaffVacationOpening) error {
			created = opening
			return nil
		},
	})
	svc.quotaRepo = &absVacationQuotaRepoMock{
		getByStaffAndYearFunc: func(context.Context, int64, int) (*StaffVacationQuota, error) {
			return &StaffVacationQuota{EntitledDays: 28, CarryoverDays: 4}, nil
		},
	}
...
func TestDeleteVacationOpening_WritesTombstoneBeforeDeleting(t *testing.T) {
	...
	opening := &StaffVacationOpening{
		StaffID:         41,
		Year:            TodayDate().Year(),
		EffectiveDate:   TodayDate().AddDays(-10),
		TakenBeforeDays: 5,
		Note:            "Übernahme aus Altsystem",
	}
	opening.ID = 77
...
func TestDeleteVacationOpening_Rejects(t *testing.T) {
	...
	t.Run("audit write failure keeps the row", func(t *testing.T) {
		...
		svc, _, deletionRepo := voService(&voOpeningRepoMock{
			getByStaffAndYearFunc: func(context.Context, int64, int) (*StaffVacationOpening, error) {
				return &StaffVacationOpening{StaffID: 41, Year: year}, nil
			},
...
func TestRejectVacationBeforeOpening(t *testing.T) {
	...
	year := TodayDate().Year()
	cutoff := NewDate(year, time.June, 8) // Monday

	t.Run("ignores non-vacation absences", func(t *testing.T) {
		svc, _, _ := voService(&voOpeningRepoMock{
			getByStaffAndYearFunc: func(context.Context, int64, int) (*StaffVacationOpening, error) {
				return &StaffVacationOpening{EffectiveDate: cutoff}, nil
			},
		})

		err := svc.rejectVacationBeforeOpening(context.Background(), &StaffAbsence{
...
func TestRejectVacationAbsencesBefore(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			svc, absenceRepo, _ := voService(&voOpeningRepoMock{})
			absenceRepo.getByStaffAndDateRangeFunc = func(context.Context, int64, Date, Date) ([]*StaffAbsence, error) {
...
func TestRejectVacationAbsencesBefore_PropagatesLookupFailures(t *testing.T) {
	...
	err := svc.ValidateVacationOpeningAbsencesBefore(context.Background(), 41, TodayDate())
