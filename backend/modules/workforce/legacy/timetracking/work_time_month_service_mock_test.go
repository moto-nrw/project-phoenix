...
func (m *wtmMockSessionReader) ListOverlappingByStaffID(_ context.Context, _ int64, from time.Time, to *time.Time) ([]*WorkSession, error) {
	m.calls++
	m.lastFrom = DateFromTime(from)
...
func (m *wtmMockBreakReader) GetBySessionID(_ context.Context, sessionID int64) ([]*WorkSessionBreak, error) {
	return m.breaks[sessionID], nil
}

func (m *wtmMockBreakReader) GetBySessionIDs(_ context.Context, _ []int64) (map[int64][]*WorkSessionBreak, error) {
	m.batchCalls++
	return m.breaks, nil
}
...
func (m *wtmMockAbsenceReader) GetByStaffAndDateRange(_ context.Context, _ int64, from, to Date) ([]*StaffAbsence, error) {
	var result []*StaffAbsence
...
func (m *wtmMockScheduleReader) TargetsForStaff(_ context.Context, _ int64, _, _ Date) (WorkScheduleTargets, error) {
	return WorkScheduleTargets{HasEntries: len(m.entries) > 0, DailyTarget: func(anchor *Date, date Date) (int, bool) {
...
func targetModelFixture(model *WorkTimeTemplate) *WorkTimeTargetModel {
	...
	return &WorkTimeTargetModel{ID: model.ID, RotationAnchorDate: Date(model.RotationAnchorDate),
		DailyTarget: func(anchor, date Date) (int, bool) {
			return DailyTargetFromTemplate(model, Date(anchor), Date(date))
		},
	}
}
...
func (m *wtmMockShiftReader) FindByStaffAndDateRange(_ context.Context, _ int64, from, to Date) ([]*TimeTrackingShift, error) {
...
func newWTMFixture() *wtmFixture {
	f := &wtmFixture{
		sessions: &wtmMockSessionReader{},
		breaks:   &wtmMockBreakReader{breaks: map[int64][]*WorkSessionBreak{}},
		absences: &wtmMockAbsenceReader{},
		shifts:   &wtmMockShiftReader{},
...
func wtmSession(date Date, startHour, minutes, breakMinutes int) *WorkSession {
...
func wtmShift(date Date, minutes int, cancelled bool) *TimeTrackingShift {
...
func TestWTMMonthSummary_RealisticAccountStartNotClamped(t *testing.T) {
	...
	assert.Equal(t, NewDate(2026, time.June, 1), f.sessions.lastFrom,
...
func TestWTMMonthSummary_PastMonthComputation(t *testing.T) {
	...
	f.sessions.sessions = []*WorkSession{
		wtmSession(NewDate(2026, time.June, 1), 8, 480, 30),
		wtmSession(NewDate(2026, time.June, 8), 8, 240, 0),
	}
	f.absences.absences = []*StaffAbsence{
		// Full sick Monday: credits 480, counts 1 sick day.
		{Model: Model{ID: 1}, StaffID: wtmStaffID, AbsenceType: AbsenceTypeSick, Status: AbsenceStatusReported,
			DateStart: NewDate(2026, time.June, 15), DateEnd: NewDate(2026, time.June, 15)},
		// Overlapping second absence on the same day: must NOT double-credit.
		{Model: Model{ID: 2}, StaffID: wtmStaffID, AbsenceType: AbsenceTypeSick, Status: AbsenceStatusApproved,
			DateStart: NewDate(2026, time.June, 15), DateEnd: NewDate(2026, time.June, 16)},
		// Half-day vacation Monday: credits 240, counts 0.5 vacation days.
		{Model: Model{ID: 3}, StaffID: wtmStaffID, AbsenceType: AbsenceTypeVacation, Status: AbsenceStatusApproved,
			HalfDay: true, DateStart: NewDate(2026, time.June, 22), DateEnd: NewDate(2026, time.June, 22)},
		// Vacation on a Tuesday (target 0): no credit, no day count.
		{Model: Model{ID: 4}, StaffID: wtmStaffID, AbsenceType: AbsenceTypeVacation, Status: AbsenceStatusApproved,
			DateStart: NewDate(2026, time.June, 2), DateEnd: NewDate(2026, time.June, 2)},
		// Requested (not approved) vacation: never counts.
		{Model: Model{ID: 5}, StaffID: wtmStaffID, AbsenceType: AbsenceTypeVacation, Status: AbsenceStatusRequested,
			DateStart: NewDate(2026, time.June, 29), DateEnd: NewDate(2026, time.June, 29)},
	}
	f.shifts.shifts = []*TimeTrackingShift{
		wtmShift(NewDate(2026, time.June, 1), 180, false),
		wtmShift(NewDate(2026, time.June, 8), 240, true), // cancelled → not counted
	}
...
func TestWTMMonthSummary_LiveCarry(t *testing.T) {
	...
	f.sessions.sessions = []*WorkSession{
		wtmSession(NewDate(2026, time.May, 4), 8, 480, 0),
	}
	...
	f.sessions.sessions = append(f.sessions.sessions,
		wtmSession(NewDate(2026, time.May, 11), 8, 480, 0))
	summary, err = f.svc.GetMonthSummary(context.Background(), wtmStaffID, 2026, 6)
	require.NoError(t, err)
...
func TestWTMMonthSummary_CurrentMonthProRated(t *testing.T) {
	...
	f.sessions.sessions = []*WorkSession{
		wtmSession(NewDate(2026, time.July, 6), 8, 480, 0),
	}
	f.absences.absences = []*StaffAbsence{
		// Future vacation (Jul 20) inside the current month: no credit yet.
		{Model: Model{ID: 1}, StaffID: wtmStaffID, AbsenceType: AbsenceTypeVacation, Status: AbsenceStatusApproved,
			DateStart: NewDate(2026, time.July, 20), DateEnd: NewDate(2026, time.July, 20)},
	}
...
func TestWTMMonthSummary_ModelFallback(t *testing.T) {
	...
	f.models.model = &WorkTimeTemplate{
		RotationLength:     1,
		RotationAnchorDate: NewDate(2020, time.January, 1),
...
func TestWTMMonthSummary_MidMonthAccountStartClampsAnchorMonth(t *testing.T) {
	...
	f.sessions.sessions = []*WorkSession{
		...
		wtmSession(NewDate(2026, time.June, 1), 8, 480, 0),
		wtmSession(NewDate(2026, time.June, 15), 8, 480, 0),
	}
	f.absences.absences = []*StaffAbsence{
		// Sick Monday before the account start — must not credit.
		{Model: Model{ID: 1}, StaffID: wtmStaffID, AbsenceType: AbsenceTypeSick, Status: AbsenceStatusApproved,
			DateStart: NewDate(2026, time.June, 8), DateEnd: NewDate(2026, time.June, 8)},
	}
...
func TestWTMMonthSummary_FutureSessionExcludedFromActual(t *testing.T) {
	...
	f.sessions.sessions = []*WorkSession{
		wtmSession(NewDate(2026, time.July, 13), 8, 480, 0),
		// Dated after today — must be ignored entirely.
		wtmSession(NewDate(2026, time.July, 20), 8, 480, 0),
	}
...
func TestWTMMonthSummary_AssignsOvernightSessionToBothDays(t *testing.T) {
	t.Parallel()
	f := newWTMFixture()
	start := NewDate(2026, time.June, 30).BerlinMidnight().Add(22 * time.Hour)
	end := start.Add(4 * time.Hour)
	f.sessions.sessions = []*WorkSession{{
		StaffID:      wtmStaffID,
		Date:         NewDate(2026, time.June, 30),
		Status:       WorkSessionStatusPresent,
		CheckInTime:  start,
		CheckOutTime: &end,
	}}
...
func TestBalanceSessionEnd_AllowsOpenNightBlockButCapsStaleBlock(t *testing.T) {
	t.Parallel()
	yesterday := NewDate(2026, time.July, 21)
	now := yesterday.AddDays(1).BerlinMidnight().Add(6 * time.Hour)
	night := &WorkSession{Date: yesterday, CheckInTime: yesterday.BerlinMidnight().Add(22 * time.Hour)}
	assert.Equal(t, now, balanceSessionEnd(night, now, DateFromTime(now)))

	stale := &WorkSession{Date: yesterday, CheckInTime: yesterday.BerlinMidnight().Add(8 * time.Hour)}
	assert.Equal(t, stale.CheckInTime.Add(maxOpenWorkSessionDuration), balanceSessionEnd(stale, now, DateFromTime(now)))

	lateCheckout := now
	stale.CheckOutTime = &lateCheckout
	assert.Equal(t, lateCheckout, balanceSessionEnd(stale, now, DateFromTime(now)), "a completed block must retain its recorded check-out")
}

func TestBalanceSessionEnd_PreservesCompletedLongNightBlock(t *testing.T) {
	t.Parallel()
	day := NewDate(2026, time.July, 21)
	...
	assert.Equal(t, checkOut, balanceSessionEnd(session, checkOut.Add(time.Hour), DateFromTime(checkOut.Add(time.Hour))))
}
...
func TestWTMMonthSummary_HistoricalRowAnchorWinsOverStaffAnchor(t *testing.T) {
	...
	staffAnchor := NewDate(2026, time.June, 8)
	versionAnchor := NewDate(2026, time.June, 1)
	svc := NewWorkTimeMonthService(f.sessions, f.breaks, f.absences,
...
func TestWTMDailyTargets_UsesDateValidScheduleVersion(t *testing.T) {
	...
	closedAt := NewDate(2026, time.July, 1)
	closedAtWorkforce := closedAt
	f.schedules.entries = []*WorkScheduleRow{
		{
			StaffID: wtmStaffID, DayOfWeek: DayMonday, TargetMinutes: 480, RotationLength: 1,
			ValidFrom: NewDate(2020, time.January, 1), ValidUntil: &closedAtWorkforce,
		},
		{
	...
	targets, err := f.svc.GetDailyTargets(context.Background(), wtmStaffID,
		NewDate(2026, time.June, 29), NewDate(2026, time.July, 6))
	require.NoError(t, err)
	...
	assert.Equal(t, 480, byDate[NewDate(2026, time.June, 29)], "Monday under the old version")
	assert.Equal(t, 240, byDate[NewDate(2026, time.July, 6)], "Monday under the new version")
	assert.Equal(t, 0, byDate[NewDate(2026, time.June, 30)], "Tuesday is a planned day off")
}
...
func TestWTMDailyTargets_ZeroesDaysBeforeMidMonthAccountStart(t *testing.T) {
	...
	targets, err := f.svc.GetDailyProjection(context.Background(), wtmStaffID,
		NewDate(2026, time.June, 29), NewDate(2026, time.July, 13))
	require.NoError(t, err)
	...
	assert.Equal(t, 0, byDate[NewDate(2026, time.July, 6)],
		"Monday in the anchor month but before the start day carries no Soll")
	assert.Equal(t, 480, byDate[NewDate(2026, time.July, 13)],
		"Monday after the start day keeps its Soll")
	assert.Equal(t, 480, byDate[NewDate(2026, time.June, 29)],
...
func TestWTMRangeAggregate_ZeroesBalanceBeforeMidWeekAccountStart(t *testing.T) {
	...
	aggregate, err := f.svc.GetRangeAggregate(
		context.Background(),
		wtmStaffID,
		NewDate(2026, time.July, 6),
		NewDate(2026, time.July, 12),
	)
	require.NoError(t, err)
...
func TestWTMRangeAggregate_ExcludesActualBeforeMidWeekAccountStart(t *testing.T) {
	...
	f.sessions.sessions = []*WorkSession{
		wtmSession(NewDate(2026, time.July, 6), 8, 480, 0),
		wtmSession(NewDate(2026, time.July, 8), 8, 120, 0),
	}
...
func TestWTMRangeAggregate_ExcludesPreviousMonthBeforeAccountStart(t *testing.T) {
	...
	f.settings.accountStart = "2026-07-01"
	f.sessions.sessions = []*WorkSession{
		wtmSession(NewDate(2026, time.June, 29), 8, 60, 0),
		wtmSession(NewDate(2026, time.July, 1), 8, 120, 0),
	}

	aggregate, err := f.svc.GetRangeAggregate(
		context.Background(),
		wtmStaffID,
		NewDate(2026, time.June, 29),
		NewDate(2026, time.July, 5),
	)
	require.NoError(t, err)
...
func TestWTMRangeAggregate_UnsetAccountStartUsesPeriodEndYear(t *testing.T) {
	...
	f.sessions.sessions = []*WorkSession{
		wtmSession(NewDate(2025, time.December, 29), 8, 120, 0),
	}

	aggregate, err := f.svc.GetRangeAggregate(
		context.Background(),
		wtmStaffID,
		NewDate(2025, time.December, 29),
		NewDate(2026, time.January, 4),
	)
	require.NoError(t, err)
...
func TestPrefetchedMonthService_FiltersSessionsToRequestedRange(t *testing.T) {
	...
	accountStart := NewDate(2026, time.July, 8)
	prefetch := &monthPrefetch{
		staff: map[int64]*StaffScheduleAssignment{
			wtmStaffID: {},
		},
...
func TestWTMDailyTargets_UnsetAccountStartZeroesNothing(t *testing.T) {
	...
	targets, err := f.svc.GetDailyProjection(context.Background(), wtmStaffID,
		NewDate(2026, time.January, 1), NewDate(2026, time.January, 31))
	require.NoError(t, err)
...
func TestWTMDailyTargets_RejectsInvalidRange(t *testing.T) {
	...
	_, err := f.svc.GetDailyProjection(ctx, wtmStaffID,
		NewDate(2026, time.July, 10), NewDate(2026, time.July, 1))
	assert.Error(t, err, "to before from")

	_, err = f.svc.GetDailyProjection(ctx, wtmStaffID,
		NewDate(2020, time.January, 1), NewDate(2026, time.July, 1))
...
func TestWTMDailyTargets_InvalidRangeIsTyped(t *testing.T) {
	...
	cases := map[string]struct{ from, to Date }{
		"missing from": {Date(""), NewDate(2026, time.July, 1)},
		"missing to":   {NewDate(2026, time.July, 1), Date("")},
		"inverted":     {NewDate(2026, time.July, 10), NewDate(2026, time.July, 1)},
		"over the cap": {NewDate(2020, time.January, 1), NewDate(2026, time.July, 1)},
	}
...
func TestWTMMonthSummary_ActiveBreakDeductedFromLiveActual(t *testing.T) {
	...
	f := newWTMFixture()
	now := time.Date(2026, time.July, 15, 12, 0, 0, 0, Berlin)
	...
	// span.
	today := DateFromTime(now)
	f.svc.todayFunc = func() Date { return today }
	...
	f.sessions.sessions = []*WorkSession{session}
	f.breaks.breaks[session.ID] = []*WorkSessionBreak{{
		Model:     Model{ID: 5},
...
func TestWTMMonthSummary_EndedBreakNotDeductedTwice(t *testing.T) {
	...
	f.settings.accountStart = "2026-07-01"
	f.sessions.sessions = []*WorkSession{
		wtmSession(NewDate(2026, time.July, 13), 8, 480, 30),
	}
...
func TestWTMMonthSummary_TodayFutureCheckOutClampedAtNow(t *testing.T) {
	...
	f := newWTMFixture()
	now := time.Date(2026, time.July, 15, 12, 0, 0, 0, Berlin)
	todayDate := DateFromTime(now)
	f.svc.todayFunc = func() Date { return todayDate }
	f.svc.nowFunc = func() time.Time { return now }
...
func TestWTMMonthSummary_PastFutureCheckOutClampedAtDayEnd(t *testing.T) {
	...
	sessionDay := NewDate(2026, time.July, 10) // well before today (07-15)
	checkIn := time.Date(2026, time.July, 10, 8, 0, 0, 0, Berlin)
	checkOut := time.Date(2030, time.January, 1, 0, 0, 0, 0, Berlin) // absurd future
	f.sessions.sessions = []*WorkSession{{
		Model:        Model{ID: 88},
		StaffID:      wtmStaffID,
		Date:         sessionDay,
...
func TestWTMMonthSummary_PastUnclosedSessionClampedAtDayEnd(t *testing.T) {
	...
	sessionDay := NewDate(2026, time.July, 10)
	checkIn := time.Date(2026, time.July, 10, 8, 0, 0, 0, Berlin)
	f.sessions.sessions = []*WorkSession{{
		Model:       Model{ID: 88},
		StaffID:     wtmStaffID,
