...
// per-category checks.
func (rs *StaffAdminResource) documentActor(r *http.Request) (workforce.StaffDocumentActor, error) {
	claims := rs.identity(r.Context())
	if claims.AccountID == 0 {
		return workforce.StaffDocumentActor{}, errors.New("invalid token: no account id")
	}
	return workforce.StaffDocumentActor{
...
func newStaffDocumentResponse(info workforce.StaffDocumentInfo) StaffDocumentResponse {
	doc := info.Document
	return StaffDocumentResponse{
		...
		RetainUntil: optionalDate(info.RetainUntil),
		ReviewDue:   optionalDate(info.ReviewDue),
	}
}
...
func renderDocumentError(w http.ResponseWriter, r *http.Request, err error) {
	switch {
	case errors.Is(err, workforce.ErrStaffDocumentForbidden):
		common.RenderError(w, r, common.ErrorForbidden(err))
	case errors.Is(err, workforce.ErrStaffDocumentInvalid):
...
func (rs *StaffAdminResource) uploadStaffDocument(w http.ResponseWriter, r *http.Request) {
	...
	uploadCtx, cancelUpload := context.WithTimeout(r.Context(), workforce.StaffDocumentUploadDeadline)
	defer cancelUpload()
	...
	info, err := rs.StaffDocumentService.CreateStaffDocument(uploadCtx, workforce.CreateStaffDocumentInput{
		StaffID:         id,
