...
func parseTimeExportRequest(r *http.Request) (workforce.TimeExportRequest, error) {
	query := r.URL.Query()
	req := workforce.TimeExportRequest{
...
func (rs *StaffAdminResource) exportTimeTracking(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		rs.logger.Error("failed to export time tracking",
			"error", err.Error(),
		)
...
func (rs *StaffAdminResource) datevExportReport(w http.ResponseWriter, r *http.Request) {
	req, err := parseTimeExportRequest(r)
	if err != nil {
		common.RenderError(w, r, common.ErrorInvalidRequest(err))
		return
