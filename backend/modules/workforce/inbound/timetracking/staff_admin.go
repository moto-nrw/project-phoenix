...
type StaffAdminResource struct {
	...
	BalanceAdjustService workforce.BalanceAdjustments
	MonthCloseService    workforce.MonthClosing
	StaffOverviewService workforce.StaffOverview
	AuditLogService      workforce.TimeTrackingAuditLog
	TimeExportService    workforce.StaffTimeExport
	// ExportTransfer is nil when the school has no transfer capability wired;
...
// resource. Schedules is the Workforce query the schedule views read the
// assigned template and the running schedule version from.
...
func NewStaffAdminResource(deps StaffAdminDependencies) *StaffAdminResource {
	...
	return &StaffAdminResource{
		...
		BalanceAdjustService: deps.BalanceAdjustments,
		MonthCloseService:    deps.MonthClosing,
		StaffOverviewService: deps.Overview,
		AuditLogService:      deps.AuditLog,
