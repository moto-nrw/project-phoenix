...
func (rs *StaffAdminResource) getStaffHistory(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		rs.logger.Error("failed to get staff history",
			"staff_id", staffID,
			"error", err.Error(),
		)
...
func (rs *StaffAdminResource) exportStaffSessions(w http.ResponseWriter, r *http.Request) {
	staffID, err := common.ParseID(r)
	if err != nil {
		common.RenderError(w, r, common.ErrorInvalidRequest(err))
		return
	...
	if err != nil {
		rs.logger.Error("failed to export staff sessions",
			"staff_id", staffID,
			"error", err.Error(),
		)
		common.RenderError(w, r, common.ErrorInternalServer(err))
	...
	if _, err := w.Write(file.Data); err != nil {
		rs.logger.Error("failed to write export response", "error", err.Error())
	}
}

// listPendingAbsenceRequests handles GET /api/staff/absences/pending
func (rs *StaffAdminResource) listPendingAbsenceRequests(w http.ResponseWriter, r *http.Request) {
...
func (rs *StaffAdminResource) listAbsenceRequests(w http.ResponseWriter, r *http.Request) {
	query := r.URL.Query()
	req := workforce.AbsenceRequestListQuery{
...
type absenceRequestResponse struct {
	*workforce.StaffAbsenceRequestItem
	ID         string  `json:"id"`
...
func (rs *StaffAdminResource) approveAbsence(w http.ResponseWriter, r *http.Request) {
	...
	var req workforce.VacationDecisionRequest
...
func (rs *StaffAdminResource) denyAbsence(w http.ResponseWriter, r *http.Request) {
	absenceID, err := parseInt64Param(r, "absenceId")
	if err != nil {
		common.RenderError(w, r, common.ErrorInvalidRequest(err))
		return
...
func (rs *StaffAdminResource) questionAbsence(w http.ResponseWriter, r *http.Request) {
	absenceID, err := parseInt64Param(r, "absenceId")
	if err != nil {
		common.RenderError(w, r, common.ErrorInvalidRequest(err))
		return
...
func (rs *StaffAdminResource) getStaffVacationQuota(w http.ResponseWriter, r *http.Request) {
	staffID, err := common.ParseID(r)
	if err != nil {
		common.RenderError(w, r, common.ErrorInvalidRequest(err))
		return
...
func (rs *StaffAdminResource) setStaffVacationQuota(w http.ResponseWriter, r *http.Request) {
	staffID, err := common.ParseID(r)
	if err != nil {
		common.RenderError(w, r, common.ErrorInvalidRequest(err))
		return
	...
	summary, err := rs.StaffAbsenceService.VacationQuotaSummary(r.Context(), staffID, body.Year)
	if err != nil {
		common.RenderError(w, r, common.ErrorInternalServer(err))
		return
	}
...
func (rs *StaffAdminResource) adminUpdateStaffSession(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		rs.logger.Error("admin session update failed",
	...
	common.Respond(w, r, http.StatusOK, workforce.WorkSessionWire{WorkSession: session}, "Session updated")
}

// adminCreateStaffSession handles POST /api/staff/{id}/time-tracking/sessions
// the "nachtragen" flow when an MA forgot to stamp.
func (rs *StaffAdminResource) adminCreateStaffSession(w http.ResponseWriter, r *http.Request) {
	targetStaffID, err := common.ParseID(r)
	if err != nil {
		common.RenderError(w, r, common.ErrorInvalidRequest(err))
		return
	...
	var req workforce.AdminCreateSessionRequest
	if err := render.DecodeJSON(r.Body, &req); err != nil {
		common.RenderError(w, r, common.ErrorInvalidRequest(err))
		return
	}
	...
	if err != nil {
		rs.logger.Error("admin session create failed",
			"target_staff_id", targetStaffID,
			"editor_staff_id", editorStaffID,
			"error", err.Error(),
		)
	...
	common.Respond(w, r, http.StatusCreated, workforce.WorkSessionWire{WorkSession: session}, "Session created")
}
...
func (rs *StaffAdminResource) getStaffSessionEdits(w http.ResponseWriter, r *http.Request) {
	staffID, err := common.ParseID(r)
	if err != nil {
		common.RenderError(w, r, common.ErrorInvalidRequest(err))
		return
	...
	edits, err := rs.WorkSessionService.SessionEditsForStaff(r.Context(), staffID, sessionID)
	if err != nil {
		rs.logger.Error("failed to get staff session edits",
			"staff_id", staffID,
			"session_id", sessionID,
			"error", err.Error(),
		)
...
// staff in the tenant.
func (rs *StaffAdminResource) getStaffAbsences(w http.ResponseWriter, r *http.Request) {
	staffID, err := common.ParseID(r)
	if err != nil {
		common.RenderError(w, r, common.ErrorInvalidRequest(err))
		return
	...
	absences, err := rs.StaffAbsenceService.AbsencesForRange(r.Context(), staffID, from.String(), to.String())
	if err != nil {
		rs.logger.Error("failed to get staff absences",
			"staff_id", staffID,
			"error", err.Error(),
		)
		common.RenderError(w, r, common.ErrorInternalServer(err))
...
func (rs *StaffAdminResource) adminCreateStaffAbsence(w http.ResponseWriter, r *http.Request) {
	staffID, err := common.ParseID(r)
	if err != nil {
		common.RenderError(w, r, common.ErrorInvalidRequest(err))
		return
...
func (rs *StaffAdminResource) adminDeleteStaffAbsence(w http.ResponseWriter, r *http.Request) {
	staffID, err := common.ParseID(r)
	if err != nil {
		common.RenderError(w, r, common.ErrorInvalidRequest(err))
		return
...
func (rs *StaffAdminResource) getCompTimeBalancePreview(w http.ResponseWriter, r *http.Request) {
	staffID, err := common.ParseID(r)
	if err != nil {
		common.RenderError(w, r, common.ErrorInvalidRequest(err))
		return
	...
	preview, err := rs.StaffAbsenceService.PreviewCompTimeBalance(r.Context(), staffID, dateStart.String(), dateEnd.String(), halfDay)
	if err != nil {
		rs.logger.Error("failed to compute comp_time preview",
			"staff_id", staffID,
			"error", err.Error(),
		)
		common.RenderError(w, r, common.RenderWithRules(err, adminAbsenceErrorRules, common.ErrorInternalServer))
