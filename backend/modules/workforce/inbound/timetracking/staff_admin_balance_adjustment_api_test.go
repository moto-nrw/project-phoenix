...
func TestBalanceAdjustmentAPI(t *testing.T) {
	...
	accrualCheckIn := time.Date(accrualDate.Year(), accrualDate.Month(), accrualDate.Day(), 8, 0, 0, 0, time.UTC)
	insertWorkSession(t, tc, subject.ID, accrualDate.String(), accrualCheckIn, accrualCheckIn.Add(4*time.Hour))
	...
	t.Run("open-day resets rejected", func(t *testing.T) {
		for _, date := range []string{today.String(), today.AddDays(7).String()} {
			rec := do(http.MethodPost, resetPath,
	...
	t.Run("delete removes the adjustment", func(t *testing.T) {
		...
		require.NoError(t, err)
		sessionDate := today
		checkOut := time.Now().In(berlin).Add(-time.Minute)
		...
		insertWorkSession(t, tc, subject.ID, sessionDate.String(), checkIn, checkOut)
