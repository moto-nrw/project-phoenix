...
func (rs *StaffAdminResource) getStammdaten(w http.ResponseWriter, r *http.Request) {
	...
	data, err := rs.PersonService.StaffStammdaten(r.Context(), id)
	if err != nil {
...
func newStammdatenResponse(data *workforce.StaffStammdaten) *StammdatenResponse {
	resp := &StammdatenResponse{
		StaffID:         data.Staff.ID,
		Qualifikationen: make([]StammdatenQualificationPayload, 0, len(data.Qualifications)),
	}
	resp.Person.FirstName = data.Staff.FirstName
	resp.Person.LastName = data.Staff.LastName
	resp.Person.Birthday = optionalDate(data.Staff.Birthday)
	...
	if md := data.MasterData; md != nil {
		...
		resp.Arbeitsvertrag.EntryDate = optionalDate(md.EntryDate)
		resp.Arbeitsvertrag.ContractEndDate = optionalDate(md.ContractEndDate)
		resp.Arbeitsvertrag.ProbationEndDate = optionalDate(md.ProbationEndDate)
	...
	for _, q := range data.Qualifications {
		...
		resp.Qualifikationen = append(resp.Qualifikationen, StammdatenQualificationPayload{
			ID:         &qualID,
			Name:       q.Name,
			AcquiredOn: optionalDate(q.AcquiredOn),
			ExpiresOn:  optionalDate(q.ExpiresOn),
		})
	}
	return resp
}
...
func (rs *StaffAdminResource) updateStammdatenPerson(w http.ResponseWriter, r *http.Request) {
	...
	rs.runStammdatenUpdate(w, r, func(staffID, changedBy int64) error {
		return rs.PersonService.UpdateStaffStammdatenPerson(r.Context(), staffID, workforce.StammdatenPersonInput{
			FirstName: req.FirstName,
			LastName:  req.LastName,
			Birthday:  optionalDateString(req.Birthday),
			Gender:    req.Gender,
		}, changedBy, req.Note)
	})
}
...
func (rs *StaffAdminResource) updateStammdatenKontakt(w http.ResponseWriter, r *http.Request) {
	...
	rs.runStammdatenUpdate(w, r, func(staffID, changedBy int64) error {
		return rs.PersonService.UpdateStaffStammdatenKontakt(r.Context(), staffID, workforce.StammdatenKontaktInput{
...
func (rs *StaffAdminResource) updateStammdatenArbeitsvertrag(w http.ResponseWriter, r *http.Request) {
	...
	rs.runStammdatenUpdate(w, r, func(staffID, changedBy int64) error {
		return rs.PersonService.UpdateStaffStammdatenArbeitsvertrag(r.Context(), staffID, workforce.StammdatenArbeitsvertragInput{
			EntryDate:        optionalDateString(req.EntryDate),
			ContractEndDate:  optionalDateString(req.ContractEndDate),
			ProbationEndDate: optionalDateString(req.ProbationEndDate),
...
func (rs *StaffAdminResource) updateStammdatenQualifikationen(w http.ResponseWriter, r *http.Request) {
	...
	rs.runStammdatenUpdate(w, r, func(staffID, changedBy int64) error {
		inputs := make([]workforce.StammdatenQualificationInput, 0, len(req.Qualifikationen))
		for _, q := range req.Qualifikationen {
			inputs = append(inputs, workforce.StammdatenQualificationInput{
				Name:       q.Name,
				AcquiredOn: optionalDateString(q.AcquiredOn),
				ExpiresOn:  optionalDateString(q.ExpiresOn),
			})
		}
		return rs.PersonService.ReplaceStaffQualificationList(r.Context(), staffID, inputs, changedBy, req.Note)
	})
}
...
// audit shape.
func (rs *StaffAdminResource) financialActor(r *http.Request) (int64, string, error) {
	claims := rs.identity(r.Context())
	if claims.AccountID == 0 {
		return 0, "", errors.New("invalid token: no account id")
	}
	return claims.AccountID, strings.Join(claims.Roles, ","), nil
}
...
func (rs *StaffAdminResource) getStammdatenFinancial(w http.ResponseWriter, r *http.Request) {
	...
	accountID, role, err := rs.financialActor(r)
	if err != nil {
		common.RenderError(w, r, common.ErrorUnauthorized(err))
		return
	}
	masked, err := rs.PersonService.StaffFinancialMasked(r.Context(), id, accountID, role)
	if err != nil {
		if common.IsNotFound(err) {
			common.RenderError(w, r, common.ErrorNotFound(err))
			return
...
func (rs *StaffAdminResource) updateStammdatenFinancial(w http.ResponseWriter, r *http.Request) {
	...
	accountID, _, err := rs.financialActor(r)
	if err != nil {
		common.RenderError(w, r, common.ErrorUnauthorized(err))
		return
	}
	if err := rs.PersonService.UpdateStaffFinancial(r.Context(), staffID, workforce.StammdatenFinancialInput{
		IBAN:                 req.IBAN,
		TaxID:                req.TaxID,
