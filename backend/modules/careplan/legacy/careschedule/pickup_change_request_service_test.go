...
func newPickupChangeFixture(t *testing.T) *careFixture {
	return newCareFixture(t, careschedule.WithCareRequestToday(func() timezone.Date { return timezone.NewDate(2026, 8, 24) }))
}
...
func TestPickupChangeRequestAppliesOnlyAfterStaffApproval(t *testing.T) {
	...
	decided, err := f.svc.Decide(ctx, careschedule.CareRequestDecideInput{
		RequestID:  req.ID,
		Approve:    true,
...
func TestPickupApprovalRejectsStaleAffectedBlockList(t *testing.T) {
	...
	require.ErrorIs(t, err, careschedule.ErrPickupChangeImpactChanged)
...
func TestPickupApprovalRequiresImpactTokenForHTTPDecision(t *testing.T) {
	...
	require.ErrorIs(t, err, careschedule.ErrPickupChangeImpactRequired)
	kept, findErr := f.repos.CareScheduleChangeRequest.FindByID(ctx, req.ID)
	require.NoError(t, findErr)
	assert.Equal(t, scheduleModels.CareRequestStatusPending, kept.Status)
}
...
func TestPickupChangeApprovalRollsBackWhenAutoExcusalFails(t *testing.T) {
	...
	err = testpkg.WithTenantTx(t, ctx, f.db, f.chain.TenantID, func(txCtx context.Context, tx bun.Tx) error {
		...
		_, decideErr := f.svc.Decide(txCtx, careschedule.CareRequestDecideInput{
			RequestID:  req.ID,
			Approve:    true,
			ReviewedBy: f.staffAccount,
		})
...
func TestPickupChangeApprovalYieldsToStaffException(t *testing.T) {
	...
	require.ErrorIs(t, err, careschedule.ErrPickupChangeConflict)
...
func TestPickupChangeApprovalRejectsCompletedSameDayPickup(t *testing.T) {
	...
	require.ErrorIs(t, err, careschedule.ErrPickupChangeAlreadyCompleted)
}
...
func TestPickupChangeApprovalRejectsExpiredRequest(t *testing.T) {
	...
	require.ErrorIs(t, err, careschedule.ErrPickupChangeExpired)
