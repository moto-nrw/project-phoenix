...
func arrivalServiceWithClassTimes(t *testing.T, repos *repositories.Factory) careschedule.ArrivalScheduleService {
	t.Helper()
	db := testpkg.SetupTestDB(t)
	return careschedule.NewArrivalScheduleServiceWithBaselines(
...
func TestBulkUpsertBySchoolClassWritesTheClassTimetable(t *testing.T) {
	...
	result, err := svc.BulkUpsertArrivalSchedules(ctx,
		careschedule.ArrivalScheduleBulkFilter{SchoolClass: "7c"},
		[]careschedule.ArrivalScheduleInput{{Weekday: scheduleModel.WeekdayMonday, ArrivalTime: "11:45"}},
		staff.ID,
	)
...
) (*careschedule.ArrivalBaselineProjection, error) {
	projection := &careschedule.ArrivalBaselineProjection{
		WeeklyByStudentDate:   make(careschedule.ArrivalPlansByStudent, len(studentIDs)),
		DerivedByStudentDate:  make(careschedule.ArrivalPlansByStudent, len(studentIDs)),
		BookingsAuthoritative: true,
	}
	for _, studentID := range studentIDs {
		weekly := careschedule.ArrivalPlanByDate{}
		for date := from; !date.After(to); date = date.AddDays(1) {
			week := careschedule.ArrivalWeek{}
		...
		projection.DerivedByStudentDate[studentID] = careschedule.ArrivalPlanByDate{}
	}
...
func TestArrivalDataForDateRangeKeepsMidweekBookingStart(t *testing.T) {
	...
	repos := repositories.NewFactory(db, repositories.NewUnobservedTimetableDependencies(db))
	ctx := testpkg.Ctx(t)
	svc := careschedule.NewArrivalScheduleServiceWithBaselines(
		repos.StudentArrivalSchedule,
		repos.StudentArrivalException,
		repos.StudentArrivalNote,
		repos.Student,
...
func (bookingMondayBaseline) Project(
	...
	for _, studentID := range studentIDs {
		projection.WeeklyByStudentDate[studentID] = careschedule.ArrivalPlanByDate{
			from: careschedule.ArrivalWeek{
