...
func TestListHistory_DecidedRequestsWithReviewerAndSummary(t *testing.T) {
	...
	_, err := f.svc.Decide(staffCtx, careschedule.CareRequestDecideInput{
	...
	byID := map[int64]*careschedule.CareRequestHistoryItem{}
...
func TestListHistory_IncludesPickupChangeWithPayloadSummary(t *testing.T) {
	...
	require.NoError(t, err)
	var got *careschedule.CareRequestHistoryItem
	for _, item := range items {
	...
	require.Equal(t, []careschedule.RequestDiffEntry{{
		Label: date.Format("02.01.2006") + " · Abholzeit", New: "15:30", CareKind: careschedule.DiffCareKindPickup,
	}}, got.Requested)
}
