...
// (care_request_service.go): CreateRequest, Decide
...
type careFixture struct {
	db           *bun.DB
	svc          careschedule.CareScheduleRequestService
	sf           *services.Factory
	repos        *repositories.Factory
	chain        testpkg.ParentChain
	staffAccount int64
	staffID      int64
	autoExcusal  *careschedule.PickupAutoExcusalSyncer
}
...
func newCareScheduleRequestService(
	...
	arrival careschedule.ArrivalScheduleService,
	pickup careschedule.PickupScheduleService,
	factory *services.Factory,
	emitter *parentmessaging.Emitter,
	broadcaster realtime.Broadcaster,
	logger *slog.Logger,
) careschedule.CareScheduleRequestService {
	return careschedule.NewCareScheduleRequestServiceWithPickupChangesAndPolicy(
...
func newPickupChangePresence(t *testing.T, db *bun.DB) interface {
	careschedule.PickupChangePresence
...
func newCareFixture(t *testing.T, options ...careschedule.CareRequestOption) *careFixture {
	t.Helper()
	return newCareFixtureWithEmitter(t, nil, options...)
}
...
func newCareFixtureWithEmitter(t *testing.T, emitter *parentmessaging.Emitter, options ...careschedule.CareRequestOption) *careFixture {
	t.Helper()
	...
	autoExcusal := careschedule.NewPickupAutoExcusalSyncer(
	...
	svc := careschedule.NewCareScheduleRequestServiceWithPickupChangesAndPolicy(
...
func (f *careFixture) seedGuardianPickupAutoExcusal(t *testing.T, date timezone.Date, pickupTime time.Time) {
	...
	require.NoError(t, testpkg.WithTenantTx(t, ctx, f.db, f.chain.TenantID, func(txCtx context.Context, _ bun.Tx) error {
		if err := careschedule.LockCareExceptionDay(txCtx, f.db, f.chain.StudentID, date); err != nil {
			return err
		}
...
func TestDecide_ForbiddenWithoutStaffRecord(t *testing.T) {
	...
	_, err = f.svc.Decide(denyCtx, careschedule.CareRequestDecideInput{RequestID: req.ID, Approve: true, ReviewedBy: f.staffAccount})
	assert.ErrorIs(t, err, careschedule.ErrCareRequestForbidden)
	_, err = f.svc.Decide(denyCtx, careschedule.CareRequestDecideInput{RequestID: req.ID, Approve: false, Reason: "nope", ReviewedBy: f.staffAccount})
	assert.ErrorIs(t, err, careschedule.ErrCareRequestForbidden)
	...
	item, err := f.svc.Decide(staffCtx, careschedule.CareRequestDecideInput{RequestID: req.ID, Approve: false, Reason: "closing out", ReviewedBy: f.staffAccount})
	require.NoError(t, err)
...
func TestDecide_ApproveAppliesModeArrivalPickup(t *testing.T) {
	...
	item, err := f.svc.Decide(ctx, careschedule.CareRequestDecideInput{
...
func TestDecide_ApproveMergesPreservingOtherDaysAndModes(t *testing.T) {
	...
	_, err = f.svc.Decide(ctx, careschedule.CareRequestDecideInput{RequestID: req.ID, Approve: true, ReviewedBy: f.staffAccount})
	require.NoError(t, err)
...
func TestDecide_ApproveInactiveCareDayRemovesWeeklyPlan(t *testing.T) {
	...
	err := testpkg.WithTenantTx(t, ctx, f.db, f.chain.TenantID, func(txCtx context.Context, _ bun.Tx) error {
		_, err := f.svc.Decide(txCtx, careschedule.CareRequestDecideInput{
			RequestID: req.ID, Approve: true, ReviewedBy: f.staffAccount,
		})
		return err
	})
...
func TestDecide_InactiveCareDayRollsBackWhenPickupDeleteFails(t *testing.T) {
	...
	err := testpkg.WithTenantTx(t, ctx, f.db, f.chain.TenantID, func(txCtx context.Context, _ bun.Tx) error {
		_, err := failingService.Decide(txCtx, careschedule.CareRequestDecideInput{
			RequestID: req.ID, Approve: true, ReviewedBy: f.staffAccount,
		})
		return err
	})
...
func TestDecide_InactiveCareDayRejectsOfferingManagedPickup(t *testing.T) {
	...
	err := testpkg.WithTenantTx(t, ctx, f.db, f.chain.TenantID, func(txCtx context.Context, _ bun.Tx) error {
		_, decideErr := service.Decide(txCtx, careschedule.CareRequestDecideInput{
			RequestID: req.ID, Approve: true, ReviewedBy: f.staffAccount,
		})
		return decideErr
	})
	require.ErrorIs(t, err, careschedule.ErrCareDayManagedByBooking)
...
func TestDecide_NonStaffConfirmerForbidden(t *testing.T) {
	...
	_, err := f.svc.Decide(f.staffCtx(f.chain.AccountID), careschedule.CareRequestDecideInput{
		RequestID: req.ID, Approve: true, ReviewedBy: f.chain.AccountID,
	})
	require.ErrorIs(t, err, careschedule.ErrCareRequestForbidden)
}
...
func TestDecide_ApproveRefusedWhenGuardianAccessRevoked(t *testing.T) {
	...
	_, err = svc.Decide(ctx, careschedule.CareRequestDecideInput{RequestID: req.ID, Approve: true, ReviewedBy: f.staffAccount})
	require.ErrorIs(t, err, careschedule.ErrCareRequestGuardianAccessRevoked, "a revoked guardian's request must not be applied")
	...
	item, err := svc.Decide(ctx, careschedule.CareRequestDecideInput{RequestID: req.ID, Approve: false, Reason: "Elternteil nicht mehr berechtigt", ReviewedBy: f.staffAccount})
...
func TestDecide_RejectClosesRequestPillEvenWhenMessagingDisabled(t *testing.T) {
	...
	item, err := svc.Decide(ctx, careschedule.CareRequestDecideInput{RequestID: req.ID, Approve: false, Reason: "telefonisch geklärt", ReviewedBy: f.staffAccount})
	require.NoError(t, err)
	assert.Equal(t, scheduleModels.CareRequestStatusRejected, item.Request.Status)
...
func TestCreateRequest_RequestCreatedPillDoesNotAdvanceThreadPreview(t *testing.T) {
	...
	_, err = svc.Decide(ctx, careschedule.CareRequestDecideInput{RequestID: req.ID, Approve: false, Reason: "telefonisch geklärt", ReviewedBy: f.staffAccount})
	require.NoError(t, err)
...
func TestDecide_NoReconcilePillWhenRequestFiledWhileDisabled(t *testing.T) {
	...
	ctx := f.staffCtx(f.staffAccount)
	item, err := svc.Decide(ctx, careschedule.CareRequestDecideInput{RequestID: req.ID, Approve: false, Reason: "kein Kanal", ReviewedBy: f.staffAccount})
	require.NoError(t, err)
	assert.Equal(t, scheduleModels.CareRequestStatusRejected, item.Request.Status)
...
func TestDecide_ApproveAllowedWhenMessagingDisabled(t *testing.T) {
	...
	item, err := svc.Decide(ctx, careschedule.CareRequestDecideInput{RequestID: req.ID, Approve: true, ReviewedBy: f.staffAccount})
	require.NoError(t, err)
	assert.Equal(t, scheduleModels.CareRequestStatusApproved, item.Request.Status)
...
func TestDecide_SecondDecideNotPending(t *testing.T) {
	...
	_, err = f.svc.Decide(ctx, careschedule.CareRequestDecideInput{RequestID: req.ID, Approve: false, Reason: "zu spät", ReviewedBy: f.staffAccount})
...
func TestDecide_RejectRequiresReasonAndBounds(t *testing.T) {
	...
	_, err := f.svc.Decide(ctx, careschedule.CareRequestDecideInput{RequestID: req.ID, Approve: false, Reason: "  \n\t ", ReviewedBy: f.staffAccount})
	require.ErrorIs(t, err, careschedule.ErrCareRequestRejectReasonRequired)

	_, err = f.svc.Decide(ctx, careschedule.CareRequestDecideInput{RequestID: req.ID, Approve: false, Reason: strings.Repeat("x", 2001), ReviewedBy: f.staffAccount})
	require.ErrorIs(t, err, careschedule.ErrCareRequestRejectReasonTooLong)

	item, err := f.svc.Decide(ctx, careschedule.CareRequestDecideInput{RequestID: req.ID, Approve: false, Reason: "Bitte mit Datum erneut", ReviewedBy: f.staffAccount})
	require.NoError(t, err)
	assert.Equal(t, scheduleModels.CareRequestStatusRejected, item.Request.Status)
...
func TestCreateRequest_RejectsMidnight(t *testing.T) {
	...
	require.ErrorIs(t, err, careschedule.ErrInvalidCareRequestPayload)
...
func TestCreateRequest_OnePendingPerStudent(t *testing.T) {
	...
	require.ErrorIs(t, err, careschedule.ErrCareRequestAlreadyPending)
}
...
func TestDecide_ApproveBroadcastsCacheInvalidation(t *testing.T) {
	...
	_, err := svc.Decide(f.staffCtx(f.staffAccount), careschedule.CareRequestDecideInput{
		RequestID: req.ID, Approve: true, ReviewedBy: f.staffAccount,
	})
	require.NoError(t, err)
...
func TestDecide_BogusIDNotFound(t *testing.T) {
	...
	_, err := f.svc.Decide(f.staffCtx(f.staffAccount), careschedule.CareRequestDecideInput{
...
func TestCareRequestLifecycle_WakesAllGuardians(t *testing.T) {
	...
	require.NoError(t, err)
	broadcaster.Reset()
	_, err = svc.Decide(f.staffCtx(f.staffAccount), careschedule.CareRequestDecideInput{RequestID: req2.ID, Approve: true, ReviewedBy: f.staffAccount})
	require.NoError(t, err)
