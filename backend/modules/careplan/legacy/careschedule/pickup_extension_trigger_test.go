...
type pickupExtensionHarness struct {
	db      *bun.DB
	ctx     context.Context
	svc     careschedule.PickupScheduleService
	student int64
	staff   int64
...
func setupPickupExtensionHarness(t *testing.T) *pickupExtensionHarness {
	...
	baselines := carescheduletest.NewPickupBaselineService(repos.StudentPickupSchedule, approvedOfferingProjection(t), repos.CareOffering)
	syncer := careschedule.NewPickupAutoExcusalSyncer(
		repos.StudentPickupException,
		baselines,
		repos.InstanceStudent,
		db,
		careschedule.WithPickupExtensions(timetabletest.New(t, db)),
	)
