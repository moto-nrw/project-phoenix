...
func newProjectionWriteService(
	...
	studentID int64,
	hhmm string,
) (careschedule.PickupScheduleService, *repositories.Factory) {
	t.Helper()
	repos := repositories.NewFactory(db, repositories.NewUnobservedTimetableDependencies(db))
	service := careschedule.NewPickupScheduleServiceWithBulk(
		...
		repos.Person,
		nil,
		carescheduletest.FixedPickupBaseline{StudentID: studentID, Weekday: scheduleModels.WeekdayMonday, HHMM: hhmm},
		db,
		nil,
	)
...
func TestUpsertBulkPickupSchedules_DoesNotMaterializeUnchangedProjection(t *testing.T) {
	...
	service := careschedule.NewPickupScheduleServiceWithBulk(
		...
		repos.Person,
		nil,
		carescheduletest.FixedPickupBaseline{StudentID: student.ID, Weekday: scheduleModels.WeekdayMonday, HHMM: "14:30"},
		db,
		nil,
	)
	ctx := testpkg.Ctx(t)
