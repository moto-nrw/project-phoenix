...
func cutoffAt(t *testing.T, hour, minute int) careschedule.SameDayCutoff {
	t.Helper()
	cutoff, err := careschedule.NewSameDayCutoff("11:00",
...
func createPickupChangeWithCutoff(
	t *testing.T, f *careFixture, date timezone.Date, cutoff careschedule.SameDayCutoff,
) (*scheduleModels.CareScheduleChangeRequest, error) {
	t.Helper()
	return f.svc.CreatePickupChange(f.staffCtx(f.staffAccount), careschedule.PickupChangeCreateInput{
...
func TestCreatePickupChangeHonoursSameDayCutoff(t *testing.T) {
	...
	t.Run("heute nach der Frist", func(t *testing.T) {
		...
		require.ErrorIs(t, err, careschedule.ErrPickupChangeCutoffPassed)
	...
	t.Run("ohne Frist spät am Tag", func(t *testing.T) {
		t.Parallel()
		f := newPickupChangeFixture(t)
		none, err := careschedule.NewSameDayCutoff("", time.Date(2026, 8, 24, 23, 0, 0, 0, timezone.Berlin))
		require.NoError(t, err)
...
func TestEditPickupChangeHonoursSameDayCutoff(t *testing.T) {
	...
	edit := func(t *testing.T, f *careFixture, req *scheduleModels.CareScheduleChangeRequest, date timezone.Date, cutoff careschedule.SameDayCutoff) error {
		t.Helper()
		_, err := f.svc.EditRequest(f.staffCtx(f.staffAccount), careschedule.CareRequestEditInput{
			RequestID:         req.ID,
			StudentID:         f.chain.StudentID,
			GuardianAccountID: f.chain.AccountID,
			Date:              date,
	...
	t.Run("heutige Anfrage ändern", func(t *testing.T) {
		...
		require.ErrorIs(t, edit(t, f, req, cutoffFixtureToday, cutoffAt(t, 11, 1)), careschedule.ErrPickupChangeCutoffPassed)
	...
	t.Run("heutige Anfrage auf morgen verschieben", func(t *testing.T) {
		...
		require.ErrorIs(t, edit(t, f, req, cutoffFixtureToday.AddDays(1), cutoffAt(t, 11, 1)), careschedule.ErrPickupChangeCutoffPassed)
	})
...
func TestStaffDecidesTodaysPickupChangeAfterCutoff(t *testing.T) {
	...
	decided, err := f.svc.Decide(f.staffCtx(f.staffAccount), careschedule.CareRequestDecideInput{
		RequestID:  req.ID,
		Approve:    true,
