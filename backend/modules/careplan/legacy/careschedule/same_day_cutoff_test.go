...
func sameDayCutoff(t *testing.T, clock string, now time.Time) careschedule.SameDayCutoff {
	t.Helper()
	cutoff, err := careschedule.NewSameDayCutoff(clock, now)
...
func TestSameDayCutoffEmptyNeverCloses(t *testing.T) {
	...
	assert.False(t, careschedule.SameDayCutoff{}.Closed(timezone.NewDate(2026, 8, 24)))
}
...
func TestSameDayCutoffAtUsesTimeAtValidation(t *testing.T) {
	...
	cutoff, err := careschedule.NewSameDayCutoffAt("11:00", func() time.Time { return now })
	require.NoError(t, err)
...
func TestNewSameDayCutoffRejectsMalformedClock(t *testing.T) {
	...
	for _, clock := range []string{"11", "25:00", "11:60", "elf"} {
		_, err := careschedule.NewSameDayCutoff(clock, berlinAt(2026, 8, 24, 10, 0, 0))
		require.Error(t, err, clock)
	}
	// A hand-built value that bypassed validation fails closed for today only.
	broken := careschedule.SameDayCutoff{Clock: "elf", Now: berlinAt(2026, 8, 24, 10, 0, 0)}
