...
var (
	errTestDateframeNotFound      = errors.New("dateframe not found")
	errTestTimeframeNotFound      = errors.New("timeframe not found")
	errTestRecurrenceRuleNotFound = errors.New("recurrence rule not found")
	errTestInvalidDateRange       = errors.New("invalid date range")
	errTestInvalidTimeRange       = errors.New("invalid time range")
	errTestInvalidDuration        = errors.New("invalid duration")
)
...
func TestScheduleError_ErrorsIs(t *testing.T) {
	...
	err := &careplan.ScheduleError{
		Op:  "FindTimeframe",
		Err: errTestTimeframeNotFound,
	}

	assert.True(t, errors.Is(err, errTestTimeframeNotFound))
	assert.False(t, errors.Is(err, errTestDateframeNotFound))
}
...
func TestScheduleError_ChainedWrapping(t *testing.T) {
	...
	wrapped1 := &careplan.ScheduleError{
		Op:  "ValidateDateRange",
		Err: baseErr,
	}
	wrapped2 := &careplan.ScheduleError{
		Op:  "CreateSchedule",
		Err: wrapped1,
	}
