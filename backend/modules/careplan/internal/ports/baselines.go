...
type BookingBaselineSource interface {
	...
	BookingRows(context.Context, []int64, timezone.Date, timezone.Date) ([]*careplan.ApprovedBooking, error)
...
type ArrivalBaselineSource interface {
	...
	ClassArrivalExceptions(context.Context, map[int64]string, timezone.Date, timezone.Date) (map[string]careplan.ClassArrivalExceptionsByDate, error)
}
