...
func TestListHistory_DecidedRequestsWithReviewerAndSummary(t *testing.T) {
	...
	_, err := f.svc.Decide(staffCtx, carerequests.DecideInput{
	...
	items, next, err := nativeCareReviews(t, f).ListHistory(staffCtx, careplan.RequestQueueFilter{Limit: 25})
	require.NoError(t, err)
	assert.Nil(t, next)

	byID := map[int64]*carerequests.HistoryItem{}
	...
	page1, next1, err := nativeCareReviews(t, f).ListHistory(staffCtx, careplan.RequestQueueFilter{Limit: 1})
	require.NoError(t, err)
	require.Len(t, page1, 1)
	require.NotNil(t, next1)
	page2, next2, err := nativeCareReviews(t, f).ListHistory(staffCtx, careplan.RequestQueueFilter{BeforeInstant: next1.UpdatedAt, BeforeID: next1.ID, Limit: 1})
	require.NoError(t, err)
...
func TestListHistory_IncludesPickupChangeWithPayloadSummary(t *testing.T) {
	...
	require.NoError(t, err)
	var got *carerequests.HistoryItem
	for _, item := range items {
	...
	require.Equal(t, []carerequests.DiffEntry{{
		Label: date.Format("02.01.2006") + " · Abholzeit", New: "15:30", CareKind: carerequests.KindPickup,
	}}, got.Requested)
}
