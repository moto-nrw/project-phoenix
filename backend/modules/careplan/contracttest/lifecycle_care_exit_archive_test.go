...
func TestCareExitArchive_RecordedAtUsesTheBerlinDay(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, carePlanRecords(db).UpsertCareExit(ctx, careplan.CareExit{
	...
	err = testpkg.WithTenantTx(t, context.Background(), db, testpkg.Tenant(t),
		func(txCtx context.Context, tx bun.Tx) error {
			...
			rows, total, err := svc.ListEnded(txCtx,
				careplan.EndedCareFilter{Search: class})
			if err != nil {
				return err
			}
...
// The binding confirmation freezes the person rows it read: an RFID assignment
// must not change behind it. The confirmation joins the caller's transaction,
// so its locks are still held when the probe runs.
func TestCareExitConfirmation_LocksPersonImpactRows(t *testing.T) {
	t.Parallel()
	...
	input := careplan.CareExitInput{
		StudentIDs: []int64{student.ID}, LastCareDay: timezone.TodayDate(), Reason: careplan.CareExitReasonMovedAway,
	}
	...
	err = testpkg.WithTenantTx(t, ctx, db, testpkg.Tenant(t), func(txCtx context.Context, _ bun.Tx) error {
		if _, err := svc.Confirm(txCtx, preview.Token, input, actorID); err != nil {
			return err
		}
		go func() {
