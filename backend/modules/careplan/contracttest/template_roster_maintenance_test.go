...
func TestDeriveTemplateRosterMaintenance(t *testing.T) {
	...
	active := func(id int64, name string) careplan.TemplateRosterFeedOffering {
		return careplan.TemplateRosterFeedOffering{ID: id, Name: name, IsActive: true}
	}
	inactive := func(id int64, name string) careplan.TemplateRosterFeedOffering {
		return careplan.TemplateRosterFeedOffering{ID: id, Name: name}
	}
	...
	t.Run("class target without offering is manual", func(t *testing.T) {
		...
		got := careplan.DeriveTemplateRosterMaintenance(careplan.TemplateRosterMaintenanceInput{
			HasDynamicTargets: true,
			Feeds:             careplan.TemplateRosterFeeds{CareOfferingsEnabled: true},
		})
		assert.Equal(t, careplan.RosterMaintenanceManual, got.Mode)
	...
	t.Run("source offering with class filter is automatic", func(t *testing.T) {
		...
		got := careplan.DeriveTemplateRosterMaintenance(careplan.TemplateRosterMaintenanceInput{
			...
			Feeds: careplan.TemplateRosterFeeds{
				...
				Sources:              []careplan.TemplateRosterFeedOffering{active(7, "Bis 16 Uhr")},
			},
		})
		assert.Equal(t, careplan.RosterMaintenanceAutomatic, got.Mode)
		assert.Equal(t, []careplan.RosterMaintenanceOffering{{ID: 7, Name: "Bis 16 Uhr"}}, got.Offerings)
	...
	t.Run("offering link on the offering is automatic", func(t *testing.T) {
		...
		got := careplan.DeriveTemplateRosterMaintenance(careplan.TemplateRosterMaintenanceInput{
			Feeds: careplan.TemplateRosterFeeds{
				CareOfferingsEnabled: true,
				LinkedOfferings:      []careplan.TemplateRosterFeedOffering{active(3, "Mittagessen")},
			},
		})
		assert.Equal(t, careplan.RosterMaintenanceAutomatic, got.Mode)
		assert.Equal(t, []careplan.RosterMaintenanceOffering{{ID: 3, Name: "Mittagessen"}}, got.Offerings)
	...
	t.Run("invalid offering link is manual", func(t *testing.T) {
		...
		got := careplan.DeriveTemplateRosterMaintenance(careplan.TemplateRosterMaintenanceInput{
			Feeds: careplan.TemplateRosterFeeds{
				CareOfferingsEnabled: true,
				LinkedOfferings: []careplan.TemplateRosterFeedOffering{{
					ID: 3, Name: "Mittagessen", IsActive: true, IsInvalid: true,
				}},
			},
		})
		assert.Equal(t, careplan.RosterMaintenanceManual, got.Mode)
		assert.Equal(t, []careplan.RosterMaintenanceOffering{{ID: 3, Name: "Mittagessen"}}, got.InvalidOfferings)
	})
	...
	t.Run("offering link plus class target is partial", func(t *testing.T) {
		...
		assert.Equal(t, careplan.RosterMaintenancePartial, got.Mode)
		assert.True(t, got.DynamicTargetsManual)
	})
	...
	t.Run("inactive offering link is named", func(t *testing.T) {
		...
		got := careplan.DeriveTemplateRosterMaintenance(careplan.TemplateRosterMaintenanceInput{
			Feeds: careplan.TemplateRosterFeeds{
				CareOfferingsEnabled: true,
				LinkedOfferings:      []careplan.TemplateRosterFeedOffering{inactive(3, "Mittagessen")},
			},
		})
		assert.Equal(t, careplan.RosterMaintenanceManual, got.Mode)
		assert.Equal(t, []careplan.RosterMaintenanceOffering{{ID: 3, Name: "Mittagessen"}}, got.InactiveOfferings)
	})
	...
	t.Run("inactive offering link beside an active link is partial", func(t *testing.T) {
		...
		assert.Equal(t, careplan.RosterMaintenancePartial, got.Mode)
		assert.Equal(t, []careplan.RosterMaintenanceOffering{{ID: 4, Name: "Spätbetreuung"}}, got.InactiveOfferings)
	})
	...
	t.Run("inactive offering is not automatic", func(t *testing.T) {
		...
		got := careplan.DeriveTemplateRosterMaintenance(careplan.TemplateRosterMaintenanceInput{
			...
			Feeds: careplan.TemplateRosterFeeds{
				CareOfferingsEnabled: true,
				Sources: []careplan.TemplateRosterFeedOffering{
					active(7, "Bis 16 Uhr"), inactive(8, "Musik"),
				},
				LinkedOfferings: []careplan.TemplateRosterFeedOffering{inactive(9, "Alt")},
			},
		})
		assert.Equal(t, careplan.RosterMaintenanceManual, got.Mode,
		...
		assert.Equal(t, []careplan.RosterMaintenanceOffering{
			{ID: 8, Name: "Musik"},
			{ID: 9, Name: "Alt"},
	...
	t.Run("broken source beside a working link is partial", func(t *testing.T) {
		...
		got := careplan.DeriveTemplateRosterMaintenance(careplan.TemplateRosterMaintenanceInput{
			...
			Feeds: careplan.TemplateRosterFeeds{
				CareOfferingsEnabled: true,
				Sources:              []careplan.TemplateRosterFeedOffering{inactive(8, "Musik")},
				LinkedOfferings:      []careplan.TemplateRosterFeedOffering{active(3, "Mittagessen")},
			},
		})
		assert.Equal(t, careplan.RosterMaintenancePartial, got.Mode)
	...
	t.Run("same offering as source and link is named once", func(t *testing.T) {
		...
		got := careplan.DeriveTemplateRosterMaintenance(careplan.TemplateRosterMaintenanceInput{
			...
			Feeds: careplan.TemplateRosterFeeds{
				CareOfferingsEnabled: true,
				Sources:              []careplan.TemplateRosterFeedOffering{active(7, "Bis 16 Uhr")},
				LinkedOfferings:      []careplan.TemplateRosterFeedOffering{active(7, "Bis 16 Uhr")},
			},
		})
		assert.Equal(t, careplan.RosterMaintenanceAutomatic, got.Mode)
...
func TestTemplateRosterMaintenanceFeeds_ResolvesSourcesLinksAndSeries(t *testing.T) {
	...
	reader, ok := env.bookings.(careplan.OfferingSourceEditor)
	require.True(t, ok)
	queries := []careplan.TemplateRosterFeedQuery{
...
func TestTemplateRosterMaintenanceFeeds_ReportsSwitchedOffCareOfferings(t *testing.T) {
	...
	reader, ok := env.bookings.(careplan.OfferingSourceEditor)
	require.True(t, ok)
	feeds, err := reader.TemplateRosterMaintenanceFeeds(testpkg.Ctx(t), []careplan.TemplateRosterFeedQuery{{TemplateID: template.ID}})
	require.NoError(t, err)
...
func TestTemplateRosterMaintenanceFeeds_RejectsSourceOutsideTemplatePeriod(t *testing.T) {
	...
	feeds, err := reader.TemplateRosterMaintenanceFeeds(ctx, []careplan.TemplateRosterFeedQuery{{
	...
	maintenance := careplan.DeriveTemplateRosterMaintenance(careplan.TemplateRosterMaintenanceInput{
		Feeds: careplan.TemplateRosterFeeds{
			CareOfferingsEnabled: feeds[987654321].CareOfferingsEnabled,
			Sources:              feeds[987654321].Sources,
		},
	})
	assert.Equal(t, careplan.RosterMaintenanceManual, maintenance.Mode)
	assert.Equal(t, []careplan.RosterMaintenanceOffering{{ID: source.ID, Name: source.Name}}, maintenance.InvalidOfferings)
}
...
func TestTemplateRosterMaintenanceFeeds_RejectsLinkedOfferingOutsideTemplatePeriod(t *testing.T) {
	...
	assert.Equal(t, careplan.RosterMaintenanceManual, maintenance.Mode)
	assert.Equal(t, []careplan.RosterMaintenanceOffering{{ID: linked.ID, Name: linked.Name}}, maintenance.InvalidOfferings)
}
