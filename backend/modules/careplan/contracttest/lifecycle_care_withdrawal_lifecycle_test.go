...
func TestCareWithdrawalLifecycle_AllowsRetroactiveExitButNotBeforeAttendance(t *testing.T) {
	...
	completion := testpkg.CreateTestCareWithdrawalCompletion(t, careWithdrawals(db), student.ID, actorID, today)

	_, err = svc.PreviewWithdrawalCareEnd(ctx, completion.ID, careplan.CareExitInput{
		LastCareDay: today.AddDays(-2), Reason: careplan.CareExitReasonNoCareNeed,
	})
	require.Error(t, err)
	assert.Contains(t, err.Error(), today.AddDays(-1).Format("02.01.2006"))

	input := careplan.CareExitInput{LastCareDay: today.AddDays(-1), Reason: careplan.CareExitReasonNoCareNeed}
...
func TestCareWithdrawalLifecycle_CompletionEndsBookingsFromEveryEnrollmentRequest(t *testing.T) {
	...
	input := careplan.CareExitInput{
		LastCareDay: timezone.NewDate(2026, 8, 24), Reason: careplan.CareExitReasonNoCareNeed,
	}
	svc := newCareLifecycleService(t, db)
	preview, err := svc.PreviewWithdrawalCareEnd(ctx, completion.ID, input)
	require.NoError(t, err)
...
func TestCareWithdrawalLifecycle_ConcurrentCompletionWritesOneResult(t *testing.T) {
	...
	completion := testpkg.CreateTestCareWithdrawalCompletion(t, careWithdrawals(db), student.ID, actorID, timezone.TodayDate().AddDays(1))
	input := careplan.CareExitInput{LastCareDay: timezone.TodayDate(), Reason: careplan.CareExitReasonNoCareNeed}
	preview, err := svc.PreviewWithdrawalCareEnd(ctx, completion.ID, input)
	require.NoError(t, err)
	...
	for _, err := range errs {
		...
		case err == nil:
			successes++
		case errors.Is(err, careplan.ErrCareWithdrawalAlreadyResolved):
			conflicts++
		default:
...
func TestCareWithdrawalLifecycle_ResolvedCompletionIsAConflict(t *testing.T) {
	...
	completion := testpkg.CreateTestCareWithdrawalCompletion(t, careWithdrawals(db), student.ID, actorID, timezone.TodayDate())
	...
	require.ErrorIs(t, err, careplan.ErrCareWithdrawalAlreadyResolved)
}
...
func TestCareWithdrawalLifecycle_CancellingPlannedExitRestoresTask(t *testing.T) {
	...
	completion := testpkg.CreateTestCareWithdrawalCompletion(t, careWithdrawals(db), student.ID, actorID, firstGap)
	input := careplan.CareExitInput{
		LastCareDay: firstGap.AddDays(-1), Reason: careplan.CareExitReasonNoCareNeed,
	}
	preview, err := svc.PreviewWithdrawalCareEnd(ctx, completion.ID, input)
	require.NoError(t, err)
	_, err = svc.ConfirmWithdrawalCareEnd(ctx, completion.ID, preview.Token, input, actorID)
	...
	rows, _, err := svc.ListPendingWithdrawals(ctx, careplan.CareWithdrawalFilter{StudentID: student.ID})
	require.NoError(t, err)
	assert.Empty(t, rows)
	...
	assert.Equal(t, firstGap.String(), pending.FirstBookinglessDay.String())
}
...
func TestCareWithdrawalLifecycle_CancellingLaterOrdinaryExitDoesNotRestoreOldTask(t *testing.T) {
	...
	completion := testpkg.CreateTestCareWithdrawalCompletion(t, careWithdrawals(db), student.ID, actorID, firstGap)
	withdrawal := careplan.CareExitInput{
		LastCareDay: firstGap.AddDays(-1), Reason: careplan.CareExitReasonNoCareNeed,
	}
	...
	later := careplan.CareExitInput{
		StudentIDs: []int64{student.ID}, LastCareDay: firstGap.AddDays(7),
		Reason: careplan.CareExitReasonMovedAway,
	}
