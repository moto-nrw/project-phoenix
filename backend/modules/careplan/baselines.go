...
type ArrivalPlanByDate map[calendar.Date]ArrivalWeek
...
func (p *ArrivalBaselineProjection) ForDate(studentID int64, date calendar.Date) *ArrivalSchedule {
	if p == nil {
		return nil
	}
...
func (p *ArrivalBaselineProjection) DerivedForDate(studentID int64, date calendar.Date) *ArrivalSchedule {
	if p == nil {
		return nil
	}
...
func (p *ArrivalBaselineProjection) HasPlan(studentID int64, date calendar.Date) bool {
...
func (p *ArrivalBaselineProjection) WeeklyForDate(studentID int64, date calendar.Date) ArrivalWeek {
	if p == nil {
		return nil
	}
...
type ArrivalBaselineReader interface {
	Project(ctx context.Context, studentIDs []int64, from, to calendar.Date) (*ArrivalBaselineProjection, error)
}
...
type PickupPlanByDate map[calendar.Date]PickupWeek
...
func (p *PickupBaselineProjection) ForDate(studentID int64, date calendar.Date) *PickupSchedule {
	if p == nil {
		return nil
	}
...
// override, if one exists for the date's weekday.
func (p *PickupBaselineProjection) OfferingForDate(studentID int64, date calendar.Date) *PickupSchedule {
	if p == nil {
		return nil
	}
...
func (p *PickupBaselineProjection) HasPlan(studentID int64, date calendar.Date) bool {
	return p != nil && len(p.WeeklyByStudentDate[studentID][date]) > 0
}

// WeeklyForDate returns the full recurring week applicable on date.
func (p *PickupBaselineProjection) WeeklyForDate(studentID int64, date calendar.Date) PickupWeek {
	if p == nil {
		return nil
	}
	return p.WeeklyByStudentDate[studentID][date]
...
func (p *PickupBaselineProjection) OfferingWeeklyForDate(studentID int64, date calendar.Date) PickupWeek {
	if p == nil {
		return nil
	}
...
type PickupBaselineReader interface {
	Project(ctx context.Context, studentIDs []int64, from, to calendar.Date) (*PickupBaselineProjection, error)
	OfferingPickupForDate(ctx context.Context, studentID int64, date calendar.Date) (*PickupSchedule, error)
...
type ApprovedBookingReader interface {
	ListApprovedByStudentIDsInRange(context.Context, []int64, calendar.Date, calendar.Date) ([]*ApprovedBooking, error)
}
...
type ClassArrivalExceptionsByDate map[calendar.Date]*ArrivalBaselineException
type CareDayIndex map[int64]map[calendar.Date]map[int]bool

func (c CareDayIndex) Covers(studentID int64, date calendar.Date, weekday int) bool {
...
func classExceptionRow(
	...
	effective := *row
	effective.ExpectedArrival = calendar.NormalizeWallClock(exception.ArrivalTime)
