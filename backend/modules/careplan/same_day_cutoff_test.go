...
func berlinAt(year int, month time.Month, day, hour, minute, second int) time.Time {
	return time.Date(year, month, day, hour, minute, second, 0, timezone.Berlin)
}
...
func TestSameDayCutoffEmptyNeverCloses(t *testing.T) {
	...
	assert.False(t, sameDayCutoff(t, "", now).Closed(timezone.NewDate(2026, 8, 24)))
	assert.False(t, careplan.SameDayCutoff{}.Closed(timezone.NewDate(2026, 8, 24)))
}
...
func TestSameDayCutoffUsesBerlinTimeAcrossDaylightSaving(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"Winter 10:59 Berlin", time.Date(2026, 1, 15, 9, 59, 0, 0, time.UTC), timezone.NewDate(2026, 1, 15), false},
		{"Winter 11:01 Berlin", time.Date(2026, 1, 15, 10, 1, 0, 0, time.UTC), timezone.NewDate(2026, 1, 15), true},
		// Summer (CEST, UTC+2): 11:00 Berlin is 09:00 UTC.
		{"Sommer 10:59 Berlin", time.Date(2026, 7, 1, 8, 59, 0, 0, time.UTC), timezone.NewDate(2026, 7, 1), false},
		{"Sommer 11:01 Berlin", time.Date(2026, 7, 1, 9, 1, 0, 0, time.UTC), timezone.NewDate(2026, 7, 1), true},
		// 10:01 UTC in summer is already 12:01 Berlin — a UTC reading would say open.
		{"Sommer 10:01 UTC ist 12:01 Berlin", time.Date(2026, 7, 1, 10, 1, 0, 0, time.UTC), timezone.NewDate(2026, 7, 1), true},
		// Changeover days: 29 March 2026 (clocks go forward), 25 October 2026 (back).
		{"Umstellung März 10:59 Berlin", time.Date(2026, 3, 29, 8, 59, 0, 0, time.UTC), timezone.NewDate(2026, 3, 29), false},
		{"Umstellung März 11:01 Berlin", time.Date(2026, 3, 29, 9, 1, 0, 0, time.UTC), timezone.NewDate(2026, 3, 29), true},
		{"Umstellung Oktober 10:59 Berlin", time.Date(2026, 10, 25, 9, 59, 0, 0, time.UTC), timezone.NewDate(2026, 10, 25), false},
		{"Umstellung Oktober 11:01 Berlin", time.Date(2026, 10, 25, 10, 1, 0, 0, time.UTC), timezone.NewDate(2026, 10, 25), true},
		...
		{"UTC-Tag ist in Berlin schon gestern", time.Date(2026, 6, 30, 23, 30, 0, 0, time.UTC), timezone.NewDate(2026, 6, 30), false},
		{"Berliner Tag nach UTC-Mitternacht", time.Date(2026, 6, 30, 23, 30, 0, 0, time.UTC), timezone.NewDate(2026, 7, 1), false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
	...
	assert.True(t, early.Closed(timezone.NewDate(2026, 7, 1)))
	assert.False(t, early.Closed(timezone.NewDate(2026, 7, 2)))
}
...
func TestNewSameDayCutoffRejectsMalformedClock(t *testing.T) {
	...
	assert.True(t, broken.Closed(timezone.NewDate(2026, 8, 24)))
	assert.False(t, broken.Closed(timezone.NewDate(2026, 8, 25)))
}
