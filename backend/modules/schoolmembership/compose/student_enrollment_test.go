...
func TestStudentMembershipAlumniNeedExplicitReactivation(t *testing.T) {
	...
	membership, err := New(Dependencies{DB: db, Observe: func(Observation) {}, Employment: testEmployment, ChildQuota: newChildQuotas()})
	require.NoError(t, err)
	...
	ids, err := membership.Reactivate(ctx, []int64{student.ID}, "active", schoolmembership.EnforceChildQuota)
	require.NoError(t, err)
