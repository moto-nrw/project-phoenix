...
func TestAuthService_StartStaffPreview(t *testing.T) {
	...
	t.Run("refuses an inactive target", func(t *testing.T) {
		...
		requirePreviewErr(t, err, identityaccess.ErrAccountInactive)
	})
	...
	t.Run("refuses a target without a mapping at this school", func(t *testing.T) {
		...
		_, err := service.StartStaffPreview(ctx, admin.ID, tenantID, target.ID, "", "", "")
		requirePreviewErr(t, err, identityaccess.ErrTenantAccessDenied)
	})
	...
	t.Run("refuses a guardian-only target", func(t *testing.T) {
		...
		_, err := service.StartStaffPreview(ctx, admin.ID, tenantID, target.ID, "", "", "")
		requirePreviewErr(t, err, identityaccess.ErrPreviewTargetNotStaff)
	})
	...
	t.Run("refuses a Lehrkraft-only target", func(t *testing.T) {
		...
		_, err := service.StartStaffPreview(ctx, admin.ID, tenantID, target.ID, "", "", "")
		requirePreviewErr(t, err, identityaccess.ErrMustUseSchoolPortal)
	})
}
...
func TestAuthService_ListStaffPreviewCandidates(t *testing.T) {
	...
	ids := make(map[int64]identityaccess.StaffPreviewCandidate, len(candidates))
...
func TestAuthService_StartStaffPreviewWaitsForConcurrentEnd(t *testing.T) {
	...
	remint := make(chan *identityaccess.StaffPreviewSession, 1)
	go func() {
