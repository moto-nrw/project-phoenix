...
func TestMFAService_GetTenantMFAOverride_DefaultNone(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, identityaccess.MFAAdminOverrideNone, globalOverride)
}
...
func TestMFAService_SetMFAOverride_RejectsInvalidValue(t *testing.T) {
	...
	assert.ErrorIs(t, err, identityaccess.ErrMFAInvalidOverride)
}
...
func TestMFAService_SetMFAOverride_PermissionGate(t *testing.T) {
	...
	err := svc.SetMFAOverride(ctx, 0, actorTenantID, acc.ID, identityaccess.MFAAdminOverrideForceOff, "reason", []string{"something:else"})
	assert.ErrorIs(t, err, identityaccess.ErrMFAPermissionDenied)

	// users:manage + matching tenant works.
	require.NoError(t, svc.SetMFAOverride(ctx, 0, actorTenantID, acc.ID, identityaccess.MFAAdminOverrideForceOff, "reason", []string{"users:manage"}))
	got, err := svc.GetTenantMFAOverride(ctx, acc.ID, actorTenantID)
	require.NoError(t, err)
	assert.Equal(t, identityaccess.MFAAdminOverrideForceOff, got)
}
...
func TestMFAService_SetMFAOverride_ForceOff_RevokesTrustedDevices(t *testing.T) {
	...
	require.NoError(t, svc.SetMFAOverride(ctx, 0, actorTenantID, acc.ID, identityaccess.MFAAdminOverrideForceOff, "lost mailbox", []string{"users:manage"}))
...
func TestMFAService_IsRequired_HonorsOverride(t *testing.T) {
	...
	require.NoError(t, svc.SetMFAOverride(ctx, 0, actorTenantID, acc.ID, identityaccess.MFAAdminOverrideForceOn, "always 2FA", []string{"users:manage"}))
	required, err = svc.IsRequired(ctx, acc.ID, nil, actorTenantID)
	require.NoError(t, err)
...
func TestMFAService_IsRequired_TenantOverride_StaysInTenant(t *testing.T) {
	...
	require.NoError(t, svc.SetMFAOverride(ctx, 0, tenantA, acc.ID, identityaccess.MFAAdminOverrideForceOff, "lost mailbox in school A", []string{"users:manage"}))
	...
	require.NoError(t, svc.SetMFAOverride(ctx, 0, tenantB, acc.ID, identityaccess.MFAAdminOverrideForceOn, "tenant B keeps 2FA", []string{"users:manage"}))
	requiredB, err = svc.IsRequired(ctx, acc.ID, nil, tenantB)
	require.NoError(t, err)
...
func TestMFAService_IsRequired_GlobalOverride_AppliesEverywhere(t *testing.T) {
	...
	require.NoError(t, svc.OperatorSetGlobalMFAOverride(ctx, op.ID, acc.ID, identityaccess.MFAAdminOverrideForceOff, "mailbox lockout account-wide"))
	...
	// resolution.
	require.NoError(t, svc.OperatorSetGlobalMFAOverride(ctx, op.ID, acc.ID, identityaccess.MFAAdminOverrideNone, "mailbox restored"))
...
func TestMFAService_SetMFAOverride_RejectionDoesNotPartialWrite(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, identityaccess.MFAAdminOverrideNone, override, "override must stay none after rejected write")
...
func TestMFAService_SetMFAOverride_RejectsCrossTenant(t *testing.T) {
	...
	tenantB := testpkg.UniqueTestTenantID(t)
	err := svc.SetMFAOverride(ctx, 7777, tenantB, target.ID, identityaccess.MFAAdminOverrideForceOff, "cross-tenant attack", []string{"users:manage"})
	assert.ErrorIs(t, err, identityaccess.ErrMFAPermissionDenied)
	...
	require.NoError(t, err)
	assert.Equal(t, identityaccess.MFAAdminOverrideNone, overrideA,
		"rejected cross-tenant override must NOT write tenant A's row")
	overrideB, err := svc.GetTenantMFAOverride(ctx, target.ID, tenantB)
	require.NoError(t, err)
	assert.Equal(t, identityaccess.MFAAdminOverrideNone, overrideB,
...
func TestMFAService_SetMFAOverride_RejectsZeroActorTenant(t *testing.T) {
	...
	err := svc.SetMFAOverride(ctx, 7777, 0, target.ID, identityaccess.MFAAdminOverrideForceOff, "no tenant", []string{"users:manage"})
	assert.ErrorIs(t, err, identityaccess.ErrMFAPermissionDenied)
}
...
func TestMFAService_OperatorSetMFAOverride_RejectsZeroSchoolID(t *testing.T) {
	...
	err := svc.OperatorSetMFAOverride(ctx, 1, 0, acc.ID, identityaccess.MFAAdminOverrideForceOff, "test")
	assert.ErrorIs(t, err, identityaccess.ErrMFAPermissionDenied)
}
...
func TestMFAService_OperatorSetMFAOverride_RejectsCrossSchool(t *testing.T) {
	...
	// guess.
	err := svc.OperatorSetMFAOverride(ctx, 1, 999, acc.ID, identityaccess.MFAAdminOverrideForceOn, "test")
	assert.ErrorIs(t, err, identityaccess.ErrMFAPermissionDenied)

	override, err := svc.GetTenantMFAOverride(ctx, acc.ID, 999)
	require.NoError(t, err)
	assert.Equal(t, identityaccess.MFAAdminOverrideNone, override, "override row must remain unset after rejected cross-school write")
}
...
func TestMFAService_StartChallenge_RateLimitAfter3Codes(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		_, err := svc.StartMFAChallenge(ctx, acc.ID, 0, identityaccess.MFAChallengeScopeTenant, ip)
...
func TestMFAService_VerifyChallenge_LocksAccountAfter5Failures(t *testing.T) {
	...
	require.NoError(t, svc.EnrollMFA(ctx, acc.ID))
	challenge, err := svc.StartMFAChallenge(ctx, acc.ID, 0, identityaccess.MFAChallengeScopeTenant, net.ParseIP("203.0.113.40"))
	require.NoError(t, err)
...
func TestMFAService_VerifyChallenge_ExpiredChallengeRejected(t *testing.T) {
	...
	require.NoError(t, svc.EnrollMFA(ctx, acc.ID))
	challenge, err := svc.StartMFAChallenge(ctx, acc.ID, 0, identityaccess.MFAChallengeScopeTenant, net.ParseIP("203.0.113.50"))
	require.NoError(t, err)
