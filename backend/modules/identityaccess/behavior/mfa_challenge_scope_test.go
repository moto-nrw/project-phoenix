...
func TestMFAService_ResendChallengeForScope_InvalidToken_ReturnsTokenInvalid(t *testing.T) {
	...
	renewed, err := svc.ResendMFAChallengeForScope(
		context.Background(), "not-a-jwt", net.ParseIP("127.0.0.1"), identityaccess.MFAChallengeScopeSchool)
...
func TestMFAService_ResendChallengeForScope_ForeignScope_RefusedBeforeSending(t *testing.T) {
	...
	renewed, err := svc.ResendMFAChallengeForScope(ctx, token, net.ParseIP("127.0.0.1"), identityaccess.MFAChallengeScopeSchool)
...
func TestMFAService_ResendChallengeForScope_SchoolScope_HappyPath(t *testing.T) {
	...
	token, err := svc.StartMFAChallenge(ctx, accID, 0, identityaccess.MFAChallengeScopeSchool, net.ParseIP("127.0.0.1"))
	require.NoError(t, err)
	require.NotEmpty(t, token)
