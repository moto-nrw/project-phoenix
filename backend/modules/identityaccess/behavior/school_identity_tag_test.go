...
func TestEnsureSchoolIdentityAcceptsTheReaderSpellingOfATag(t *testing.T) {
	...
	var identity *identityaccess.SchoolIdentity
	err = tenant.WithinCurrentTenant(ctx, func(txCtx context.Context) error {
		var ensureErr error
		identity, ensureErr = provisioning.EnsureSchoolIdentity(txCtx, identityaccess.SchoolIdentityInput{
			AccountID:    account.ID,
			TenantID:     testpkg.Tenant(t),
			Role:         &identityaccess.RoleFacts{Name: "admin", IsSystem: true},
			FirstName:    "Tag",
			LastName:     "Spelling",
			TagID:        &submitted,
			CreatePerson: true,
...
func TestEnsureSchoolIdentityReportsAnUnknownTagAsTheRetainedSentinel(t *testing.T) {
	...
	unknown := "NOTACARD"
	err = tenant.WithinCurrentTenant(ctx, func(txCtx context.Context) error {
		_, ensureErr := provisioning.EnsureSchoolIdentity(txCtx, identityaccess.SchoolIdentityInput{
			AccountID:    account.ID,
			TenantID:     testpkg.Tenant(t),
			Role:         &identityaccess.RoleFacts{Name: "admin", IsSystem: true},
			FirstName:    "Tag",
	...
	require.ErrorIs(t, err, identityaccess.ErrSchoolIdentityTagUnknown)
