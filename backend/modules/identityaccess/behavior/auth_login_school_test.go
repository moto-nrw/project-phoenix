...
func TestLoginSchool_NoPortalRole_Refused(t *testing.T) {
	...
	assert.ErrorIs(t, authErr.Err, identityaccess.ErrAccountNoSchoolPortalRole)
}
...
func TestLoginSchool_Lehrkraft_IssuesSchoolScopedTokens(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, identityaccess.LoginStatusAuthenticated, result.Status)
...
func TestRefreshToken_SchoolScopeWithoutTenant_RejectedBeforeRotation(t *testing.T) {
	...
	var authErr *identityaccess.AuthenticationError
	require.ErrorAs(t, err, &authErr)
	assert.ErrorIs(t, authErr.Err, identityaccess.ErrTenantAccessDenied)
...
func TestLoginSchool_MFARequiredEnrolled_StartsSchoolScopedChallenge(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, identityaccess.LoginStatusMFARequired, result.Status)
...
func TestLoginSchool_MFAStatusUnavailable_FailsClosed(t *testing.T) {
	...
	setup := func(t *testing.T, mfa *MFAServiceMock) (gatedAuthService, string) {
		t.Helper()
	...
	t.Run("IsRequired lookup fails", func(t *testing.T) {
		service, email := setup(t, &MFAServiceMock{
		...
		var authErr *identityaccess.AuthenticationError
		require.ErrorAs(t, err, &authErr)
		assert.ErrorIs(t, authErr.Err, identityaccess.ErrMFAStatusUnavailable)
	})
	...
	t.Run("challenge start fails", func(t *testing.T) {
		...
		var authErr *identityaccess.AuthenticationError
		require.ErrorAs(t, err, &authErr)
		assert.ErrorIs(t, authErr.Err, identityaccess.ErrMFARateLimited)
	})
}
...
func TestIssueSchoolTokens_AccountStateGates(t *testing.T) {
	...
	t.Run("unknown account", func(t *testing.T) {
		...
		var authErr *identityaccess.AuthenticationError
		require.ErrorAs(t, err, &authErr)
		assert.ErrorIs(t, authErr.Err, identityaccess.ErrAccountNotFound)
	})
	...
	t.Run("deactivated account", func(t *testing.T) {
		...
		var authErr *identityaccess.AuthenticationError
		require.ErrorAs(t, err, &authErr)
		assert.ErrorIs(t, authErr.Err, identityaccess.ErrAccountInactive)
	})
...
func TestLoginSchool_MFAEnrollmentRequired_MintsSchoolScopedEnrollmentToken(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, identityaccess.LoginStatusMFAEnrollmentRequired, result.Status)
	require.NotEmpty(t, result.AccessToken)
...
func TestSchoolTokenMints_RevokedMembership_Refused(t *testing.T) {
	...
	login, err := service.LoginSchoolWithMFAGate(context.Background(), email, testPassword, "", "", "")
	require.NoError(t, err)
	require.Equal(t, identityaccess.LoginStatusAuthenticated, login.Status)
...
func TestLoginSchool_MFARequirementAppearingMidLogin_ChallengesInsteadOfMinting(t *testing.T) {
	...
	service := newGatedAuthService(t, db, &MFAServiceMock{
		...
		ResolveMFAPolicyInTxFn: func(context.Context, int64, int64) (identityaccess.MFAPolicy, error) {
			return requiredAdminsPolicy(), nil
		},
		HasMFAEnrollmentFn: func(context.Context, int64) (bool, error) { return true, nil },
