...
func withActorTenant(r *http.Request, tenantID int64) *http.Request {
	claims := testutil.ClaimsFromContext(r.Context())
	claims.TenantID = tenantID
	return r.WithContext(testutil.WithAuthenticatedContext(r.Context(), claims, nil))
}
...
func reqWithAccountID(t *testing.T, method, accountIDParam string, body any, claimID int, perms []string) *http.Request {
	...
	if claimID > 0 {
		ctx = testutil.WithAuthenticatedContext(ctx, testutil.Claims{
			ID:          claimID,
			Permissions: perms,
		}, nil)
	}
