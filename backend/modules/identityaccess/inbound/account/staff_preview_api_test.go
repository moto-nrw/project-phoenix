...
func TestStaffPreviewEndpoints(t *testing.T) {
	...
	t.Run("end rejects anything that is not a preview token", func(t *testing.T) {
		cases := map[string]string{
			...
			"a preview token without preview id": testutil.MintTestJWT(t, testutil.Claims{ID: int(target.ID), TenantID: testpkg.Tenant(t), Sub: target.Email, ReadOnly: true, ActingAdminID: admin.ID}),
			"garbage":                            "not-a-token",
		}
