...
func setupPlatformPermissionRoute(t *testing.T) chi.Router {
	...
	svc := testutil.SetupAuthModuleOn(t, serveDB)
	authResource := authAPI.NewResource(svc.Auth, svc.Invitation, nil, svc.AccountAuthentication, testutil.AccountRouteTenantRuntime())
	authResource.RoleGrants = testutil.AccountRoleGrantPolicy()
