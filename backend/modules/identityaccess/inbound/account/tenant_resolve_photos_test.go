...
func TestResolveTenant_StudentPhotosEnabled_OverrideTrue(t *testing.T) {
	...
	ctx := scope.Context()
	require.NoError(t,
		tenantSettings.SetValue(ctx, settings.KeyStudentPhotosEnabled, true, nil, nil),
		"enable student_photos_enabled for the isolated tenant")
	t.Cleanup(func() {
		require.NoError(t, tenantSettings.ResetValue(ctx, settings.KeyStudentPhotosEnabled, nil, nil))
	})
...
func TestResolveTenant_GradeLevelMax_Override(t *testing.T) {
	...
	ctx := scope.Context()
	require.NoError(t,
		tenantSettings.SetValue(ctx, settings.KeyEnrollmentGradeLevelMax, 13, nil, nil))
	t.Cleanup(func() {
		require.NoError(t, tenantSettings.ResetValue(ctx, settings.KeyEnrollmentGradeLevelMax, nil, nil))
	})
...
func TestResolveTenant_StaffMessagingSettingsFailureIsGeneric500(t *testing.T) {
	...
	resource.SettingsService = &configtest.Mock{
		ResolveBoolForTenantFn: func(_ context.Context, _ int64, key string) (bool, error) {
			if key == settings.KeyStaffMessagingEnabled {
