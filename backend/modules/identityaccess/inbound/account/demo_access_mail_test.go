...
func TestDemoAccessRequestMailsTheLinkAndTheLead(t *testing.T) {
	...
	assert.Equal(t, "Ihr Team von", content["PoweredByLabel"])
	assert.Equal(t, content["LogoURL"], content["MotoLogoURL"])
	assert.NotEmpty(t, content["MotoLogoURL"])
	...
	require.True(t, ok)
	assert.Equal(t, "Kim Beispiel", content["PersonName"], "the lead names first and last name")
	assert.Equal(t, "Kim Beispiel", lead.ReplyTo.Name)
