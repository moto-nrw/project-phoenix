...
func TestInvitationHTTPRequiresVerifiedOwner(t *testing.T) {
	...
	resource := authAPI.NewResource(module.Auth, service, nil, nil, testutil.AccountRouteTenantRuntime())
	...
	req.Header.Set("Authorization", "Bearer "+testutil.MintTestJWT(t, testutil.Claims{ID: int(owner.ID), Sub: owner.Email, Roles: []string{}, Scope: "school", TenantID: testpkg.Tenant(t)}))
	rr = testutil.ExecuteRequest(router, req)
