...
func setupAuthDependenciesRoute(t *testing.T) (*bun.DB, *authAPI.Resource) {
	t.Helper()
	db, resource, _ := setupAuthDependenciesRouteWithSettings(t)
	return db, resource
}

// tenantSettingsWriter is the real settings service the resource reads its
// tenant settings from, with the writes a test sets its overrides with.
type tenantSettingsWriter interface {
...
// setupAuthDependenciesRouteWithSettings composes the resource the way the
// root does and also returns the settings service it reads, so a test can
// store tenant overrides.
func setupAuthDependenciesRouteWithSettings(t *testing.T) (*bun.DB, *authAPI.Resource, tenantSettingsWriter) {
	t.Helper()
	db, svc := testutil.SetupAuthModule(t)
	resource := authAPI.NewResource(svc.Auth, svc.Invitation, testSchoolDirectory{schools: svc.Schools, db: db, runtime: testpkg.TenantRuntime(t, db)}, svc.AccountAuthentication, testutil.AccountRouteTenantRuntime())
	resource.SettingsService = svc.Settings
	resource.RoleGrants = testutil.AccountRoleGrantPolicy()
	return db, resource, svc.Settings
}
