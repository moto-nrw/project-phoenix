...
type RolePermissionRepository struct {
	*base.Repository[*authmodels.RolePermission]
	db *bun.DB
}

// NewRolePermissionRepository creates a new RolePermissionRepository
func NewRolePermissionRepository(db *bun.DB) authmodels.RolePermissionRepository {
	return &RolePermissionRepository{
		Repository: base.NewRepository[*authmodels.RolePermission](db, rolePermissionTable, "RolePermission"),
		db:         db,
	}
}

// FindByRoleID retrieves all role-permission mappings for a role
func (r *RolePermissionRepository) FindByRoleID(ctx context.Context, roleID int64) ([]*authmodels.RolePermission, error) {
	return r.List(ctx, map[string]any{"role_id": roleID})
}

// Update overrides the base Update method for schema consistency
func (r *RolePermissionRepository) Update(ctx context.Context, rolePermission *authmodels.RolePermission) error {
...
func (r *RolePermissionRepository) DeleteByRoleID(ctx context.Context, roleID int64) error {
	_, err := base.GetDB(ctx, r.db).NewDelete().
		Model((*authmodels.RolePermission)(nil)).
		ModelTableExpr(rolePermissionTable).
