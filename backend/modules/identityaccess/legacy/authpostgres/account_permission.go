...
type AccountPermissionRepository struct {
	*base.Repository[*authmodels.AccountPermission]
	db *bun.DB
}

// NewAccountPermissionRepository creates a new AccountPermissionRepository
func NewAccountPermissionRepository(db *bun.DB) authmodels.AccountPermissionRepository {
	repo := base.NewRepository[*authmodels.AccountPermission](db, accountPermissionTable, "AccountPermission")
...
func (r *AccountPermissionRepository) FindByAccountID(ctx context.Context, accountID int64) ([]*authmodels.AccountPermission, error) {
	var accountPermissions []*authmodels.AccountPermission
...
func (r *AccountPermissionRepository) GrantPermission(ctx context.Context, accountID, permissionID int64) error {
	...
	existsQuery := db.NewSelect().
		Model((*authmodels.AccountPermission)(nil)).
		ModelTableExpr(accountPermissionTableAlias).
	...
	if exists {
		...
		perm := &authmodels.AccountPermission{
			AccountID:    accountID,
...
func (r *AccountPermissionRepository) Create(ctx context.Context, accountPermission *authmodels.AccountPermission) error {
...
func (r *AccountPermissionRepository) Update(ctx context.Context, accountPermission *authmodels.AccountPermission) error {
	if accountPermission == nil {
		return fmt.Errorf("account permission cannot be nil")
	}
...
func (r *AccountPermissionRepository) List(ctx context.Context, filters map[string]interface{}) ([]*authmodels.AccountPermission, error) {
	var accountPermissions []*authmodels.AccountPermission
	query := base.GetDB(ctx, r.db).NewSelect().
		Model(&accountPermissions).
		ModelTableExpr(accountPermissionTableAlias)
