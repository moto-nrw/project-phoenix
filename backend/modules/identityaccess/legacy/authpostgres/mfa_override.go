...
type MFAOverrideRepository struct {
	*base.Repository[*authmodels.MFAOverride]
	db *bun.DB
}

// NewMFAOverrideRepository wires the BUN repository.
func NewMFAOverrideRepository(db *bun.DB) authmodels.MFAOverrideRepository {
	return &MFAOverrideRepository{
		Repository: base.NewRepository[*authmodels.MFAOverride](db, mfaOverrideTable, "MfaOverride"),
		db:         db,
	}
}
...
// error.
func (r *MFAOverrideRepository) FindGlobal(ctx context.Context, accountID int64) (*authmodels.MFAOverride, error) {
	row := new(authmodels.MFAOverride)
...
func (r *MFAOverrideRepository) FindByAccountAndTenant(ctx context.Context, accountID, tenantID int64) (*authmodels.MFAOverride, error) {
	if tenantID == 0 {
...
func (r *MFAOverrideRepository) UpsertGlobal(ctx context.Context, override *authmodels.MFAOverride) error {
	if override == nil {
	...
	if override.Override != authmodels.MFAAdminOverrideForceOff && override.Override != authmodels.MFAAdminOverrideForceOn {
		return fmt.Errorf("override must be force_off or force_on, got %q", override.Override)
	}
	if override.SetByType != authmodels.MFAOverrideSetByTypeOperator {
...
func (r *MFAOverrideRepository) UpsertTenant(ctx context.Context, override *authmodels.MFAOverride) error {
	if override == nil {
		return fmt.Errorf("mfa override cannot be nil")
	}
	if override.AccountID == 0 {
...
func (r *MFAOverrideRepository) DeleteGlobal(ctx context.Context, accountID int64) error {
	_, err := base.GetDB(ctx, r.db).NewDelete().
		Model((*authmodels.MFAOverride)(nil)).
		ModelTableExpr(mfaOverrideTable).
		Where("account_id = ?", accountID).
		Where("tenant_id IS NULL").
		Exec(ctx)
...
func (r *MFAOverrideRepository) ListByAccount(ctx context.Context, accountID int64) ([]*authmodels.MFAOverride, error) {
	var rows []*authmodels.MFAOverride
	err := base.GetDB(ctx, r.db).NewSelect().
		Model(&rows).
		ModelTableExpr(mfaOverrideTableAlias).
		Where("account_id = ?", accountID).
