...
type MFATrustedDeviceRepository struct {
	*base.Repository[*authmodels.MFATrustedDevice]
	db *bun.DB
}

// NewMFATrustedDeviceRepository creates a new repository for trusted-device records.
func NewMFATrustedDeviceRepository(db *bun.DB) authmodels.MFATrustedDeviceRepository {
	return &MFATrustedDeviceRepository{
		Repository: base.NewRepository[*authmodels.MFATrustedDevice](db, mfaTrustedDeviceTable, "MfaTrustedDevice"),
		db:         db,
	}
}

// Create persists a new trusted-device record after light validation.
func (r *MFATrustedDeviceRepository) Create(ctx context.Context, device *authmodels.MFATrustedDevice) error {
	if device == nil {
...
func (r *MFATrustedDeviceRepository) FindActiveByAccountTenantAndTokenHash(ctx context.Context, accountID, tenantID int64, tokenHash string) (*authmodels.MFATrustedDevice, error) {
	device := new(authmodels.MFATrustedDevice)
...
func (r *MFATrustedDeviceRepository) ListActiveByAccountTenant(ctx context.Context, accountID, tenantID int64) ([]*authmodels.MFATrustedDevice, error) {
	var devices []*authmodels.MFATrustedDevice
	err := base.GetDB(ctx, r.db).NewSelect().
		Model(&devices).
		ModelTableExpr(mfaTrustedDeviceTableAlias).
		Where("account_id = ?", accountID).
...
func (r *MFATrustedDeviceRepository) UpdateLastUsedAt(ctx context.Context, id int64, when time.Time) error {
	device := &authmodels.MFATrustedDevice{Model: modelBase.Model{ID: id}, LastUsedAt: &when}
...
func (r *MFATrustedDeviceRepository) Revoke(ctx context.Context, id int64, revokedAt time.Time) error {
	res, err := base.GetDB(ctx, r.db).NewUpdate().
		Model((*authmodels.MFATrustedDevice)(nil)).
