...
type RFIDCardRepository struct {
	*base.Repository[*authmodels.RFIDCard]
	db *bun.DB
}

// NewRFIDCardRepository creates a new RFIDCardRepository
func NewRFIDCardRepository(db *bun.DB) authmodels.RFIDCardRepository {
	repo := base.NewRepository[*authmodels.RFIDCard](db, "users.rfid_cards", "RfidCard")
...
func (r *RFIDCardRepository) Delete(ctx context.Context, id string) error {
	...
	normalizedID := authmodels.NormalizeTagID(id)

	query := base.GetDB(ctx, r.db).NewDelete().
		Model((*authmodels.RFIDCard)(nil)).
...
func (r *RFIDCardRepository) FindByID(ctx context.Context, id string) (*authmodels.RFIDCard, error) {
	// Normalize the tag ID to match stored format
	normalizedID := authmodels.NormalizeTagID(id)

	card := new(authmodels.RFIDCard)
...
func (r *RFIDCardRepository) List(ctx context.Context, filters map[string]interface{}) ([]*authmodels.RFIDCard, error) {
