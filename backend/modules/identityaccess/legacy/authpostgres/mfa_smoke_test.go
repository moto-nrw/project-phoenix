...
func TestMFARepositoriesSmoke(t *testing.T) {
	...
	t.Run("credential lifecycle", func(t *testing.T) {
		cred := &authmodels.MFACredential{
			AccountID: account.ID,
			Method:    authmodels.MFAMethodEmail,
		}
		...
		assert.Equal(t, authmodels.MFAMethodEmail, fetched.Method)
	...
	t.Run("email challenge lifecycle + rate-limit count", func(t *testing.T) {
		challenge := &authmodels.MFAEmailChallenge{
			AccountID: account.ID,
		...
		expired := &authmodels.MFAEmailChallenge{
			AccountID: account.ID,
			Scope:     authjwt.MFAChallengeScopeTenant,
	...
	t.Run("trusted device lifecycle + revoke", func(t *testing.T) {
		...
		device := &authmodels.MFATrustedDevice{
			AccountID: account.ID,
			TenantID:  tenantID,
		...
		another := &authmodels.MFATrustedDevice{
			AccountID: account.ID,
			TenantID:  tenantID,
		...
		expired := &authmodels.MFATrustedDevice{
			AccountID: account.ID,
			TenantID:  tenantID,
