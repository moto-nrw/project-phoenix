...
type ProfileRepository struct {
	*base.Repository[*authmodels.Profile]
	db *bun.DB
}

// NewProfileRepository creates a new ProfileRepository
func NewProfileRepository(db *bun.DB) authmodels.ProfileRepository {
	repo := base.NewRepository[*authmodels.Profile](db, "users.profiles", "Profile")
...
func (r *ProfileRepository) FindByAccountID(ctx context.Context, accountID int64) (*authmodels.Profile, error) {
	profile := new(authmodels.Profile)
...
func (r *ProfileRepository) UpdateAvatar(ctx context.Context, id int64, avatar string) error {
	profile := &authmodels.Profile{Model: modelBase.Model{ID: id}, Avatar: avatar}
...
func (r *ProfileRepository) List(ctx context.Context, filters map[string]interface{}) ([]*authmodels.Profile, error) {
	var profiles []*authmodels.Profile
	query := base.GetDB(ctx, r.db).NewSelect().
		Model(&profiles).
		ModelTableExpr(`users.profiles AS "profile"`)
