...
type MFACredentialRepository struct {
	*base.Repository[*authmodels.MFACredential]
	db *bun.DB
}

// NewMFACredentialRepository creates a new repository for MFA credential records.
func NewMFACredentialRepository(db *bun.DB) authmodels.MFACredentialRepository {
	return &MFACredentialRepository{
		...
		Repository: base.NewRepository[*authmodels.MFACredential](db, mfaCredentialTable, "MfaCredential"),
		db:         db,
	}
}

// Update validates and persists modifications to an existing credential.
func (r *MFACredentialRepository) Update(ctx context.Context, credential *authmodels.MFACredential) error {
	if credential == nil {
...
func (r *MFACredentialRepository) FindByAccountID(ctx context.Context, accountID int64) (*authmodels.MFACredential, error) {
	credential := new(authmodels.MFACredential)
...
func (r *MFACredentialRepository) UpdateLastUsedAt(ctx context.Context, id int64, when time.Time) error {
	credential := &authmodels.MFACredential{Model: modelBase.Model{ID: id}, LastUsedAt: &when}
...
func (r *MFACredentialRepository) DeleteByAccountID(ctx context.Context, accountID int64) error {
	_, err := base.GetDB(ctx, r.db).NewDelete().
		Model((*authmodels.MFACredential)(nil)).
		ModelTableExpr(mfaCredentialTable).
		Where("account_id = ?", accountID).
		Exec(ctx)
	if err != nil {
...
func (r *MFACredentialRepository) List(ctx context.Context, filters map[string]interface{}) ([]*authmodels.MFACredential, error) {
	var credentials []*authmodels.MFACredential
