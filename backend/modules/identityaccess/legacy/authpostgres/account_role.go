...
type AccountRoleRepository struct {
	*base.Repository[*authmodels.AccountRole]
	db *bun.DB
}

// NewAccountRoleRepository creates a new AccountRoleRepository
func NewAccountRoleRepository(db *bun.DB) authmodels.AccountRoleRepository {
	repo := base.NewRepository[*authmodels.AccountRole](db, accountRoleTable, "AccountRole")
...
func (r *AccountRoleRepository) FindByAccountID(ctx context.Context, accountID int64) ([]*authmodels.AccountRole, error) {
	var accountRoles []*authmodels.AccountRole
...
func (r *AccountRoleRepository) FindByRoleID(ctx context.Context, roleID int64) ([]*authmodels.AccountRole, error) {
	return r.List(ctx, map[string]any{"role_id": roleID})
}

// FindByAccountAndRole retrieves a specific account-role mapping
func (r *AccountRoleRepository) FindByAccountAndRole(ctx context.Context, accountID, roleID int64) (*authmodels.AccountRole, error) {
	accountRole := new(authmodels.AccountRole)
	query := base.GetDB(ctx, r.db).NewSelect().
		Model(accountRole).
		ModelTableExpr(accountRoleTableAlias).
...
func (r *AccountRoleRepository) Create(ctx context.Context, accountRole *authmodels.AccountRole) error {
	if accountRole == nil {
...
func (r *AccountRoleRepository) Update(ctx context.Context, accountRole *authmodels.AccountRole) error {
	if accountRole == nil {
		return fmt.Errorf("account role cannot be nil")
	}
...
func (r *AccountRoleRepository) DeleteByAccountAndRole(ctx context.Context, accountID, roleID int64) error {
	query := base.GetDB(ctx, r.db).NewDelete().
		Model((*authmodels.AccountRole)(nil)).
		ModelTableExpr(accountRoleTableAlias).
		Where(`"account_role".account_id = ? AND "account_role".role_id = ?`, accountID, roleID)
