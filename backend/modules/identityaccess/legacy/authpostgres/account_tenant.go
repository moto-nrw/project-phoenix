...
func (r *AccountTenantRepository) FindActiveGuardianByAccountID(ctx context.Context, accountID int64) ([]authmodels.AccountTenant, error) {
	var items []authmodels.AccountTenant
	err := base.GetDB(ctx, r.db).NewSelect().
		Model(&items).
		ModelTableExpr(accountTenantTableAlias).
...
func (r *AccountTenantRepository) ListAccountsByTenantID(ctx context.Context, tenantID int64) ([]authmodels.TenantAccountInfo, error) {
	db := base.GetDB(ctx, r.db)

	var accounts []authmodels.TenantAccountInfo
	err := db.NewSelect().
	...
	var invitations []authmodels.TenantAccountInfo
...
func (r *AccountTenantRepository) scanPendingInvitationsTenant(ctx context.Context, db bun.IDB, tenantID int64, out *[]authmodels.TenantAccountInfo) error {
	return db.NewSelect().
		...
		)`, tenantID, authmodels.AccountTenantStatusActive).
		Scan(ctx, out)
}
...
func (r *AccountTenantRepository) ListAccountsByOrganizationID(ctx context.Context, organizationID int64) ([]authmodels.OrgAccountInfo, error) {
...
// plus pending invitations. Each result includes school_id and school_name for context.
func (r *AccountTenantRepository) ListAllAccounts(ctx context.Context) ([]authmodels.OrgAccountInfo, error) {
...
func (r *AccountTenantRepository) ListAccountsBySchoolIDs(ctx context.Context, schoolIDs []int64) ([]authmodels.OrgAccountInfo, error) {
	if len(schoolIDs) == 0 {
		return []authmodels.OrgAccountInfo{}, nil
	}
	db := base.GetDB(ctx, r.db)
...
func (r *AccountTenantRepository) queryOrgAccounts(ctx context.Context, db bun.IDB, schoolIDs []int64) ([]authmodels.OrgAccountInfo, error) {
	var accounts []authmodels.OrgAccountInfo
	q := db.NewSelect().
...
func (r *AccountTenantRepository) queryOrgInvitations(ctx context.Context, db bun.IDB, schoolIDs []int64) ([]authmodels.OrgAccountInfo, error) {
	var invitations []authmodels.OrgAccountInfo
	q := db.NewSelect().
	...
	err := q.
		...
		)`, authmodels.AccountTenantStatusActive).
