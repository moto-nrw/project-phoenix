...
type RoleRepository struct {
	*base.Repository[*authmodels.Role]
	db *bun.DB
}

// NewRoleRepository creates a new RoleRepository
func NewRoleRepository(db *bun.DB) authmodels.RoleRepository {
	repo := base.NewRepository[*authmodels.Role](db, roleTable, "Role")
...
func (r *RoleRepository) FindByName(ctx context.Context, name string) (*authmodels.Role, error) {
	role := new(authmodels.Role)
...
func (r *RoleRepository) FindByAccountID(ctx context.Context, accountID int64) ([]*authmodels.Role, error) {
...
func (r *RoleRepository) FindByID(ctx context.Context, id any) (*authmodels.Role, error) {
	role := new(authmodels.Role)
	query := base.GetDB(ctx, r.db).NewSelect().
		Model(role).
		ModelTableExpr(roleTableAlias).
...
func (r *RoleRepository) FindByIDForUpdate(ctx context.Context, id int64) (*authmodels.Role, error) {
	role := new(authmodels.Role)
	query := base.GetDB(ctx, r.db).NewSelect().
		Model(role).
		ModelTableExpr(roleTableAlias).
...
func (r *RoleRepository) Update(ctx context.Context, role *authmodels.Role) error {
	if role == nil {
...
func (r *RoleRepository) List(ctx context.Context, filters map[string]interface{}) ([]*authmodels.Role, error) {
	var roles []*authmodels.Role
	query := base.GetDB(ctx, r.db).NewSelect().
		Model(&roles).
		ModelTableExpr(roleTableAlias)
