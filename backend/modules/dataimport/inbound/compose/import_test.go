...
type testContext struct {
	db       *bun.DB
	student  func(context.Context, int64) (peopledirectory.EnrollmentRecord, error)
...
func setupImportRoute(t *testing.T) *testContext {
	...
	resource := importAPI.NewResource(importAPI.Dependencies{Students: svc.Import, Staff: svc.StaffImport, ClassList: svc.ClassListImport, Files: fileformat.Decoder{}, Runtime: importCompose.HTTPRuntime(db, svc.PeopleDirectory, svc.Membership, nil)})

	return &testContext{
		db: db,
		student: func(ctx context.Context, id int64) (peopledirectory.EnrollmentRecord, error) {
			return svc.PeopleDirectory.ReadEnrollmentStudent(ctx, id, "")
		},
		resource: resource,
	}
}
...
func TestImportStudents_PersistsDepartureFromGehweise(t *testing.T) {
	...
	assert.Equal(t, "bus", hydrated.DepartureDays["mon"])
	assert.Equal(t, "pickup", hydrated.DepartureDays["wed"])
	assert.True(t, hydrated.BusDays["mon"], "derived bus_days mirror")
	assert.True(t, hydrated.PickupDays["wed"], "derived pickup_days mirror")
}
...
func TestImportStudents_LegacyTemplateStillImports(t *testing.T) {
	...
	require.NoError(t, err)
	for _, day := range []string{"mon", "tue", "wed", "thu", "fri"} {
		assert.Equal(t, "bus", hydrated.DepartureDays[day],
...
func TestImportStudents_PersistsEnrollmentDatesAndStatus(t *testing.T) {
	...
	read := func(firstName string) peopledirectory.EnrollmentRecord {
		...
		record, err := tc.student(testpkg.Ctx(t), id)
		require.NoError(t, err)
		return record
	}

	future := read("Zukunft")
	require.NotEmpty(t, future.EnrolledFrom, "enrolled_from must be persisted")
	assert.Equal(t, "2099-08-01", future.EnrolledFrom)
	require.NotEmpty(t, future.EnrolledUntil, "enrolled_until must be persisted")
	assert.Equal(t, "2100-07-31", future.EnrolledUntil)
	assert.Equal(t, "pending", future.Status,
		"a future enrollment start must be imported as pending")

	active := read("Aktiv")
	require.NotEmpty(t, active.EnrolledFrom, "enrolled_from must be persisted")
	assert.Equal(t, "2020-08-01", active.EnrolledFrom)
...
func TestImportStudents_PersistsConsentDates(t *testing.T) {
	...
	record, err := tc.student(testpkg.Ctx(t), student.ID)
	require.NoError(t, err)

	require.NotNil(t, record.AGBAcceptedAt, "AGB consent date must be persisted")
	assert.Equal(t, "2024-08-01", record.AGBAcceptedAt.Format("2006-01-02"))
	require.NotNil(t, record.DataProcessingAcceptedAt)
	assert.Equal(t, "2024-08-02", record.DataProcessingAcceptedAt.Format("2006-01-02"))
	require.NotNil(t, record.EmailContactAcceptedAt)
	assert.Equal(t, "2024-08-03", record.EmailContactAcceptedAt.Format("2006-01-02"))
	require.NotNil(t, record.PhotoConsentGivenAt)
	assert.Equal(t, "2024-08-04", record.PhotoConsentGivenAt.Format("2006-01-02"))
	assert.Nil(t, record.PhotoConsentGivenBy, "photo_consent_given_by must be left NULL on import")
}
...
func findImportAuditRecords(t *testing.T, db *bun.DB, filename string) []*importAuditRecord {
	t.Helper()
	var records []*importAuditRecord
	require.NoError(t, db.NewSelect().
		Model(&records).
		Column("data_import.entity_type", "data_import.dry_run", "data_import.imported_by", "data_import.created_count").
