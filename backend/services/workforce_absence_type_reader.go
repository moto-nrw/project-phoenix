...
func AbsenceTypes(catalog workforce.AbsenceTypeQuery) active.AbsenceTypeReader {
	if catalog == nil {
...
func (r absenceTypeReader) PreviewAllowanceBooking(ctx context.Context, staffID, typeID int64, start, end timezone.Date, halfDay bool) ([]*active.AbsenceTypeAllowanceSummary, error) {
	...
	result := make([]*active.AbsenceTypeAllowanceSummary, 0, len(values))
	for _, value := range values {
		result = append(result, &active.AbsenceTypeAllowanceSummary{
