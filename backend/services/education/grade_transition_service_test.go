...
func TestGradeTransitionWorkflow_Apply(t *testing.T) {
	...
	t.Run("apply transition promotes students", func(t *testing.T) {
		...
		err = db.NewSelect().
			...
			Column("school_class").
			Where("deleted_at IS NULL").Where("student_profile_id = ?", student1.ID).
...
func TestGradeTransitionWorkflow_Apply_CascadingGraduation(t *testing.T) {
	...
	readStudent := func(id int64) (string, string) {
		...
		err := db.NewSelect().
			...
			ColumnExpr("status").
			Where("deleted_at IS NULL").Where("student_profile_id = ?", id).
...
func TestGradeTransitionWorkflow_Revert_ReconcilesOnlyReactivatedStudents(t *testing.T) {
	...
	_, err = db.NewRaw(`UPDATE users.student_school_memberships SET status = 'inactive' WHERE student_profile_id = ? AND deleted_at IS NULL`, handChanged.ID).
		Exec(ctx)
	require.NoError(t, err)
...
func TestGradeTransitionWorkflow_Revert_SkipsRosterReplayForNonActiveRestores(t *testing.T) {
	...
	_, err := db.NewRaw(`UPDATE users.student_school_memberships SET status = 'pending' WHERE student_profile_id = ? AND deleted_at IS NULL`, pendingChild.ID).
		Exec(ctx)
	require.NoError(t, err)
