...
func (r *WorkScheduleTargets) TargetsForStaff(ctx context.Context, id int64, from, to timezone.Date) (active.WorkScheduleTargets, error) {
...
func (r *WorkScheduleTargets) TargetsByStaff(ctx context.Context, ids []int64, from, to timezone.Date) (map[int64]active.WorkScheduleTargets, error) {
	...
	result := make(map[int64]active.WorkScheduleTargets, len(byStaff))
...
func workScheduleTargets(rows []*config.StaffWorkSchedule) active.WorkScheduleTargets {
	return active.WorkScheduleTargets{HasEntries: len(rows) > 0, DailyTarget: func(anchor *timezone.Date, date timezone.Date) (int, bool) {
