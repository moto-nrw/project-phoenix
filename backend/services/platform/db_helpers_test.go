...
func TestIsUniqueViolation_WrappedInDatabaseError_UniqueViolation(t *testing.T) {
	...
	assert.True(t, isUniqueViolation(dbErr), "should detect unique violation wrapped in DatabaseError")
}
...
func TestIsUniqueViolation_DirectPgError_UniqueViolation(t *testing.T) {
	...
	pgErr := newPgError("23505")
	assert.True(t, isUniqueViolation(pgErr), "should detect unique violation from direct pgdriver.Error")
}
...
func TestIsUniqueViolation_NonUniqueConstraintPgError(t *testing.T) {
	...
	assert.False(t, isUniqueViolation(pgErr), "foreign key violation should not be detected as unique violation")
}
...
func TestIsUniqueViolation_DatabaseErrorWrappingGenericError(t *testing.T) {
	...
	assert.False(t, isUniqueViolation(dbErr), "generic error wrapped in DatabaseError should return false")
}
...
func TestIsUniqueViolation_DatabaseErrorWrappingNilError(t *testing.T) {
	...
	assert.False(t, isUniqueViolation(dbErr), "DatabaseError with nil inner error should return false")
}
