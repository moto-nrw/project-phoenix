...
// Operator invitation token validity decisions (issue #586, Rule 12: models
// hold data, not decisions). Expiry and consumability are wall-clock policy
// decisions, so they live next to the flow that consumes them with the clock
// injected instead of calling time.Now() inside the model. The production
// read paths already filter expiry and use in SQL (FindValidByToken); these
// helpers are the in-memory equivalents for non-DB decisions.
