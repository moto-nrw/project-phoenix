...
func sameDayCutoff(t *testing.T, clock string, now time.Time) schedule.SameDayCutoff {
	t.Helper()
	cutoff, err := schedule.NewSameDayCutoff(clock, now)
...
func TestNewSameDayCutoffRejectsMalformedClock(t *testing.T) {
	...
	for _, clock := range []string{"11", "25:00", "11:60", "elf"} {
		_, err := schedule.NewSameDayCutoff(clock, berlinAt(2026, 8, 24, 10, 0, 0))
		require.Error(t, err, clock)
	}
	// A hand-built value that bypassed validation fails closed for today only.
	broken := schedule.SameDayCutoff{Clock: "elf", Now: berlinAt(2026, 8, 24, 10, 0, 0)}
