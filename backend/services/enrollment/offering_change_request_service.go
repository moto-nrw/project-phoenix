...
func (s *offeringChangeRequestService) EarliestEffectiveFrom(ctx context.Context) (timezone.Date, error) {
	...
	return EarliestOfferingChangeDate(s.Today(), leadDays), nil
}
...
func (s *offeringChangeRequestService) keepCompleteWithdrawalStatus(ctx context.Context, studentID int64) (bool, error) {
	...
	if student.CareEndedOn(s.Today()) {
		return true, nil
	}
...
func (s *offeringChangeRequestService) ListPending(ctx context.Context, filters modelBase.RequestQueueFilters) ([]*OfferingChangeView, *usersService.HistoryCursor, error) {
	...
	for _, row := range visibleRows {
		...
		// filed.
		reviewRow.EffectiveFrom = appliedOfferingChangeDate(row.EffectiveFrom, s.Today())
...
func (s *offeringChangeRequestService) Decide(ctx context.Context, input DecideOfferingChangeInput) error {
	...
	if input.EffectiveFrom == nil &&
		usersService.ParentRequestIsPast(row.EffectiveFrom, s.Today()) {
...
func (s *offeringChangeRequestService) PreviewDecision(
	...
	if student == nil || student.IsAlumnus() || student.CareEndedOn(s.Today()) {
		return nil, enrollmentModels.ErrOfferingChangeNotFound
	}
	allowed, authErr := s.canReviewStudent(ctx, student)
	if authErr != nil {
...
func (s *offeringChangeRequestService) payloadDecisionDiff(
	...
	current, err := s.RequestChildOfferingRepo.ListByRequestChildIDAtDate(
		ctx, row.RequestChildID, appliedOfferingChangeDate(row.EffectiveFrom, s.Today()),
	)
	if err != nil {
...
func (s *offeringChangeRequestService) applyApproved(
	...
	effectiveFrom, err := confirmedEffectiveFrom(input.EffectiveFrom, row.EffectiveFrom, phase, s.Today())
	if err != nil {
		return nil, err
	}
...
func (s *offeringChangeRequestService) materializeDecisionSelections(
	...
	rowCopy.EffectiveFrom, err = confirmedEffectiveFrom(effectiveFrom, rowCopy.EffectiveFrom, phase, s.Today())
	if err != nil {
		return nil, err
	}
	allowCompleteWithdrawal, err := s.approvalAllowsCompleteWithdrawal(ctx)
...
func (s *offeringChangeRequestService) MarkDone(
	...
	if !usersService.ParentRequestIsPast(row.EffectiveFrom, s.Today()) {
