...
func TestPasskeyRegistration_RefusedAttestationSpendsTheCeremony(t *testing.T) {
	...
	options, err := module.Passkeys.BeginAccountPasskeyRegistration(ctx, auth.AccountPasskeyRegistrationStart{
	...
	_, err = module.Passkeys.FinishAccountPasskeyRegistration(ctx, auth.AccountPasskeyRegistrationFinish{
		AccountID:          account.ID,
	...
	require.ErrorIs(t, err, auth.ErrPasskeySessionInvalid,
