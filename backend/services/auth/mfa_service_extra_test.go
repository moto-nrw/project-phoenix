...
func TestMFAService_VerifyCodeForAccount_NoActiveChallenge_ReturnsInvalid(t *testing.T) {
	...
	err := svc.VerifyMFACodeForAccount(context.Background(), accID, 0, "123456", auth.MFAChallengeScopeTenant)
...
func TestMFAService_VerifyCodeForAccount_WrongAccount_ReturnsInvalid(t *testing.T) {
	...
	err := svc.VerifyMFACodeForAccount(context.Background(), 99999999, 0, "123456", auth.MFAChallengeScopeTenant)
...
func TestMFAService_VerifyCodeForAccount_WrongCode_RecordsFailureAndReturnsInvalid(t *testing.T) {
	...
	_, err := svc.StartMFAChallenge(context.Background(), accID, 0, auth.MFAChallengeScopeTenant, net.ParseIP("127.0.0.1"))
	require.NoError(t, err)

	err = svc.VerifyMFACodeForAccount(context.Background(), accID, 0, "000000", auth.MFAChallengeScopeTenant)
...
func TestMFAService_ResendChallenge_HappyPath(t *testing.T) {
	...
	token, err := svc.StartMFAChallenge(context.Background(), accID, 0, auth.MFAChallengeScopeTenant, net.ParseIP("127.0.0.1"))
	require.NoError(t, err)
...
func TestMFAService_StartChallenge_RateLimitLookupFails_IssuesNoCode(t *testing.T) {
	...
	challengeToken, err := svc.StartMFAChallenge(
		context.Background(), acc.ID, 0, auth.MFAChallengeScopeTenant, net.ParseIP("127.0.0.1"),
	)
