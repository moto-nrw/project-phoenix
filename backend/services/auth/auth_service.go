...
func (s *Service) Register(ctx context.Context, email, username, name, password string, roleID *int64) (*auth.Account, error) {
	...
	_, err := s.repos.Account.FindByEmail(ctx, email)
	if err == nil {
		return nil, &AuthError{Op: "register", Err: ErrEmailAlreadyExists}
	}

	// Check if username already exists
	_, err = s.repos.Account.FindByUsername(ctx, username)
	if err == nil {
	...
	err = s.txHandler.RunInTx(ctx, func(ctx context.Context, tx bun.Tx) error {
		...
		if err := txService.(*Service).repos.Account.Create(ctx, account); err != nil {
			return err
		}
		...
		if err := txService.(*Service).repos.AccountRole.Create(ctx, accountRole); err != nil {
