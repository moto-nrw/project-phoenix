...
func TestMFAService_EnrollDisableLifecycle(t *testing.T) {
	...
	require.ErrorIs(t, svc.EnrollMFA(ctx, acc.ID), auth.ErrMFAAlreadyEnrolled)
...
func TestMFAService_StartAndVerifyChallenge(t *testing.T) {
	...
	tokenString, err := svc.StartMFAChallenge(ctx, acc.ID, 0, auth.MFAChallengeScopeTenant, net.ParseIP("203.0.113.42"))
	require.NoError(t, err)
	require.NotEmpty(t, tokenString)

	// Active challenge row exists.
	active, err := repos.MFAEmailChallenge.FindActiveByAccountIDInScope(ctx, acc.ID, 0, auth.MFAChallengeScopeTenant)
	require.NoError(t, err)
