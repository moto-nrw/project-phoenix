...
func TestAuthService_Register_Extended(t *testing.T) {
	...
	t.Run("returns error for empty username", func(t *testing.T) {
		// ACT
		_, err := service.Register(ctx, fmt.Sprintf("empty-user-%d@test.local", time.Now().UnixNano()), "", "Test1234%", nil)
...
func TestAuthService_Logout_Extended(t *testing.T) {
	...
	t.Run("is idempotent - double logout succeeds", func(t *testing.T) {
		...
		require.Error(t, err) // Token no longer exists, but shouldn't panic
	})
}
