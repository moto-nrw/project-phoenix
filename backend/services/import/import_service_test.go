...
func TestNewImportService(t *testing.T) {
	...
	t.Run("creates service with config", func(t *testing.T) {
		...
		// ACT
		service := NewImportService[testRow](config, ImportRuntime{Transactions: newTestTransactions()})
...
func TestImportService_RecordAuditInTransaction(t *testing.T) {
	...
	service := NewImportService[testRow](&mockImportConfig{}, ImportRuntime{Transactions: newTestTransactions(), Audit: audit})
	...
	unwired := NewImportService[testRow](&mockImportConfig{}, ImportRuntime{Transactions: newTestTransactions()})
