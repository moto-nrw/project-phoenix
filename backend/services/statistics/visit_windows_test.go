...
func TestVisitWindowsUseEligibilityAndBerlinDayBoundaries(t *testing.T) {
	...
	student := &ReportStudent{ID: 7, EnrolledOn: func(day, reportToday timezone.Date) bool {
		require.Equal(t, today, reportToday)
		return day != first.AddDays(2)
	}}
	windows := visitWindows([]*ReportStudent{nil, {}, student}, first, today, today)
	require.Equal(t, []StudentVisitWindow{
