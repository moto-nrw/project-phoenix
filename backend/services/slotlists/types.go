...
type Result struct {
	...
	// deferred set — so a rowless selected slot cancelled between the verified
	// preview and the export rebuild would change the printed header while the
	// row/counter hash stayed equal and slipped past the 409 drift guard (#1565
	// review pass 7). Precomputed in BuildList because the header needs Params
	// (the slot selection, classes, grouping); unexported so it never serializes.
