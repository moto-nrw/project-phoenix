...
func TestFilterStudentsEligibleOnDate_UsesEnrollmentWindowForPastDates(t *testing.T) {
	...
	filtered := filterStudentsEligibleOnDate([]*userModels.Student{eligible, laterEnrolled, departed}, date)

	require.Len(t, filtered, 1)
	assert.Same(t, eligible, filtered[0])
}

func TestFilterStudentsEligibleOnDate_ExcludesFutureActiveStudentToday(t *testing.T) {
	...
	require.Len(t, filtered, 1)
	assert.Equal(t, today, *filtered[0].EnrolledFrom)
}
