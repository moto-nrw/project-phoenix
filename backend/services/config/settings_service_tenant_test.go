...
func TestResolveBoolForTenant_RegistryDefault_NoOverride(t *testing.T) {
	...
	svc := configSvc.NewSettingsService(newInMemoryValueRepo(), noopAuditRepo{}, nil, testpkg.SettingsRuntime(t, db), slog.Default())
...
func TestResolveBoolForTenant_TenantOverrideWins(t *testing.T) {
	...
	svc := configSvc.NewSettingsService(repo, noopAuditRepo{}, nil, testpkg.SettingsRuntime(t, db), slog.Default())
	testpkg.SetTenantRuntime(t, svc, db)
