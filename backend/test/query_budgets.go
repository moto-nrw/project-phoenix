...
var queryBudgets = map[string]queryBudget{
	...
	// 4 rows each. Every read resolves ids at its owner and loads groups in
	// one batch; without an ambient transaction School Membership and
	// Workforce each open their own tenant transaction.
	"modules.schoolstructure.staff_group_reads": {max: 13},
