...
// CreateTestInvitationTokenWithDetails creates an invitation token with first/last name.
func CreateTestInvitationTokenWithDetails(tb testing.TB, db *bun.DB, email string, roleID, createdBy int64, expiresAt time.Time, firstName, lastName string) *auth.InvitationToken {
	tb.Helper()
	...
	err := db.NewInsert().
		...
		ModelTableExpr(`auth.invitation_tokens`).
		Scan(ctx)
	require.NoError(tb, err, "Failed to create test invitation token with details")
