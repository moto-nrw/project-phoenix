...
// per ready school. It runs under the demo lease, so it is the only one.
...
func runDemoSchools(ctx context.Context, schools *backendapi.DemoRuntime, baseURL, heartbeat string, once bool) error {
	...
	scheduler := &demoScheduler{
		...
		ticking: map[string]bool{}, seeds: make(chan struct{}, demoSeedWorkers),
	}
...
// startTickers keeps one ticker per ready school.
func (s *demoScheduler) startTickers(ctx context.Context) error {
	...
	for _, slug := range slugs {
		...
		s.work.Go(func() {
			...
			if err == nil {
				err = school.run(ctx, false, false, func() {})
			}
			if err != nil && ctx.Err() == nil {
