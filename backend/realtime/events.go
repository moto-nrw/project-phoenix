...
const (
	...
	// EventSubstitutionChanged is a tenant-wide signal that a group substitution
	// row changed: an admin Vertretung was created, edited or deleted, or a
	// group leader handed their group over to a colleague (Gruppenübergabe) or
	// took that handover back. Both flows write education.group_substitution,
	// which decides WHICH groups a staff member may open in "Meine Gruppe".
	//
	// It exists BECAUSE the receiving staff member's own client never made the
	// write and no other event covers it: a handover changes no attendance, no
	// activity session and no timetable block, so none of the check-in,
	// activity or instance events fire. Until #2083 the visibility rode along
	// by accident — the OGS page's BFF was refetched on every check-in
	// anywhere in the school — and after that refetch herd was removed, a
	// group handed to someone stayed invisible in their open tab until the
	// five-minute safety poll or a manual reload (#2084).
	//
	// Carries no staff identifier: the event reaches every staff client of the
	// tenant, and naming the substitute would tell colleagues outside the
	// affected group who is standing in for whom. Clients refetch their own
	// access-filtered views (Source names the emitting flow for log review).
	EventSubstitutionChanged EventType = "substitution_changed"
