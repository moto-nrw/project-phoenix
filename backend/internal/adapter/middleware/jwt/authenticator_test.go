...
func TestAuthenticator_WrongSignature(t *testing.T) {
	...
	auth1, err := NewTokenAuthWithSecret("different-secret-32-chars!!!!!!!", viper.GetDuration("auth_jwt_expiry"), viper.GetDuration("auth_jwt_refresh_expiry"))
	require.NoError(t, err)
	...
	auth2, err := NewTokenAuthWithSecret(testSecret, viper.GetDuration("auth_jwt_expiry"), viper.GetDuration("auth_jwt_refresh_expiry"))
	require.NoError(t, err)
