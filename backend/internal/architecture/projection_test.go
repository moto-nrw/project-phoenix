...
func TestTargetProjectionContainsOnlyPolicyAllowedOwnerEdges(t *testing.T) {
	...
	projection := TargetProjection(policy)
...
func TestTargetProjectionIncludesDeclaredOwnersWithoutCurrentPackages(t *testing.T) {
	...
	policy := loadProjectionPolicy(t)
	policy.Owners = append(policy.Owners, Owner{ID: "future", Kind: "domain"})
...
func TestTargetProjectionExcludesNonModuleOwners(t *testing.T) {
	...
	policy := loadProjectionPolicy(t)
	policy.Owners = append(policy.Owners, Owner{ID: "http-adapter", Kind: "inbound"})
	policy.Packages = append(policy.Packages, Package{Path: "http", Owner: "http-adapter", Role: "application"})
	projection := TargetProjection(policy)
...
func TestMigrationProjectionPreservesEveryStatusBetweenTheSameOwners(t *testing.T) {
	...
	policy, graph, baseline := loadProjectionInputs(t)
	legacy := Violation{
		Scope: ScopeProduction, Rule: "tables.foreign-read",
		Source: policy.ModulePath + "/alpha", Target: "beta.legacy",
	}
	current := Violation{
		Scope: ScopeProduction, Rule: "tables.foreign-write",
		Source: policy.ModulePath + "/alpha", Target: "beta.current",
	}
	policy.DataObjects = append(policy.DataObjects,
		DataObject{Name: legacy.Target, WriteOwner: "beta"},
		DataObject{Name: current.Target, WriteOwner: "beta"},
	)
	graph.SemanticViolations = append(graph.SemanticViolations, legacy, current)
	baseline.Entries = append(baseline.Entries, LegacyEntry{Violation: legacy, Issue: "https://github.com/moto-nrw/project-phoenix/issues/2584"})
	...
	if want := []ProjectionStatus{ProjectionAllowed, ProjectionLegacy, ProjectionNew}; !slices.Equal(statuses, want) {
...
func TestMigrationProjectionDrawsExternalAndUnclassifiedViolations(t *testing.T) {
	...
	graph.Edges = append(graph.Edges, Edge{
		Scope: ScopeProduction, Source: policy.ModulePath + "/alpha", Target: unknownExternal,
	})
...
func TestMigrationProjectionIncludesSemanticViolationsAndOwnership(t *testing.T) {
	...
	for _, violation := range projection.Violations {
		if violation.Key == semantic.Key() {
			found = violation.SourceOwner == "alpha" && violation.TargetOwner == "gamma" && violation.Status == ProjectionNew
		}
	}
	if !found {
...
func TestMigrationProjectionAddsSemanticTargetOwnersWithoutPackages(t *testing.T) {
	...
	policy, graph, baseline := loadProjectionInputs(t)
	policy.Owners = append(policy.Owners, Owner{ID: "future", Kind: "domain"})
	policy.DataObjects = append(policy.DataObjects, DataObject{Name: "future.records", WriteOwner: "future"})
	graph.SemanticViolations = append(graph.SemanticViolations, Violation{
		Scope: ScopeProduction, Rule: "tables.foreign-write",
	...
	if _, err := RenderSVG(projection); err != nil {
...
func TestSVGAndMachineProjectionAreByteStable(t *testing.T) {
	...
	firstJSON, err := MarshalProjection(projection)
	if err != nil {
		t.Fatalf("marshal projection: %v", err)
	}
	secondJSON, err := MarshalProjection(projection)
	if err != nil {
...
func TestSVGEdgesTerminateAtNodeBoundaries(t *testing.T) {
	...
	policy := loadProjectionPolicy(t)
	svg := renderValidSVG(t, TargetProjection(policy))
...
func TestDependenciesProjectionValidatesAndFiltersFocus(t *testing.T) {
	...
	policy, graph, baseline := loadProjectionInputs(t)
	module, err := DependenciesProjection(policy, graph, baseline, "module:beta")
	if err != nil {
	...
	pkg, err := DependenciesProjection(policy, graph, baseline, "package:alpha")
	if err != nil {
	...
	for focus, want := range map[string]string{
		...
		if _, err := DependenciesProjection(policy, graph, baseline, focus); err == nil || !strings.Contains(err.Error(), want) {
...
func TestDependenciesProjectionRejectsStalePolicyFocus(t *testing.T) {
	...
	policy, graph, baseline := loadProjectionInputs(t)
	policy.Owners = append(policy.Owners, Owner{ID: "future", Kind: "domain"})
	policy.Packages = append(policy.Packages, Package{Path: "future", Owner: "future", Role: "domain"})
	for _, focus := range []string{"module:future", "package:future"} {
		if _, err := DependenciesProjection(policy, graph, baseline, focus); err == nil || !strings.Contains(err.Error(), "current graph") {
...
func TestPackageDependenciesProjectionDrawsSemanticViolations(t *testing.T) {
	...
	graph.SemanticViolations = append(graph.SemanticViolations, semantic)
	projection, err := DependenciesProjection(policy, graph, baseline, "package:alpha")
	if err != nil {
...
func TestToolProjectionsComeFromPolicyAndBuildContext(t *testing.T) {
	...
	policy := loadProjectionPolicy(t)
	config := string(GoArchLintProjection(policy))
	...
	policy.Packages = append(policy.Packages, Package{Path: "future", Owner: "alpha", Role: "domain"})
	query, err := GodaQuery(policy, graph, "module:alpha")
	if err != nil {
		t.Fatalf("generate Goda query: %v", err)
	}
	policy.Owners = append(policy.Owners, Owner{ID: "future", Kind: "domain"})
	policy.Packages = append(policy.Packages, Package{Path: "future", Owner: "future", Role: "domain"})
	if _, err := GodaQuery(policy, graph, "package:future"); err == nil || !strings.Contains(err.Error(), "current graph") {
...
func loadProjectionPolicy(t *testing.T) *Policy {
	t.Helper()
	policy, err := LoadPolicy(fixturePath(t, "projection", "policy.json"))
	if err != nil {
...
func loadProjectionInputs(t *testing.T) (*Policy, *Graph, *LegacyManifest) {
	t.Helper()
	policy := loadProjectionPolicy(t)
	graph, err := LoadGraph(fixturePath(t, "projection"), policy)
	if err != nil {
		t.Fatalf("load projection graph: %v", err)
	}
	baseline, err := LoadLegacyManifest(fixturePath(t, "projection", "legacy.jsonl"))
	if err != nil {
...
func projectionEdgeSummaries(projection Projection) []string {
...
func projectionNodeIDs(projection Projection) []string {
...
func renderValidSVG(t *testing.T, projection Projection) []byte {
	t.Helper()
	result, err := RenderSVG(projection)
	if err != nil {
