...
type DataDeletionRepository interface {
	...
	FindByID(ctx context.Context, id any) (*DataDeletion, error)
	...
	List(ctx context.Context, filters map[string]any) ([]*DataDeletion, error)

	// Statistics methods
	GetDeletionStats(ctx context.Context, startDate time.Time) (map[string]any, error)
...
type AuthEventRepository interface {
	...
	FindByID(ctx context.Context, id any) (*AuthEvent, error)
	...
	List(ctx context.Context, filters map[string]any) ([]*AuthEvent, error)
}
...
type DataImportRepository interface {
	...
	List(ctx context.Context, filters map[string]any) ([]*DataImport, error)
}
