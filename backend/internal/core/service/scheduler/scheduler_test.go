...
func TestNewScheduler_NilServices(t *testing.T) {
	s := NewScheduler(nil, nil, nil, nil, Config{})
...
func TestNewScheduler_OnlyAuthService(t *testing.T) {
	...
	s := NewScheduler(nil, nil, auth, nil, Config{})
...
func TestNewScheduler_OnlyInvitationService(t *testing.T) {
	...
	s := NewScheduler(nil, nil, nil, invitations, Config{})
...
func TestScheduler_StartStop(t *testing.T) {
	...
	s := NewScheduler(nil, nil, nil, nil, cfg)
...
func TestScheduler_StartWithTokenCleanupOnly(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		s := NewScheduler(nil, nil, auth, nil, cfg)
		s.Start()
...
func TestScheduler_DisabledByConfig(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		cfg := Config{
			CleanupEnabled:        false,
			SessionEndEnabled:     false,
...
func TestScheduler_DefaultConfigValues(t *testing.T) {
	s := NewScheduler(nil, nil, nil, nil, Config{})

	// Default values should be set
	assert.Equal(t, 0, s.config.SessionCleanupIntervalMinutes)
	assert.Equal(t, 0, s.config.SessionAbandonedThresholdMinutes)
}
	...
	synctest.Test(t, func(t *testing.T) {
		s := &Scheduler{
			...
			config: Config{
				...
				SessionEndSchedule:       "invalid",
				SessionEndTimeoutMinutes: 30,
			},
		}
...
// NOTE: Session cleanup task tests are limited because they spawn goroutines with
// a 30-second delay before execution, which makes them flaky when the test suite
// takes longer than 30 seconds. Configuration validation is covered by Config
// validation tests and the general scheduler lifecycle tests.
...
func (m *mockActiveService) GetVisitsWithDisplayData(_ context.Context, _ int64) ([]activeService.VisitWithDisplayData, error) {
	return nil, nil
}
	...
	synctest.Test(t, func(t *testing.T) {
		s := &Scheduler{
			...
			config: Config{
				...
				SessionAbandonedThresholdMinutes: 30,
			},
		}
		...
		assert.Equal(t, 30, s.config.SessionCleanupIntervalMinutes)
	...
	synctest.Test(t, func(t *testing.T) {
		s := &Scheduler{
			...
			config: Config{
				...
				SessionAbandonedThresholdMinutes: 120,
			},
		}
		...
		// Verify config was parsed
		assert.Equal(t, 120, s.config.SessionAbandonedThresholdMinutes)
