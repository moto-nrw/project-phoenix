...
func TestAuthResource_token(t *testing.T) {
	...
	authStore.CreateOrUpdateTokenFn = func(a *jwt2.Token) error {
		return nil
	}
...
func TestAuthResource_refresh(t *testing.T) {
	...
	authStore.GetTokenFn = func(token string) (*jwt2.Token, error) {
		var err error
		var t jwt2.Token
	...
	authStore.DeleteTokenFn = func(t *jwt2.Token) error {
		return nil
	}
	...
	tests := []struct {
		...
		{"not_found", "not_found", 1, http.StatusUnauthorized, jwt2.ErrTokenExpired},
		{"expired", "expired", -1, http.StatusUnauthorized, jwt2.ErrTokenExpired},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// }
			refreshJWT := genRefreshJWT(jwt2.RefreshClaims{
				Token: tc.token,
				CommonClaims: jwt2.CommonClaims{
...
func TestAuthResource_logout(t *testing.T) {
	...
	authStore.DeleteTokenFn = func(a *jwt2.Token) error {
		return nil
	}
	...
	tests := []struct {
		...
		{"notfound", "notfound", 1, http.StatusUnauthorized, jwt2.ErrTokenExpired},
...
func genRefreshJWT(c jwt2.RefreshClaims) string {
	claims, _ := jwt2.ParseStructToMap(c)
