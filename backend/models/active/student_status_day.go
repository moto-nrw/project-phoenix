...
type StudentStatusDayRepository interface {
	...
	// FindActiveByStudentIDsAndDateRange returns the active and end-of-day
	// archived rows of many students within a date range, ordered by date,
	// then student, then newest report (#2288 absence overview).
	FindActiveByStudentIDsAndDateRange(ctx context.Context, studentIDs []int64, startDate, endDate timezone.Date) ([]*StudentStatusDay, error)
