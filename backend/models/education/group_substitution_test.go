...
func TestGroupSubstitution_EntityInterface(t *testing.T) {
	now := time.Now()
	gs := &GroupSubstitution{
		Model: base.Model{
			ID:        123,
	...
	t.Run("GetID", func(t *testing.T) {
		got := gs.GetID()
		if got != int64(123) {
			t.Errorf("GroupSubstitution.GetID() = %v, want %v", got, int64(123))
		}
	})
	...
	t.Run("GetCreatedAt", func(t *testing.T) {
		got := gs.GetCreatedAt()
		if !got.Equal(now) {
			t.Errorf("GroupSubstitution.GetCreatedAt() = %v, want %v", got, now)
		}
	})
	...
	t.Run("GetUpdatedAt", func(t *testing.T) {
		...
		if !got.Equal(expected) {
			t.Errorf("GroupSubstitution.GetUpdatedAt() = %v, want %v", got, expected)
		}
	})
}
