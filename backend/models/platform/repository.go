...
type AnnouncementViewRepository interface {
	...
	// Query unread announcements for a user (filtered by roles and full account_tenants membership)
	GetUnreadForUser(ctx context.Context, userID int64, userRoles []string) ([]*Announcement, error)

	// Count unread announcements for a user (filtered by roles and full account_tenants membership)
