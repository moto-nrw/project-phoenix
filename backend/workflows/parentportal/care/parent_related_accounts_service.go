...
func (s *Service) ListRelatedAccounts(ctx context.Context, accountID, studentID int64) ([]*RelatedAccount, error) {
	...
	txErr := InTenant(ctx, child.TenantID, func(txCtx context.Context) error {
		...
		for _, link := range links {
			account, err := s.relatedAccountForLink(txCtx, link, accountID, studentID)
			if err != nil {
				return err
			}
			if account == nil {
...
// relatedAccountForLink projects one guardian link into a RelatedAccount. It
// returns nil when the link's guardian profile no longer exists.
func (s *Service) relatedAccountForLink(txCtx context.Context, link *userModels.StudentGuardian, accountID, studentID int64) (*RelatedAccount, error) {
...
func (s *Service) relatedAccountStatus(ctx context.Context, profile *userModels.GuardianProfile, link *userModels.StudentGuardian, studentID int64) RelatedAccountStatus {
	...
	if s.hasOpenInvitationForStudent(ctx, profile.ID, studentID) {
...
// hasOpenInvitationForStudent reports whether the guardian profile holds an
// open invitation for this student. A lookup error counts as no invitation.
func (s *Service) hasOpenInvitationForStudent(ctx context.Context, guardianProfileID, studentID int64) bool {
	if s.GuardianInvitations == nil {
		return false
	}
	invitations, err := s.GuardianInvitations.ListByProfile(ctx, guardianProfileID)
	if err != nil {
		return false
	}
	now := time.Now()
...
func (s *Service) InviteRelatedAccount(ctx context.Context, accountID, studentID int64, email, firstName, lastName string, confirmRoleUpgrade bool) (*InviteRelatedAccountResult, error) {
	...
	requireApproval, err := s.inviteRequiresStaffApproval(ctx, child.TenantID)
	if err != nil {
		return nil, err
	}
...
// validateInviteEmail rejects a missing or malformed invite email.
func validateInviteEmail(email string) error {
	if strings.TrimSpace(email) == "" {
		return ErrEmailRequired
	}
	if _, err := mail.ParseAddress(strings.TrimSpace(email)); err != nil {
		return fmt.Errorf("%w: invalid email", ErrInvalidInviteInput)
	}
	return nil
}

// inviteRequiresStaffApproval resolves guardians.parent_invite_mode for the
// tenant: ErrInviteDisabled when parents may not invite, otherwise whether the
// invite waits for staff approval.
func (s *Service) inviteRequiresStaffApproval(ctx context.Context, tenantID int64) (bool, error) {
	mode, err := s.Settings.ResolveStringForTenant(ctx, tenantID, configModels.KeyGuardianParentInviteMode)
	if err != nil {
		return false, fmt.Errorf("parent: resolve invite mode: %w", err)
	}
	if mode == configModels.ParentInviteModeDisabled {
		return false, ErrInviteDisabled
	}
	return mode == configModels.ParentInviteModeStaffApproval, nil
}
...
func (s *Service) RemoveRelatedAccount(ctx context.Context, accountID, studentID, guardianProfileID int64) error {
	...
	return InTenant(ctx, child.TenantID, func(txCtx context.Context) error {
		if err := s.RequireCareRunningForUpdate(txCtx, studentID); err != nil {
			return err
		}
