...
func TestOwnExcusedRequests_HidesAnotherGuardiansRequest(t *testing.T) {
	...
	visible := visibleExcusedRequests(requests, 42, ownRequestsOnly{})
	require.Len(t, visible, 1)
	assert.Equal(t, "mine", visible[0].Note)
}

// ownRequestsOnly is the visibility of a family without any share: every
// guardian sees only the requests they submitted themselves.
type ownRequestsOnly struct{}

func (ownRequestsOnly) Allows(_ string, _ int64, accountID, submittedBy int64) bool {
	return accountID == submittedBy
}
