...
func buildParentService(t *testing.T, repo *stubEnrollmentRequestRepo) *parentService.Portal {
	t.Helper()
...
// portalLocalePeople serves the stub's locale write as People Directory's
// tenant-scoped command: the account has one profile in tenantID, and the
// stub's ErrGuardianProfileNotFound stands for an UPDATE that matched no row.
type portalLocalePeople struct {
	parentportalcompose.PeopleDirectory
	repo      *stubGuardianProfileRepo
	tenantIDs []int64
}

func (p portalLocalePeople) ListGuardiansByAccount(context.Context, []int64) ([]peopledirectory.Guardian, error) {
	guardians := make([]peopledirectory.Guardian, 0, len(p.tenantIDs))
	for _, tenantID := range p.tenantIDs {
		guardians = append(guardians, peopledirectory.Guardian{TenantID: tenantID})
	}
	return guardians, nil
}

func (p portalLocalePeople) SetGuardianPortalLocale(ctx context.Context, accountID int64, locale string) (int64, error) {
	err := p.repo.UpdatePortalLocaleByAccountID(ctx, accountID, locale)
...
func buildParentServiceWithGuardian(t *testing.T, repo *stubGuardianProfileRepo) *parentService.Portal {
	t.Helper()
	testpkg.SetupTestDB(t)
	return parentportalcompose.New(parentportalcompose.Dependencies{
		People:              portalLocalePeople{repo: repo, tenantIDs: []int64{testpkg.Tenant(t)}},
...
// A parent with a profile at two schools gets the language at both: People
// Directory writes one school per tenant unit of work, and every school the
// account has a profile in is written.
func TestService_UpdatePortalLocale_WritesEverySchool(t *testing.T) {
	t.Parallel()
	...
	repo := &stubGuardianProfileRepo{}
	svc := parentportalcompose.New(parentportalcompose.Dependencies{
		People:              portalLocalePeople{repo: repo, tenantIDs: []int64{testpkg.Tenant(t), otherTenant}},
		GuardianProfileRepo: repo,
		Logger:              slog.Default(),
	})
	...
	require.NoError(t, err)
	assert.Equal(t, "en", profile.Locale)
	assert.Equal(t, 2, repo.updateCallCount, "one write per school the account has a profile in")
	assert.Equal(t, "en", repo.gotUpdateLocale)
}
