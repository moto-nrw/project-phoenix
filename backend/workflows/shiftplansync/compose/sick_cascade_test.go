// Hermetic integration tests for the #1843 sick cascade and its wiring
// through the staff absence service.
//
// Covers:
...
type sickCascadeEnv struct {
	planSyncEnv
	// absences is the retained absence lifecycle the composition root binds
	// the cascade into; the end-to-end cases drive it.
	absences services.WorkforceTestModule
	ctx      context.Context
...
// sickCascadeNow fixes the clock every guard in this suite compares against.
func sickCascadeNow() time.Time {
	return time.Date(2026, 8, 24, 12, 0, 0, 0, time.UTC)
}

func buildSickCascadeEnv(t *testing.T) *sickCascadeEnv {
	t.Helper()
	return buildSickCascadeEnvWith(t, nil)
}

// buildSickCascadeEnvWith composes the workflow with an explicit SSE sink for
// the cascade, so a recording broadcaster sees exactly the cascade's events.
func buildSickCascadeEnvWith(t *testing.T, cascadeBroadcaster realtime.Broadcaster) *sickCascadeEnv {
	t.Helper()
	env := newPlanSyncEnv(t, cascadeBroadcaster, sickCascadeNow)
	absences, err := services.NewWorkforceTestModule(env.db, testpkg.TenantRuntime(t, env.db), sickCascadeNow)
	require.NoError(t, err)

	suffix := time.Now().UnixNano()
	room := testpkg.CreateTestRoom(t, env.db, fmt.Sprintf("Sick-Room-%d", suffix))
	subject := testpkg.CreateTestStaff(t, env.db, "Sick", fmt.Sprintf("Subject-%d", suffix))
	admin := testpkg.CreateTestStaff(t, env.db, "Sick", fmt.Sprintf("Admin-%d", suffix))
	sub := testpkg.CreateTestStaff(t, env.db, "Sick", fmt.Sprintf("Sub-%d", suffix))
	tmpl := testpkg.CreateTestActivityGroup(t, env.db, fmt.Sprintf("Sick-Tmpl-%d", suffix))
...
// clock renders a HH:MM fixture as the wall clock the Dienstplan contract
// carries (workforce.ClockLayout).
func clock(hhmm string) string { return hhmm + ":00" }

// createShift plans one staff shift through the row capability (fixture path,
// no planning-service validation).
func (e *sickCascadeEnv) createShift(t *testing.T, staffID int64, date timezone.Date, start, end string, mutate func(*workforce.StaffShift)) workforce.StaffShift {
	t.Helper()
	shift := workforce.StaffShift{
		StaffID:   staffID,
		Date:      date.String(),
	...
	created, err := e.shifts.CreateStaffShift(e.ctx, shift)
	require.NoError(t, err)
	return created
}
...
func (e *sickCascadeEnv) reloadShift(t *testing.T, id int64) workforce.StaffShift {
	t.Helper()
	shift, err := e.shifts.FindStaffShift(e.ctx, id)
	require.NoError(t, err)
	return shift
}
...
// sickCascadeInput builds the public cascade input for a full-day range.
func sickCascadeInput(subjectStaffID, actorStaffID, absenceID int64, start, end timezone.Date) workforce.SickCascadeInput {
	return workforce.SickCascadeInput{
...
func TestSickCascade_MarkSickForRange(t *testing.T) {
	...
	preCancelled := e.createShift(t, e.subject.ID, tomorrow, "13:00", "15:00", func(s *workforce.StaffShift) {
		s.Cancelled = true
		s.ChangeReason = &adminReason
	})
	colleagueGap := e.createShift(t, e.sub.ID, tomorrow, "15:00", "17:00", func(s *workforce.StaffShift) {
		s.Cancelled = true
	})
	coverOfColleague := e.createShift(t, e.subject.ID, tomorrow, "15:00", "16:00", func(s *workforce.StaffShift) {
	...
	e.inTx(t, func(ctx context.Context) error {
		resp, err := e.absences.StaffAbsence.CreateAbsenceFor(ctx, e.subject.ID, e.admin.ID, nil, services.CreateAbsenceRequest{
			AbsenceType: "sick",
	...
	absence, err := e.shifts.FindStaffAbsence(e.ctx, absenceID)
	require.NoError(t, err)
...
func TestSickCascade_PastShiftsRemainHistoricalDuringMarkAndReconcile(t *testing.T) {
	...
	e.inTx(t, func(ctx context.Context) error {
		return e.cascade.MarkSickForRange(ctx, sickCascadeInput(
			e.subject.ID, e.admin.ID, time.Now().UnixNano(), yesterday, yesterday))
	})
	...
	_, err := e.shifts.UpdateStaffShift(e.ctx, stored)
	require.NoError(t, err)

	before := sickCascadeInput(e.subject.ID, e.admin.ID, absenceID, yesterday, yesterday)
	after := before
	after.DateStart = tomorrow.String()
	after.DateEnd = tomorrow.String()
	e.inTx(t, func(ctx context.Context) error {
		return e.cascade.ReconcileSickRange(ctx, before, after)
	})
...
func TestSickCascade_ShiftOnlyChangesBroadcastTenantInvalidation(t *testing.T) {
	...
	e := buildSickCascadeEnvWith(t, broadcaster)
	...
	input := sickCascadeInput(e.subject.ID, e.admin.ID, time.Now().UnixNano(), dayOne, dayOne)

	e.inTx(t, func(ctx context.Context) error { return e.cascade.MarkSickForRange(ctx, input) })
	after := input
	after.DateStart = dayTwo.String()
	after.DateEnd = dayTwo.String()
	e.inTx(t, func(ctx context.Context) error {
		return e.cascade.ReconcileSickRange(ctx, input, after)
	})
	e.inTx(t, func(ctx context.Context) error { return e.cascade.ClearSickForRange(ctx, after) })
...
func TestSickCascade_ConcurrentOverlappingReportsSerializeBeforeOverlapRead(t *testing.T) {
	...
	go func() {
		lockerDone <- testpkg.WithTenantTx(t, context.Background(), e.db, e.tenantID, func(ctx context.Context, _ bun.Tx) error {
			if err := e.shifts.LockStaffAbsenceWrites(ctx, e.subject.ID); err != nil {
				return err
			}
			close(lockHeld)
			<-releaseLock
			_, err := e.absences.StaffAbsence.CreateAbsenceFor(ctx, e.subject.ID, e.admin.ID, nil, services.CreateAbsenceRequest{
				AbsenceType: "sick",
	...
	rows, err := e.shifts.ListStaffAbsences(e.ctx, workforce.StaffAbsenceFilter{
		StaffID: e.subject.ID, OverlapFrom: day.String(), OverlapTo: day.String(),
	})
	require.NoError(t, err)
...
func TestSickCascade_HalfDayRules(t *testing.T) {
	...
	e.inTx(t, func(ctx context.Context) error {
		_, err := e.absences.StaffAbsence.CreateAbsenceFor(ctx, e.subject.ID, e.subject.ID, nil, services.CreateAbsenceRequest{
			AbsenceType: "sick",
	...
	// Boundary half days of a range are skipped by the cascade itself.
	e.inTx(t, func(ctx context.Context) error {
		return e.cascade.MarkSickForRange(ctx, workforce.SickCascadeInput{
			SubjectStaffID: e.subject.ID,
			DateStart:      tomorrow.String(),
			DateEnd:        dayAfter.String(),
			SkipStartDay:   true,
			AbsenceID:      424242, // direct cascade call: provenance target not persisted
...
func TestSickCascade_ReconcileSickRangeAppliesOnlyDateDelta(t *testing.T) {
	...
	var absenceID int64
	e.inTx(t, func(ctx context.Context) error {
		created, err := e.absences.StaffAbsence.CreateAbsenceFor(ctx, e.subject.ID, e.admin.ID, nil, services.CreateAbsenceRequest{
			AbsenceType: "sick",
	...
	before := sickCascadeInput(e.subject.ID, e.admin.ID, absenceID, dayOne, dayTwo)
...
func TestSickCascade_UpdateRangeRollsBackWhenRemovedShiftCannotReactivate(t *testing.T) {
	...
	err := testpkg.WithTenantTx(t, context.Background(), e.db, e.tenantID, func(ctx context.Context, _ bun.Tx) error {
		_, updateErr := e.absences.StaffAbsence.UpdateAbsence(ctx, e.subject.ID, nil, absenceID, services.UpdateAbsenceRequest{
			DateStart: &newStart,
			DateEnd:   &newEnd,
		})
		return updateErr
	...
	absence, loadErr := e.shifts.FindStaffAbsence(e.ctx, absenceID)
	require.NoError(t, loadErr)
	assert.Equal(t, oldDay.String(), absence.DateStart, "absence update must roll back with its cascade")
	assert.Equal(t, oldDay.String(), absence.DateEnd)
...
func TestSickCascade_MarkWaitsForConcurrentShiftWrite(t *testing.T) {
	...
	input := sickCascadeInput(e.subject.ID, e.admin.ID, absenceID, day, day)
	...
	go func() {
		writerDone <- testpkg.WithTenantTx(t, context.Background(), e.db, e.tenantID, func(ctx context.Context, _ bun.Tx) error {
			if err := e.lock(ctx, e.subject.ID); err != nil {
				return err
			}
			shift, err := e.shifts.CreateStaffShift(ctx, workforce.StaffShift{
				StaffID: e.subject.ID, Date: day.String(), StartTime: clock("08:00"),
				EndTime: clock("09:00"), CreatedBy: e.admin.ID,
			})
			if err != nil {
				return err
			}
	...
	go func() {
		markDone <- testpkg.WithTenantTx(t, context.Background(), e.db, e.tenantID, func(ctx context.Context, _ bun.Tx) error {
			return e.cascade.MarkSickForRange(ctx, input)
		})
	}()
	select {
	case err := <-markDone:
	...
	stored := e.reloadShift(t, shiftID)
	assert.True(t, stored.Cancelled)
	require.NotNil(t, stored.SickAbsenceID)
	assert.Equal(t, absenceID, *stored.SickAbsenceID)
}
...
func TestSickCascade_ClearWaitsForConcurrentReplacement(t *testing.T) {
	...
	go func() {
		writerDone <- testpkg.WithTenantTx(t, context.Background(), e.db, e.tenantID, func(ctx context.Context, _ bun.Tx) error {
			...
			cover, err := e.shifts.CreateStaffShift(ctx, workforce.StaffShift{
				StaffID: e.sub.ID, Date: day.String(), StartTime: clock("08:00"),
				EndTime: clock("10:00"), CreatedBy: e.admin.ID, OriginShiftID: &origin.ID,
			})
			if err != nil {
				return err
			}
	...
	go func() {
		clearDone <- testpkg.WithTenantTx(t, context.Background(), e.db, e.tenantID, func(ctx context.Context, _ bun.Tx) error {
			return e.cascade.ClearSickForRange(ctx, input)
		})
	}()
	select {
	case err := <-clearDone:
	...
	_, err := e.shifts.FindStaffShift(e.ctx, coverID)
...
func TestSickCascade_ClearLocksCommittedReplacementStaffBeforeReversal(t *testing.T) {
	...
	origin := e.createShift(t, e.subject.ID, day, "08:00", "10:00", nil)
	absenceID := e.createSickAbsence(t, day, day)
	cover := e.createShift(t, e.sub.ID, day, "08:00", "10:00", func(shift *workforce.StaffShift) {
	...
	go func() {
		lockerDone <- testpkg.WithTenantTx(t, context.Background(), e.db, e.tenantID, func(ctx context.Context, _ bun.Tx) error {
			if err := e.lock(ctx, cover.StaffID); err != nil {
				return err
			}
			close(locked)
			<-release
	...
	_, err := e.shifts.FindStaffShift(e.ctx, cover.ID)
...
func TestSickCascade_ClearSickForRange(t *testing.T) {
	...
	cover := e.createShift(t, e.sub.ID, tomorrow, "13:00", "14:00", func(s *workforce.StaffShift) {
	...
	e.inTx(t, func(ctx context.Context) error {
		return e.absences.StaffAbsence.DeleteAbsenceFor(ctx, e.subject.ID, e.admin.ID, nil, absenceID)
	})

	// Absence row is gone.
	_, err = e.shifts.FindStaffAbsence(e.ctx, absenceID)
...
func TestSickCascade_DeleteDoesNotReactivateManuallyRetainedCancellation(t *testing.T) {
	...
	e.inTx(t, func(ctx context.Context) error {
		_, err := e.planning.ApplyCancellation(ctx, workforce.CancelStaffShift{
			ShiftID:      shift.ID,
			Cancelled:    true,
...
func TestSickCascade_ReassignSickStamps(t *testing.T) {
	...
	e.inTx(t, func(ctx context.Context) error {
		return e.cascade.ReassignSickStamps(ctx, absenceID, newOwner)
	})
