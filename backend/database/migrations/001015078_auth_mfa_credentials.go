...
const (
	authMFACredentialsVersion     = "1.15.78"
...
func authMFACredentialsUp(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.78: Creating auth.mfa_credentials table...")
...
func authMFACredentialsDown(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.78: Dropping auth.mfa_credentials...")
