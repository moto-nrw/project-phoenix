...
const (
	dropFeedbackSuggestionsVersion     = "1.15.312"
...
func init() {
	MigrationRegistry.Register(&Migration{
		...
		DependsOn:   []string{guardianEnrollmentAccountBackfillVersion},
	})
...
func dropFeedbackSuggestionsUp(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.312: Dropping feedback and suggestions...")
...
func dropFeedbackSuggestionsDown(_ context.Context, _ *bun.DB) error {
	fmt.Println("Migration 1.15.312 down: no-op — dropped feedback/suggestions data can only be restored from a backup")
	return nil
}
