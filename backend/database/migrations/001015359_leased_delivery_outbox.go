...
const (
	leasedDeliveryOutboxVersion     = "1.15.359"
...
func leasedDeliveryOutboxUp(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.359: Expanding durable delivery into fenced email and push outboxes...")
	if _, err := db.NewRaw(`
...
func leasedDeliveryOutboxDown(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.359: Restoring the legacy email outbox schema...")
	if _, err := db.NewRaw(`
