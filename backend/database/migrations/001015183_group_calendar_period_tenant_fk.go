...
const (
	groupCalendarPeriodTenantFKVersion     = "1.15.183"
...
func groupCalendarPeriodTenantFKUp(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.183: Enforcing tenant-safe group calendar-period references...")
	_, err := db.NewRaw(`
...
func groupCalendarPeriodTenantFKDown(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.183: Restoring the legacy group calendar-period reference...")
	_, err := db.NewRaw(`
		ALTER TABLE activities.groups
			DROP CONSTRAINT IF EXISTS fk_activities_groups_calendar_period_tenant,
