...
const (
	vacationWorkflowVersion     = "1.15.59"
...
func addVacationWorkflow(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.59: Adding vacation workflow fields + quota table...")
	...
	fmt.Println("Migration 1.15.59: vacation workflow ready")
	return tx.Commit()
}

func removeVacationWorkflow(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.59: Removing vacation workflow...")
	...
	fmt.Println("Migration 1.15.59 rollback complete")
	return tx.Commit()
}
