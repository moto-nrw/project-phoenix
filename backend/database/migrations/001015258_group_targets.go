...
func groupTargetsUp(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.258: Creating activities.group_targets...")
	_, err := db.NewRaw(`
...
func groupTargetsDown(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.258: Dropping activities.group_targets...")
	if _, err := db.NewRaw(`
