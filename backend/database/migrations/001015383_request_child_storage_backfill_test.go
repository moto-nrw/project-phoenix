...
func createRequestChildStorageFixture(t *testing.T, db *testpkg.DB) requestChildStorageFixture {
	...
	rows := []legacyOffering{
		...
		{child: firstChild, offering: first.ID, selected: `["mon","tue","wed"]`, manual: `["mon","wed"]`,
...
func requireVerifiedBookings(t *testing.T, db *testpkg.DB, tenantID int64) RequestChildStorageVerification {
	t.Helper()
	...
	require.Zero(t, verification.Mismatches(), "bookings and empty trusted-selection projection must match")
	if verification.SourceBookings > 0 {
		require.False(t, verification.Equal(), "booking parity cannot certify submission provenance")
		require.Positive(t, verification.UnresolvedOrigins)
	} else {
...
func TestRequestChildStorageBackfillBlocksUnprovenOrigins(t *testing.T) {
	t.Parallel()
	db := testpkg.SetupIsolatedTestDB(t)
	fixture := createRequestChildStorageFixture(t, db)
	report := runBackfill(t, db, RequestChildStorageBackfillOptions{})
	require.False(t, report.Complete, "surviving intervals do not establish immutable submission history")
	require.False(t, report.Verification.Equal())
	require.EqualValues(t, 5, report.Verification.TargetBookings, "effective bookings still copy")
	require.Zero(t, countRows(t, db, "enrollment.request_child_offering_selections", fixture.tenant), "never populate guessed submissions")
	require.ErrorContains(t, requestChildStorageBackfillUp(t.Context(), db), "did not reach")
}

func TestRequestChildStorageOriginProofCannotComeFromTargets(t *testing.T) {
	t.Parallel()
	db := testpkg.SetupIsolatedTestDB(t)
	fixture := createRequestChildStorageFixture(t, db)
	// Only a later approved interval survives before the first copy. Even a
	// singleton cannot establish the deleted original submission's fields.
	...
	require.EqualValues(t, 1, first.Verification.UnresolvedOrigins)
	...
	_, err = db.NewRaw(`INSERT INTO enrollment.request_child_offering_selections
		...
		FROM enrollment.request_child_offerings WHERE tenant_id = ?`, fixture.tenant).Exec(t.Context())
	require.NoError(t, err)
	...
	require.False(t, verified.Complete, "matching guessed targets and old success cannot become origin evidence")
	require.EqualValues(t, 1, verified.Verification.UnresolvedReasons[requestChildOriginReason])
	...
	require.Equal(t, 1, countRows(t, db, "enrollment.request_child_offering_selections", fixture.tenant), "verify-only changes evidence, not target data")
	...
	require.Equal(t, first.Verification.UnresolvedReasons, replayed.Verification.UnresolvedReasons)
	require.False(t, replayed.Complete)
	require.Zero(t, countRows(t, db, "enrollment.request_child_offering_selections", fixture.tenant))
	// Empty schools can still be proven complete; unresolved schools remain
	// visible in an all-school report and block automatic migration.
	report, err := RunRequestChildStorageBackfill(t.Context(), db, RequestChildStorageBackfillOptions{})
	require.NoError(t, err)
	require.Equal(t, []int64{fixture.tenant}, report.IncompleteTenants())
	var emptyComplete bool
	for _, tenant := range report.Tenants {
		if tenant.TenantID != fixture.tenant {
			emptyComplete = emptyComplete || tenant.Complete
		}
	}
	require.True(t, emptyComplete)
	require.ErrorContains(t, requestChildStorageBackfillUp(t.Context(), db), "did not reach")
}
...
func TestRequestChildStorageDoesNotTrustPartialOrForeignChangeHistory(t *testing.T) {
	...
	require.EqualValues(t, 3, report.Verification.UnresolvedOrigins)
	require.Zero(t, report.Verification.TargetSelections)
	requireVerifiedBookings(t, db, fixture.tenant)
	var changes int
...
func TestRequestChildStorageBackfillCopiesOwnedFields(t *testing.T) {
	...
	require.False(t, report.Complete, "legacy submission origins remain unproven")
	...
	assert.EqualValues(t, 5, report.RowsCopied, "five bookings; no guessed submission selections")
	assert.EqualValues(t, 0, report.RowsSkipped)
	assert.EqualValues(t, 5, report.Verification.SourceBookings)
	assert.Zero(t, report.Verification.SourceSelections)
	assert.EqualValues(t, 3, report.Verification.UnresolvedOrigins)
	assert.EqualValues(t, 3, report.Verification.UnresolvedReasons[requestChildOriginReason])
	assert.Zero(t, report.Verification.Mismatches())
	assert.Positive(t, report.Verification.OldestUnmigrated, "unresolved submission origins remain backlog despite booking parity")
	...
	// Null, empty, and populated interval payloads are all unproven originals.
	require.Zero(t, countRows(t, db, "enrollment.request_child_offering_selections", fixture.tenant))
...
func TestRequestChildStorageBackfillResumesAtEveryBatchBoundary(t *testing.T) {
	...
	for {
		...
		if err == nil {
			require.Equal(t, []int64{fixture.tenant}, report.IncompleteTenants(), "resume copies bookings but cannot establish submission history")
			break
		}
...
func TestRequestChildStorageBackfillReplaysCompletedBatchesWithoutEffect(t *testing.T) {
	...
	assert.Equal(t, first.Verification.UnresolvedOrigins, second.Verification.UnresolvedOrigins)
...
func TestRequestChildStorageBackfillWaitsOutApplicationLocks(t *testing.T) {
	...
	fixture := createRequestChildStorageFixture(t, db)
	runBackfill(t, db, RequestChildStorageBackfillOptions{})
	// A legacy change makes the copied booking stale; the reconcile must delete it.
	_, err := db.NewRaw(`UPDATE enrollment.request_child_offerings SET manual_selected_days = '["fri"]' WHERE id = ?`, fixture.legacyIDs[0]).Exec(t.Context())
	require.NoError(t, err)
	...
	assert.EqualValues(t, 1, report.RowsDeleted)
	requireVerifiedBookings(t, db, fixture.tenant)
}
...
func TestRequestChildStorageBackfillReconcilesChangedLegacyRows(t *testing.T) {
	...
	assert.EqualValues(t, 2, drift.Verification.MissingOrDifferent, "changed booking and new booking")
	assert.EqualValues(t, 1, drift.Verification.Orphans, "the deleted row leaves a booking; no guessed selection exists")
	...
	checkpoint := checkpointRow(t, db, fixture.tenant)
	assert.Equal(t, false, checkpoint["complete"], "verify-only invalidates acceptance when provenance is unresolved")
	assert.EqualValues(t, 3, checkpoint["mismatch_count"])
	assert.EqualValues(t, 3, checkpoint["verify_only_mismatch_count"])
	...
	assert.EqualValues(t, 2, report.RowsDeleted, "stale booking replaced and orphan booking removed")
	assert.Equal(t, newID, report.HighWaterMark)
	requireVerifiedBookings(t, db, fixture.tenant)
	require.Zero(t, countRows(t, db, "enrollment.request_child_offering_selections", fixture.tenant))
	assert.Positive(t, report.Verification.UnresolvedOrigins, "edited interval notes are not submission evidence")

	// Deleting the oldest interval must not promote an approved change into submission history.
	_, err = db.NewRaw(`DELETE FROM enrollment.request_child_offerings WHERE id = ?`, fixture.legacyIDs[0]).Exec(t.Context())
	require.NoError(t, err)
	report = runBackfill(t, db, RequestChildStorageBackfillOptions{})
	require.False(t, report.Complete, "legacy submission origins remain unproven")
	require.Zero(t, countRows(t, db, "enrollment.request_child_offering_selections", fixture.tenant))
	assert.Positive(t, report.Verification.UnresolvedOrigins, "later approved intervals cannot reconstruct original notes")
...
func TestRequestChildStorageBackfillIsolatesTenants(t *testing.T) {
	...
	require.NoError(t, err)
	require.ElementsMatch(t, []int64{fixtures[0].tenant, fixtures[1].tenant}, report.IncompleteTenants())
	...
	assert.EqualValues(t, 5, report.Tenants[0].RowsCopied)
	assert.EqualValues(t, 5, report.Tenants[1].RowsCopied)
	...
	for i, own := range fixtures {
		...
		require.NoError(t, err)
		for table, want := range map[string]int{"enrollment.care_offering_bookings": 5, "enrollment.request_child_offering_selections": 0} {
			var visible int
...
func TestRequestChildStorageBackfillMigrationUpDownAndRestart(t *testing.T) {
	...
	require.ErrorContains(t, runPresenceMigration(t.Context(), db, "001015383", true), "did not reach")
	requireVerifiedBookings(t, db, fixture.tenant)
	assert.Equal(t, false, checkpointRow(t, db, fixture.tenant)["complete"])
	require.ErrorContains(t, requestChildStorageBackfillUp(t.Context(), db), "did not reach", "resuming bookings must not certify unknown history")
...
func TestRequestChildStorageBackfillSchemaAndPlans(t *testing.T) {
	...
	assert.Zero(t, report.Verification.TargetSelections)
	assert.EqualValues(t, 1000, report.Verification.UnresolvedOrigins)
	...
	for name, query := range map[string]string{
		...
		"orphan selections": orphanSelectionDelete,
	} {
	...
	requireVerifiedBookings(t, db, tenant)
}
