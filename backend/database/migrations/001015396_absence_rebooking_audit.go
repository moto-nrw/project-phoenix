...
func init() {
	MigrationRegistry.Register(&Migration{
		Version:     absenceRebookingAuditVersion,
		Description: absenceRebookingAuditDescription,
		DependsOn:   []string{vacationQuotaChangesVersion},
	})
	Migrations.MustRegister(absenceRebookingAuditUp, absenceRebookingAuditDown)
}

// absenceRebookingAuditUp lets an absence audit row carry a type change next
// to its status change. A rebooking keeps the status and records which type
// the entry had before, which it has now and the Leitung's reason (note).
// The type IDs carry no foreign key: the row describes the past, and a
// school type is never deleted while an absence references it.
func absenceRebookingAuditUp(ctx context.Context, db *bun.DB) error {
	_, err := db.NewRaw(`
		ALTER TABLE active.staff_absence_audit
			ADD COLUMN IF NOT EXISTS from_absence_type TEXT,
			ADD COLUMN IF NOT EXISTS to_absence_type TEXT,
			ADD COLUMN IF NOT EXISTS from_absence_type_id BIGINT,
			ADD COLUMN IF NOT EXISTS to_absence_type_id BIGINT;
	`).Exec(ctx)
	if err != nil {
		return fmt.Errorf("add absence type change to staff absence audit: %w", err)
	}
	if _, err := db.NewRaw(absenceRebookingAuditViewSQL).Exec(ctx); err != nil {
		return fmt.Errorf("show absence rebookings in the time-tracking audit log: %w", err)
	}
	return nil
}

func absenceRebookingAuditDown(ctx context.Context, db *bun.DB) error {
	_, err := db.NewRaw(`
		DROP VIEW IF EXISTS audit.time_tracking_audit_log;
		` + previousAuditLogViewSQL + `
		ALTER TABLE active.staff_absence_audit
			DROP COLUMN IF EXISTS from_absence_type,
			DROP COLUMN IF EXISTS to_absence_type,
			DROP COLUMN IF EXISTS from_absence_type_id,
			DROP COLUMN IF EXISTS to_absence_type_id;
	`).Exec(ctx)
	if err != nil {
		return fmt.Errorf("drop absence type change from staff absence audit: %w", err)
	}
	return nil
}

// absenceRebookingAuditViewSQL is the 1.15.389 view with the absence branch
// extended by the type change. The other branches are unchanged.
const absenceRebookingAuditViewSQL = `
...
// previousAuditLogViewSQL is the 1.15.389 view body, restored on rollback.
const previousAuditLogViewSQL = `
		CREATE OR REPLACE VIEW audit.time_tracking_audit_log WITH (security_invoker = true) AS
		SELECT e.tenant_id, e.created_at AS occurred_at, 'session_edit'::text AS source,
			MIN(e.id) AS entry_id, e.staff_id, ARRAY[e.staff_id] AS staff_ids,
			NULLIF(e.edited_by, 0) AS actor_staff_id, e.edited_by = 0 AS actor_is_system,
