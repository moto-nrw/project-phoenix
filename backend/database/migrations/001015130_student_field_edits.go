...
const (
	studentFieldEditsVersion     = "1.15.130"
...
func init() {
	MigrationRegistry.Register(&Migration{
		...
		DependsOn:   []string{"1.15.124"}, // students table + departure_days exist
	})
...
func createStudentFieldEditsTable(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.130: Creating audit.student_field_edits...")
	...
	_, err = tx.ExecContext(ctx, `
		...
		GRANT SELECT, INSERT, UPDATE, DELETE ON audit.student_field_edits TO phoenix_tenant;
...
func dropStudentFieldEditsTable(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.130: Dropping audit.student_field_edits...")
