...
const (
	calendarOverviewVisibilityVersion     = "1.15.158"
...
func init() {
	...
	Migrations.MustRegister(
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Migration 1.15.158: Adding calendar overview visibility...")
			if _, err := db.NewRaw(`
		...
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Rolling back migration 1.15.158: Removing calendar overview visibility...")
			if _, err := db.NewRaw(`
				ALTER TABLE calendar.appointments
