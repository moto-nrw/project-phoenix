...
const (
	vacationWorkflowGrantsVersion     = "1.15.107"
	vacationWorkflowGrantsDescription = "Grant phoenix_tenant access to staff_vacation_quota + staff_absence_audit tables (missing from 1.15.105)"
)
...
func addVacationWorkflowGrants(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.107: Granting phoenix_tenant access to vacation tables...")
	...
	fmt.Println("Migration 1.15.107: vacation table grants ready")
	return tx.Commit()
}

func removeVacationWorkflowGrants(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.107: Revoking vacation table grants...")
