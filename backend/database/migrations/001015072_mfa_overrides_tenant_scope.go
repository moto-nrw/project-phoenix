...
const (
	mfaOverridesTenantScopeVersion     = "1.15.72"
...
func init() {
	...
	Migrations.MustRegister(
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Migration 1.15.72: creating auth.mfa_overrides + migrating auth.accounts.mfa_admin_override...")
			_, err := db.ExecContext(ctx, `
				...
				-- Production never ran 1.15.70 (the column is feature-
		...
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Rolling back migration 1.15.72: restoring auth.accounts.mfa_admin_override + dropping auth.mfa_overrides...")
			_, err := db.ExecContext(ctx, `
