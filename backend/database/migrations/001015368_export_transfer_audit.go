...
const (
	exportTransferAuditVersion     = "1.15.368"
...
func exportTransferAuditUp(ctx context.Context, db *bun.DB) error {
	slog.Info("migration 1.15.368: creating audit.export_transfers")
...
func exportTransferAuditDown(ctx context.Context, db *bun.DB) error {
	slog.Info("migration 1.15.368: rolling back, dropping audit.export_transfers")
	if _, err := db.NewRaw(`
