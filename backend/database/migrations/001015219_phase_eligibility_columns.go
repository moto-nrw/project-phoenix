...
const (
	phaseEligibilityColumnsVersion     = "1.15.219"
...
func phaseEligibilityColumnsUp(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.219: Adding eligibility config columns to enrollment.phases...")
...
func phaseEligibilityColumnsDown(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.219: Removing eligibility config columns from enrollment.phases...")
