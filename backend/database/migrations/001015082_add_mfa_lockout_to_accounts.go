...
const (
	addMFALockoutToAccountsVersion     = "1.15.82"
...
func addMFALockoutToAccountsUp(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.82: Adding mfa_attempts + mfa_locked_until to auth.accounts and platform.operators...")
...
func addMFALockoutToAccountsDown(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.82: Dropping mfa_attempts + mfa_locked_until from auth.accounts and platform.operators...")
