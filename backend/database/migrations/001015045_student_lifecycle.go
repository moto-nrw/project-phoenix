...
const (
	studentLifecycleVersion     = "1.15.45"
...
func studentLifecycleUp(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.45: Adding lifecycle columns (status, enrolled_from, enrolled_until) to users.students...")
...
func studentLifecycleDown(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.45: Removing lifecycle columns from users.students...")
