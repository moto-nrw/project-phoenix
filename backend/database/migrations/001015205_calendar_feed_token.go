...
const (
	calendarFeedTokenVersion     = "1.15.205"
...
func init() {
	...
	Migrations.MustRegister(
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Migration 1.15.205: Adding calendar_feed_token to auth.accounts...")
			if _, err := db.NewRaw(`
		...
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Rolling back migration 1.15.205: Dropping calendar_feed_token...")
			if _, err := db.NewRaw(`
