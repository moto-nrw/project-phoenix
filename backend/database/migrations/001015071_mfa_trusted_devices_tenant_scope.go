...
const (
	mfaTrustedDevicesTenantScopeVersion     = "1.15.71"
...
func init() {
	MigrationRegistry.Register(&Migration{
		...
		DependsOn:   []string{"1.15.70"},
	})

	Migrations.MustRegister(
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Migration 1.15.71: adding tenant_id to auth.mfa_trusted_devices...")
		...
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Rolling back migration 1.15.71: dropping tenant_id from auth.mfa_trusted_devices...")
			_, err := db.ExecContext(ctx, `
