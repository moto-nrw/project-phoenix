...
const (
	pushSubscriptionTokenFamilyVersion     = "1.15.294"
...
func pushSubscriptionTokenFamilyUp(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.294: Binding push subscriptions to token families...")
	_, err := db.ExecContext(ctx, `
		...
		COMMENT ON COLUMN iot.push_subscriptions.token_family_id IS
			'Refresh-token family that registered this device; empty on pre-1.15.294 rows';
