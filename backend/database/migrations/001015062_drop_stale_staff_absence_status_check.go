...
const (
	dropStaleStaffAbsenceStatusCheckVersion     = "1.15.62"
...
func dropStaleStaffAbsenceStatusCheck(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.61: Dropping legacy chk_sa_status constraint...")
	...
	// Migration 1.15.59 introduced staff_absences_status_check with the
	...
	fmt.Println("Migration 1.15.61: chk_sa_status dropped")
	return tx.Commit()
}

func restoreStaleStaffAbsenceStatusCheck(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.61: Restoring legacy chk_sa_status...")
