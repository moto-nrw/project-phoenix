...
const (
	dropFeedbackSuggestionsVersion     = "1.15.313"
...
func init() {
	MigrationRegistry.Register(&Migration{
		...
		DependsOn:   []string{guardianEnrollmentAccountBackfillVersion, staffAbsenceTypesVersion},
	})
...
func dropFeedbackSuggestionsUp(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.313: Dropping feedback and suggestions...")
...
func dropFeedbackSuggestionsDown(_ context.Context, _ *bun.DB) error {
	fmt.Println("Migration 1.15.313 down: no-op — dropped feedback/suggestions data can only be restored from a backup")
	return nil
}
