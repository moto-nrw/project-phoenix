...
const (
	vacationHalfDayEndpointsVersion     = "1.15.60"
...
func addVacationHalfDayEndpoints(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.60: Adding start_half_day/end_half_day columns...")
	...
	fmt.Println("Migration 1.15.60: half-day endpoints ready")
	return tx.Commit()
}

func removeVacationHalfDayEndpoints(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.60...")
	tx, err := db.BeginTx(ctx, &sql.TxOptions{})
	if err != nil {
		return fmt.Errorf("failed to begin transaction: %w", err)
	}
