...
const (
	grantOperatorMFATablePermissionsVersion     = "1.15.85"
	grantOperatorMFATablePermissionsDescription = "Grant phoenix_auth CRUD permissions on platform.operator_mfa_* tables (missed in migration 1.15.83)"
)
...
func init() {
	MigrationRegistry.Register(&Migration{
		...
		DependsOn:   []string{"1.15.84"},
	})

	Migrations.MustRegister(
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Migration 1.15.85: Granting phoenix_auth access to platform.operator_mfa_* tables...")
			...
			fmt.Println("Migration 1.15.85: Done")
			return nil
		},
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Rolling back migration 1.15.85: Revoking phoenix_auth access to platform.operator_mfa_* tables...")
