...
func TestOptionalActivityCapacity(t *testing.T) {
	...
	spontaneous := testpkg.CreateTestActivityGroupForTenant(t, db, tenantID, "LegacySpontaneousCapacity")
	...
	require.NoError(t, err)
	_, err = db.NewRaw(`UPDATE activities.categories SET name = 'Spontan' WHERE tenant_id = ? AND id = ?`, tenantID, spontaneous.CategoryID).Exec(ctx)
	require.NoError(t, err)
	_, err = db.NewRaw(`UPDATE activities.groups SET is_template = FALSE, max_participants = 999 WHERE id = ?`, spontaneous.ID).Exec(ctx)
	require.NoError(t, err)
	...
	var templateCapacity, operationalCapacity, spontaneousCapacity, limitedCapacity *int
	...
	require.NoError(t, db.NewRaw(`SELECT max_participants FROM activities.groups WHERE id = ?`, spontaneous.ID).Scan(ctx, &spontaneousCapacity))
	...
	assert.Nil(t, spontaneousCapacity)
