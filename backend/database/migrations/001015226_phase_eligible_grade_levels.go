...
const (
	phaseEligibleGradeLevelsVersion     = "1.15.226"
...
func phaseEligibleGradeLevelsUp(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.226: Adding eligible_grade_levels to enrollment.phases...")
...
func phaseEligibleGradeLevelsDown(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.226: Removing eligible_grade_levels from enrollment.phases...")
