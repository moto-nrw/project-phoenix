...
const (
	mfaOverridesPolicyNullIfVersion     = "1.15.73"
...
// Existing deployments that ran 1.15.72 have an RLS policy that does
...
//
// 1.15.72 has been patched in-source too, so a fresh `migrate reset`
// gets the same shape without this follow-up. This migration only
// matters for environments that already ran 1.15.72 in its original
// form.
...
func init() {
	...
	Migrations.MustRegister(
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Migration 1.15.73: rewriting auth.mfa_overrides RLS policy with NULLIF...")
		...
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Rolling back migration 1.15.73: restoring crash-on-unset policy shape...")
			_, err := db.ExecContext(ctx, `
				DROP POLICY IF EXISTS mfa_overrides_tenant_isolation ON auth.mfa_overrides;
				CREATE POLICY mfa_overrides_tenant_isolation ON auth.mfa_overrides
					USING (
