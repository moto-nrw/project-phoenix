...
const (
	mfaOverridesPolicyNullIfVersion     = "1.15.91"
...
func init() {
	...
	Migrations.MustRegister(
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Migration 1.15.91: rewriting auth.mfa_overrides RLS policy with NULLIF...")
		...
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Rolling back migration 1.15.91: restoring crash-on-unset policy shape...")
			_, err := db.ExecContext(ctx, `
				DROP POLICY IF EXISTS mfa_overrides_tenant_isolation ON auth.mfa_overrides;
				CREATE POLICY mfa_overrides_tenant_isolation ON auth.mfa_overrides
					USING (
