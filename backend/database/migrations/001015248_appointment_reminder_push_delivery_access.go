...
func appointmentReminderPushDeliveryAccessUp(ctx context.Context, db *bun.DB) error {
	_, err := db.ExecContext(ctx, `
		...
		BEGIN
			...
			VALUES
				(current_tenant_id, appointment_id, appointment_revision, occurrence_date, guardian_profile_id)
		...
		BEGIN
			IF current_tenant_id IS NULL THEN
				RAISE EXCEPTION 'app.current_tenant_id is required';
			END IF;
			DELETE FROM calendar.appointment_reminder_push_deliveries
			WHERE tenant_id = current_tenant_id
				AND appointment_id = release_appointment_reminder_push_delivery.appointment_id
				AND appointment_revision = release_appointment_reminder_push_delivery.appointment_revision
				AND occurrence_date = release_appointment_reminder_push_delivery.occurrence_date
				AND guardian_profile_id = release_appointment_reminder_push_delivery.guardian_profile_id;
			RETURN FOUND;
		END;
		$$;
