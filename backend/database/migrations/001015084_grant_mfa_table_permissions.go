...
const (
	grantMFATablePermissionsVersion     = "1.15.84"
	grantMFATablePermissionsDescription = "Grant phoenix_auth + phoenix_tenant CRUD permissions on auth.mfa_* tables (missed in migrations 1.15.78–63)"
)
...
func init() {
	MigrationRegistry.Register(&Migration{
		...
		DependsOn:   []string{"1.15.83"},
	})

	Migrations.MustRegister(
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Migration 1.15.84: Granting phoenix_tenant access to auth.mfa_* tables...")
			if err := execMFAPermissionStatements(ctx, db, mfaGrantStatements(), "grant failed"); err != nil {
				return err
			}
			fmt.Println("Migration 1.15.84: Done")
			return nil
		},
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Rolling back migration 1.15.84: Revoking phoenix_auth + phoenix_tenant access to auth.mfa_* tables...")
