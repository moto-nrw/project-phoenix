...
const (
	calendarAppointmentRevisionVersion     = "1.15.206"
...
func init() {
	...
	Migrations.MustRegister(
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Migration 1.15.206: Adding revision to calendar.appointments...")
			if _, err := db.NewRaw(`
		...
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Rolling back migration 1.15.206: Dropping calendar.appointments.revision...")
			if _, err := db.NewRaw(`
