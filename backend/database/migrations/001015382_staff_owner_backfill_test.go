...
func TestStaffOwnerBackfillRetiresBeforeSameBatchRejoin(t *testing.T) {
	...
	// active row will sort first by ID, so ID ordering alone cannot fix this.
	var replacementID int64
	require.NoError(t, db.NewRaw(`SELECT nextval(pg_get_serial_sequence('users.staff', 'id'))`).Scan(ctx, &replacementID))
	first := testpkg.CreateTestStaffForTenant(t, db, tenantID, "Same batch", "Rejoin")
	_, err := RunStaffOwnerBackfill(ctx, db, StaffOwnerBackfillOptions{})
	require.NoError(t, err)
	...
	report, err := RunStaffOwnerBackfill(ctx, db, StaffOwnerBackfillOptions{BatchSize: 10, MaxAttempts: 2})
	require.NoError(t, err)
	cp := requireStaffOwnerTenantEqual(t, db, report, tenantID)
	require.EqualValues(t, 2, cp.SourceCount)
	require.Zero(t, cp.BatchesRetried, "retire the old membership before inserting its replacement in the same statement")
	require.Zero(t, cp.RowsRemoved, "soft-deleted memberships are retained")
	var activeID int64
...
func TestStaffOwnerBackfillResetAndRollback(t *testing.T) {
	...
	require.ElementsMatch(t, []string{"other-backfill", StudentOwnerBackfillName, GuardianOwnerBackfillName}, remaining,
	...
	_, err = db.ExecContext(ctx, `DELETE FROM platform.storage_backfill_checkpoints WHERE backfill IN (?, ?, ?)`,
		"other-backfill", StudentOwnerBackfillName, GuardianOwnerBackfillName)
	require.NoError(t, err)
	require.NoError(t, staffOwnerBackfillDown(ctx, db))
