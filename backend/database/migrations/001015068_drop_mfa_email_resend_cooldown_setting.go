...
const (
	dropMFAEmailResendCooldownSettingVersion     = "1.15.68"
...
func init() {
	MigrationRegistry.Register(&Migration{
		...
		DependsOn:   []string{"1.15.67"},
	})

	Migrations.MustRegister(
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Migration 1.15.68: Deleting orphaned setting rows for security.mfa_email_resend_cooldown_seconds...")
			...
			fmt.Println("Migration 1.15.68: Done")
			return nil
		},
		func(ctx context.Context, db *bun.DB) error {
			...
			fmt.Println("Rolling back migration 1.15.68: no-op (orphaned rows cannot be restored)")
			return nil
		},
	)
}
