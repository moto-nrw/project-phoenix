...
const (
	backfillMensaCategoryVersion     = "1.15.255"
...
func backfillMensaCategoryUp(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.255: Backfilling 'Mensa' activity category per tenant...")
	...
	if affected, rowsErr := res.RowsAffected(); rowsErr == nil {
		fmt.Printf("Migration 1.15.255: Added 'Mensa' category to %d tenant(s)\n", affected)
	}
...
func backfillMensaCategoryDown(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.255: Removing unused 'Mensa' categories...")
