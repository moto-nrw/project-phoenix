...
const (
	normalizeRoomCapacityVersion     = "1.15.285"
...
func init() {
	MigrationRegistry.Register(&Migration{
		...
		DependsOn:   []string{optionalActivityCapacityVersion},
	})
...
func normalizeRoomCapacityUp(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.285: Normalizing optional room capacities...")
	...
	if affected, rowsErr := result.RowsAffected(); rowsErr == nil {
		fmt.Printf("Migration 1.15.285: Converted %d non-positive room capacities to NULL\n", affected)
	}
...
func normalizeRoomCapacityDown(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.285: Removing the positive room capacity constraint...")
