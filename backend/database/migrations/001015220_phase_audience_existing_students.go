...
const (
	phaseAudienceExistingStudentsVersion     = "1.15.220"
...
func phaseAudienceExistingStudentsUp(ctx context.Context, db *bun.DB) error {
	fmt.Println("Migration 1.15.220: Adding existing_students to the enrollment.phases audience CHECK...")
...
func phaseAudienceExistingStudentsDown(ctx context.Context, db *bun.DB) error {
	fmt.Println("Rolling back migration 1.15.220: Removing existing_students from the enrollment.phases audience CHECK...")
