...
const (
	calendarTargetAllSchoolParentsVersion     = "1.15.210"
...
func init() {
	...
	Migrations.MustRegister(
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Migration 1.15.210: Adding all_school_parents to the calendar target check...")
			if _, err := db.NewRaw(`
		...
		func(ctx context.Context, db *bun.DB) error {
			fmt.Println("Rolling back migration 1.15.210: Removing all_school_parents from the calendar target check...")
