...
func (d activityBookingDirectory) RemovePlannedRosterForCareExit(ctx context.Context, studentIDs []int64, after string) ([]carelifecycle.CareExitRemoval, error) {
	rows, err := d.capability.RemovePlannedRosterForCareExit(ctx, studentIDs, after)
	if err != nil {
		return nil, err
	}
	result := make([]carelifecycle.CareExitRemoval, 0, len(rows))
	for _, row := range rows {
		result = append(result, carelifecycle.CareExitRemoval{
			TenantID: row.TenantID, StudentID: row.StudentID, Kind: carelifecycle.CareExitRemovalRoster,
...
func (d activityBookingDirectory) RestoreRosterForCareExit(ctx context.Context, studentIDs []int64, removals []carelifecycle.CareExitRemoval) (int, error) {
	rows := make([]timetable.CareExitRosterRow, 0, len(removals))
	for _, removal := range removals {
		if removal.Kind != carelifecycle.CareExitRemovalRoster || removal.InstanceID == nil {
			continue
		}
...
func (d activityBookingDirectory) EndStudentEnrollmentsForCareExit(ctx context.Context, studentIDs []int64, validUntil string) (carelifecycle.ActivityBookingChanges, error) {
	changes, err := d.capability.EndStudentEnrollmentsForCareExit(ctx, studentIDs, validUntil)
	if err != nil {
		return carelifecycle.ActivityBookingChanges{}, err
	}
	result := carelifecycle.ActivityBookingChanges{
		Deleted: make([]carelifecycle.ActivityBooking, 0, len(changes.Deleted)),
		Capped:  make([]carelifecycle.ActivityBookingCap, 0, len(changes.Capped)),
	}
	...
	for _, value := range changes.Capped {
		result.Capped = append(result.Capped, carelifecycle.ActivityBookingCap{
...
func (d activityBookingDirectory) RestoreStudentEnrollmentsForCareExit(ctx context.Context, studentIDs, periodIDs []int64, removals []carelifecycle.ActivityBookingRemoval) (int, error) {
...
func activityBooking(value timetable.CareExitEnrollment) carelifecycle.ActivityBooking {
	return carelifecycle.ActivityBooking{
...
func publicActivityBooking(value carelifecycle.ActivityBooking) timetable.CareExitEnrollment {
