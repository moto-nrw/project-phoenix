...
func NewAuthTestRepositories(db *bun.DB, command auditModels.Command) (*Factory, error) {
	...
	r := &Factory{
		...
		ParentEnrollmentRequest: parentRepo.NewEnrollmentRequestRepository(carePlanLegacy.NewParentRuntime(db), enrollmentCompose.New(), identityAccountDirectory{accounts: newUnobservedIdentityAccess(db)}),
