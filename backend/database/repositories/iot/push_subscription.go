...
const (
	...
	// childAccessFilter keeps a notification about one child to the accounts
	// that still hold parent_portal.access for it. The relationship row, not the
	// account, carries that permission: the same person can be a full guardian
	// for one child and hidden from another.
...
//
// A positive studentID narrows the result to accounts that still hold
// parent_portal.access for that child. The producer decided its audience in an
// earlier transaction; this one sends. Asking again here is what keeps a
// notification about a child from reaching an account whose access was revoked
// in between — the same containment predicate the parent read paths and the
// messaging fan-out use, so the three cannot drift apart.
func (r *PushSubscriptionRepository) FindForGuardians(ctx context.Context, guardianAccountIDs []int64, studentID int64) ([]*iot.PushSubscription, error) {
	...
	if studentID > 0 {
		query = query.Where(childAccessFilter, studentID, guardianPortalAccessPermission)
	}
	query = base.WithTenantFilter(ctx, query, "push_subscription")
	if err := query.Scan(ctx); err != nil {
