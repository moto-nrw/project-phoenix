...
func statusDayFromPublic(value careplan.StudentStatusDay) *activeModels.StudentStatusDay {
	row := &activeModels.StudentStatusDay{StudentID: value.StudentID, Date: activeModels.Date(value.Date), Status: value.Status, ReportedAt: value.ReportedAt, ClearedAt: value.ClearedAt, Source: value.Source, GuardianAccountID: value.GuardianAccountID, Note: value.Note}
...
func (r studentStatusDayRepository) CountEffectiveDashboardAbsences(ctx context.Context, date activeModels.Date) (*activeModels.StudentStatusCounts, error) {
...
func (r studentStatusDayRepository) FindActiveByStudentAndDateRange(ctx context.Context, studentID int64, from, to activeModels.Date) ([]*activeModels.StudentStatusDay, error) {
...
func (r studentStatusDayRepository) FindActiveByStudentIDsAndDate(ctx context.Context, studentIDs []int64, date activeModels.Date) ([]*activeModels.StudentStatusDay, error) {
	if len(studentIDs) == 0 {
...
func (r studentStatusDayRepository) FindSignedOffByStudentIDsAndDate(ctx context.Context, studentIDs []int64, date activeModels.Date) ([]*activeModels.StudentStatusDay, error) {
	if len(studentIDs) == 0 {
		return []*activeModels.StudentStatusDay{}, nil
	}
...
func (r studentStatusDayRepository) FindByStudentAndDateRange(ctx context.Context, studentID int64, from, to activeModels.Date) ([]*activeModels.StudentStatusDay, error) {
	return r.list(ctx, careplan.StudentStatusDayFilter{StudentIDs: []int64{studentID}, From: careplan.Date(from), To: careplan.Date(to)})
}

func (r studentStatusDayRepository) ListOverviewWithOptions(ctx context.Context, options *userModels.QueryOptions, orderedStudentIDs []int64) ([]*activeModels.StudentStatusDay, error) {
