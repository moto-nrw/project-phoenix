...
func (s *Seeder) Seed(ctx context.Context) (*Result, error) {
	...
	if s.config.Reset {
		...
		if s.config.Verbose {
			logger.Logger.Info("Data reset completed")
		}
	}
	...
	if !s.config.RuntimeOnly {
		...
		if s.config.Verbose {
			logger.Logger.Info("Fixed data seeding completed")
		}
	}
	...
	if !s.config.FixedOnly && s.config.CreateActiveState {
		if s.config.Verbose {
			logger.Logger.Info("Starting runtime state creation...")
		}
		...
		if s.config.Verbose {
			logger.Logger.Info("Runtime state creation completed")
		}
	}
...
func (s *Seeder) resetData(ctx context.Context) error {
	...
	for _, table := range tables {
		...
		if err := s.db.NewSelect().ColumnExpr("to_regclass(?) IS NOT NULL", table).Scan(ctx, &exists); err != nil {
			if s.config.Verbose {
				logger.Logger.Warnf("Could not check existence for %s: %v", table, err)
			}
			continue
		}
		if !exists {
		...
		if _, err := s.db.ExecContext(ctx, query); err != nil {
			...
			if s.config.Verbose {
				logger.Logger.Warnf("Could not truncate %s: %v", table, err)
			}
		}
	}
