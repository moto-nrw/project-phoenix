...
// need. The HTTP layer must not import models/users (architecture policy:
// inbound-time-tracking/http may not import people-directory/domain), so the
// staff row is projected where its type is inferred.
