...
// Leserarten, wie sie der Service versteht (users.StaffNoticeAudience*). Die
// HTTP-Schicht darf models/users nicht importieren, deshalb stehen die beiden
// Werte hier noch einmal; TestStaffNoticeReaderKindsMatchModel hält sie fest.
const (
	noticeReaderStaff     = "staff"
	noticeReaderLehrkraft = "lehrkraft"
)
...
func (rs *StaffNoticeResource) Router() chi.Router {
	...
	common.ProtectedTenantGroup(r, rs.db, func(r chi.Router, withTx common.Middleware) {
		...
		r.With(read, withTx).Get("/today", rs.todayFor(noticeReaderStaff))
		r.With(read, withTx).Post("/{noticeId}/acknowledge", rs.acknowledgeFor(noticeReaderStaff))
...
func (rs *StaffNoticeResource) SchoolRouter() chi.Router {
	...
	common.ProtectedSchoolGroup(r, rs.db, func(r chi.Router, withTx common.Middleware) {
		...
		r.With(read, withTx).Get("/today", rs.todayFor(noticeReaderLehrkraft))
		r.With(read, withTx).Post("/{noticeId}/acknowledge", rs.acknowledgeFor(noticeReaderLehrkraft))
	})
...
func (rs *StaffNoticeResource) todayFor(reader string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		views, err := rs.Service.Today(ctx, noticeAccountID(ctx), timezone.TodayDate(), reader)
		if err != nil {
...
func (rs *StaffNoticeResource) acknowledgeFor(reader string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := rs.Service.Acknowledge(ctx, id, noticeAccountID(ctx), reader); err != nil {
			renderNoticeServiceError(w, r, err)
			return
		}
		render.JSON(w, r, map[string]any{"status": "ok"})
