...
func (rs *Resource) moveStudentsToOpenRoom(w http.ResponseWriter, r *http.Request) {
	...
	// results alike.
	response := &activeSvc.StudentMoveResult{
		Moved:         nonNilIDs(result.Moved),
		Unchanged:     nonNilIDs(result.Unchanged),
		Skipped:       make([]activeSvc.StudentMoveSkipped, 0, len(result.Skipped)),
		ActiveGroupID: &result.RoomSessionID,
		RoomID:        &result.RoomID,
	}
	for _, skipped := range result.Skipped {
		response.Skipped = append(response.Skipped, activeSvc.StudentMoveSkipped{StudentID: skipped.StudentID, Reason: skipped.Reason})
	}
