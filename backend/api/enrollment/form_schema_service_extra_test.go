...
func TestFormSchemaService_UpdateSchema_RepointsPhasesButKeepsRequestSchemaPin(t *testing.T) {
	...
	require.NoError(t, enrollmentAPI.InsertOwnerPhaseForTest(ctx, repoFactory.Enrollment(), phase))
	...
	require.NoError(t, enrollmentAPI.InsertOwnerRequestForTest(ctx, repoFactory.Enrollment(), request))
	...
	gotRequest, err := enrollmentAPI.ReadOwnerRequestForTest(ctx, repoFactory.Enrollment(), request.ID)
	require.NoError(t, err)
