...
func TestPhaseExpiryProjection_ListSnapshots_RequiresDatesAndTenant(t *testing.T) {
	...
	repo := enrollmentAPI.NewTestPhaseExpirySnapshots(nil, nil, nil, nil)
...
func TestPhaseExpiryProjection_ListSnapshots_PreservesDependencyFailures(t *testing.T) {
	...
	for _, stage := range []string{"students", "care_plan", "bookings", "enrollment"} {
		t.Run(stage, func(t *testing.T) {
			...
			repo := enrollmentAPI.NewTestPhaseExpirySnapshots(failingExpiryOwner{err: ownerErr}, failingExpiryStudents{err: studentsErr}, failingExpiryOfferings{err: offeringsErr}, failingExpiryBookings{err: bookingsErr})
