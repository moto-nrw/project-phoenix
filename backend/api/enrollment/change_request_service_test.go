...
func newChangeRequestServiceForTest(env *requestTestEnv) enrollmentAPI.ChangeRequestService {
...
func newChangeRequestServiceForTestWithAuthorizer(
	env *requestTestEnv,
	authorizer testutil.EnrollmentGuardianAuthorizer,
) enrollmentAPI.ChangeRequestService {
	repoFactory := repositories.NewFactory(env.db, repositories.NewUnobservedTimetableDependencies(env.db))
	return newTestChangeRequestService(testutil.EnrollmentChangeRequestSources{
		...
		CareOfferingRepo:    testutil.NewEnrollmentCareOfferingRecords(repoFactory.CarePlan()),
...
func TestChangeRequestService_ApproveLateInviteRenewalUsesInviteEmailForAuthorization(t *testing.T) {
	...
	withLateInviteRenewalFixture(t, func(
		env *requestTestEnv,
		_ enrollmentAPI.RequestService,
		result *enrollmentAPI.SubmitResult,
		_ enrollmentAPI.SubmitRequest,
		...
		created, err := svc.Create(ctx, result.Request.StatusToken, enrollmentAPI.CreateChangeRequestInput{
			Submission: proposed,
		...
		authorizer.email = ""
		_, err = svc.Approve(ctx, created.ChangeRequest.ID, enrollmentAPI.ReviewChangeRequestInput{
			Note:           "Freigegeben.",
...
func newChangeRequestServiceWithDecisionForTest(t *testing.T, env *decisionTestEnv) enrollmentAPI.ChangeRequestService {
	t.Helper()
...
func newChangeRequestServiceWithDecisionAndIntakeForTest(
	t *testing.T,
	env *decisionTestEnv,
	requests testutil.EnrollmentChangeRequestRecords,
) enrollmentAPI.ChangeRequestService {
	t.Helper()
	return newTestChangeRequestService(testutil.EnrollmentChangeRequestSources{
		Bookings:            requestTestBookingCommands(),
		...
		CareOfferingRepo:    testutil.NewEnrollmentCareOfferingRecords(env.repos.CarePlan()),
...
type failAdminAuditChangeRequestIntake struct {
	testutil.EnrollmentChangeRequestRecords
}
...
func liftTakeoverStamp(t *testing.T, env *decisionTestEnv, requestID int64) func() {
	...
	children, err := enrollmentAPI.ReadOwnerRequestChildrenForTest(ctx, env.repos.Enrollment(), requestID)
	require.NoError(t, err)
...
func TestNewChangeRequestService_ParentsURLRequired(t *testing.T) {
	...
	assert.PanicsWithValue(t, "PARENTS_URL is required", func() {
		newTestChangeRequestService(testutil.EnrollmentChangeRequestSources{
			Bookings:    requestTestBookingCommands(),
			FrontendURL: "http://localhost:3000",
		})
	})
...
func TestChangeRequestService_CorrectApprovedChildData_RequiresReason(t *testing.T) {
	...
	_, err := svc.CorrectApprovedChildData(context.Background(), enrollmentAPI.CorrectApprovedChildDataInput{
		RequestID:      11,
		ChildID:        12,
		FirstName:      "Lina",
		LastName:       "Muster",
	...
	require.ErrorIs(t, err, capability.ErrChangeRequestInvalidData)
}

func makeLegacyNoSchemaRequest(t *testing.T, env *requestTestEnv, childStatus string) *enrollmentAPI.SubmitResult {
	t.Helper()
	ctx := testpkg.Ctx(t)
...
func proposedChangeSubmission(t *testing.T, env *requestTestEnv, result *enrollmentAPI.SubmitResult) enrollmentAPI.SubmitRequest {
	t.Helper()
...
func TestRequestService_ReplaceEditable_PreservesChildCustomDataByIDWhenMiddleChildRemoved(t *testing.T) {
	...
	base.Children = []enrollmentAPI.SubmitChild{
		{
			FirstName:        "Lina",
			LastName:         "Beispiel",
			DateOfBirth:      timezone.NewDate(2018, 4, 15),
	...
	edit := base
	edit.Children = []enrollmentAPI.SubmitChild{
		{
...
func TestChangeRequestService_Create_RequiresStableChildIdentity(t *testing.T) {
	...
	base := validSubmission(t, env.phaseID)
	base.Children = append(base.Children, enrollmentAPI.SubmitChild{
		FirstName:        "Mika",
		LastName:         "Beispiel",
		DateOfBirth:      timezone.NewDate(2017, 7, 22),
		TargetGradeLevel: testpkg.Int16Ptr(2),
	...
	missingID := base
	_, err = svc.Create(ctx, result.Request.StatusToken, enrollmentAPI.CreateChangeRequestInput{
		Submission: missingID,
		ParentNote: "Bitte pruefen.",
	})
	require.Error(t, err)
	assert.True(t, errors.Is(err, capability.ErrChangeRequestInvalidData))
...
func TestChangeRequestService_Approve_RejectsStaleBaseSnapshot(t *testing.T) {
	...
	repoFactory := repositories.NewFactory(env.db, repositories.NewUnobservedTimetableDependencies(env.db))
	newer, err := enrollmentAPI.ReadOwnerRequestForTest(ctx, repoFactory.Enrollment(), result.Request.ID)
	require.NoError(t, err)
	newer.GuardianLastName = "Neuer"
	require.NoError(t, enrollmentAPI.UpdateOwnerRequestGuardianForTest(ctx, repoFactory.Enrollment(), newer, true))
	...
	require.Error(t, err)
	assert.True(t, errors.Is(err, capability.ErrChangeRequestConflict))
}
...
func TestChangeRequestService_Approve_RejectsActiveDuplicateAfterRename(t *testing.T) {
	...
	created, err := svc.Create(ctx, firstResult.Request.StatusToken, enrollmentAPI.CreateChangeRequestInput{
		Submission: proposed,
		ParentNote: "Bitte Namen korrigieren.",
	})
	require.NoError(t, err)
	...
	require.Error(t, err)
	assert.True(t, errors.Is(err, capability.ErrDuplicateEnrollment))

	child, err := enrollmentAPI.ReadOwnerChildForTest(ctx, repositories.NewFactory(env.db, repositories.NewUnobservedTimetableDependencies(env.db)).Enrollment(), firstResult.Children[0].ID)
	require.NoError(t, err)
...
func TestChangeRequestService_Approve_PreservesAdditionalGuardianProfileID(t *testing.T) {
	...
	req.AdditionalGuardians = []enrollmentAPI.SubmitGuardian{
...
func TestChangeRequestService_Create_AllowsOnlyOneOpenRequest(t *testing.T) {
	...
	require.Error(t, err)
	assert.True(t, errors.Is(err, capability.ErrChangeRequestNotAllowed))
}
...
func TestChangeRequestService_Create_AllowsKeepingInactiveCurrentOffering(t *testing.T) {
	...
	require.NoError(t, newCareOfferingFixtures(repoFactory.CarePlan()).Update(ctx, offering))
...
func TestChangeRequestService_Create_PreservesGradeCapabilityForInactiveConditionalOffering(t *testing.T) {
	...
	created, err := newChangeRequestServiceForTest(env).Create(
		...
		enrollmentAPI.CreateChangeRequestInput{
			Submission: proposed,
			ParentNote: "Telefon korrigieren.",
		},
	)
...
func TestChangeRequestService_Approve_AppliesPinnedOfferingChangeToApprovedChildAfterDisable(t *testing.T) {
	...
	submission := enrollmentAPI.SubmitRequest{
		...
		Children: []enrollmentAPI.SubmitChild{{
			...
			OfferingDays: []enrollmentAPI.SubmitOfferingDays{{
	...
	proposed.Children[0].OfferingDays = []enrollmentAPI.SubmitOfferingDays{{
	...
	_, err = env.decision.UpdateChildOfferings(ctx, enrollmentAPI.UpdateChildOfferingsInput{
		...
		Offerings: []enrollmentAPI.OfferingAdjustmentSelection{{
			OfferingID:   currentOffering.ID,
			SelectedDays: []string{"mon"},
		}},
	})
	require.ErrorIs(t, err, capability.ErrCareOfferingsDisabled)
...
func TestChangeRequestService_CorrectApprovedChildData_UpdatesEnrollmentStudentAndAudit(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, result.Children, 1)
	storedChild, err := enrollmentAPI.ReadOwnerChildForTest(ctx, env.repos.Enrollment(), result.Children[0].ID)
	require.NoError(t, err)
	storedChild.CustomData = map[string]any{"allergies": "keine"}
	require.NoError(t, enrollmentAPI.UpdateOwnerChildForTest(ctx, env.repos.Enrollment(), storedChild))
	outcome, err := env.decision.Decide(ctx, enrollmentAPI.DecideInput{
		RequestID:  result.Request.ID,
		ChildID:    result.Children[0].ID,
		Status:     capability.DecisionApproved,
	...
	svc := newChangeRequestServiceWithDecisionForTest(t, env)
	var audit *enrollmentAPI.ChangeRequestAggregate
	err = testpkg.WithTenantTx(t, ctx, env.db, testpkg.Tenant(t), func(txCtx context.Context, _ bun.Tx) error {
		var correctionErr error
		audit, correctionErr = svc.CorrectApprovedChildData(txCtx, enrollmentAPI.CorrectApprovedChildDataInput{
			RequestID:        result.Request.ID,
			ChildID:          result.Children[0].ID,
			FirstName:        "Lina",
			LastName:         "Richtig",
	...
	child, err := enrollmentAPI.ReadOwnerChildForTest(ctx, env.repos.Enrollment(), result.Children[0].ID)
	require.NoError(t, err)
...
func TestChangeRequestService_CorrectApprovedChildData_RejectsOpenParentChangeRequest(t *testing.T) {
	...
	for _, openStatus := range []string{
		...
		t.Run(openStatus, func(t *testing.T) {
			...
			svc := newChangeRequestServiceWithDecisionForTest(t, env)
			proposed := submission
			proposed.Children = append([]enrollmentAPI.SubmitChild(nil), submission.Children...)
			proposed.Children[0].ID = result.Children[0].ID
			proposed.Children[0].FirstName = "Lina-Marie"
			restoreTakeover := liftTakeoverStamp(t, env, result.Request.ID)
			created, err := svc.Create(ctx, result.Request.StatusToken, enrollmentAPI.CreateChangeRequestInput{
			...
			if openStatus == capability.ChangeRequestStatusNeedsParentResponse {
				_, err = svc.AskQuestion(ctx, created.ChangeRequest.ID, enrollmentAPI.ChangeRequestMessageInput{
			...
			_, err = svc.Approve(ctx, rejected.ID, enrollmentAPI.ReviewChangeRequestInput{
				Note:           "Darf nicht mehr freigegeben werden.",
				ActorAccountID: env.creatorID,
				ActorRole:      "admin",
			})
			require.ErrorIs(t, err, capability.ErrChangeRequestInvalidStatus)
		})
	}
}
...
func TestChangeRequestService_CorrectApprovedChildData_PreservesTerminalParentChangeRequest(t *testing.T) {
	...
	require.NoError(t, err)
	restoreTakeover()
	_, err = svc.Reject(ctx, created.ChangeRequest.ID, enrollmentAPI.ReviewChangeRequestInput{
	...
	err = testpkg.WithTenantTx(t, ctx, env.db, testpkg.Tenant(t), func(txCtx context.Context, _ bun.Tx) error {
		_, correctionErr := svc.CorrectApprovedChildData(txCtx, enrollmentAPI.CorrectApprovedChildDataInput{
			RequestID:        result.Request.ID,
			ChildID:          result.Children[0].ID,
			FirstName:        submission.Children[0].FirstName,
			LastName:         "Richtig",
...
func TestChangeRequestService_CorrectApprovedChildData_RollsBackRejectedRequestWhenAuditFails(t *testing.T) {
	...
	proposed.Children[0].FirstName = "Lina-Marie"
	restoreTakeover := liftTakeoverStamp(t, env, result.Request.ID)
	created, err := regularService.Create(ctx, result.Request.StatusToken, enrollmentAPI.CreateChangeRequestInput{
		Submission: proposed,
		ParentNote: "Bitte den Vornamen ändern.",
	})
	require.NoError(t, err)
	...
	childAfter, err := enrollmentAPI.ReadOwnerChildForTest(ctx, env.repos.Enrollment(), result.Children[0].ID)
	require.NoError(t, err)
...
) (enrollmentAPI.SubmitRequest, *enrollmentAPI.SubmitResult) {
	t.Helper()
	ctx := testpkg.Ctx(t)
	grade := int16(1)
	submission := enrollmentAPI.SubmitRequest{
...
func TestChangeRequestService_CorrectApprovedChildData_PreservesHistoricalTargetsWhenCollectionIsDisabled(t *testing.T) {
	...
	gradeTwo := int16(2)
	targetClass := "2a"
	result, err := env.requestSvc.Submit(ctx, enrollmentAPI.SubmitRequest{
		TenantID:          testpkg.Tenant(t),
		PhaseID:           env.sourcePhase.ID,
		GuardianFirstName: "Mara",
	...
	svc := newChangeRequestServiceWithDecisionForTest(t, env)
	correct := func(input enrollmentAPI.CorrectApprovedChildDataInput) error {
	...
	baseInput := enrollmentAPI.CorrectApprovedChildDataInput{
		RequestID:         result.Request.ID,
		ChildID:           result.Children[0].ID,
		FirstName:         "Lina",
		LastName:          "Richtig",
	...
	require.ErrorIs(t, correct(changedGrade), capability.ErrChangeRequestInvalidData)
	...
	require.ErrorIs(t, correct(changedClass), capability.ErrChangeRequestInvalidData)
...
func TestChangeRequestService_Create_RejectsInactiveOfferingOnlyCurrentForAnotherChild(t *testing.T) {
	...
	require.NoError(t, newCareOfferingFixtures(repoFactory.CarePlan()).Update(ctx, secondOffering))
	...
	require.Error(t, err)
	assert.True(t, errors.Is(err, capability.ErrCareOfferingClosed), "expected ErrCareOfferingClosed, got %v", err)
}
...
func TestChangeRequestService_Approve_DoesNotReopenUnchangedRejectedChild(t *testing.T) {
	...
	child, err := enrollmentAPI.ReadOwnerChildForTest(ctx, repoFactory.Enrollment(), result.Children[0].ID)
	require.NoError(t, err)
...
func TestChangeRequestService_Approve_WaitlistsNonApprovedChildMovedOntoFullOffering(t *testing.T) {
	...
	proposed.Children[0].OfferingIDs = []int64{offering.ID}
	svc := newChangeRequestServiceForTest(env)
	created, err := svc.Create(ctx, candidate.Request.StatusToken, enrollmentAPI.CreateChangeRequestInput{
		Submission: proposed,
		ParentNote: "Bitte Betreuungsangebot wechseln.",
	})
	require.NoError(t, err)
	...
	child, err := enrollmentAPI.ReadOwnerChildForTest(ctx, repoFactory.Enrollment(), candidate.Children[0].ID)
	require.NoError(t, err)
...
func TestChangeRequestService_Approve_DoesNotDoubleCountPreservedOfferingCapacity(t *testing.T) {
	...
	for _, child := range result.Children {
		stored, err := enrollmentAPI.ReadOwnerChildForTest(ctx, repoFactory.Enrollment(), child.ID)
		require.NoError(t, err)
...
func TestChangeRequestService_Approve_RejectsNonApprovedChildMovedOntoFullOffering(t *testing.T) {
	...
	require.Error(t, err)
	assert.True(t, errors.Is(err, capability.ErrCareOfferingFull), "expected ErrCareOfferingFull, got %v", err)
...
func TestChangeRequestService_Approve_RollsBackApprovedChildScheduleReplacementFailure(t *testing.T) {
	...
	requestRow, err := enrollmentAPI.ReadOwnerRequestForTest(ctx, env.repos.Enrollment(), reqID)
	require.NoError(t, err)
	grade := int16(2)
	proposed := enrollmentAPI.SubmitRequest{
		TenantID:          testpkg.Tenant(t),
		PhaseID:           env.sourcePhase.ID,
		GuardianFirstName: "Eltern",
	...
	created, err := svc.Create(ctx, requestRow.StatusToken, enrollmentAPI.CreateChangeRequestInput{
		Submission: proposed,
...
type failingChangeRequestCatalog struct {
	testutil.EnrollmentIntakeCatalog
}
...
func TestChangeRequestService_PreservesReadFailures(t *testing.T) {
	...
	created, err := healthy.Create(ctx, result.Request.StatusToken, enrollmentAPI.CreateChangeRequestInput{Submission: proposed})
	require.NoError(t, err)
	for _, tc := range []struct{ operation, fail string }{
		...
		t.Run(tc.operation+"/"+tc.fail, func(t *testing.T) {
			...
			switch tc.operation {
			case "create":
				_, readErr = svc.Create(ctx, result.Request.StatusToken, enrollmentAPI.CreateChangeRequestInput{Submission: proposed})
			case "detail":
				_, readErr = svc.GetAdmin(ctx, created.ChangeRequest.ID)
			case "question":
				_, readErr = svc.AskQuestion(ctx, created.ChangeRequest.ID, enrollmentAPI.ChangeRequestMessageInput{Body: "Question"})
			case "reply":
				_, readErr = svc.ParentReply(ctx, result.Request.StatusToken, created.ChangeRequest.ID, enrollmentAPI.ChangeRequestMessageInput{Body: "Reply"})
			}
			require.ErrorIs(t, readErr, context.Canceled)
			require.NotErrorIs(t, readErr, capability.ErrChangeRequestNotFound)
			require.NotErrorIs(t, readErr, capability.ErrRequestNotFound)
		})
	}
