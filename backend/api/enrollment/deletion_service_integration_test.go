...
func newDeletionTestFixture(t *testing.T, db *bun.DB, label string) *deletionTestFixture {
	...
	require.NoError(t, enrollmentAPI.InsertOwnerPhaseForTest(f.scope.Context(), f.repos.Enrollment(), phase))
	f.phase = phase.ID
	return f
}
...
func (f *deletionTestFixture) request(label string, guardianAccountID *int64) *enrollmentModels.Request {
	...
	require.NoError(f.t, enrollmentAPI.InsertOwnerRequestForTest(f.scope.Context(), f.repos.Enrollment(), request))
	return request
}

func (f *deletionTestFixture) child(requestID int64, label, status string, createdStudentID *int64) *enrollmentAPI.RequestChild {
	f.t.Helper()
	now := time.Now()
	...
	require.NoError(f.t, enrollmentAPI.InsertOwnerChildForTest(f.scope.Context(), f.repos.Enrollment(), child))
	return child
}
...
func TestEnrollmentDeletionOwner_MismatchedRequestPreservesChildSelections(t *testing.T) {
	...
	require.NoError(t, newCareOfferingFixtures(f.repos.CarePlan()).Create(f.scope.Context(), offering))
...
func TestEnrollmentDeletion_DeleteRequestCleansDependenciesAndPreservesPeople(t *testing.T) {
	...
	require.NoError(t, requestTestBookingCommands().RecordCareBookings(f.scope.Context(), child.ID, []capability.CareBookingInput{
...
func TestEnrollmentDeletion_BlocksExistingStudent(t *testing.T) {
	...
	require.ErrorIs(t, err, capability.ErrEnrollmentDeletionStudentExists)
	assert.Equal(t, 1, tableCount(t, db, "enrollment.requests", "id = ?", request.ID))
	assert.Equal(t, 1, tableCount(t, db, "users.student_profiles", "id = ?", student.ID))
...
func TestEnrollmentDeletion_RLSDeniesOtherTenant(t *testing.T) {
	...
	require.ErrorIs(t, err, capability.ErrEnrollmentDeletionNotFound)
...
func TestEnrollmentDeletion_ConcurrentDecisionIsRecheckedUnderLock(t *testing.T) {
	...
	require.ErrorIs(t, err, capability.ErrEnrollmentDeletionNotAllowed)
...
func (f *deletionTestFixture) guardians() deletionTestGuardians {
	...
	return deletionTestGuardians{
		byAccount: func(ctx context.Context, ids []int64) ([]enrollmentAPI.TestDirectoryGuardian, error) {
			values, err := people.ListGuardiansByAccount(ctx, ids)
			if err != nil {
				return nil, err
			}
			result := make([]enrollmentAPI.TestDirectoryGuardian, 0, len(values))
			for _, value := range values {
				result = append(result, enrollmentAPI.TestDirectoryGuardian{ID: value.ID, AccountID: value.AccountID})
			}
			return result, nil
		},
		byID: func(ctx context.Context, ids []int64) ([]enrollmentAPI.TestDirectoryGuardian, error) {
...
type deletionTestGuardians struct {
	byAccount func(context.Context, []int64) ([]enrollmentAPI.TestDirectoryGuardian, error)
	byID      func(context.Context, []int64) ([]enrollmentAPI.TestDirectoryGuardian, error)
	count     func(context.Context, []int64) (map[int64]int, error)
}

func (d deletionTestGuardians) ListGuardiansByAccount(ctx context.Context, ids []int64) ([]enrollmentAPI.TestDirectoryGuardian, error) {
	return d.byAccount(ctx, ids)
}
func (d deletionTestGuardians) ListGuardiansByID(ctx context.Context, ids []int64) ([]enrollmentAPI.TestDirectoryGuardian, error) {
