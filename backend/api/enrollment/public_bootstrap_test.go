...
func TestRequestService_LoadPublicFormBootstrap_ReturnsValidLateInvitePrefill(t *testing.T) {
	...
	firstName := "Mara"
	lastName := "Muster"
	created, err := svc.CreateLateInvite(ctx, enrollmentAPI.CreateLateInviteInput{
		PhaseID:           env.phaseID,
...
func TestRequestService_LoadPublicFormBootstrap_CapabilityFailureIsStageError(t *testing.T) {
	...
	var stageErr *enrollmentAPI.BootstrapStageError
	require.True(t, errors.As(err, &stageErr))
	assert.Equal(t, enrollmentAPI.BootstrapStageCapabilities, stageErr.Stage)
}
