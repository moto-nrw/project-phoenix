...
type StudentResponseOpts struct {
	...
	// PhotosEnabled mirrors operations.student_photos_enabled for the
	// current tenant. When false, populatePhotoFields skips photo_url +
	// consent metadata so an admin disabling the feature actually
	// suppresses the API surface, not just the UI rendering. Resolve
	// once per request and thread through — never resolve inside
	// populatePhotoFields, which runs per-student in list paths.
	PhotosEnabled bool
}
