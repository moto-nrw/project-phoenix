...
//
// The substrings and codes below must match the contract lines of
// api/testdata/iot_error_strings.golden one to one. That golden, generated by
// api.TestFullProductionRouterGolden, pins each string to the Go declaration
// that emits it and records every deliberate exclusion, so a one-sided edit of
...
// for errors emitted by /api/iot/* routes: the 56 business patterns of
// ERROR_MESSAGE_MAPPINGS minus the "locked" exclusion recorded in the golden.
//
// Source rules:
...
//     visit found for student" — verified absent 2026-07-12; the generic
//     "locked" fallback has no emitter). Those are PyrePortal cleanup
//     candidates, not backend contracts; the golden names each exclusion.
...
var pyreportalErrorStrings = []string{
	...
	// Staff clock RFID (POST /staff-clock), the prose fallback next to the
	// codes below (origin: modules/devicescan/devicescan.go)
	"invalid RFID tag",
	"RFID tag is inactive",
...
var extraGuardSources = []string{
	...
	// Staff-clock RFID sentinels whose text PyrePortal maps next to the
	// staff-clock codes (#3419).
	"modules/devicescan/devicescan.go",
...
func TestPyrePortalErrorStringsGuard(t *testing.T) {
	...
	assertReconciledWithGolden(t, filepath.Join(backendRoot, "api", "testdata", "iot_error_strings.golden"))

	for _, want := range pyreportalErrorStrings {
		assert.Truef(t, strings.Contains(corpus, want),
...
// lines of the IoT error-string golden disagree in either direction.
func assertReconciledWithGolden(t *testing.T, goldenPath string) {
	t.Helper()

	assert.Lenf(t, pyreportalErrorStrings, wantPyrePortalErrorStrings,
		"pyreportalErrorStrings changed size. Change the pattern in PyrePortal/src/services/apiErrors.ts, "+
			"api/iot_error_strings_golden_test.go and here together.")
	assert.Lenf(t, pyreportalErrorCodes, wantPyrePortalErrorCodes,
		"pyreportalErrorCodes changed size. Change the code in PyrePortal/src/services/apiErrors.ts, "+
			"api/iot_error_strings_golden_test.go and here together.")

	data, err := os.ReadFile(goldenPath) // #nosec G304 -- repo-local testdata
	require.NoError(t, err, "reading the IoT error-string golden")
	...
	for line := range strings.Lines(string(data)) {
		...
		require.Truef(t, ok, "malformed golden line %q", line)
		pattern, _, ok := strings.Cut(rest, " -> ")
		require.Truef(t, ok, "malformed golden line %q", line)
		golden[kind] = append(golden[kind], pattern)
	}

	const hint = "this guard and api/testdata/iot_error_strings.golden disagree. Both mirror " +
		"PyrePortal/src/services/apiErrors.ts; change the golden table, this list and PyrePortal together."
	assert.ElementsMatch(t, golden["message"], pyreportalErrorStrings, hint)
	assert.ElementsMatch(t, golden["code"], pyreportalErrorCodes, hint)
}
