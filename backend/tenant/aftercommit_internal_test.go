...
// TestAfterCommitHooks_AddAndRun locks down the queue contract used by
// WithTenantTx: callbacks accumulate in registration order and run all at
// once, leaving the queue empty for re-use.
func TestAfterCommitHooks_AddAndRun(t *testing.T) {
	t.Parallel()
	...
	assert.Empty(t, h.fns, "run must reset the queue")
}
...
func TestRunAfterCommitHooks_DrainsAndExecutes(t *testing.T) {
	...
	assert.Empty(t, h.fns, "queue must be empty after run")
}
...
func TestRegisterAfterCommit_QueuesOntoHolderInContext(t *testing.T) {
	...
	assert.Len(t, h.fns, 1, "fn must be queued onto the holder")
}
