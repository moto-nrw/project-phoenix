...
if [[ -n "$HARDCODED_VIOLATIONS" ]]; then
    echo -e "${RED}Found potential hardcoded ID violations:${NC}" >&2
    echo "" >&2
    echo "$HARDCODED_VIOLATIONS" | while IFS= read -r line; do
        ...
        echo -e "  ${RED}!${NC} $REL_PATH:$LINE_NUM" >&2
        echo -e "    ${CONTENT}" >&2
        echo "" >&2
    done

    if $SHOW_FIX_SUGGESTIONS; then
        echo -e "${YELLOW}Fix suggestion:${NC}" >&2
        echo "  Instead of using hardcoded IDs like int64(1), use test fixtures:" >&2
        echo "" >&2
        echo "    // Before (non-hermetic):" >&2
        echo "    result, err := repo.FindByID(ctx, int64(1))" >&2
        echo "" >&2
        echo "    // After (hermetic):" >&2
        echo "    student := testpkg.CreateTestStudent(t, db, \"First\", \"Last\", \"1a\")" >&2
        echo "    defer testpkg.CleanupTableRecords(t, db, \"users.students\", student.ID)" >&2
        echo "    result, err := repo.FindByID(ctx, student.ID)" >&2
        echo "" >&2
    fi
...
if [[ -n "$MISSING_SETUP_VIOLATIONS" ]]; then
    echo -e "${RED}Found test files with DB operations missing SetupTestDB:${NC}" >&2
    echo "" >&2
    ...
    if $SHOW_FIX_SUGGESTIONS; then
        echo -e "${YELLOW}Fix suggestion:${NC}" >&2
        echo "  Add SetupTestDB to initialize the test database:" >&2
        echo "" >&2
        echo "    func TestExample(t *testing.T) {" >&2
        echo "        db := testpkg.SetupTestDB(t)" >&2
        echo "        defer func() { _ = db.Close() }()" >&2
        echo "" >&2
        echo "        // ... test code using real database" >&2
        echo "    }" >&2
        echo "" >&2
    fi
...
if [[ -n "$EMAIL_VIOLATIONS" ]]; then
    echo -e "${YELLOW}Warning: Found potentially non-unique email addresses:${NC}" >&2
    echo "" >&2
    echo "$EMAIL_VIOLATIONS" | head -10 | while IFS= read -r line; do
        FILE=$(echo "$line" | cut -d: -f1)
        LINE_NUM=$(echo "$line" | cut -d: -f2)
        REL_PATH="${FILE#$PROJECT_ROOT/}"
        echo -e "  ${YELLOW}?${NC} $REL_PATH:$LINE_NUM" >&2
    done
    echo "" >&2
    echo -e "  ${YELLOW}Note:${NC} These may be intentional for error case testing." >&2
    echo "" >&2
else
...
if [[ $VIOLATIONS_FOUND -eq 0 ]]; then
    ...
    echo -e "${RED}Hermetic test violations found.${NC}" >&2
    echo "" >&2
    echo "Please fix the violations above to ensure tests are hermetic." >&2
    echo "Hermetic tests should:" >&2
    echo "  1. Create their own test data using fixtures" >&2
    echo "  2. Never rely on hardcoded IDs that may not exist" >&2
    echo "  3. Clean up after themselves" >&2
    echo "  4. Be runnable in any order and in parallel" >&2
    echo "" >&2
    echo "Run with --fix-suggestions for detailed fix examples." >&2
    echo "" >&2
    exit 1
fi
