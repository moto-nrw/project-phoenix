...
# 3. Run the tests. Playwright starts `pnpm run dev` itself with env
#    pointing at server-e2e (:8081), so the dev frontend container does
#    not need to be up. Stop the dev frontend if it is — both want :3000.
cd frontend
...
The password (`E2EPass1234!`) is set via the seeder's `--staff-password` flag and applies to all 20 demo accounts. The tenant subdomain is `demo-school.localtest.me:3000`.
...
The bare domain shows a tenant selector, not a login form (see `src/app/page.tsx`). The login lives at `{slug}.TENANT_DOMAIN/`, so `baseURL` is `http://demo-school.localtest.me:3000`.
...
| -------------------------------------------------------- | --------------------------------------------------------------------------- |
| Setup test times out on `input[name="email"]`            | Wrong subdomain — `baseURL` should be `http://demo-school.localtest.me:3000`. |
