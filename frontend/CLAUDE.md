...
- Linting: **oxlint** (`.oxlintrc.json` — plugins react/nextjs/jsx-a11y/import/promise; correctness+perf = error). Disabled rules and their rationale live in `.oxlintrc.json`; custom plugins live in `scripts/oxlint-plugin-date-safety.mjs`, `scripts/oxlint-plugin-ui-kit.mjs` (UI-kit drift ratchet — five hard-zero rules, see `.claude/rules/frontend-ui-kit.md`) and `scripts/oxlint-plugin-bauart.mjs` (Bauarten ratchet — `bauart/one-delete-confirm`, `bauart/no-unconfirmed-destructive-click`, `bauart/no-autosave`, `bauart/no-toast-form-error`, `bauart/no-manage-surface-in-overlay`, `bauart/one-detail-per-type`, `bauart/no-disabled-menu-item`, `bauart/no-local-field-grid`, `bauart/no-own-skeleton` and `bauart/no-raw-status-hex` hard-zero, `bauart/no-row-action-buttons` with a location-bound exception list for form-internal remove buttons, `bauart/no-edit-overlay` with a shrink-only baseline for list entries without an object route, `bauart/no-autosave` with a named baseline; see `src/components/ui/BAUARTEN-SPEC.md`). Every page under `app/[tenant]/(protected)` also declares its Bauart in `src/components/ui/tenant-page-scaffold.test.ts` (`BAUART` / `OHNE_BAUART`); a new page without an entry fails the test.
