...
export function DatabasePage<T extends { id: string }>({
  ...
  const searchableFields = useMemo(
    () => config.list.searchableFields ?? ["name", "title"],
    [config.list.searchableFields],
  );
    ...
    async (
      ...
      try {
        ...
        const apiFilters: Record<string, string | number> = {
          ...Object.fromEntries(
            Object.entries(customFilters ?? filters).filter(
              ([_, v]) => v !== null,
            ),
          ),
          page: page,
        ...
        try {
          ...
          console.error(
            `API error when fetching ${config.name.plural}:`,
            apiErr,
          );
          setError(
            ...
            if (filterId === "supervisor_id") {
              ...
              if (Array.isArray(supervisors)) {
                return supervisors.some(
                  (sup: { staff_id?: unknown; id?: unknown }) =>
                    String(sup.staff_id) === filterValue ||
                    String(sup.id) === filterValue,
                );
              }
              return false;
            }
      ...
      for (const filter of config.list.filters) {
        if (typeof filter.options === "function") {
          try {
            ...
            console.error(
              `Failed to load options for filter ${filter.id}:`,
              error,
            );
          }
  ...
  useEffect(() => {
    if (searchStrategy === "frontend") {
      ...
      const filteredItems = performFrontendSearch(
        searchFilter,
        allItems,
        filters,
      );
  ...
  const handleCreateItem = async (itemData: Partial<T>) => {
    try {
      ...
      toastSuccess(
        getDbOperationMessage("create", config.name.singular, displayName),
      );
  ...
  const handleUpdateItem = async (itemData: Partial<T>) => {
    ...
    try {
      ...
      toastSuccess(
        getDbOperationMessage("update", config.name.singular, displayName),
      );
  ...
  const handleDeleteItem = async () => {
    ...
    const confirmMessage =
      config.labels?.deleteConfirmation ??
    ...
    if (window.confirm(confirmMessage)) {
      try {
        ...
        const displayName = config.list.item.title(selectedItem);
        toastSuccess(
          getDbOperationMessage("delete", config.name.singular, displayName),
        );
      ...
      const uniqueValues = Array.from(
        new Set(
          allItems
            ...
            .map((item) =>
              String((item as Record<string, unknown>)[filter.id]),
            ),
        ),
      ).sort();
  ...
  return (
    <>
      <DatabaseListPage
        ...
        addButton={
          config.list.features?.create !== false
            ? {
                label:
                  config.labels?.createButton ??
                  `Neuen ${config.name.singular} erstellen`,
        ...
        itemLabel={{
          singular: config.name.singular,
          plural: config.name.plural,
        }}
        ...
        renderItem={(item: T) => {
          ...
          return (
            <DatabaseListItem
              ...
              subtitle={
                config.list.item.subtitle?.(item) ??
                config.list.item.description?.(item)
              }
              onClick={() => handleSelectItem(item)}
              accent={config.theme.accent}
              leftIcon={
                config.list.item.avatar ? (
                  <div
                    className={`h-10 w-10 rounded-full bg-gradient-to-br ${config.theme.avatarGradient} flex items-center justify-center font-medium text-white`}
                  >
                    {config.list.item.avatar.text(item)}
                  </div>
                ) : undefined
              }
              badges={config.list.item.badges
                ?.filter((badge) => !badge.showWhen || badge.showWhen(item))
                .map((badge, index) => (
                  <span
                    key={index}
      ...
      <CreateFormModal
        ...
        title={
          config.labels?.createModalTitle ?? `Neuer ${config.name.singular}`
        }
        size="lg"
      >
        {createError && (
        ...
        title={
          isEditing
            ? (config.labels?.editModalTitle ??
              `${config.name.singular} bearbeiten`)
      ...
      >
        {selectedItem && !isEditing && (
          <DatabaseDetailView
              ...
              config.detail.header
                ? {
                    ...
                    avatar: config.detail.header.avatar
                      ...
                      : undefined,
                    badges: config.detail.header.badges
                      ?.filter((badge) => badge.showWhen(selectedItem))
                      .map((badge) => ({
                        label:
                          typeof badge.label === "function"
                            ? badge.label(selectedItem)
                            : badge.label,
                        color: badge.color,
                      })),
                  }
            ...
            actions={{
              onEdit:
                config.detail.actions?.edit !== false
                  ? () => setIsEditing(true)
                  : undefined,
              onDelete:
                config.detail.actions?.delete !== false
              ...
              .filter(
                (section) =>
                  ...
                  !(
                    ...
                    config.name.singular === "Pädagogische Fachkraft"
                  ),
              )
              .map((section) => ({
                title: section.title,
