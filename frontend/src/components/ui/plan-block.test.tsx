...
describe("PlanBlock", () => {
  ...
  it("applies the 3px edge color and the 10% tint as inline styles", () => {
    render(<PlanBlock timeRange="12:00–14:00" label="Mensa" color="#5A8E1F" />);

    const block = screen.getByRole("button");
    expect(block).toHaveStyle({ borderLeft: "3px solid #5A8E1F" });
    expect(block).toHaveStyle({ backgroundColor: "#5A8E1F1A" });
  });
