    ...
    void (async function fetchRoomHistory() {
      try {
        ...
        if (!historyResponse.ok) {
          throw new Error(
            `Failed to fetch room history: ${historyResponse.status}`,
          );
        }
        ...
        const backendEntries = Array.isArray(historyResponseData)
          ...
          : typeof historyResponseData === "object" &&
              ...
              Array.isArray(
                (historyResponseData as Record<string, unknown>).data,
              )
        ...
        const frontendHistoryData = backendEntries.map(
          (entry: BackendRoomHistoryEntry): RoomHistoryEntry => {
            if (
              typeof entry.id !== "number" ||
              typeof entry.room_id !== "number" ||
              typeof entry.date !== "string" ||
              typeof entry.group_name !== "string" ||
              typeof entry.student_count !== "number" ||
        ...
        setError(
          "Fehler beim Laden der Raumhistorie. Bitte versuchen Sie es später erneut.",
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="mt-6 space-y-6">
      <div className="border-b border-gray-200 pb-3">
        ...
        <p className="text-sm text-gray-500">Verlauf der letzten Nutzungen</p>
      </div>
      ...
      <div className="relative space-y-6 border-l-2 border-gray-200 pl-6">
        {historyData.map((entry, index) => (
          <div key={entry.id || index} className="relative">
            {/* Timeline dot */}
            <div className="absolute top-1.5 -left-[31px] h-4 w-4 rounded-full border-2 border-white bg-blue-500"></div>
