 ...
 * `GUIDE_SOURCES`, while the help landing page advertises them via
 * `guideEntryPoints`. If a guide is renamed/added/removed in one place but not
 * the other, it would still render on the page yet vanish from (or never enter)
 * search — an invisible gap. These tests force the two lists to stay aligned.
 */
describe("help search ↔ guide entry point sync", () => {
  it("every searchable guide path has a matching landing-page entry point", () => {
    const entryHrefs = new Set(guideEntryPoints.map((e) => e.href));
  ...
  it("every landing-page entry point is covered by the search index", () => {
    ...
    for (const entry of guideEntryPoints) {
      expect(sourcePaths).toContain(entry.href);
    }
  });

  it("guide labels match their advertised titles", () => {
    const titleByHref = new Map(guideEntryPoints.map((e) => [e.href, e.title]));
