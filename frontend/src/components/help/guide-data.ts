  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        icon: ClipboardList,
        summary:
          "Zeigt den Status des Teams: wer anwesend ist, in welchem Raum, im Homeoffice oder abwesend. Darüber steht die Schul-Übersicht mit den Kennzahlen des ganzen Teams.",
        steps: [
          "`Mitarbeiter` öffnen.",
          "Im Bereich `Schul-Übersicht` die Kennzahlen des Teams prüfen: aktive Mitarbeitende, aktuell Eingestempelte gegenüber den jetzt Erwarteten, Krank- und Urlaubsmeldungen für heute sowie Soll und Ist des Zeitraums. Oben rechts lässt sich zwischen `Woche` und `Monat` umschalten.",
          "Die Karte `Saldo-Veränderung` ist nicht Ist minus Soll: Krankheits-, Urlaubs- und Fortbildungstage werden mit dem Tagessoll gutgeschrieben. Die Zahl entspricht deshalb der Summe der Stundenkonten und nicht der Differenz der beiden Karten daneben.",
          "Das `Stundenkonto der Schule` darunter ist ein Kontostand über alle Mitarbeitenden und ändert sich beim Umschalten zwischen Woche und Monat bewusst nicht.",
          "Mit der Berechtigung `time_tracking:manage` erscheinen unter der Übersicht die Reiter `Status` und `Zeitkonten`. `Zeitkonten` zeigt eine Tabelle mit Soll, Ist, Saldo und Resturlaub pro Person für den laufenden Monat; jede Spalte lässt sich über die Kopfzeile sortieren, die Schaltflächen darüber filtern nach `Minusstunden`, `Plusstunden` oder `über +20 Std.`, und `Eigene Grenze` erlaubt eine eigene Untergrenze in Stunden. Ein Klick auf eine Zeile öffnet das Mitarbeiterprofil.",
        ...
        screenshot:
          "Mitarbeiterseite mit Schul-Übersicht, Reitern Status und Zeitkonten sowie Statuskarten des Teams.",
    ...
    steps: [
      ...
      },
      {
        ...
        steps: [
          ...
          "Unter `Auf anderen Seiten` führt `Tageslisten` zu den Listen aus geplanten Angeboten (Mensa, Lernzeit, AG-Angebote, Randstunden, Ganztag) mit den Datenbasen Plan, Ist und Abgleich - der Einstieg liegt im Bereich `Planung`, siehe das Kapitel `Tageslisten`. Daneben führen `Anmeldungen` und `Zeitnachweis` auf die Seite, zu der der jeweilige Export gehört: Anmeldungen werden je Anmeldephase exportiert, Zeitnachweise je Person.",
        ],
        callout: {
