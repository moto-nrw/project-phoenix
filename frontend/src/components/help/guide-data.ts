    ...
    steps: [
      ...
      },
      {
        ...
        steps: [
          ...
          "Eine vorhandene Schicht anklicken, um sie zu bearbeiten oder über `Schicht löschen` zu entfernen. Serien-Schichten (graues Wiederhol-Symbol auf dem Block) zeigen im Fenster direkt die Serie selbst, zum Beispiel `Di, Do · 08:00–15:00 · bis Ende des Kalenderzeitraums`. Beim Speichern fragt die App nach dem Umfang: `Nur diese Woche` ändert genau diesen Termin (die Serie plant ihn nicht erneut ein, das Symbol färbt sich gelb), `Ab jetzt dauerhaft` teilt die Serie ab diesem Datum, `Alle Termine der Serie` öffnet die ganze Serie zum Bearbeiten.",
          "Die ganze Serie ändern Sie über `Ganze Serie bearbeiten` im Serien-Hinweis oder über die Auswahl `Alle Termine der Serie` beim Speichern. Dort lassen sich zusätzlich `Wochentage`, `Wochenrhythmus` und `Gültig bis` anpassen. Geplante Termine ab morgen werden mit den neuen Angaben neu aufgebaut; Termine bis heute bleiben unverändert. Wurden einzelne Termine zuvor per `Nur diese Woche` geändert, nennt die App sie mit Datum, bevor sie überschrieben werden. Ausgefallene Termine und einzeln gelöschte Termine bleiben in jedem Fall bestehen.",
          "Beim Löschen einer Serien-Schicht stehen dieselben drei Umfänge zur Wahl: `Nur diese Woche`, `Ab jetzt dauerhaft` oder `Ganze Serie beenden`. Termine bis heute und einzeln angepasste Termine bleiben dabei erhalten.",
