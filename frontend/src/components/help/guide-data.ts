    ...
    steps: [
      ...
      },
      {
        ...
        steps: [
          ...
          "Tab `Betreuungszeiten`: die Ankunfts- und Abholzeiten des Kindes verwalten. Auf einen Tag in der Wochenübersicht klicken öffnet den Editor; oben wählen Sie unter `Gilt für` eine von drei Möglichkeiten: `Nur an diesem Tag` (eine Ausnahme für dieses Datum), `Mehrere Tage` (dieselbe Ausnahme bis zu einem Enddatum, Wochenenden werden übersprungen) oder `Wochenplan bearbeiten` (die festen Zeiten aller Wochentage, gilt jede Woche). Der Wochenplan lässt sich auch direkt über die Schaltfläche `Wochenplan` oben rechts öffnen. Bei einer Ausnahme stehen neben `Regulär` und `Andere Zeit` auch `Kommt nicht` und `Keine Abholung` zur Verfügung. Notizen gelten entweder nur für den gewählten Tag oder, wenn sie im Wochenplan hinterlegt sind, für jede Woche; im Tageseditor sind Wochennotizen mit `Jede Woche` gekennzeichnet und über `Im Wochenplan ändern` erreichbar.",
