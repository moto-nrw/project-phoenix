    ...
    steps: [
      ...
      },
      {
        ...
        steps: [
          ...
          "Tab `Betreuungszeiten`: die Ankunfts- und Abholzeiten des Kindes verwalten. Auf einen Tag in der Wochenübersicht klicken öffnet den Editor; oben wählen Sie unter `Gilt für`, wie weit die Änderung reicht: `Nur an diesem Tag`, `Mehrere Tage` (bis zu einem Enddatum, Wochenenden werden übersprungen) oder `Jeden Dienstag` und so weiter, was die feste Zeit dieses einen Wochentags dauerhaft ändert. Alle fünf Wochentage auf einmal bearbeiten Sie über die Schaltfläche `Wochenplan` oben rechts. Für einen einzelnen Tag oder Zeitraum stehen neben `Regulär` und `Andere Zeit` auch `Kommt nicht` und `Keine Abholung` zur Verfügung; dauerhaft entfernen Sie einen Tag ebenfalls über `Kommt nicht` bzw. `Keine Abholung`. Notizen gelten entweder nur für den gewählten Tag oder, wenn sie am Wochentag hinterlegt sind, für jede Woche; im Tageseditor sind Wochennotizen mit `Jede Woche` gekennzeichnet und über `Im Wochenplan ändern` bearbeitbar.",
