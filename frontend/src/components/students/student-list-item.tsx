...
export function StudentListItem({ student, onClick }: StudentListItemProps) {
  ...
  if (student.school_class) {
    badges.push(<ClassBadge key="class" className={student.school_class} />);
  }
  ...
  const avatar = (
    <div
      className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-r md:h-12 md:w-12 ${getGradientColors(student.school_class || "")} text-sm font-medium text-white md:text-base`}
    >
  ...
  return (
    <DatabaseListItem
      ...
      subtitle={isPresent && <StatusBadge status="present" />}
      minHeight="md"
    />
  );
}
