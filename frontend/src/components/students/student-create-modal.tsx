...
export function StudentCreateModal({
  ...
  const validateForm = (): boolean => {
    ...
    const hasValidGuardian = guardians.some(
      (g) =>
        g.name.trim() || g.contact.trim() || g.email.trim() || g.phone.trim(),
    );
    if (!hasValidGuardian) {
      newErrors.guardians =
        "Bitte geben Sie mindestens einen Erziehungsberechtigten mit Kontaktinformationen an";
    }
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateGuardian(guardian.id, "contact", e.target.value)
                        }
                        className="block w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm transition-colors focus:border-[#5080D8] focus:ring-1 focus:ring-[#5080D8]"
