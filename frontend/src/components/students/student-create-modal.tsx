...
export function StudentCreateModal({
  ...
  const validateForm = (): boolean => {
    ...
    // Validate guardians: either completely empty (will be skipped) or must have first+last name
    const invalidGuardians = guardians.filter((g) => {
      const hasAnyData =
        g.first_name.trim() ||
        g.last_name.trim() ||
        g.email.trim() ||
        g.phone.trim();
      const hasRequiredData = g.first_name.trim() && g.last_name.trim();
      return hasAnyData && !hasRequiredData;
    });

    if (invalidGuardians.length > 0) {
      newErrors.guardians =
        "Bitte füllen Sie Vor- und Nachname für alle angegebenen Erziehungsberechtigten aus";
    }

    // Ensure at least one valid guardian exists
    const validGuardians = guardians.filter(
      (g) => g.first_name.trim() && g.last_name.trim(),
    );
    if (validGuardians.length === 0) {
      newErrors.guardians =
        "Bitte geben Sie mindestens einen Erziehungsberechtigten mit Vor- und Nachnamen an";
    }
  ...
  const updateGuardian = (
    ...
    setGuardians(
      guardians.map((g, i) => (i === index ? { ...g, [field]: value } : g)),
    );
          ...
          <div className="rounded-xl border border-gray-100 bg-blue-50/30 p-3 md:p-4">
            <div className="mb-3 flex items-center justify-between md:mb-4">
              ...
              <button
                type="button"
                onClick={addGuardian}
                className="flex items-center gap-1 rounded-lg bg-purple-100 px-3 py-1.5 text-xs font-medium text-purple-700 transition-colors hover:bg-purple-200"
              >
                <svg
                  className="h-3 w-3"
                  fill="none"
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateGuardian(index, "first_name", e.target.value)
                        }
                        className="block w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm transition-colors focus:border-[#5080D8] focus:ring-1 focus:ring-[#5080D8]"
                        placeholder="Maria"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateGuardian(index, "last_name", e.target.value)
                        }
                        className="block w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm transition-colors focus:border-[#5080D8] focus:ring-1 focus:ring-[#5080D8]"
                        placeholder="Mustermann"
                      />
