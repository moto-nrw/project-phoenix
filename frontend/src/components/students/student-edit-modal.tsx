...
export function StudentEditModal({
  ...
  const isPayload = useCallback(
    (x: unknown): x is { guardians: Guardian[]; additionalInfo?: string } => {
  ...
  const parseGuardiansAndInfo = useCallback(
    (student: Student): { guardians: Guardian[]; additionalInfo: string } => {
      try {
      ...
      const hasGuardianData = [
        legacyGuardian.name,
        legacyGuardian.contact,
        legacyGuardian.email,
        legacyGuardian.phone,
      ].some((v) => typeof v === "string" && v.length > 0);

      return {
        guardians: hasGuardianData ? [legacyGuardian] : [],
        additionalInfo:
          !hasGuardianData && student.extra_info ? student.extra_info : "",
      };
    },
    [isPayload],
  );
  ...
  useEffect(() => {
    if (student) {
      const { guardians: parsedGuardians, additionalInfo: parsedInfo } =
        parseGuardiansAndInfo(student);
