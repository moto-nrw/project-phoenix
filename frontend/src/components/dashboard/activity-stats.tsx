            ...
            {activities.slice(0, 3).map((activity) => (
              <li key={activity.id} className="py-2">
                <Link
                  ...
                  className="-mx-2 block rounded-lg p-2 hover:bg-gray-50"
                >
                  <div className="flex justify-between">
                    ...
                    <div
                      className={`h-2 w-2 self-center rounded-full ${activity.is_open_ags ? "bg-green-500" : "bg-gray-400"}`}
                    ></div>
                  </div>
                  <p className="mt-1 text-xs text-gray-500">
                    {activity.category_name ?? "Keine Kategorie"} •{" "}
                    {activity.participant_count ?? 0} von{" "}
                    {activity.max_participant} Teilnehmern
                  </p>
                </Link>
              </li>
            ))}
