...
export function GlobalSearch({ className = '', placeholder = 'Schüler, Räume, Aktivitäten suchen...', onResultSelect }: GlobalSearchProps) {
    ...
    <div className={`relative ${className}`}>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="block w-full pl-9 pr-12 py-2 border border-gray-200 rounded-lg text-sm placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#5080d8]/50 focus:border-transparent bg-gray-50/50 hover:bg-white/80 transition-colors duration-200"
          autoComplete="off"
          spellCheck="false"
        />
        ...
        <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
          {isLoading && (
            <div className="animate-spin rounded-full h-4 w-4 border-2 border-gray-300 border-t-[#5080d8]"></div>
          )}
        </div>
      </div>
        ...
        >
          {results.length > 0 ? (
              ...
              <div className="py-2">
                {results.map((result, index) => (
                  <button
                    ...
                    className={`w-full px-3 py-3 text-left hover:bg-gray-50 active:bg-gray-100 transition-colors duration-150 focus:outline-none focus:bg-gray-50 ${
                      index === selectedIndex ? 'bg-[#5080d8]/5 border-r-2 border-[#5080d8]' : ''
                    }`}
                  >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            ...
                            {result.badge && (
                              <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getBadgeStyles(result.badge.color)}`}>
                                {result.badge.text}
                              </span>
                            )}
                          </div>
            ...
            <div className="px-3 py-8 text-center">
              ...
              <h3 className="text-sm font-medium text-gray-900 mb-1">Keine Ergebnisse gefunden</h3>
