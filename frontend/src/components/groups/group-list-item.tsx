...
import { DatabaseListItem, Badge, Link } from "@/components/ui";
...
export function GroupListItem({ group, onClick }: GroupListItemProps) {
  ...
  if (group.supervisors && group.supervisors.length > 0) {
    badges.push(
      ...
      >
        {group.supervisors.length} Aufsicht
        {group.supervisors.length === 1 ? "" : "en"}
      </Badge>,
    );
  } else {
    badges.push(
  ...
  if (group.student_count !== undefined) {
    badges.push(
      ...
      >
        {group.student_count}{" "}
        {group.student_count === 1 ? "Schüler" : "Schüler"}
      </Badge>,
    );
  }
  ...
  // Group icon
  const groupIcon = (
    <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-r from-purple-400 to-pink-400 text-white md:h-12 md:w-12">
      <svg
