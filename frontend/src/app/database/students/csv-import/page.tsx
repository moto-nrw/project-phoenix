...
export default function StudentCSVImportPage() {
    ...
    reader.onload = (e) => {
      ...
      const parsedData: CSVStudent[] = dataRows.map((line, index) => {
        ...
        if (!values[5] && !values[7] && !values[8] && !values[9]) {
          errors.push(
            "Mindestens ein Erziehungsberechtigter mit Kontaktinformationen erforderlich",
          );
        }
  ...
  const convertCSVStudentToStudent = (
  ...
  const handleSaveFromModal = async (updatedStudent: Partial<Student>) => {
    ...
    // Validate guardian
    if (
      !updatedCSVStudent.guardian_name &&
      !updatedCSVStudent.guardian_email &&
      !updatedCSVStudent.guardian_phone &&
      !updatedCSVStudent.guardian_contact
    ) {
      errors.push(
        "Mindestens ein Erziehungsberechtigter mit Kontaktinformationen erforderlich",
      );
    ...
    if (
      updatedCSVStudent.data_retention_days < 1 ||
      updatedCSVStudent.data_retention_days > 31
    ) {
      errors.push("Aufbewahrungsdauer muss zwischen 1 und 31 Tagen liegen");
    }

    updatedCSVStudent.errors = errors;
    updatedCSVStudent.status =
      errors.length > 0 ? "error" : csvData[editingRowIndex]!.status;

    // Update csvData
    setCsvData((prev) =>
      prev.map((row, idx) =>
        idx === editingRowIndex ? updatedCSVStudent : row,
      ),
    );
  ...
  const getStatusBadge = (status: RowStatus) => {
    switch (status) {
      case "new":
        return (
          <span
            className="inline-flex items-center rounded-full bg-[#83CD2D]/10 px-2 py-1 text-xs font-medium text-[#83CD2D]"
            style={{
              backgroundColor: "rgba(131, 205, 45, 0.1)",
              color: "#83CD2D",
            }}
          >
            Neu
      ...
      case "existing":
        return (
          <span
            className="inline-flex items-center rounded-full bg-[#5080D8]/10 px-2 py-1 text-xs font-medium text-[#5080D8]"
            style={{
              backgroundColor: "rgba(80, 128, 216, 0.1)",
              color: "#5080D8",
            }}
          >
            Vorhanden
      ...
      case "error":
        return (
          <span
            className="inline-flex items-center rounded-full bg-[#FF3130]/10 px-2 py-1 text-xs font-medium text-[#FF3130]"
            style={{
              backgroundColor: "rgba(255, 49, 48, 0.1)",
              color: "#FF3130",
            }}
          >
            Fehler
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <ul className="list-inside list-disc space-y-1 text-sm text-gray-600">
                ...
                <li>
                  Korrigieren Sie eventuelle Fehler direkt in der Vorschau
                </li>
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              ...
              <div
                ...
                style={{
                  borderColor: "rgba(131, 205, 45, 0.2)",
                  backgroundColor: "rgba(131, 205, 45, 0.05)",
                }}
              >
                <div className="flex items-center gap-3">
                  <div
                    className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full"
                    style={{ backgroundColor: "rgba(131, 205, 45, 0.15)" }}
                  >
                    <svg
                      className="h-5 w-5"
                      style={{ color: "#83CD2D" }}
                  ...
                  <div>
                    ...
                    <p className="text-xs" style={{ color: "#83CD2D" }}>
                      Neu
                    </p>
                  </div>
                </div>
              ...
              <div
                className="rounded-xl border bg-white p-4"
                style={{
                  borderColor: "rgba(80, 128, 216, 0.2)",
                  backgroundColor: "rgba(80, 128, 216, 0.05)",
                }}
              >
                <div className="flex items-center gap-3">
                  <div
                    className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full"
                    style={{ backgroundColor: "rgba(80, 128, 216, 0.15)" }}
                  >
                    <svg
                      className="h-5 w-5"
                      style={{ color: "#5080D8" }}
                  ...
                  <div>
                    ...
                    <p className="text-xs" style={{ color: "#5080D8" }}>
                      Vorhanden
                    </p>
                  </div>
                </div>
              ...
              <div
                className="rounded-xl border bg-white p-4"
                style={{
                  borderColor: "rgba(255, 49, 48, 0.2)",
                  backgroundColor: "rgba(255, 49, 48, 0.05)",
                }}
              >
                <div className="flex items-center gap-3">
                  <div
                    className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full"
                    style={{ backgroundColor: "rgba(255, 49, 48, 0.15)" }}
                  >
                    <svg
                      className="h-5 w-5"
                      style={{ color: "#FF3130" }}
                  ...
                  <div>
                    ...
                    <p className="text-xs" style={{ color: "#FF3130" }}>
                      Fehler
                    </p>
                  </div>
                </div>
            ...
            <div className="overflow-hidden rounded-xl border border-gray-100 bg-white">
              <div className="border-b border-gray-100 p-4">
                ...
                <p className="mt-1 text-xs text-gray-600">
                  Überprüfen Sie die Daten und korrigieren Sie Fehler durch
                ...
                {csvData.map((student, index) => {
                  ...
                  return (
                    <div
                      ...
                      className="group relative cursor-pointer overflow-hidden rounded-2xl border border-gray-100/50 bg-white/90 shadow-sm backdrop-blur-md transition-all duration-300 hover:-translate-y-0.5 hover:scale-[1.005] hover:border-blue-200/50 hover:bg-white hover:shadow-md active:scale-[0.995]"
                    >
                      {/* Status-based overlay */}
                      <div
                        className={`absolute inset-0 rounded-2xl ${
                          student.status === "new"
                            ? "bg-gradient-to-br from-[#83CD2D]/5 to-[#83CD2D]/10 opacity-40"
                            : student.status === "existing"
                              ? "bg-gradient-to-br from-[#5080D8]/5 to-[#5080D8]/10 opacity-40"
                              : student.status === "error"
                                ? "bg-gradient-to-br from-[#FF3130]/5 to-[#FF3130]/10 opacity-60"
                      ...
                      <div
                        className={`absolute inset-0 rounded-2xl ring-1 ${
                          student.status === "error"
                            ? "ring-[#FF3130]/20"
                            : "ring-white/20"
                        } transition-all duration-300 group-hover:ring-blue-200/60`}
                      ></div>
                      ...
                      <div className="relative flex items-center gap-3 p-3">
                        ...
                        <div className="relative flex-shrink-0">
                          <div
                            className={`flex h-9 w-9 items-center justify-center rounded-full text-xs font-semibold text-white shadow-sm transition-transform duration-300 group-hover:scale-105 ${
                              student.status === "new"
                                ? "bg-gradient-to-br from-[#83CD2D] to-[#70b525]"
                                : student.status === "existing"
                                  ? "bg-gradient-to-br from-[#5080D8] to-[#4070c8]"
                                  : student.status === "error"
                                    ? "bg-gradient-to-br from-[#FF3130] to-[#e02020]"
                          ...
                          {/* Row number badge */}
                          <div className="absolute -right-1 -bottom-1 flex h-4 w-4 items-center justify-center rounded-full bg-gray-700 text-[8px] font-bold text-white shadow-sm">
                            {student.row}
                          </div>
                        </div>
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <h4 className="text-sm font-semibold text-gray-900 transition-colors duration-300 group-hover:text-blue-600">
                          ...
                          {student.errors.length > 0 && (
                            <p className="mt-1 flex items-center gap-1 text-xs text-[#FF3130]">
                              <svg
                        ...
                        <div className="flex-shrink-0">
                          <svg
                            className="h-5 w-5 text-gray-400 transition-all duration-300 group-hover:translate-x-0.5 group-hover:text-blue-600"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                          >
                      ...
                      <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-transparent via-blue-100/20 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100"></div>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="sticky bottom-6 flex flex-col gap-4 rounded-xl border border-gray-200 bg-white/95 p-4 shadow-lg backdrop-blur-sm sm:flex-row">
              ...
              <button
                ...
                className="flex-1 rounded-lg px-6 py-3 text-sm font-medium text-white shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-xl active:scale-95 disabled:cursor-not-allowed disabled:hover:scale-100"
                style={{
                  background:
                    stats.errors > 0
                ...
                onMouseEnter={(e) => {
                  if (stats.errors === 0) {
                    e.currentTarget.style.background =
                      "linear-gradient(to bottom right, #70b525, #5d9920)";
                  }
                }}
                onMouseLeave={(e) => {
                  if (stats.errors === 0) {
