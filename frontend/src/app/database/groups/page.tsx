  ...
  return (
    <ResponsiveLayout>
      <div className="w-full">
        ...
        <button
          onClick={() => setShowCreateModal(true)}
          className={`group fixed right-4 bottom-24 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-[#83CD2D] to-[#70b525] text-white shadow-[0_8px_30px_rgb(0,0,0,0.12)] transition-all duration-300 ease-out hover:shadow-[0_8px_40px_rgba(112,181,37,0.3)] active:scale-95 md:hidden ${isFabVisible ? "pointer-events-auto translate-y-0 opacity-100" : "pointer-events-none translate-y-32 opacity-0"}`}
          aria-label="Gruppe erstellen"
        >
          <div className="pointer-events-none absolute inset-[2px] rounded-full bg-gradient-to-br from-white/20 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100"></div>
          <svg
