...
export default function ActivitiesPage() {
  ...
  const uniqueCategories = useMemo(() => {
    ...
    activities.forEach(a => { if (a.category_name) set.add(a.category_name); });
      ...
      <div className="w-full">
        ...
        <div className="mb-4">
          <PageHeaderWithSearch
            ...
            search={{ value: searchTerm, onChange: setSearchTerm, placeholder: "Aktivitäten suchen..." }}
            filters={filters}
            ...
            actionButton={!isMobile && (
              <button
                ...
                className="relative w-10 h-10 bg-gradient-to-br from-[#FF3130] to-[#e02020] text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center group hover:scale-110 active:scale-95"
        ...
        <button
          onClick={() => setShowCreateModal(true)}
          className={`md:hidden fixed right-4 bottom-24 z-40 w-14 h-14 bg-gradient-to-br from-[#FF3130] to-[#e02020] text-white rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.12)] hover:shadow-[0_8px_40px_rgb(255,49,48,0.3)] flex items-center justify-center group active:scale-95 transition-all duration-300 ease-out ${isFabVisible ? 'translate-y-0 opacity-100 pointer-events-auto' : 'translate-y-32 opacity-0 pointer-events-none'}`}
          aria-label="Aktivität erstellen"
        >
          <div className="absolute inset-[2px] rounded-full bg-gradient-to-br from-white/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
        ...
        {filteredActivities.length === 0 ? (
          <div className="flex min-h-[300px] items-center justify-center">
            <div className="text-center">
              ...
              <h3 className="mt-4 text-lg font-medium text-gray-900">{searchTerm || categoryFilter !== 'all' ? 'Keine Aktivitäten gefunden' : 'Keine Aktivitäten vorhanden'}</h3>
              <p className="mt-2 text-sm text-gray-600">{searchTerm || categoryFilter !== 'all' ? 'Versuchen Sie andere Suchkriterien oder Filter.' : 'Es wurden noch keine Aktivitäten erstellt.'}</p>
            </div>
          </div>
        ) : (
          <div className="space-y-3">
            {filteredActivities.map((activity, index) => {
              ...
              return (
                ...
                >
                  <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-red-50/80 to-rose-100/80 opacity-[0.03] rounded-3xl"></div>
                  <div className="pointer-events-none absolute inset-px rounded-3xl bg-gradient-to-br from-white/80 to-white/20"></div>
                  <div className="pointer-events-none absolute inset-0 rounded-3xl ring-1 ring-white/20 md:group-hover:ring-red-200/60 transition-all duration-300"></div>

                  <div className="relative flex items-center gap-4 p-5">
                    <div className="flex-shrink-0">
                      <div className="h-12 w-12 rounded-full bg-gradient-to-br from-[#FF3130] to-[#e02020] flex items-center justify-center text-white font-semibold shadow-md md:group-hover:scale-110 transition-transform duration-300">
                        {initials}
                      </div>
                    </div>

                    <div className="flex-1 min-w-0">
                      <h3 className="text-lg font-semibold text-gray-900 md:group-hover:text-red-600 transition-colors duration-300">
                        {activity.name}
                      </h3>
                    ...
                    <div className="flex-shrink-0">
                      <svg className="h-6 w-6 text-gray-400 md:group-hover:text-red-600 md:group-hover:translate-x-1 transition-all duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                      </svg>
                    </div>
                  </div>

                  <div className="pointer-events-none absolute inset-0 rounded-3xl opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-r from-transparent via-red-100/30 to-transparent"></div>
                </div>
              );
            })}
