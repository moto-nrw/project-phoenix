              ...
              {filteredStudents.length > 0 && (
                <div className="absolute right-0 top-0 h-full w-full pointer-events-none">
                  {filteredStudents.map((student, index) => (
                    <div key={student.id} className="absolute right-4 flex gap-2" style={{ top: `${index * 78 + 20}px` }}>
                      <button 
                        onClick={(e) => {
                          e.stopPropagation();
                          handleUnenrollStudent(student.id);
                        }}
                        className="flex items-center justify-center h-9 w-9 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-full transition-all duration-200 pointer-events-auto border border-gray-200 hover:border-red-200 shadow-sm"
