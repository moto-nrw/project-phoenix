...
export default function OperatorProvisioningPage() {
  ...
  const handleUpdateOrg = useCallback(
    async (e: React.FormEvent) => {
      e.preventDefault();
      if (!editOrgTarget || !orgName.trim() || !orgSlug.trim()) return;
      if (!isValidSlug(orgSlug)) {
        setOrgError(
          "Slug darf nur Kleinbuchstaben, Zahlen und Bindestriche enthalten.",
        );
      ...
      setOrgError("");
      try {
        await operatorProvisioningService.updateOrganization(editOrgTarget.id, {
          name: orgName.trim(),
          slug: orgSlug.trim(),
          active: editOrgTarget.active,
        });
    ...
    [editOrgTarget, orgName, orgSlug, mutateOrgs, resetOrgForm],
  );
  ...
  const handleUpdateSchool = useCallback(
    async (e: React.FormEvent) => {
      ...
      setSchoolError("");
      try {
        const oldSubdomain = editSchoolTarget.subdomain;
        const newSubdomain = schoolSubdomain.trim();
        await operatorProvisioningService.updateSchool(editSchoolTarget.id, {
          organization_id: parseInt(schoolOrgId, 10),
          name: schoolName.trim(),
          slug: schoolSlug.trim(),
          ...
          active: editSchoolTarget.active,
        });
  ...
  return (
    <div className="-mt-1.5 w-full">
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button
              type="button"
              onClick={(e) => void handleUpdateOrg(e)}
              disabled={orgSaving || !orgName.trim() || !orgSlug.trim()}
              className="flex-1 rounded-lg bg-gray-900 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-50"
            >
      ...
      >
        <form
          onSubmit={(e) => void handleUpdateOrg(e)}
          className="space-y-4"
          id="edit-org-form"
        >
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button
              type="button"
              onClick={(e) => void handleUpdateSchool(e)}
              disabled={
                schoolSaving ||
                !schoolOrgId ||
                !schoolName.trim() ||
      ...
      >
        <form
          onSubmit={(e) => void handleUpdateSchool(e)}
          className="space-y-4"
          id="edit-school-form"
        >
          <FormField label="Träger" htmlFor="edit-school-org" required>
            ...
            {editSchoolTarget &&
              schoolOrgId !== editSchoolTarget.organizationId && (
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Slug" htmlFor="edit-school-slug" required>
              <input
                ...
                required
              />
              {editSchoolTarget && schoolSlug !== editSchoolTarget.slug && (
                <FieldWarning message="Slug-Änderungen können bestehende Verweise ungültig machen." />
              )}
            </FormField>
            <FormField
            ...
            >
              ...
              />
              {editSchoolTarget &&
                schoolSubdomain !== editSchoolTarget.subdomain && (
...
function FieldWarning({ message }: { readonly message: string }) {
  return (
