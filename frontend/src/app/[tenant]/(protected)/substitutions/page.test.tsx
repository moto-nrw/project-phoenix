...
function openAssignFor(name: RegExp) {
  ...
  fireEvent.click(screen.getByRole("menuitem", { name: "Gruppe übergeben" }));
}
...
function endFirstAccess() {
  ...
  fireEvent.click(screen.getByRole("menuitem", { name: "Übergabe beenden" }));
}
...
describe("SubstitutionsPage", () => {
  ...
  describe("Loading and Error States", () => {
    ...
    it("shows loading state while teachers are loading", () => {
      ...
      expect(
        screen.getByLabelText("Gruppenübergaben werden geladen…"),
      ).toBeInTheDocument();
    });
  ...
  describe("Active Substitutions Display", () => {
    ...
    it("shows the empty state when there are no handovers", () => {
      ...
      expect(
        screen.getByText("Keine eintägigen Übergaben"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Keine mehrtägigen Übergaben"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Assign Substitution Modal", () => {
    ...
    it("displays duration options in modal", async () => {
      ...
      expect(
        within(durationGroup).getByRole("button", { name: "1 Tag" }),
    ...
    it("assigns a multi-day handover starting in the future", async () => {
      ...
      fireEvent.click(screen.getByRole("option", { name: "Gruppe 1A" }));
      fireEvent.click(within(modal).getByRole("button", { name: "3 Tage" }));
      fireEvent.click(
