...
async function proxyExport(phaseId: string, body: string, token: string) {
  const { getServerApiUrl } = await import("~/lib/server-api-url");
  const response = await fetch(
    `${getServerApiUrl()}/api/enrollment/phases/${encodeURIComponent(phaseId)}/export`,
    {
      method: "POST",
      headers: {
...
async function POSTHandler(request: NextRequest, context: RouteContext) {
  try {
    ...
    const response = await proxyExport(id, body, session.user.token);
