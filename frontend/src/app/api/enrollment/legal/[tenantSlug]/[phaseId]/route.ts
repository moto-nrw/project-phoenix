...
const logger = createLogger({ component: "PublicEnrollmentLegalPhaseRoute" });
...
export async function GET(_request: NextRequest, context: RouteContext) {
  ...
  if (!tenantSlug || !phaseId) {
    return NextResponse.json(
      { error: "tenant slug and phase id are required" },
      { status: 400 },
    );
  }
  try {
    ...
    logger.error("public_enrollment_legal_phase_failed", {
