...
const logger = createLogger({
  component: "EnrollmentCareUsageReportExportRoute",
});

async function proxyExport(body: string, token: string) {
  const { getServerApiUrl } = await import("~/lib/server-api-url");
  const response = await fetch(
    `${getServerApiUrl()}/api/enrollment/admin/reports/care-usage/export`,
    {
      method: "POST",
      headers: {
...
async function POSTHandler(request: NextRequest) {
  try {
    ...
    logger.error("care_usage_report_export_failed", { error: message });
