...
describe("POST /api/logs", () => {
  ...
  it("redacts sensitive fields before writing client logs to stdout", async () => {
    ...
    expect(JSON.stringify(output)).not.toMatch(
      /sentinel-(password|pin|token|secret|jwt|api-key|authorization|cookie|header|staff-pin|staff-auth-pin|status-token|compound-jwt-token|compound-pin|calendar-token)/,
    );
