...
describe("POST /api/platform/announcements/[id]/dismiss", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("dismisses announcement successfully", async () => {
    mockValidSession();
    mockApiPost.mockResolvedValue({ success: true });

    const request = new NextRequest(
      "http://localhost:3000/api/platform/announcements/1/dismiss",
      {
        method: "POST",
      },
    );
    ...
    expect(mockApiPost).toHaveBeenCalledWith(
      "/api/platform/announcements/1/dismiss",
      "test-token",
      {},
    );
  });
