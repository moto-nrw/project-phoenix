...
import type { BackendSubstitution, CreateSubstitutionRequest } from "~/lib/substitution-helpers";
...
export const GET = createGetHandler(async (request: NextRequest, token: string) => {
  ...
  const paginatedResponse = await apiGet<PaginatedSubstitutionResponse>(endpoint, token);
