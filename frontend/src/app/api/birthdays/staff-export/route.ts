...
const logger = createLogger({ component: "StaffBirthdayExportRoute" });
...
async function proxyExport(body: string, token: string) {
  ...
  const response = await fetch(
    `${getServerApiUrl()}/api/birthdays/staff-export`,
    {
      method: "POST",
      headers: {
...
async function POSTHandler(request: NextRequest) {
  try {
    ...
    logger.error("staff_birthday_export_route_failed", { error: message });
