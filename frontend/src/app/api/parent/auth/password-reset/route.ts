...
const logger = createLogger({ component: "ParentPasswordResetRoute" });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${getServerApiUrl()}/parent/auth/password-reset`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        logger.warn("failed to parse parent password reset error response", {
          error:
    ...
    logger.error("parent password reset request failed", {
