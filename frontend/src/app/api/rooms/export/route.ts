...
const logger = createLogger({ component: "RoomExportRoute" });

async function proxyExport(body: string, token: string) {
  const { getServerApiUrl } = await import("~/lib/server-api-url");
  const response = await fetch(`${getServerApiUrl()}/api/rooms/export`, {
    method: "POST",
    headers: {
...
async function POSTHandler(request: NextRequest) {
  try {
    ...
    logger.error("room_export_route_failed", { error: message });
