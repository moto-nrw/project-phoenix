...
export async function POST(
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return NextResponse.json(
                { error: errorText || `Failed to assign permission: ${response.status}` } as ErrorResponse,
...
export async function DELETE(
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return NextResponse.json(
                { error: errorText || `Failed to remove permission: ${response.status}` } as ErrorResponse,
                { status: response.status }
            );
        }
