// app/api/students/[id]/photo/route.ts
//
// Proxy for student photo upload + delete. Mirrors the existing
// `/api/me/profile/avatar` flow (createFileUploadHandler validates magic
// bytes, MIME, size cap → forwards multipart to backend) so user avatars
// and student photos go through identical sanitisation.
//
// 401 retry: createFileUploadHandler hands us a single cached token. When
// it expires (15 min default) but the NextAuth refresh token is still
// valid, the first upload returns 401. We then call uncachedAuth() once
// to force a token refresh and retry — same pattern as the photo-serve
// proxy at api/students/[id]/photo/[filename]/route.ts and the generic
// createDeleteHandler. Without this, idle users hit 401 on upload while
// fetch + delete keep working until the next page reload.
