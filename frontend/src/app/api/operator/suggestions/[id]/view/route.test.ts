...
describe("POST /api/operator/suggestions/[id]/view", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("marks suggestion as viewed successfully", async () => {
    ...
    const request = new NextRequest(
      "http://localhost:3000/api/operator/suggestions/1/view",
      {
        method: "POST",
      },
    );
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "http://localhost:8080/operator/suggestions/1/view",
  ...
  it("returns 404 for non-existent suggestion", async () => {
    ...
    const request = new NextRequest(
      "http://localhost:3000/api/operator/suggestions/999/view",
      {
        method: "POST",
      },
    );
