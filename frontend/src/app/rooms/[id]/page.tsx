...
function mapBackendToFrontendRoom(backendRoom: BackendRoom): Room {
  return {
    ...
    color:
      backendRoom.color ?? categoryColors[backendRoom.category] ?? "#6B7280",
  };
}
      ...
      <div className="mx-auto max-w-7xl px-4 pb-6 sm:px-6 lg:px-8">
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          >
            ...
            <InfoItem
              label="Gebäude"
              value={room.building ?? "Nicht angegeben"}
            />
          ...
          {room.isOccupied && (
            ...
            >
              {room.groupName && (
                <InfoItem label="Gruppe" value={room.groupName} />
              )}
              {room.activityName && (
                <InfoItem label="Aktivität" value={room.activityName} />
              )}
              {room.supervisorName && (
                <InfoItem label="Aufsichtsperson" value={room.supervisorName} />
              )}
              {room.studentCount !== undefined && (
                <div>
                  <p className="mb-2 text-xs text-gray-500">Belegung</p>
