...
function mapActivitySupervisor(
...
export function mapActivitySupervisorsResponse(
...
export function formatSupervisorList(
  supervisors: ActivitySupervisor[] | undefined,
): string {
...
export function mapActivityCategoryResponse(
  backendCategory: BackendActivityCategory,
): ActivityCategory {
  return {
...
export function mapActivityScheduleResponse(
  ...
  return {
    ...
    timeframe_id: backendSchedule.timeframe_id
      ? String(backendSchedule.timeframe_id)
      : undefined,
...
export function mapActivityStudentResponse(
  backendStudent: BackendActivityStudent,
): ActivityStudent {
...
export function mapActivityStudentsResponse(
...
export function mapStudentEnrollmentResponse(
  enrollment: BackendStudentEnrollment,
): ActivityStudent {
  // Handle both direct and flattened person fields
  const firstName =
    enrollment.first_name ?? enrollment.person__first_name ?? "";
  const lastName = enrollment.last_name ?? enrollment.person__last_name ?? "";
  const fullName = `${firstName} ${lastName}`.trim() || "Unnamed Student";
  const current_location = normalizeLocation(
    enrollment.current_location ?? enrollment.student__current_location,
  );

  return {
    id: String(enrollment.id),
    activity_id: enrollment.activity_group_id
      ? String(enrollment.activity_group_id)
      : "",
    // Backend returns 'id' as the student ID, not 'student_id'
    student_id: String(enrollment.id),
    name: fullName,
    school_class:
      enrollment.school_class ?? enrollment.student__school_class ?? "",
    current_location,
...
export function mapStudentEnrollmentsResponse(
  enrollments: BackendStudentEnrollment[],
): ActivityStudent[] {
...
export function formatStudentList(
  students: ActivityStudent[] | undefined,
): string {
  ...
  return students
    .map((student) => student.name ?? `Student ${student.student_id}`)
    .join(", ");
}
...
export function prepareBatchEnrollmentForBackend(studentIds: string[]): {
  student_ids: number[];
} {
  return {
...
export function isActivityCreator(
  activity: Activity,
...
export function mapTimeframeResponse(
...
export function prepareActivityScheduleForBackend(
...
export function formatActivityTimes(
  activity: Activity | ActivitySchedule[],
): string {
...
export function sortSchedulesByWeekday(
  schedules: ActivitySchedule[],
): ActivitySchedule[] {
  return [...schedules].sort(
    (a, b) => getWeekdayOrder(a.weekday) - getWeekdayOrder(b.weekday),
  );
}

export function formatScheduleTime(
  schedule: ActivitySchedule,
  timeframes?: Array<{ id: string; start_time: string; end_time: string }>,
): string {
...
export function formatParticipantStatus(
  activityOrCurrent: Activity | number,
  max?: number,
): string {
...
export function isSupervisorAvailable(
  ...
  return !existingSupervisorSchedules.some(
    (schedule) =>
      schedule.weekday.toLowerCase() === weekday.toLowerCase() &&
      schedule.timeframe_id === timeframeId,
  );
