...
export const activityService = {
  ...
  createActivity: async (activity: Omit<Activity, "id">): Promise<Activity> => {
    ...
    const backendActivity: BackendActivity = prepareActivityForBackend(
    ...
    if (
      !backendActivity.max_participant ||
      backendActivity.max_participant < 1
    ) {
  ...
  updateActivity: async (
    ...
    console.log(
      "Backend updates prepared:",
      JSON.stringify(backendUpdates, null, 2),
    );
  ...
  getCategories: async (): Promise<ActivityCategory[]> => {
    const useProxyApi = typeof window !== "undefined";
    const url = useProxyApi
      ? "/api/database/activities/categories"
  ...
  enrollStudent: async (
    ...
    try {
      ...
      console.error(
        `Error enrolling student ${studentId} in activity ${activityId}:`,
        error,
      );
      throw error;
    }
  ...
  unenrollStudent: async (
    ...
    try {
      ...
      console.error(
        `Error unenrolling student ${studentId} from activity ${activityId}:`,
        error,
      );
      throw error;
    }
  ...
  addTimeSlot: async (
    activityId: string,
    timeSlot:
      | Omit<ActivityTime, "id" | "ag_id" | "created_at">
      | { weekday: string; start_time: string; end_time?: string },
    ...
    if ("timespan_id" in timeSlot) {
      ...
      preparedTimeSlot = {
        ...timeSlot,
        timespan_id:
          typeof timeSlot.timespan_id === "string"
      ...
      throw new Error(
        "Invalid time slot format: must include either timespan_id or start_time",
      );
    }
  ...
  deleteTimeSlot: async (
    ...
    try {
      ...
      console.error(
        `Error deleting time slot ${timeSlotId} from activity ${activityId}:`,
        error,
      );
      throw error;
    }
  ...
  getEnrolledStudents: async (activityId: string): Promise<Student[]> => {
    ...
    try {
      ...
      console.error(
        `Error fetching enrolled students for activity ${activityId}:`,
        error,
      );
      throw error;
    }
  ...
  getStudentActivities: async (studentId: string): Promise<Activity[]> => {
    ...
    try {
      ...
      console.error(
        `Error fetching activities for student ${studentId}:`,
        error,
      );
      throw error;
    }
