...
publish the sweep on the PR as native attachments; local file paths alone are
not a delivery. Upload rules: [docs/agents/operations.md](../../../../docs/agents/operations.md#pr-screenshots-and-qa-evidence).

```bash
# comment.md references every shot by its local path: ![iPhone SE – Übersicht](<shots>/dash-se.png)
gh pr comment <NR> --body-file comment.md $(printf -- '--attach %s ' <shots>/*.png)
```
...
**Re-running the sweep? UPDATE, don't stack comments**: rerun the same command
with `--edit-last`.
