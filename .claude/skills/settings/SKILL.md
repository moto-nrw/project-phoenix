...
Workflow for any settings system work. The full reference is in `.claude/rules/settings-system.md` — this skill guides you through the right files to read first.
...
Then read the full rule: `.claude/rules/settings-system.md`
...
Read the current definition in `defaults/{category}.go`, then consult the editing table in `.claude/rules/settings-system.md`. Key changes that need data migrations: renaming a key.

## Task: Delete a Setting

Follow the deletion checklist in `.claude/rules/settings-system.md` — includes removing key, definition, consumers, tests, AND data migration for orphaned DB rows.
